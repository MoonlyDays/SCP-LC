...
if CLIENT then SWEP.BoostMaterials = {"slc/hud/scp/066/speed.png", "slc/hud/scp/066/prot.png", "slc/hud/scp/066/regen.png",} end
...
function SWEP:Think()
    ...
    if self.MusicAttack < ct and eric <= ct and eric_stacks < self.MaxEricStacks then
        ...
        if can_attack or not any_player then
            ...
            if self:GetNextPrimaryFire() < ct + 1.5 then self:SetNextPrimaryFire(ct + 1.5) end
        end
    end

    if self.MusicAttack >= ct and self.NextMusicAttackTick <= ct then
        self.NextMusicAttackTick = self.NextMusicAttackTick + 1
        if self.NextMusicAttackTick < ct then self.NextMusicAttackTick = ct + 1 end
        ...
        for i, v in ipairs(player.GetAll()) do
            ...
            if is_safe and not pos:WithinAABox(safe_pos.mins, safe_pos.maxs) then continue end
            local dist = v:GetPos():Distance(pos)
            local pct = 1 - dist / max_dist
            if pct <= 0 then continue end
            v:TakeDamage(dmg * pct, owner, owner)
            if pct > 0.5 then v:ApplyEffect("scp066_blur") end
        end
    end
...
function SWEP:DashThink()
    ...
    if not self.DashActive then return end
    local owner = self:GetOwner()
...
function SWEP:TranslateActivity(act)
    if act == ACT_MP_WALK or act == ACT_MP_RUN or act == ACT_MP_CROUCHWALK or act == ACT_LAND then return ACT_WALK end
    return ACT_IDLE
end
...
function SWEP:PrimaryAttack()
    ...
    if not self:CanAttack() then
        if CLIENT then self:HUDNotify("not_threatened", 1, "npc/roller/code2.wav") end
        return
    end
...
function SWEP:Reload()
    ...
    local owner = self:GetOwner()
    if not self.Attached and not owner:IsOnGround() then return end
    self:SetNextDash(ct + self.DashCooldown * self:GetUpgradeMod("dash_cd", 1))
    ...
    if not self.Attached then self.Attached = {owner} end
    owner:SetVelocity(vel)
    controller.Start(owner, "scp066_dash")
    owner:AddTimer("SCP066DashForceStop", 3, 1, function() if controller.IsEnabled(owner, "scp066_dash") then self:CleanupDash() end end)
...
function SWEP:SpecialAttack()
    ...
    if CLIENT then
        ...
        if skill then skill:SetMaterialOverride(self.BoostMaterials[next_type + 1], "smooth") end
        local bar = self.HUDObject:GetBar("boost_bar")
        if bar then bar:SetMaterialOverride(self.BoostMaterials[special_type + 1], "smooth") end
        return
    end
    ...
    if special_type == 0 then --speed
        ...
        owner:AddTimer("SCP066Special", duration, 1, function() owner:PopSpeed("SLC_SCP066Special") end)
    elseif special_type == 1 then
        --def
        self.SpecialDefenseTime = ct + duration
        self.SpecialDefenseMult = math.Clamp(1 - power * self.SpecialDefense, 0, 1)
    elseif special_type == 2 then
        --regen
        owner:AddTimer("SCP066Special", duration / self.SpecialHealTicks, self.SpecialHealTicks, function() owner:AddHealth(math.ceil(power * self.SpecialHeal)) end)
    end
end
...
function SWEP:CanAttack()
    ...
    local any = false
    for i, v in ipairs(player.GetAll()) do
        if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(pos) > radius or not v:TestVisibility(owner) then continue end
        any = true
        local wep = v:GetActiveWeapon()
        if IsValid(wep) and wep:IsAttackWeapon() then return true end
    end
    return false, any
end
...
function SWEP:Detach()
    if not self.CurrentAttach or not IsValid(self.CurrentAttach[1]) or not self.CurrentAttach[1]:CheckSignature(self.CurrentAttach[2]) then return end
    local owner = self:GetOwner()
...
function SWEP:CleanupDash(soft)
    if IsValid(self.CurrentAttach) then self.CurrentAttach:SetProperty("scp066_attached", nil) end
...
SCPHook("SCP066", "EntityTakeDamage", function(ent, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not dmg:IsDamageType(DMG_BULLET) or not IsValid(ent) or not ent:IsPlayer() or ent:SCPClass() ~= CLASSES.SCP066 then return end
    local wep = ent:GetSCPWeapon()
    if not IsValid(wep) then return end
    if wep.SpecialDefenseTime >= CurTime() then dmg:ScaleDamage(wep.SpecialDefenseMult) end
end)

SCPHook("SCP066", "PostEntityTakeDamage", function(ent, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not IsValid(ent) or not ent:IsPlayer() or ent:SCPClass() ~= CLASSES.SCP066 then return end
...
SCPHook("SCP066", "SLCPlayerFootstep", function(ply, foot, snd)
    ...
    local ct = CurTime()
    if not ply.Next066Step or ply.Next066Step < ct then
...
local function handle_death(ply)
    ...
    if not IsValid(wep) or not wep.CurrentAttach or wep.CurrentAttach[1] ~= ply or not ply:CheckSignature(wep.CurrentAttach[2]) then return end
    wep:Detach()
    wep:CleanupDash()
end
...
EFFECTS.RegisterEffect("scp066_blur", {
    ...
    begin = function(self, ply, tier, args, refresh, decrease)
        ...
        AddRoundHook("SLCScreenMod", "SCP066ScreenBlur", function(clr) DrawMotionBlur(0.25, 0.95, 0.02) end)
    end,
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("music"):SetButton("attack"):SetMaterial("slc/hud/scp/066/music.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire"):SetActiveFunction("CanAttack")
    hud:AddSkill("dash"):SetButton("reload"):SetMaterial("slc/hud/scp/066/dash.png", "smooth"):SetCooldownFunction("GetNextDash")
    hud:AddSkill("boost"):SetButton("scp_special"):SetMaterial("slc/hud/scp/066/speed.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack"):SetActiveFunction(function(swep) return swep:GetEric() > 0 end):SetParser(function(swep, lang)
    ...
    hud:AddSkill("eric"):SetOffset(0.5):SetMaterial("slc/hud/scp/066/eric.png", "smooth"):SetCooldownFunction("GetNextEric"):SetTextFunction("GetEric")
    hud:AddBar("music_bar"):SetMaterial("slc/hud/scp/066/music.png", "smooth"):SetColor(Color(164, 195, 26)):SetTextFunction(function(swep)
        local time = swep.MusicAttack - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep) return (swep.MusicAttack - CurTime()) / swep.MusicLength end):SetVisibleFunction(function(swep) return swep.MusicAttack > CurTime() end)

    hud:AddBar("dash_bar"):SetMaterial("slc/hud/scp/066/dash.png", "smooth"):SetColor(Color(199, 169, 38)):SetTextFunction(function(swep)
        local time = swep:GetDetachTime() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep) return (swep:GetDetachTime() - CurTime()) / (swep.DashDetachTime * swep:GetUpgradeMod("detach_time", 1)) end):SetVisibleFunction(function(swep) return swep:GetDetachTime() > CurTime() end)

    hud:AddBar("boost_bar"):SetMaterial("slc/hud/scp/066/speed.png", "smooth"):SetColor(Color(43, 231, 168)):SetTextFunction(function(swep)
        local time = swep.SpecialEndTime - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep) return (swep.SpecialEndTime - CurTime()) / (swep.SpecialDuration * swep:GetUpgradeMod("boost_dur", 1)) end):SetVisibleFunction(function(swep) return swep.SpecialEndTime > CurTime() end)
end
...
controller.Register("scp066_dash", {
    ...
    Move = function(self, ply, mv)
        ...
        if not self.InAir and not on_ground then self.InAir = true end
        if self.InAir and on_ground then wep:CleanupDash() end
    end,
})

local vector_zero = Vector(0, 0, 0)
controller.Register("scp066_stop", {
    Move = function(self, ply, mv) mv:SetVelocity(vector_zero) end,
})
