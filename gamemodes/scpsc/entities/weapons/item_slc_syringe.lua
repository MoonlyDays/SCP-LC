...
function SWEP:Holster()
    return not self.PreventDropping
end
...
function SWEP:PrimaryAttack()
    ...
    timer.Simple(dur / self.SelfInjectSpeed, function()
        if not IsValid(self) or not IsValid(owner) or owner ~= self:GetOwner() then return end
...
function SWEP:SecondaryAttack()
    ...
    if not syringe_trace.Hit then util.TraceHull(syringe_trace) end
    owner:LagCompensation(false)
    if not syringe_trace.Hit then return end
    ...
    timer.Simple(dur / self.InjectSpeed, function()
        if not IsValid(self) or not IsValid(owner) or not IsValid(other) or owner ~= self:GetOwner() then return end
        self.PreventDropping = false
