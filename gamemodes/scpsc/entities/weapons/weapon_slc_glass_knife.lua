...
local paths = {
    [3] = {Vector(0, -4, 6), Vector(0, 4, -10), 25, -25,},
    [4] = {Vector(0, 4, 6), Vector(0, -4, -10), -25, 25,},
    [5] = {Vector(0, 4, 0), Vector(25, 4, 0), 0, 0,},
}
...
function SWEP:PrimaryAttack()
    ...
    if combo > 0 and self:GetComboTime() < ct then combo = 0 end
    local seq = combo + 3
    local strong = seq == 5
    local vm = owner:GetViewModel()
    ...
    self:SwingAttack({
        ...
        on_start = function() self:EmitSound("WeaponFrag.Throw") end,
        callback = function(tr, num)
            ...
            elseif tr.Hit then
                ...
                return true, not strong and num < 3 and {
                    distance = 60,
                    bounds = 1.5,
                }
            end
