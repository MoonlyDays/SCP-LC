...
function SWEP:Think()
    ...
    if self.NextPassive <= ct then
        ...
        if self.NextPassive < ct then self.NextPassive = ct + cd end
        local dur = self.DetectDuration * self:GetUpgradeMod("detect_time", 1)
        local affected = 0
        for i, v in ipairs(SLCZones.GetPlayersInZone(ZONE_FOREST)) do
            if not self:CanTargetPlayer(v) or v:GetSCP714() or v:CheckHazmat(30, true) then continue end
        ...
        if affected > 0 then
            ...
            if hp > max_hp then
                ...
                if overheal > max_overheal then overheal = max_overheal end
        ...
        if #transmit > 0 then net.SendTable("SCP8602Detect", transmit, owner) end
    end
    ...
    if charge > ct then
        ...
        if charge_trace.Hit and IsValid(ent) and ent:IsPlayer() and self:CanTargetPlayer(ent) then
            ...
            if hit_wall or has_upgrade and pct >= 0.8 then
                ...
                if has_upgrade and hit_wall then ent:ApplyEffect("fracture") end
            else
...
function SWEP:PrimaryAttack()
    ...
    if not attack_trace.Hit or not IsValid(ent) then return end
    local is_player = ent:IsPlayer()
    if is_player and not self:CanTargetPlayer(ent) then return end
...
function SWEP:SpecialAttack()
    if CLIENT or not self:CanAttack() then return end
    local ct = CurTime()
...
function SWEP:PerformDefenseAttack()
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        local pos = v:GetPos()
        if not pos:WithinRotatedRect(rect_a, rect_b, rect_c, rect_d) or math.abs(pos.z - owner_pos.z) > 48 then continue end
...
Net
---------------------------------------------------------------------------]]
if SERVER then net.AddTableChannel("SCP8602Detect") end
if CLIENT then
...
SCPHook("SCP8602", "EntityTakeDamage", function(target, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not dmg:IsDamageType(DMG_BULLET) or not IsValid(target) or not target:IsPlayer() or target:SCPClass() ~= CLASSES.SCP8602 then return end
    local wep = target:GetSCPWeapon()
    if not IsValid(wep) or not target:IsHolding(wep, "scp8602_def") then return end
...
SCPHook("SCP8602", "SLCPostScaleDamage", function(target, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not IsValid(target) or not target:IsPlayer() or target:SCPClass() ~= CLASSES.SCP8602 then return end
    local wep = target:GetSCPWeapon()
    if not IsValid(wep) then return end
...
SCPHook("SCP8602", "StartCommand", function(ply, cmd)
    ...
    local wep = ply:GetSCPWeapon()
    if not IsValid(wep) or not ply:IsHolding(wep, "scp8602_def") then return end
...
DefineUpgradeSystem("scp8602", {
    ...
    rewards = {
        --14 + 1 points -> 60% = 9 (-1 base) = 8 points
        {100, 1},
        {200, 1},
        {300, 1},
        {450, 1},
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("primary"):SetButton("attack"):SetMaterial("slc/hud/scp/8602/attack.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud:AddSkill("defense"):SetButton("attack2"):SetMaterial("slc/hud/scp/8602/def.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetParser(function(swep, lang)
        return {
            dmg_ratio = MarkupBuilder.StaticPrint((swep.DefenseDamageMultiplier * swep:GetUpgradeMod("def_mult", 1) * 100) .. "%", color_green),
        }
    end)

    hud:AddSkill("charge"):SetButton("scp_special"):SetMaterial("slc/hud/scp/8602/charge.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack")
    hud:AddSkill("passive"):SetOffset(0.5):SetMaterial("slc/hud/scp/8602/passive.png", "smooth")
    hud:AddBar("overheal_bar"):SetMaterial("slc/hud/scp/8602/overheal.png", "smooth"):SetColor(Color(150, 195, 20)):SetTextFunction(function(swep) return math.Round(swep:GetOverheal()) end):SetProgressFunction(function(swep) return swep:GetOverheal() / swep.MaxOverheal end):SetVisibleFunction(function(swep) return swep:GetOverheal() > 0 end)
    hud:AddBar("defense_bar"):SetMaterial("slc/hud/scp/8602/def.png", "smooth"):SetColor(Color(185, 175, 105)):SetTextFunction(function(swep)
        local f = LocalPlayer():HoldProgress(swep, "scp8602_def")
        if f then return math.Round(swep.DefenseReduction * swep:GetUpgradeMod("def_prot", 1) * f * 100) .. "%" end
    end):SetProgressFunction(function(swep) return LocalPlayer():HoldProgress(swep, "scp8602_def") end):SetVisibleFunction(function(swep) return LocalPlayer():IsHolding(swep, "scp8602_def") end)

    hud:AddBar("charge_bar"):SetMaterial("slc/hud/scp/8602/charge.png", "smooth"):SetColor(Color(180, 30, 30)):SetTextFunction(function(swep)
        local time = swep:GetCharge() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time)
    end):SetProgressFunction(function(swep) return (swep:GetCharge() - CurTime()) / (swep.ChargeTime * swep:GetUpgradeMod("charge_time", 1)) end):SetVisibleFunction(function(swep) return swep:GetCharge() > CurTime() end)
end
