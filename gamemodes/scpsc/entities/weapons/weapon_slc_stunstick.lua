...
function SWEP:Think()
    ...
    if SERVER or not IsFirstTimePredicted() or owner:ShouldDrawLocalPlayer() or vm:GetSequence() <= 1 or SLCRandom(0, 3) ~= 0 then return end
    if self:GetBattery() <= 0 and not self.LastAttackPowered then return end
...
function SWEP:Deploy()
    ...
    self:SetNextIdle(cd)
    if self:GetBattery() > 0 then self:DeployEffects() end
    return true
end
...
function SWEP:PrimaryAttack()
    ...
    self:SwingAttack({
        ...
        on_start = function() self:EmitSound(powered and "Weapon_StunStick.Swing" or "Weapon_Crowbar.Single") end,
        callback = function(tr, num, center)
            local ent = tr.Entity
            if not IsValid(ent) and num < 3 then
                if center then
...
function SWEP:ViewModelDrawn(vm)
    ...
    if is_swing and not self.SwingLastFrame then
        self.SwingLastFrame = true
    elseif not is_swing and self.SwingLastFrame then
    ...
    if not is_swing and fade <= 0 then return end
...
function SWEP:DrawWorldModel(flags)
    if IsValid(self:GetOwner()) and self:GetBattery() > 0 then
        ...
        if SLCRandom(0, 8) == 0 then self:CreateBeam(self, 1) end
    end
