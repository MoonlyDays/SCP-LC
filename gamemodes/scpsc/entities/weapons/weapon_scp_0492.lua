...
function SWEP:SetupDataTables()
    ...
    self:NetworkVarNotify("ZombieType", function(ent, name, old, new)
        ...
        if ent._DataDump then ent._DataDump.zombie_type = zombie.name end
    end)
end
...
function SWEP:Think()
    ...
    if self.Next049Update <= ct and self:GetProtection() < ct then
        ...
        if not scp:Alive() or scp:SCPTeam() ~= TEAM_SCP then
            self:SetSCP049(NULL)
            return
        end
...
function SWEP:PrimaryAttack()
    if not self.ZombieData or ROUND.preparing or not IsFirstTimePredicted() then return end
    local ct = CurTime()
    ...
    if self:GetNextSecondaryFire() < secondary_cd then self:SetNextSecondaryFire(secondary_cd) end
end
...
function SWEP:SecondaryAttack()
    ...
    if secondary == "heavy" then
        ...
        if self:GetNextPrimaryFire() < primary_cd then self:SetNextPrimaryFire(primary_cd) end
    elseif secondary == "rapid" then
        local primary_cd = ct + self:DoRapidAttack()
        if self:GetNextPrimaryFire() < primary_cd then self:SetNextPrimaryFire(primary_cd) end
    elseif secondary == "explode" then
        if not self:TriggerExplosion() then return end
...
local paths = {
    [2] = {Vector(0, 8, -7), Vector(0, -8, 5), -20, 20,},
    [3] = {Vector(0, -8, -7), Vector(0, 8, 5), 20, -20,},
    [4] = {Vector(0, 2, 10), Vector(0, -2, -15), 0, 0,},
}
...
function SWEP:DoAttack(strong, mod)
    ...
    self:SwingAttack({
        ...
        on_start = function() self:EmitSound("SCP0492.Miss") end,
        callback = function(tr, num)
            ...
            if IsValid(ent) then
                ...
                SuppressHostEvents(owner)
                if self.LifeSteal > 0 then owner:AddHealth(math.ceil(dmg * self.LifeSteal)) end
                return strong
            elseif tr.Hit then
                self:EmitSound("SCP0492.HitWall")
...
function SWEP:DoRapidAttack()
    ...
    if CLIENT then return dur * attacks + 0.5 end
    self:DoFastAttack(dur, speed, 0)
    owner:AddTimer("SCP0492_Rapid", dur, attacks - 1, function(this, n) self:DoFastAttack(dur, speed, n) end)
...
function SWEP:DoFastAttack(dur, speed, n)
    ...
    if not fast_attack_trace.Hit then
...
function SWEP:Shoot()
    ...
    projectile.OnHit = function(this, tr)
        if not IsValid(self) or not self:CheckOwner() then return end
        local pos = tr.HitPos
        for i, v in ipairs(player.GetAll()) do
            if v:GetPos():DistToSqr(pos) > self.ShotRadius or not self:CanTargetPlayer(v) then continue end
...
function SWEP:TriggerExplosion(ignore_hp)
    ...
    local owner = self:GetOwner()
    if not ignore_hp and owner:Health() > 50 then return false end
    local ct = CurTime()
    ...
    if SERVER then owner:PushSpeed(self.ExplosionSpeed, self.ExplosionSpeed, -1, "SLC_SCP0492Explosion") end
    return true
end
...
function SWEP:EnableBoost(duration, power)
    ...
    owner:AddTimer("SCP0492Boost", duration, 1, function() owner:PopSpeed("SLC_SCP0492Boost") end)
end
...
function SWEP:DrawSCPHUD()
    ...
    local scp = self:GetSCP049()
    if not IsValid(scp) or not scp:Alive() or scp:SCPTeam() ~= TEAM_SCP then return end
...
SCPHook("SCP0492", "EntityTakeDamage", function(ent, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not dmg:IsDamageType(DMG_BULLET) or not IsValid(ent) or not ent:IsPlayer() or ent:SCPClass() ~= CLASSES.SCP0492 then return end
...
SCPHook("SCP0492", "SLCPostScaleDamage", function(ent, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not IsValid(ent) or not ent:IsPlayer() or ent:SCPClass() ~= CLASSES.SCP0492 then return end
    local wep = ent:GetSCPWeapon()
    if not IsValid(wep) then return end
    local ct = CurTime()
    if wep:GetExplode() >= ct then return true end
    ...
    if IsValid(attacker) and attacker:IsPlayer() and attacker ~= ent then wep.LastAttacker = attacker end
...
SCPHook("SCP0492", "PostEntityTakeDamage", function(ent, dmg)
    ...
    local wep = ent:GetSCPWeapon()
    if not IsValid(wep) or wep:GetExplode() >= CurTime() then return end
...
SCPHook("SCP0492", "SLCButtonOverloaded", function(ply, btn, adv)
    ...
    if wep.Overloads <= 0 then ply:SetSCPDisableOverload(true) end
end)
...
if CLIENT then
    local function add_common(hud)
        ...
        hud:AddSkill("prot"):SetOffset(0.5):SetMaterial("slc/hud/scp/0492/prot.png", "smooth"):SetActiveFunction(function(swep) return swep:GetProtection() >= CurTime() end)
        hud:AddSkill("boost"):SetMaterial("slc/hud/scp/0492/buff.png", "smooth"):SetActiveFunction(function(swep) return swep:GetBoost() >= CurTime() end)
        hud:AddBar("boost_bar"):SetMaterial("slc/hud/scp/0492/buff.png", "smooth"):SetColor(Color(236, 170, 27)):SetTextFunction(function(swep)
            local time = swep:GetBoost() - CurTime()
            if time < 0 then time = 0 end
            return math.Round(time)
        end):SetProgressFunction(function(swep) return (swep:GetBoost() - CurTime()) / swep:GetBoostDuration() end):SetVisibleFunction(function(swep) return swep:GetBoost() >= CurTime() end)
    end

    local hud_normal = SCPHUDObject("SCP0492_normal")
    hud_normal:AddCommonSkills()
    hud_normal:AddSkill("light_attack"):SetButton("attack"):SetMaterial("slc/hud/scp/0492/light.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud_normal:AddSkill("heavy_attack"):SetButton("attack2"):SetMaterial("slc/hud/scp/0492/heavy.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire")
    add_common(hud_normal)
    local hud_assassin = SCPHUDObject("SCP0492_assassin")
    hud_assassin:AddCommonSkills()
    hud_assassin:AddSkill("light_attack"):SetButton("attack"):SetMaterial("slc/hud/scp/0492/light.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud_assassin:AddSkill("rapid"):SetButton("attack2"):SetMaterial("slc/hud/scp/0492/rapid.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire")
    ...
    hud_boomer:AddSkill("explode"):SetButton("attack2"):SetMaterial("slc/hud/scp/0492/explode.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetActiveFunction(function(swep) return swep:GetOwner():Health() <= 50 end)
    add_common(hud_boomer)
    hud_boomer:AddBar("explode_bar"):SetMaterial("slc/hud/scp/0492/explode.png", "smooth"):SetColor(Color(236, 51, 27)):SetTextFunction(function(swep)
        local time = swep:GetExplode() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time)
    end):SetProgressFunction(function(swep) return (swep:GetExplode() - CurTime()) / swep.ExplosionTime end):SetVisibleFunction(function(swep) return swep:GetExplode() > CurTime() end)

    local hud_heavy = SCPHUDObject("SCP0492_heavy")
    hud_heavy:AddCommonSkills()
    hud_heavy:AddSkill("heavy_attack"):SetButton("attack"):SetMaterial("slc/hud/scp/0492/heavy.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud_heavy:AddSkill("shot"):SetButton("attack2"):SetMaterial("slc/hud/scp/0492/shot.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire")
