    ...
    if new == true then
        ...
        if new then
            if self.ChipName ~= new then
                ...
                if chip then self:SetChipData(chip, override) end
            else
...
function SWEP:SetChipData(chip, override)
    ...
    if chip.model then self:SetModel(chip.model) end
end
...
if CLIENT then
    function SWEP:UpdateDescription(id, override)
        ...
        if chip then
            ...
            if self.Lang.NAMES[name] then name = self.Lang.NAMES[name] end
            ...
            if desc then self.Info = self.Info .. string.format("\n%s\n\t%s", self.Lang.hasaccess, desc) end
        end
    end
    ...
    function SWEP:BuildDescription(desc)
        ...
        if cd then desc:Print("\n"):Print(self.Lang.hasaccess):Print("\n\t"):Print(cd, Color(30, 200, 30)) end
    end
end
