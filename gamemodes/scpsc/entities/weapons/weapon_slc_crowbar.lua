...
function SWEP:PrimaryAttack()
    ...
    self:SwingAttack({
        ...
        on_start = function() self:EmitSound("Weapon_Crowbar.Single") end,
        callback = function(tr, num, center)
            ...
            if not IsValid(ent) and num < 3 then
                if center then
                    ...
                    if SERVER then self:DispatchImpactTrace(tr, DMG_CLUB) end
                    return
                end
                return true, {
            ...
            if ent:IsPlayer() then util.Decal("Impact.Flesh", tr.HitPos + tr.HitNormal * 20, tr.HitPos - tr.HitNormal * 20, owner) end
            self:EmitSound("Weapon_Crowbar.Melee_Hit")
            self:PlayActivity(ACT_VM_HITCENTER, self.AttackSpeed)
            if SERVER then
