...
function SWEP:Think()
    ...
    if not self.Eggs then
        self.Eggs = {SpawnSCP3199Egg(owner)}
        self:CheckEggs()
    end

    if self.NextEggCheck < ct then self:CheckEggs() end
    ...
    elseif frenzy >= ct then
        if self.NextFrenzyRegen <= ct then
            ...
            if self.NextFrenzyRegen < ct then self.NextFrenzyRegen = ct + 1 end
        ...
        if self.NextTransmit <= ct then
            ...
            for i, v in ipairs(player.GetAll()) do
                if not self:CanTargetPlayer(v) or v:HasEffect("deep_wounds") or pos:DistToSqr(v:GetPos()) > radius then continue end
            ...
            if #tab > 0 then net.SendTable("SCP3199Frenzy", tab, owner) end
        end
    end
end

local paths = {
    [1] = {Vector(0, 8, -2), Vector(0, -8, 3), -20, 20,},
    [2] = {Vector(0, -8, -2), Vector(0, 8, 3), 20, -20,},
}
...
function SWEP:PrimaryAttack()
    ...
    self:SwingAttack({
        ...
        mins = mins,
        maxs = maxs,
        on_start = function() self:EmitSound("SCP3199.Miss") end,
        callback = function(trace, id, center)
            ...
            if not IsValid(ent) or ent_filter[ent] then return end
            ent_filter[ent] = true
            ...
            if CLIENT or not self:CanTargetPlayer(ent) then return end
        ...
        on_end = function() if SERVER and not any_hit and self:GetFrenzy() > 0 then self:StopFrenzy(hit_wounded) end end
    })
end
...
function SWEP:SecondaryAttack()
    ...
    for i, v in ipairs(player.GetAll()) do
        if v:HasEffect("deep_wounds") and self:CanTargetPlayer(v) then
            ...
            v:AddTimer("SCP3199Slow", slow_dur, 1, function() v:PopSpeed("SLC_SCP3199Slow") end)
            if bleed > 0 then v:ApplyEffect("bleeding", bleed, owner) end
        end
    end
end

function SWEP:SpecialAttack()
    if CLIENT or ROUND.preparing or ROUND.post or not self:GetEggReady() then return end
    local ct = CurTime()
    if self:GetEggTime() >= ct or self:GetFrenzy() > 0 then return end
    local owner = self:GetOwner()
    if not owner:IsOnGround() or owner:Crouching() then return end
    self:CheckEggs()
...
function SWEP:TranslateFOV(fov)
    ...
    if self.FOVIncrease > 0 then return fov + self.FOVIncrease end
end
...
function SWEP:StopFrenzy(wrong)
    ...
    if wrong then self:SetTotalStacks(math.floor(self:GetTotalStacks() * (1 - self.FrenzyPenaltyStacks))) end
    owner:PushSpeed(self.FrenzyPenalty, self.FrenzyPenalty, -1, "SLC_SCP3199Penalty", 1)
    owner:AddTimer("SCP3199Penalty", 5, 1, function() owner:PopSpeed("SLC_SCP3199Penalty") end)
end

function SWEP:CheckEggs()
    local num = 0
    for i, v in rpairs(self.Eggs) do
        if IsValid(v) and not IsValid(v.Locked) then
            num = num + 1
        else
...
function SWEP:EggRespawn()
    ...
    owner:SetPos(pos)
    owner:AddTimer("SCP3199EggPos", 0.2, 5, function() owner:SetPos(pos) end)
    ...
    owner:AddTimer("SCP3199Respawn", self.RespawnTime, 1, function()
        if not IsValid(egg) then
            if not self:EggRespawn() then
...
function SWEP:DrawSCPHUD()
    ...
    for i, v in ipairs(self.FrenzyDetectTable) do
        ...
        if pos.visible then surface.DrawTexturedRect(pos.x - size * 0.5, pos.y - size * 0.5, size, size) end
    end
end
...
Net
---------------------------------------------------------------------------]]
if SERVER then net.AddTableChannel("SCP3199Frenzy") end
if CLIENT then
...
SCPHook("SCP3199", "EntityTakeDamage", function(target, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not IsValid(target) or not target:IsPlayer() or target:SCPClass() ~= CLASSES.SCP3199 then return end
    ...
    if dmg:IsDamageType(DMG_BULLET) then
        ...
        if prot < 1 then dmg:ScaleDamage(prot) end
    end
end)
...
SCPHook("SCP3199", "StartCommand", function(ply, cmd)
    ...
    local wep = ply:GetSCPWeapon()
    if not IsValid(wep) or wep:GetEggTime() < CurTime() then return end
    cmd:ClearMovement()
    cmd:ClearButtons()
...
SCPHook("SCP3199", "SLCMovementAnimSpeed", function(ply, vel, speed, len, movement)
    if ply:SCPClass() ~= CLASSES.SCP3199 then return end
    local n = len / 75
    if n < 1.5 then n = 1.5 end
    return n, true
end)

SCPHook("SCP3199", "PreDrawViewModel", function(vm, ply, wep)
    if ply:SCPClass() ~= CLASSES.SCP3199 then return end
    if not IsValid(wep) or wep:GetClass() ~= "weapon_scp_3199" or wep:GetRespawnTime() < CurTime() then return end
    return true
end)
...
DefineUpgradeSystem("scp3199", {
    ...
    rewards = {
        --18 + 1 points -> 60% = 11 (-1 base) = 10 points
        {2, 1},
        {4, 1},
        {6, 1},
        {9, 1},
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("primary"):SetButton("attack"):SetMaterial("slc/hud/scp/3199/attack.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud:AddSkill("special"):SetButton("attack2"):SetMaterial("slc/hud/scp/3199/special.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetActiveFunction(function(swep) return swep:GetFrenzyStacks() >= swep.SpecialThreshold end):SetTextFunction("GetFrenzyStacks"):SetParser(function(swep, lang)
        return {
            tokens = MarkupBuilder.StaticPrint(swep.SpecialThreshold, color_green),
        }
    end)

    hud:AddSkill("egg"):SetButton("scp_special"):SetMaterial("slc/hud/scp/3199/egg.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack"):SetActiveFunction(function(swep) return swep:GetEggReady() and swep:GetFrenzy() == 0 end):SetTextFunction(function(swep) return (math.Clamp(swep:GetTotalEggs() * swep.EggsProtection, 0, swep.EggsProtectionMax) * 100) .. "%" end):SetParser(function(swep, lang)
        return {
    ...
    hud:AddSkill("passive"):SetMaterial("slc/hud/scp/3199/passive.png", "smooth"):SetOffset(0.5):SetTextFunction("GetTotalStacks"):SetParser(function(swep, lang)
    ...
    hud:AddBar("frenzy_bar"):SetMaterial("slc/hud/scp/3199/frenzy.png", "smooth"):SetColor(Color(240, 194, 45)):SetTextFunction(function(swep)
        local time = swep:GetFrenzy() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep) return (swep:GetFrenzy() - CurTime()) / (swep.FrenzyDuration * swep:GetUpgradeMod("frenzy_duration", 1)) end):SetVisibleFunction(function(swep) return swep:GetFrenzy() > 0 end)

    hud:AddBar("egg_bar"):SetMaterial("slc/hud/scp/3199/egg.png", "smooth"):SetColor(Color(15, 69, 90)):SetTextFunction(function(swep)
        local time = swep:GetEggTime() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep) return (swep:GetEggTime() - CurTime()) / swep.EggTime end):SetVisibleFunction(function(swep) return swep:GetEggTime() > 0 end)

    hud:AddBar("respawn_bar"):SetMaterial("slc/hud/scp/3199/egg.png", "smooth"):SetColor(Color(95, 140, 140)):SetTextFunction(function(swep)
        local time = swep:GetRespawnTime() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep) return (swep:GetRespawnTime() - CurTime()) / swep.RespawnTime end):SetVisibleFunction(function(swep) return swep:GetRespawnTime() > CurTime() end)
end
