...
function SWEP:Think()
    ...
    if not IsValid(owner) or owner:SCPTeam() ~= TEAM_GOC then return end
    ...
    elseif status == false then
        if SERVER then owner:EnableProgressBar(false) end
    else
        local valid, pos, ang = self:TestIndicator()
        if owner:IsHolding(self, "gocdevice_place") then
            if not valid then owner:InterruptHold(self, "gocdevice_place") end
        else
            self.LastPos = pos
            self.LastAng = ang
    ...
    if not IsValid(self.Indicator) then
        self.Indicator = CreateIndicator(self.WorldModel, false, nil, nil, function()
            local wep = LocalPlayer():GetActiveWeapon()
            return not IsValid(wep) or wep:GetClass() ~= "item_slc_goc_device"
        end)
    end
end
...
function SWEP:PrimaryAttack()
    ...
    if not self:TestIndicator() then return end
    owner:StartHold(self, "gocdevice_place", IN_ATTACK, self.PlaceTime)
    if SERVER then owner:EnableProgressBar(true, CurTime() + self.PlaceTime, "lang:WEAPONS.GOCDEVICE.placing", Color(200, 200, 200, 255), Color(50, 225, 25, 255)) end
end
...
function SWEP:TestIndicator()
    ...
    if tr.Hit and pos:WithinAABox(GOC_DEVICE_BOUNDS[1], GOC_DEVICE_BOUNDS[2]) then
        ...
        if not tr2.Hit then valid = true end
    end
    return valid, pos, Angle(0, (owner:GetPos() - pos):Angle().y + 90, 0)
end

hook.Add("SLCCanPickupWeaponClass", "SLCGOCDevicePickup", function(ply, class) if class == "item_slc_goc_device" and ply:SCPTeam() ~= TEAM_GOC then return false, "goc_only" end end)
hook.Add("StartCommand", "SLCGOCDeviceCMD", function(ply, cmd)
    ...
    if IsValid(wep) then
        if ply:IsHolding(wep, "gocdevice_place") then
            cmd:ClearMovement()
            if not wep.ViewAngles then wep.ViewAngles = cmd:GetViewAngles() end
