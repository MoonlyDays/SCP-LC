...
function SWEP:Think()
    ...
    if self.NextPassiveAMN < ct then
        ...
        if oldest and oldest.die_time <= ct then self.Trail:PopFront() end
        for v in self.Trail:Iter() do
            ...
            for i, ply in ipairs(FindInCylinder(v.pos, trail_radius, -32, 64, nil, MASK_SOLID_BRUSHONLY, player.GetAll())) do
                if filter[ply] or ply == owner or not self:CanTargetPlayer(ply) then continue end
                filter[ply] = true
    ...
    if passive < ct and stacks < stacks_max then
        ...
        passive = passive + cd
        if passive < ct then passive = ct + cd end
...
function SWEP:PrimaryAttack()
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        if bleed then dmg_info:SetDamageType(DMG_SLASH) end
...
---------------------------------------------------------------------------]]
local scp939_sounds = {}
if SERVER then util.AddNetworkString("SCP939Sound") end
if CLIENT then
...
if SERVER then
    SCPHook("SCP939", "EntityEmitSound", function(data)
        ...
        local tab = {}
        for i, v in ipairs(player.GetAll()) do
            if v:SCPClass() ~= CLASSES.SCP939 or not v:TestPVS(pos) then continue end
            tab[n] = v
            n = n + 1
        end
...
SCPHook("SCP939", "PostDrawTranslucentRenderables", function()
    ...
    for i, v in rpairs(scp939_sounds) do
        ...
        if f <= 0 then table.remove(scp939_sounds, i) end
    end
...
EFFECTS.RegisterEffect("amnc227", {
    ...
    begin = function(self, ply, tier, args, refresh, decrease)
        ...
        if args[2] and (not self.args[2] or args[2] > self.args[2]) then self.args[2] = args[2] end
    end,
    think = function(self, ply, tier, args)
        if SERVER then
            if not args[2] or ply:SCPTeam() == TEAM_SPEC or ply:SCPTeam() == TEAM_SCP then return end
            local att = args[1]
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("primary"):SetButton("attack"):SetMaterial("slc/hud/scp/939/attack.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud:AddSkill("trail"):SetButton("attack2"):SetMaterial("slc/hud/scp/939/trail.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetTextFunction("GetPassiveStacks")
    hud:AddSkill("special"):SetButton("scp_special"):SetMaterial("slc/hud/scp/939/special.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack")
    hud:AddSkill("passive"):SetOffset(0.5):SetMaterial("slc/hud/scp/939/passive.png", "smooth"):SetCooldownFunction("GetNextPassive")
    hud:AddBar("special_bar"):SetMaterial("slc/hud/scp/939/special.png", "smooth"):SetColor(Color(90, 225, 40)):SetTextFunction(function(swep)
