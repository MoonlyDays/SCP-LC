...
function SWEP:Initialize()
    ...
    if CLIENT then self.PixelVis = util.GetPixelVisibleHandle() end
end
...
function SWEP:Deploy()
    ...
    if SERVER then
        local light = self:GetLight()
        if not IsValid(light) then self:CreateLight() end
    end
    return true
end
...
function SWEP:Holster()
    ...
    if SERVER then
        local light = self:GetLight()
        if IsValid(light) then light:Fire("TurnOff") end
    end
    return true
end
...
function SWEP:PrimaryAttack()
    if self:GetBattery() <= 0 or not IsFirstTimePredicted() then return end
...
function SWEP:DrawWorldModel()
    ...
    local beamalpha = 1
    if dot > 0.4 then beamalpha = math.Clamp((0.8 - dot) / 0.4, 0, 1) end
    if beamalpha > 0 then beamrenderer[#beamrenderer + 1] = {pos, light_trace.HitPos, beamalpha} end
