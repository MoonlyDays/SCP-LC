...
function SWEP:SetupDataTables()
    ...
    if CLIENT then self:NetworkVarNotify("Charges", function(ent, name, old, new) self.PrintName = string.format(self.Lang.name, new) end) end
end
...
function SWEP:Equip()
    if CLIENT then self.PrintName = string.format(self.Lang.name, self:GetCharges()) end
    self:StopHeal(0.5)
end
...
function SWEP:Think()
    ...
    if owner:IsHolding(self, "medkit_heal") and (not self:IsValidHealTarget(self.HealTarget) or ROUND.post) then
        self:StopHeal()
        return
    end
    ...
    if status == true then
        ...
        if isnumber(override) then heal = override end
        ...
        if charges <= 0 then self:Remove() end
...
function SWEP:PopSpeed()
    if IsValid(self.HealTarget) then self.HealTarget:PopSpeed("SLC_Medkit") end
    local owner = self:GetOwner()
    if IsValid(owner) then
...
function SWEP:Heal(ply, key)
    if not self:IsValidHealTarget(ply) then return end
    local owner = self:GetOwner()
    ...
    if SERVER then
        ...
        if ply ~= owner then ply:PushSpeed(0.2, 0.2, -1, "SLC_Medkit") end
    end
