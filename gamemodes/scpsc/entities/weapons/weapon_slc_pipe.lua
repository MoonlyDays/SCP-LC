...
function SWEP:OnDrop()
    if SERVER and self:GetHits() <= 0 then self:Remove() end
end
...
function SWEP:DoAttack()
    ...
    if not IsValid(trace_tab.Entity) then util.TraceHull(trace_tab) end
    ...
    if trace_tab.Hit then
        ...
        if hits <= 0 then self.PreventDropping = true end
    end

    local ent = trace_tab.Entity
    if not IsValid(ent) then
        if trace_tab.Hit then self:EmitSound("Weapon_Crowbar.Melee_HitWorld") end
        return
    end
