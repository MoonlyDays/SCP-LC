...
local OPTIONS = {"scan", "tesla", "intercom", "vent"}
local DURATION = {
	scan = 45,
	tesla = 10,
	intercom = 15,
...
function SWEP:Deploy()
	if self:GetState() ~= STATE.PROGRESS then self:ResetAction(STATE.LOADING, self.LoadingDuration) end
	--self:ResetViewModelBones()
end

function SWEP:Holster()
	if self:GetState() == STATE.MALO then return false end
	return true
end
...
function SWEP:PrimaryAttack()
	...
	self:QueueAction(STATE.PRESS, 1.5, function(finish, dur) self:SendWeaponAnim(ACT_VM_PRIMARYATTACK) end)
	self:QueueAction(STATE.PROGRESS, DURATION[option], function(finish, dur)
		...
		if selected == 1 then
			if SERVER then
				...
				AddTimer("ComTabFacilityScan", dur, 1, function()
					if IsValid(self) then self:ResetAction(STATE.IDLE, 0) end
					local teams = {}
					for k, v in pairs(SLCZones.GetPlayersByFlag(ZONE_FLAG_FACILITY)) do
						local t = v:SCPTeam()
						if not teams[t] then teams[t] = {} end
		...
		elseif selected == 2 then
			if SERVER then
				...
				AddTimer("ComTabTeslaRequest", dur, 1, function()
					...
					if IsValid(btn) then btn:Fire("PressIn") end
					G_COM_TABLET_DATA.notif_tesla = CurTime() + 60
					ComTab_Transmit()
					AddTimer("ComTabTeslaReset", 60, 1, function() if IsValid(btn) then btn:Fire("PressOut") end end)
				end)
			end
		...
		elseif selected == 4 then
			...
			AddTimer("ComTabVent", dur, 1, function() DisableZoneVentilation(ZONE_ALL) end)
		end
	end)
end

hook.Add("PlayerSpeakOverride", "SLCCommanderIntercom", function(talker)
	if not G_COM_TABLET_DATA or not IsValid(G_COM_TABLET_DATA.IntercomPlayer) then return end
	local wep = G_COM_TABLET_DATA.IntercomPlayer:GetActiveWeapon()
	if not IsValid(wep) or wep:GetClass() ~= "item_slc_commander_tablet" then
		G_COM_TABLET_DATA.IntercomPlayer = nil
		TransmitSound("scp_lc/misc/v_end.ogg", true, 1)
		UnPausePA()
		return
	...
	if talker == G_COM_TABLET_DATA.IntercomPlayer then return true end
end)
...
hook.Add("SLCRoundCleanup", "SLCCommanderTablet", function() G_COM_TABLET_DATA = {} end)
if SERVER then
	util.AddNetworkString("SLCComTabletUpdate")
	function ComTab_Transmit()
		local tab = {}
		for i, v in ipairs(player.GetAll()) do
			if v:HasWeapon("item_slc_commander_tablet") then table.insert(tab, v) end
		end
...
if CLIENT then
	net.Receive("SLCComTabletUpdate", function(len) G_COM_TABLET_DATA = net.ReadTable() end)
	...
	function SWEP:DrawScreen(x, y, w, h)
		...
		if alpha < 1 then surface.SetAlphaMultiplier(alpha) end
		...
		if t_cd and t_cd > ct then draw.SimpleText(self.Lang.tesla_deactivated .. string.ToMinutesSeconds(math.ceil(t_cd - ct)), "SLCTabletLarge", x + w * 0.5, y + 16, t_cd - 10 < ct and math.floor(ct) % 2 == 1 and color_red or color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP) end
		...
		for i, option in ipairs(OPTIONS) do
			...
			if i == selected then draw.SimpleText(pointer, "SLCTabletLarge", m_x, m_y, color or color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM) end
			local cd = G_COM_TABLET_DATA["t_cd_" .. option]
			if cd == true then
