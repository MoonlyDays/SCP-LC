...
function SWEP:InitializeLanguage(name)
    if CLIENT then self.Lang = LANG.WEAPONS[name] or {} end
end

function SWEP:InitializeHUD(name)
    if SERVER or not name and not self.HUDName then return end
...
function SWEP:Deploy()
    ...
    if IsValid(owner) then
        ...
        if SERVER then owner:DrawWorldModel(self.ShouldDrawWorldModel) end
...
function SWEP:SCPDamageEvent(ent, dmg)
    if SERVER then hook.Run("SCPDamage", self:GetOwner(), ent, dmg) end
end

function SWEP:CheckOwner()
    local owner = self:GetOwner()
    if not IsValid(owner) then return false end
    if not self.OwnerSignature then
        self.OwnerSignature = owner:TimeSignature()
        return true
    end
...
function SWEP:CanTargetPlayer(ply)
    local t = ply:SCPTeam()
    return t ~= TEAM_SPEC and not SCPTeams.IsAlly(TEAM_SCP, t) and not ply:HasGodMode() and not ply:HasEffect("spawn_protection")
end
...
function SWEP:DamageEvent()
    ...
    if SERVER then
        ...
        if IsValid(ent) and not ent:IsPlayer() then self:SCPDamageEvent(ent, 50) end
    end
end
...
function SWEP:HUDNotify(text, time, snd)
    ...
    if not self.HUDObject then return end
    self.HUDObject:Notify(text, time or 1, snd)
end

function SWEP:DrawHUD()
    if not hook.Run("SLCShouldDrawSCPHUD") then return end
    if self.HUDObject then self.HUDObject:Draw() end
    if self.DrawSCPHUD then self:DrawSCPHUD() end
end
...
if SERVER then
    ...
    hook.Add("PostEntityTakeDamage", "SLCGenericSCPDamage", function(target, dmg, took)
        ...
        if not IsValid(att) or not att:IsPlayer() or att == target or att:SCPTeam() ~= TEAM_SCP then return end
        local dmg_num = math.max(dmg:GetDamage(), dmg:GetDamageCustom())
        if dmg_num == 0 then return end
        local wep = att:GetSCPWeapon()
        if not IsValid(wep) then return end
        if wep.OnPlayerDamaged then wep:OnPlayerDamaged(dmg_num, target) end
        if not wep.UpgradeSystemMounted or not wep.ScoreOnDamage then return end
        wep:AddScore(dmg_num)
    end)

    hook.Add("SLCPlayerDeath", "SLCGenericSCPKill", function(ply, att)
        if not IsValid(ply) or not ply:IsPlayer() or not IsValid(att) or not att:IsPlayer() or att == ply or att:SCPTeam() ~= TEAM_SCP then return end
        local wep = att:GetSCPWeapon()
        if not IsValid(wep) then return end
        if wep.UpgradeSystemMounted and wep.ScoreOnKill then wep:AddScore(1) end
        if wep.OnPlayerKilled then wep:OnPlayerKilled(att) end
    end)
end
