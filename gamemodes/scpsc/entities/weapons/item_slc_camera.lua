...
function SWEP:Think()
    ...
    if #self.CCTV ~= #CCTV then
        for i, v in ipairs(ents.FindByClass("slc_cctv")) do
            --print( v, v:GetCam() )
            if v.GetCam then self.CCTV[v:GetCam()] = v end
        end
    end
...
function SWEP:SecondaryAttack()
    ...
    if CAM > #CCTV then CAM = 1 end
    self:SetCAM(CAM)
    self.NextChange = CurTime() + 0.1
end

function SWEP:Reload()
    if not self:GetEnabled() or not IsValid(self.CCTV[self:GetCAM()]) then return end
...
function SWEP:Scan()
    ...
    if not IsValid(self.CCTV[CAM]) then return end
    local detected = {}
...
function SWEP:DrawWorldModel()
    if not IsValid(self:GetOwner()) then self:DrawModel() end
end
...
function SWEP:DrawHUD()
    if self:GetEnabled() then
        ...
        local CAM = self:GetCAM()
        local w, h = ScrW(), ScrH()
        if not IsValid(self.CCTV[CAM]) then
