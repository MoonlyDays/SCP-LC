...
function SWEP:Think()
	...
	if self.NextPassive <= ct and self:GetPassiveCooldown() < ct then
		...
		if self.NextPassive <= ct then self.NextPassive = self.NextPassive + 0.25 end
		local burn_dmg = self.PassiveDamage * self:GetUpgradeMod("burn_power", 1)
		for i, v in ipairs(player.GetAll()) do
			if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(pos) > self.PassiveRadius or v:IsBurning() then continue end
		...
		for i, v in ipairs(player.GetAll()) do
			if self.Preys[v] or not self:CanTargetPlayer(v) or invis_radius > 0 and v:GetPos():DistToSqr(pos) > invis_radius or not owner:TestVisibility(v) then continue end
			self:AddPrey(v)
		end
	...
	elseif drain >= ct and self.NextDrain <= ct then
		...
		if self.NextDrain <= ct then self.NextDrain = ct + drain_tick end
		...
		for i, v in ipairs(player.GetAll()) do
			if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(pos) > drain_radius then continue end
			any = true
		...
		if not any then self:SetDrain(1) end
		local heal = self:GetUpgradeMod("drain_heal", 0)
		if drained > 0 and heal > 0 then owner:AddHealth(math.ceil(drained * heal)) end
	end
end
...
function SWEP:PrimaryAttack()
	...
	for i, v in ipairs(player.GetAll()) do
		if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(pos) > radius then continue end
...
function SWEP:SpecialAttack()
	if CLIENT or ROUND.preparing or ROUND.post then return end
	if not self:CheckPreys() then return end
	local ct = CurTime()
	local owner = self:GetOwner()
	...
	local new_preys = {}
	for i, v in ipairs(player.GetAll()) do
		if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(prey_pos) > rng_radius or v:GetSCP714() or v:IsInSafeSpot() then continue end
		table.insert(new_preys, v)
	end

	self:SetNextSpecialAttack(ct + 5)
	local new_prey = new_preys[SLCRandom(#new_preys)]
	if not new_prey then return end
	owner:Blink(0, 0.5)
	timer.Simple(0.4, function()
		if not IsValid(self) or not IsValid(new_prey) or not self:CheckOwner() then return end
...
function SWEP:AddPrey(ply)
	...
	ply:PlayAmbient("023", true, false, function(p) return not IsValid(self) end)
...
function SWEP:CheckPreys()
	...
	for k, v in pairs(self.Preys) do
		if not IsValid(k) then
			self.Preys[k] = nil
		elseif not k:CheckSignature(v) then
			self.Preys[k] = nil
			k:StopAmbient("023")
		else
			count = count + 1
...
function SWEP:OnUpgradeBought(name, active, group)
	...
	if IsValid(self.Clone) then self.Clone:SetInvisRadius(radius) end
end
...
SCPHook("SCP023", "ShouldCollide", function(ent1, ent2) if ent1:IsPlayer() and ent1:SCPClass() == CLASSES.SCP023 or ent2:IsPlayer() and ent2:SCPClass() == CLASSES.SCP023 then if ent1.ignorecollide106 or ent2.ignorecollide106 then return false end end end)
SCPHook("SCP023", "EntityTakeDamage", function(ent, dmg)
	if dmg:IsDamageType(DMG_DIRECT) or not dmg:IsDamageType(DMG_BULLET) or not IsValid(ent) or not ent:IsPlayer() or ent:SCPClass() ~= CLASSES.SCP023 then return end
...
DefineUpgradeSystem("scp023", {
	...
	rewards = {
		--16 + 1 points -> ~60% = 11 (-1 base) = 10 points
		{50, 1},
		{100, 1},
		{175, 1},
		{250, 1},
...
if CLIENT then
	...
	hud:AddCommonSkills()
	hud:AddSkill("drain"):SetButton("attack"):SetMaterial("slc/hud/scp/023/drain.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire"):SetActiveFunction(function(swep) return swep:GetDrain() == 0 end)
	hud:AddSkill("clone"):SetButton("attack2"):SetMaterial("slc/hud/scp/023/clone.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetTextFunction(function(swep) return swep:GetClone() and "*" or "" end)
	hud:AddSkill("hunt"):SetButton("scp_special"):SetMaterial("slc/hud/scp/023/hunt.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack"):SetTextFunction("GetPreys"):SetActiveFunction(function(swep) return swep:GetPreys() > 0 end)
	hud:AddSkill("passive"):SetOffset(0.5):SetMaterial("slc/hud/scp/023/passive.png", "smooth"):SetCooldownFunction("GetPassiveCooldown")
	hud:AddBar("drain_bar"):SetMaterial("slc/hud/scp/023/drain.png", "smooth"):SetColor(Color(45, 55, 120)):SetTextFunction(function(swep)
		local time = swep:GetDrain() - CurTime()
		if time < 0 then time = 0 end
		return math.Round(time, 1) .. "s"
	end):SetProgressFunction(function(swep) return (swep:GetDrain() - CurTime()) / (swep.DrainDuration * swep:GetUpgradeMod("drain_dur", 1)) end):SetVisibleFunction(function(swep) return swep:GetDrain() > 0 end)
end
