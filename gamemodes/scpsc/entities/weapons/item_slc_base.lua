...
function SWEP:SetupDataTables()
    ...
    if self.Durability > 0 then
        self:NetworkVar("Int", "Durability")
        self:SetDurability(self.Durability)
    end
end

function SWEP:InitializeLanguage()
    if SERVER or not self.Language then return end
    self.Lang = LANG.WEAPONS[self.Language] or {}
    if not self.Lang then return end
...
function SWEP:CanEnable()
    if not self.UseGroup then return true end
...
function SWEP:HolsterThink()
    if self.HolsterBatteryUsage then self:BatteryTick() end
end
...
function SWEP:BatteryTick()
    if CLIENT or not self.HasBattery or self.BatteryUsage <= 0 or (self.Toggleable and not self:GetEnabled()) then return end
    if self.NBatteryTake > CurTime() then return end
    self.NBatteryTake = CurTime() + self.BatteryUsage
    local battery = self:GetBattery()
    if battery > 0 then self:SetBattery(battery - 1) end
...
function SWEP:Stack()
    ...
    if count < self.Stacks then self:SetCount(count + 1) end
end
...
function SWEP:EquipAmmo(ply)
    ...
    if IsValid(wep) and wep.Stack then wep:Stack() end
end

function SWEP:DragAndDrop(wep, test)
    if not self.HasBattery or not wep:IsDerived("item_slc_battery") then return false end
    if self:GetBattery() >= (wep.Power or 100) then return false end
    if not SERVER or test then return true end
...
function SWEP:Damage(dmg)
    ...
    if dur <= 0 and self.Toggleable then self:SetEnabled(false) end
end
...
function SWEP:StoreWeapon(data)
    ...
    if self.HasBattery then data.battery = self:GetBattery() end
end

function SWEP:RestoreWeapon(data)
    if data.stacks then self:SetCount(data.stacks) end
    if data.battery then self:SetBattery(data.battery) end
end

function SWEP:DrawWorldModel()
    if not IsValid(self:GetOwner()) or self.ShouldDrawWorldModel then self:DrawModel() end
end
