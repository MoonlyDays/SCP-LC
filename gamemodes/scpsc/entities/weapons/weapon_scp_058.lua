...
function SWEP:PrimaryAttack()
    ...
    self:SwingAttack({
        ...
        callback = function(trace)
            ...
            if not trace.Hit or not IsValid(ent) then return end
            if SERVER and ent:IsPlayer() and self:CanTargetPlayer(ent) then
                ...
                if self:GetUpgradeMod("primary_poison") == true then
                    ...
                    if self:GetUpgradeMod("pp_insta2") == true and not ent:HasEffect("poison") then
    ...
    if SERVER then
        ...
        projectile.OnHit = function(proj, trace)
            ...
            if not mode then
                ...
                if trace.HitPos:DistToSqr(owner_pos) < self.ShotMinDistance then damage = damage * 0.4 end
                local direct
                if IsValid(trace.Entity) and trace.Entity:IsPlayer() and self:CanTargetPlayer(trace.Entity) then
                    ...
                    if poison and not direct:CheckHazmat(100) then direct:ApplyEffect("poison", owner, pdmg) end
                end
                ...
                for i, v in ipairs(player.GetAll()) do
                    local pos = v:GetPos()
                    if pos:DistToSqr(hit_pos) > radius or not self:CanTargetPlayer(v) then continue end
                    ...
                    if poison and not v:CheckHazmat(75) then v:ApplyEffect("poison", owner, pdmg) end
                end
            elseif mode == 1 then
            ...
            elseif mode == 2 then
                local ent = trace.Entity
                if not IsValid(ent) or not ent:IsPlayer() or not self:CanTargetPlayer(ent) or ent:CheckHazmat(60) then return end
...
function SWEP:SpecialAttack()
    if CLIENT or ROUND.post or not self:GetExplode() then return end
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        if use_poison and not v:CheckHazmat(200) then v:ApplyEffect("poison", 2, owner, 0) end
    end
end
...
function SWEP:OnUpgradeBought(name, active, group)
    if CLIENT and group then
        ...
        if skill then skill:SetMaterialOverride(Material(active, "smooth")) end
    end

    if name == "shot31" then
        self.Secondary.Automatic = true
    elseif name == "exp1" then
        if SERVER then self.ExplosionThreshold = math.max(math.floor(self:GetOwner():Health() / 1000), 1) end
    end
end
...
SCPHook("SCP058", "PostEntityTakeDamage", function(ent, dmg, took)
    if not IsValid(ent) or not ent:IsPlayer() or ent:SCPClass() ~= CLASSES.SCP058 then return end
    local wep = ent:GetSCPWeapon()
    if not IsValid(wep) or not wep.ExplosionThreshold then return end
...
DefineUpgradeSystem("scp058", {
    ...
    upgrades = {
        ...
        },
        {
            ...
            active = "slc/hud/scp/058/shot2.png",
            group = "shot"
        },
        {
            name = "shot11",
        ...
        },
        {
            ...
            active = "slc/hud/scp/058/shot3.png",
            group = "shot"
        },
        {
            name = "shot22",
        ...
        },
        {
            ...
            active = "slc/hud/scp/058/shot4.png",
            group = "shot"
        },
        {
            name = "shot32",
        ...
        {
            name = "exp2",
            icon = icons.explosion2,
            cost = 2,
            req = {"exp1"},
            reqany = false,
            pos = {5, 2},
            mod = {
                explosion_poison = true,
                explosion_radius = 1.5
            },
            active = "slc/hud/scp/058/explosion2.png",
            group = "explosion"
        },
        {
            name = "outside_buff",
    ...
    rewards = {
        --13/14 + 1 points -> ~60% = 10 (-1 base) = 9 points
        {100, 1},
        {200, 1},
        {300, 1},
        {400, 1},
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("primary_attack"):SetButton("attack"):SetMaterial("slc/hud/scp/058/attack.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud:AddSkill("shot"):SetButton("attack2"):SetMaterial("slc/hud/scp/058/shot1.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetActiveFunction(function(swep) return swep:GetShotStacks() > 0 end)
    hud:AddSkill("explosion"):SetButton("scp_special"):SetMaterial("slc/hud/scp/058/explosion1.png", "smooth"):SetVisibleFunction(function(swep) return swep:HasUpgrade("exp1") end):SetActiveFunction("GetExplode"):SetProgressFunction(function(swep)
    ...
    hud:AddSkill("shot_stacks"):SetOffset(0.5):SetMaterial("slc/hud/scp/058/shot1.png", "smooth"):SetCooldownFunction("GetNextStack"):SetTextFunction("GetShotStacks")
end
