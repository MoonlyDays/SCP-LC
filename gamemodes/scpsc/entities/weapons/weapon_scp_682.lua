...
function SWEP:TranslateActivity(act)
    if act == ACT_MP_WALK or act == ACT_MP_RUN then return controller.IsEnabled(self:GetOwner()) and ACT_HL2MP_RUN or ACT_HL2MP_WALK end
...
function SWEP:Think()
    ...
    if CLIENT and self:GetCharging() and not controller.IsEnabled(owner) then controller.Start(owner, "scp682_charge") end
...
function SWEP:ShieldThink()
    ...
    elseif shield_cd > 0 and self:HasUpgrade("shield_b") and self.NextRegen <= ct then
        ...
        if self.NextRegen <= ct then self.NextRegen = ct + 1 end
...
function SWEP:ChargeThink()
    local ct = CurTime()
    local owner = self:GetOwner()
    if not self:GetCharging() and not controller.IsEnabled(owner) then return end
    ...
    if not trace_tab.Hit or trace_tab.HitNormal.z >= 0.5 then return end
    ...
    if IsValid(ent) then
        ...
        local door_ent = ent
        if door_ent:GetClass() == "prop_dynamic" then door_ent = door_ent:GetParent() end
        local class = door_ent:GetClass()
        if class_whitelist[class] then return end
        if (class == "func_door" or class == "func_door_rotating") and (self.FullSpeed and DestroyDoor(door_ent, owner:GetAngles()) or IsDoorDestroyed(door_ent)) then return end
        if ent:IsPlayer() then
            ...
            if not ent:Alive() and IsValid(ent._RagEntity) and IsValid(ent._RagEntity:GetPhysicsObject()) then ent._RagEntity:GetPhysicsObject():SetVelocity(trace_tab.Normal * 3000) end
            return
        end
    end
    ...
    if self:GetNextSecondaryFire() - 10 < ct then self:SetNextSecondaryFire(ct + 10) end
    ...
    owner:AddTimer("SCP682ChargeSlow", 10 * self:GetUpgradeMod("charge_stun", 1), 1, function() owner:PopSpeed("SLC_SCP682Penalty") end)
...
function SWEP:PrimaryAttack()
    ...
    if IsValid(ent) then
        ...
        if ent:IsPlayer() then
            ...
            if self:HasUpgrade("attack_3") then ent:ApplyEffect("bleeding", owner) end
        else
...
function SWEP:SecondaryAttack()
    ...
    owner:StartHold(self, "scp682_bite", IN_ATTACK2, time, function() self:PerformBiteAttack() end, true)
end
...
function SWEP:PerformBiteAttack()
    ...
    if self:GetNextPrimaryFire() - 2.5 < ct then self:SetNextPrimaryFire(ct + 2.5) end
    if self:GetNextSpecialAttack() - 2.5 < ct then self:SetNextSpecialAttack(ct + 2.5) end
    local owner = self:GetOwner()
    ...
    owner:AddTimer("SCP682Bite", 1.75, 1, function() owner:StopDisableControls("scp682_bite") end)
    ...
    for i, v in ipairs(player.GetAll()) do
        if v == owner or not self:CanTargetPlayer(v) then continue end
        ...
        if has_upgrade then
            ...
            if pct >= 1 and not v:HasEffect("fracture") then v:ApplyEffect("fracture") end
        end
    end
...
function SWEP:SpecialAttack()
    local owner = self:GetOwner()
    if CLIENT or ROUND.preparing or ROUND.post or not owner:IsOnGround() or not self:HasUpgrade("charge_1") or owner:IsHolding(self, "scp682_bite") then return end
    ...
    if self:GetCharging() then
        ...
        if self:GetNextSecondaryFire() - 3 < ct then self:SetNextSecondaryFire(ct + 3) end
        return
    end
...
function SWEP:OnUpgradeBought(name, active, group)
    if group == "shield" then
        ...
        if self:GetShield() == self.DefaultShield then self:SetShieldCooldown(CurTime()) end
...
SCPHook("SCP682", "SLCPostScaleDamage", function(target, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or dmg:IsDamageType(DMG_FALL) or not IsValid(target) or not target:IsPlayer() or target:SCPClass() ~= CLASSES.SCP682 then return end
    ...
    if shield <= 0 then
        if apply_cdr then wep:SetShieldCooldown(cd - wep:GetUpgradeMod("shield_cdr") * dmg:GetDamage()) end
        return
    end
    ...
    local to_block = orig
    if has_d then to_block = math.ceil(to_block * wep:GetUpgradeMod("shield_pct")) end
    if to_block > orig then to_block = orig end
    if to_block > shield then to_block = shield end
    shield = shield - to_block
    local new_dmg = orig - to_block
    dmg:SetDamage(new_dmg)
    wep:SetShield(shield)
    if apply_cdr and new_dmg > 0 then wep:SetShieldCooldown(cd - wep:GetUpgradeMod("shield_cdr") * new_dmg) end
    local owner = wep:GetOwner()
    local inflictor = dmg:GetInflictor()
    if has_d and valid_att and wep:CanTargetPlayer(att) and (not IsValid(inflictor) or inflictor:GetClass() ~= "slc_turret") then att:TakeDamage(to_block * wep:GetUpgradeMod("reflect_pct"), owner, owner) end
    if (not has_b and not has_c and not has_d or shield == 0) and cd == 0 then
        ...
        if has_c then
            ...
            if owner:Health() > new_max then owner:SetHealth(new_max) end
        end
    end
    ...
    if shield == 0 then
        if wep:HasUpgrade("shield_1") then
            ...
            owner:AddTimer("SCP682Boost", wep:GetUpgradeMod("shield_speed_dur"), 1, function() owner:PopSpeed("SLC_SCP682Boost") end)
        end

        --owner:DoAnimationEvent( ACT_GESTURE_BIG_FLINCH )
        timer.Simple(0, function()
            --Jump out of prediction
...
SCPHook("SCP682", "SLCScaleSpeed", function(ply, mod)
    ...
    if not IsValid(wep) or not ply:IsHolding(wep, "scp682_bite") then return end
...
SCPHook("SCP682", "CalcMainActivity", function(ply, vel)
    ...
    if state == STATE_STUNNED then return ACT_INVALID, ply:LookupSequence("stun") end
end)
...
SCPHook("SCP682", "SLCMovementAnimSpeed", function(ply, vel, speed, len, movement)
    if ply:SCPClass() ~= CLASSES.SCP682 or not controller.IsEnabled(ply) then return end
...
SCPHook("SCP682", "SLCFootstepParams", function(ply, st, vel, crouch)
    ...
    if len > 125 then units = units + len / 5 end
    if crouch then units = units - 20 end
    return units
end)
...
if CLIENT then
    SCPHook("SCP682", "ScalePlayerDamage", function(ply, group, dmg)
        if not IsValid(ply) or ply:SCPClass() ~= CLASSES.SCP682 then return end
        local wep = ply:GetSCPWeapon()
        if not IsValid(wep) or wep:GetShield() <= 0 then return end
...
DefineUpgradeSystem("scp682", {
    ...
    upgrades = {
        {
            name = "shield_a", -- Empowered
            cost = 2,
            req = {},
            block = {"shield_b", "shield_c", "shield_d"},
            reqany = false,
            pos = {1, 1},
            mod = {
                shield = 1.6,
        ...
        },
        {
            ...
            cost = 2,
            req = {},
            block = {"shield_a", "shield_c", "shield_d"},
            reqany = false,
            pos = {1, 2},
            mod = {
                shield = 0.8,
            ...
            icon = icons.shield_b,
            active = {icons.shield_b, Color(20, 157, 20)},
            group = "shield"
        },
        {
            name = "shield_c", -- Sacri
            cost = 2,
            req = {},
            block = {"shield_a", "shield_b", "shield_d"},
            reqany = false,
            pos = {1, 3},
            mod = {
                shield_cd = 0.5,
            ...
            icon = icons.shield_c,
            active = {icons.shield_c, Color(183, 0, 0)},
            group = "shield"
        },
        {
            name = "shield_d", -- Refelct
            cost = 2,
            req = {},
            block = {"shield_a", "shield_b", "shield_c"},
            reqany = false,
            pos = {1, 4},
            mod = {
                shield = 0.6,
            ...
            icon = icons.shield_d,
            active = {icons.shield_d, Color(173, 0, 163)},
            group = "shield"
        },
        {
            name = "shield_1",
    ...
    rewards = {
        --13 + 1 points -> 60% = 10 (-1 base) = 9 points
        {75, 1},
        {150, 1},
        {250, 1},
        {350, 1},
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("primary"):SetButton("attack"):SetMaterial("slc/hud/scp/682/attack.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud:AddSkill("secondary"):SetButton("attack2"):SetMaterial("slc/hud/scp/682/bite.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire")
    hud:AddSkill("charge"):SetButton("scp_special"):SetMaterial("slc/hud/scp/682/charge.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack"):SetActiveFunction(function(swep) return swep:HasUpgrade("charge_1") end)
    hud:AddSkill("shield"):SetOffset(0.5):SetMaterial("slc/hud/scp/682/shield.png", "smooth"):SetCooldownFunction("GetShieldCooldown")
    hud:AddBar("shield_bar"):SetMaterial("slc/hud/scp/682/shield.png", "smooth"):SetColor(Color(110, 40, 205)):SetTextFunction(function(swep) return math.Round(swep:GetShield()) end):SetProgressFunction(function(swep)
        local max_shield
    ...
    end):SetVisibleFunction(function(swep) return swep:GetShield() > 0 end)
end
...
controller.Register("scp682_charge", {
    StartCommand = function(self, ply, cmd)
        if not self.Angle then self.Angle = cmd:GetViewAngles() end
        local ang = cmd:GetViewAngles()
        local diff = ang - self.Angle
        diff:Normalize()
        local abs_diff = math.sqrt(diff.y * diff.y + diff.p * diff.p)
        local max_diff = 18 * FrameTime()
        if abs_diff > max_diff then diff:Div(abs_diff / max_diff, max_diff) end
        self.Angle:Add(diff)
    ...
    Move = function(self, ply, mv)
        ...
        if not self.Gravity then self.Gravity = Vector(0) end
