...
function SWEP:Think()
	...
	if self:GetStealth() > ct and owner:IsInZone(ZONE_FLAG_RESTRICT173) then self:SetStealth(0) end
	if self:GetInStealth() then
		if self:GetStealth() < ct then self:DisableStealth() end
		return
	end

	if not self.KillerEntity then self.KillerEntity = owner end
	...
	for i, v in ipairs(player.GetAll()) do
		...
		if not v:Alive() or team == TEAM_SPEC or team == TEAM_SCP or v:GetBlink() or not test_ent:TestVisibility(v) then continue end
		freeze = true
...
function SWEP:UpdateFreeze(freeze, restrict)
	...
	if not freeze then
		...
		if self:GetStealth() == -1 then self:EnableStealth() end
	else
	...
	if restrict then self.RestrictFreeze = true end
end
...
function SWEP:KillThink()
	...
	for i, v in ipairs(player.GetAll()) do
		if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(killer_pos) > kill_dist then continue end
...
function SWEP:PrimaryAttack()
	...
	for i, v in ipairs(player.GetAll()) do
		if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(emit_pos) > gas_dist then continue end
		gas_trace.endpos = v:GetPos() + gas_offset
		util.TraceLine(gas_trace)
		if not gas_trace.Hit then v:ApplyEffect("gas_choke") end
	end
end
...
function SWEP:SecondaryAttack()
	...
	for i, v in rpairs(self.Decoys) do
		if not IsValid(v) or v.Attacked then table.remove(self.Decoys, i) end
	end
...
function SWEP:SpecialAttack()
	...
	if self:GetStealth() > CurTime() then self:SetStealth(0) end
...
function SWEP:DisableStealth()
	...
	for i, v in ipairs(player.GetAll()) do
		...
		if visible and not v:GetBlink() then return end
		if dist <= self.StealthOuterRange then all_out = false end
		if visible then any_can_see = true end
	end

	if any_can_see and not owner:IsInZone(ZONE_FLAG_RESTRICT173) then
...
SCPHook("SCP173", "EntityTakeDamage", function(ent, dmg)
	if dmg:IsDamageType(DMG_DIRECT) or not dmg:IsDamageType(DMG_BULLET) or not IsValid(ent) or not ent:IsPlayer() or ent:SCPClass() ~= CLASSES.SCP173 then return end
...
SCPHook("SCP173", "ShouldCollide", function(ent1, ent2)
	...
	if ent1:IsPlayer() and ent1:SCPClass() == CLASSES.SCP173 then
		if not ent2.ignorecollide106 then return end
		ply = ent1
	elseif ent2:IsPlayer() and ent2:SCPClass() == CLASSES.SCP173 then
		if not ent1.ignorecollide106 then return end
		ply = ent2
	end

	if not ply then return end
	local wep = ply:GetSCPWeapon()
	if not IsValid(wep) or not wep:GetInStealth() then return end
...
SCPHook("SCP173", "PlayerUse", function(ply, ent)
	...
	local wep = ply:GetSCPWeapon()
	if not IsValid(wep) or (not wep:GetFreeze() and not wep:GetInStealth()) then return end
	return false
end)
...
SCPHook("SCP173", "Move", function(ply, mv)
	if ply:SCPClass() ~= CLASSES.SCP173 then return end
	local wep = ply:GetSCPWeapon()
	if not IsValid(wep) or not wep:GetFreeze() then return end
	...
	if ply:IsInZone(ZONE_SURFACE) then move_dist = move_dist * 1.3 end
...
SCPHook("SCP173", "SLCPlayerFootstep", function(ply, foot, snd)
	...
	local ct = CurTime()
	if not ply.Next173Step or ply.Next173Step < ct then
...
if CLIENT then
	SCPHook("SCP173", "ScalePlayerDamage", function(ply, group, dmg)
		if not IsValid(ply) or ply:SCPClass() ~= CLASSES.SCP173 then return end
		return true
	end)
	...
	SCPHook("SCP173", "PreDrawEffects", function()
		...
		if ply:IsInZone(ZONE_SURFACE) then radius = radius * 1.3 end
		local prev_s = 0
		local prev_c = 1
		for i = 1, steps do
			local s = i == steps and 0 or math.sin(ang_diff * i)
			local c = i == steps and 1 or math.cos(ang_diff * i)
			render.DrawQuad(Vector(pos.x + prev_s * radius, pos.y + prev_c * radius, pos.z + 30), Vector(pos.x + s * radius, pos.y + c * radius, pos.z + 30), Vector(pos.x + s * radius, pos.y + c * radius, pos.z), Vector(pos.x + prev_s * radius, pos.y + prev_c * radius, pos.z), ring_color)
			prev_s = s
			prev_c = c
		end
	end)
	...
	hook.Add("SLCGamemodeLoaded", "SCP173ThermoFilter", function() SLC_THERMAL_FILTER["models/scp/173.mdl"] = true end)
end
...
DefineUpgradeSystem("scp173", {
	...
	rewards = {
		--19 + 1 points -> ~70% = 14 (-1 base) = 13 points
		{1, 1},
		{2, 1},
		{3, 1},
		{4, 1},
...
if CLIENT then
	...
	hud:AddCommonSkills()
	hud:AddSkill("gas"):SetButton("attack"):SetMaterial("slc/hud/scp/173/gas.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
	hud:AddSkill("decoy"):SetButton("attack2"):SetMaterial("slc/hud/scp/173/decoy.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetActiveFunction(function(swep) return not swep:HasUpgrade("decoy2") or swep:GetDecoys() > 0 end)
	hud:AddSkill("stealth"):SetButton("scp_special"):SetMaterial("slc/hud/scp/173/stealth.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack"):SetActiveFunction(function(swep) return swep:GetStealth() ~= -1 and not swep:GetOwner():IsInZone(ZONE_FLAG_RESTRICT173) end)
	hud:AddSkill("looked_at"):SetOffset(0.5):SetMaterial("slc/hud/scp/173/freeze.png", "smooth"):SetActiveFunction("GetFreeze")
	hud:AddSkill("next_decoy"):SetMaterial("slc/hud/scp/173/decoy.png", "smooth"):SetCooldownFunction("GetNextDecoy"):SetVisibleFunction(function(swep) return swep:HasUpgrade("decoy2") end):SetActiveFunction(function(swep) return swep:GetDecoys() > 0 end):SetTextFunction(function(swep) return swep:GetDecoys() end)
	hud:AddBar("stealth_bar"):SetMaterial("slc/hud/scp/173/stealth.png", "smooth"):SetColor(Color(150, 150, 150)):SetProgressFunction(function(swep, hud_obj, this)
		if this._restricted then return 1 end
		return (swep:GetStealth() - CurTime()) / (swep.StealthDuration * swep:GetUpgradeMod("stealth_dur", 2))
	end):SetTextFunction(function(swep, hud_obj, this) if this._restricted then return swep.Lang.restricted end end):SetVisibleFunction(function(swep, hud_obj, this)
