...
function SWEP:HolsterThink()
	if self.NThink < CurTime() and self:GetEnabled() then
		...
		if SERVER then
			...
			if IsValid(owner) then owner:TakeSanity(-1, SANITY_TYPE.ANOMALY, "scp714") end
		end
	end
end
...
function SWEP:OnDrop()
	...
	if IsValid(self.LastOwner) then self:StateChanged(self.LastOwner, false) end
end
...
if SERVER then
	local func = function(ply, arg1, arg2, data)
		if data ~= "scp714" then
			...
			if IsValid(scp) and scp:GetEnabled() then return true end
		end
	end
...
function PLAYER:GetSCP714()
	...
	if IsValid(scp) then return scp:GetEnabled() end
	return false
end