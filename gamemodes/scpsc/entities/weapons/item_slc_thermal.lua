...
function SWEP:OnSelect()
    if self:GetBattery() <= 0 or not self:CanEnable() then return end
...
if CLIENT then
    ...
    SLC_THERMAL_CLASSES = {}
    local TEX_SIZE = 256
    local thermal_rt = GetRenderTargetEx("slc_thermal_texture", TEX_SIZE, TEX_SIZE, RT_SIZE_NO_CHANGE, MATERIAL_RT_DEPTH_SEPARATE, bit.bor(2, 256), 0, IMAGE_FORMAT_BGRA8888)
    ...
    local function update_thermal(pct)
        ...
        for i, v in ipairs(ents.GetAll()) do
            ...
            if SLC_THERMAL_FILTER[v:GetModel()] or (not isp or not v:Alive()) and not isr and not SLC_THERMAL_CLASSES[class] then continue end
            render.ClearStencil()
    ...
    hook.Add("PreDrawEffects", "SLCThermal", function()
        ...
        if not IsValid(wep) or not wep:GetEnabled() then return end
        local ct = CurTime()
        if wep.NextFrame <= ct then
            wep.NextFrame = wep.NextFrame + 1 / wep.FrameRate
            if wep.NextFrame <= ct then wep.NextFrame = ct + 1 / wep.FrameRate end
            update_thermal()
        end
    end)
    ...
    hook.Add("SLCScreenMod", "SLCThermal", function(clr)
        ...
        if not ply:Alive() then return end
        local wep = ply:GetWeapon("item_slc_thermal")
        if not IsValid(wep) or not wep:GetEnabled() then
