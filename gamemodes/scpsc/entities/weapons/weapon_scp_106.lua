...
function SWEP:Think()
    ...
    if passive == 0 then
        ...
        for i, v in ipairs(player.GetAll()) do
            if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(pos) > max_dist then continue end
            ...
            if v:Alive() then SendToPocketDimension(v, owner) end
            self:AddScore(1)
...
function SWEP:SoundThink()
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        if status and not self.SoundPlayers[v] then v:PlayAmbient("106", true, false, function(ply) return not IsValid(self) or not self.SoundPlayers[ply] end) end
...
function SWEP:TrapThink()
    if SERVER or not self:GetPlacingTrap() or not IsValid(self.GhostEntity) then return end
...
function SWEP:SpotThink()
    ...
    local ct = CurTime()
    if self.ShowSpots < ct then self.LastCurrentSpot = nil end
    local owner = self:GetOwner()
    ...
    if not self.CurrentSpot then
        self.SpotHolding = false
        return
    end

    self.ShowSpots = ct + self.SpotDrawDuration
    if not self.SpotHolding then return end
    self.SelectedSpot = best_spot
    if not input.IsKeyDown(GetBindButton("scp_special")) then
        if self.SelectedSpot then net.Ping("SCP106Teleport", self.SelectedSpot:EntIndex()) end
        self.SelectedSpot = nil
        self.SpotHolding = false
    end
end
...
function SWEP:PrimaryAttack()
    ...
    if not attack_trace.Hit then util.TraceHull(attack_trace) end
    owner:LagCompensation(false)
    if not attack_trace.Hit then return end
...
function SWEP:SecondaryAttack()
    ...
    if not self:CanAttack() and not self:GetPlacingTrap() then return end
    local ct = CurTime()
    local owner = self:GetOwner()
    if self:GetTrapTime() >= ct then
        if CLIENT or not owner:IsOnGround() then return end
        self:SetTrapTime(0)
        local cd = self.LastTrapCD or 0
        if cd < ct + 1 then cd = ct + 1 end
        self:SetNextSecondaryFire(cd)
        if not IsValid(self.LastTrap) then return end
        local pos = owner:GetPos()
        util.Decal("Decal106", pos + Vector(0, 0, 8), pos - Vector(0, 0, 8), owner)
        SuppressHostEvents(NULL)
...
function SWEP:Reload()
    if not self:GetPlacingTrap() then return end
...
function SWEP:SpecialAttack()
    if ROUND.preparing or ROUND.post or not self:CanAttack() then return end
    local owner = self:GetOwner()
    if not owner:IsOnGround() then return end
    local ct = CurTime()
    if CLIENT then
        if self.CurrentSpot then self.SpotHolding = true end
        return
    end
    ...
    for y = -1, 1 do
        for x = -1, 1 do
            ...
            if not spot_trace.Hit or spot_trace.HitNormal.z ~= 1 or (prev_z and spot_trace.HitPos.z ~= prev_z) then return end
    ...
    if num_spots >= self.MaxSpots + self:GetUpgradeMod("spot_max", 0) then
        ...
        if IsValid(spot) then spot:Remove() end
    end
...
function SWEP:CanAttack()
    return not self:GetPlacingTrap() and self:GetPassiveCooldown() == 0 and self:GetTeleportSequence() < CurTime()
end
...
function SWEP:SpotTeleport(idx)
    ...
    if not IsValid(ent) or ent:GetClass() ~= "slc_106_spot" then return end
    local owner = self:GetOwner()
...
function SWEP:TeleportSequence(dest, wall, despawn_speed, spawn_speed)
    ...
    if wall then owner:SetEyeAngles(wall) end
    self:SetTeleportSequence(CurTime() + total)
    owner:DisableControls("scp106_tp")
    owner:PlaySequence(seq_despawn, false, despawn_speed)
    owner:AddTimer("SCP106FinishTPSound", dur_despawn - 2, 1, function() owner:EmitSound("SCP106.Disappear") end)
    owner:AddTimer("SCP106FinishTPFade", dur_despawn - 0.75, 1, function() owner:ScreenFade(SCREENFADE.OUT, fade_color, 0.5, 0.25) end)
    ...
    owner:AddTimer("SCP106EnableMovement", total, 1, function() owner:StopDisableControls("scp106_tp") end)
    return total
end
...
function SWEP:GetTrapPosition()
    ...
    if not trap_trace.Hit or math.abs(normal.z) > 0.1 then return false, pos, (-owner:GetAimVector()):Angle() end
    ...
    util.TraceLine(trap_trace)
    if not trap_trace.Hit then return false, pos, normal:Angle() end
    ...
    if trap_trace.Hit then return false, pos, normal:Angle() end
    for y = -1, 1 do
        for x = -1, 1 do
            trap_trace.start = pos + right * x * trap_bounds_x + up * y * trap_bounds_y + normal * 4
            trap_trace.endpos = trap_trace.start - normal * 8
            util.TraceLine(trap_trace)
            if not trap_trace.Hit or trap_trace.HitNormal ~= normal then
                all_ok = false
                break
            end
        end
...
function SWEP:RemoveGhostEntity()
    if CLIENT and IsValid(self.GhostEntity) then self.GhostEntity:Remove() end
end
...
function SWEP:CalcView(ply, pos, ang, fov, view)
    if self:GetTeleportSequence() < CurTime() then
        ...
        if self.ThirdPersonDistance < 0 then self.ThirdPersonDistance = 0 end
    elseif self.ThirdPersonDistance < 1 then
        self.ThirdPersonDistance = self.ThirdPersonDistance + FrameTime() * 2
        if self.ThirdPersonDistance > 1 then self.ThirdPersonDistance = 1 end
    end
...
SCPHook("SCP106", "SLCPostScaleDamage", function(target, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not IsValid(target) or not target:IsPlayer() or target:SCPClass() ~= CLASSES.SCP106 then return end
...
if CLIENT then
    SCPHook("SCP106", "ScalePlayerDamage", function(ply, group, dmg)
        if not IsValid(ply) or ply:SCPClass() ~= CLASSES.SCP106 then return end
        return true
    end)
end

SCPHook("SCP106", "ShouldCollide", function(ent1, ent2) if ent1:IsPlayer() and ent1:SCPClass() == CLASSES.SCP106 or ent2:IsPlayer() and ent2:SCPClass() == CLASSES.SCP106 then if ent1.ignorecollide106 or ent2.ignorecollide106 then return false end end end)
if SERVER then
...
EFFECTS.RegisterEffect("scp106_withering", {
    ...
    cantarget = function(ply)
        ...
        return not ply:CheckHazmat(75, true)
    end,
    begin = function(self, ply, tier, args, refresh, decrease)
        ...
        AddRoundHook("SLCScaleSpeed", "SCP106Withering" .. ply:SteamID64(), function(pl, mod)
            if pl ~= ply or not pl.SLCSCP106WitheringSlow then return end
            if IsFirstTimePredicted() then pl.SLCSCP106WitheringSlowPct = math.Approach(pl.SLCSCP106WitheringSlowPct, 1, FrameTime() * 0.1) end
            mod[1] = mod[1] * (1 - pl.SLCSCP106WitheringSlow * pl.SLCSCP106WitheringSlowPct)
        end)
    end,
    finish = function(self, ply, tier, args, interrupt, all) RemoveRoundHook("SLCScaleSpeed", "SCP106Withering" .. ply:SteamID64()) end,
})
...
DefineUpgradeSystem("scp106", {
    ...
    rewards = {
        --17 + 1 points -> 60% = 11 (-1 base) = 10 points
        {1, 1},
        {2, 1},
        {3, 1},
        {5, 1},
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("withering"):SetButton("attack"):SetMaterial("slc/hud/scp/106/withering.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire"):SetParser(function(swep, lang)
    ...
    hud:AddSkill("trap"):SetButton("attack2"):SetMaterial("slc/hud/scp/106/trap.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire")
    hud:AddSkill("teleport"):SetButton("scp_special"):SetMaterial("slc/hud/scp/106/teleport.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack")
    hud:AddSkill("passive"):SetOffset(0.5):SetMaterial("slc/hud/scp/106/passive.png", "smooth"):SetCooldownFunction("GetPassiveCooldown"):SetTextFunction("GetTeeth")
    hud:AddSkill("teleport_cd"):SetMaterial("slc/hud/scp/106/teleport.png", "smooth"):SetCooldownFunction("GetNextSpot")
    hud:AddBar("passive_bar"):SetMaterial("slc/hud/scp/106/passive.png", "smooth"):SetColor(Color(250, 65, 250)):SetTextFunction(function(swep)
        local hp = swep:GetPassiveDamage()
        if hp < 0 then hp = 0 end
        return math.Round(hp)
    end):SetProgressFunction(function(swep) return swep:GetPassiveDamage() / (wep.PassiveDamage * swep:GetUpgradeMod("passive_dmg", 1)) end):SetVisibleFunction(function(swep) return swep:GetPassiveDamage() > 0 end)

    hud:AddBar("trap_bar"):SetMaterial("slc/hud/scp/106/shield.png", "smooth"):SetColor(Color(230, 190, 110)):SetTextFunction(function(swep, hud_obj, this) return math.Round(this.last_time or 0) .. "s" end):SetProgressFunction(function(swep, hud_obj, this)
        local time = swep:GetTrapTime() - CurTime()
        if time > 0 then this.last_time = time end
        return (this.last_time or 0) / swep.TrapTPTime
    end):SetVisibleFunction(function(swep) return swep:GetTrapTime() >= CurTime() end)
end
