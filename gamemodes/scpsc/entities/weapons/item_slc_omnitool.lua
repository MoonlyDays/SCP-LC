...
function SWEP:Holster()
    local state = self:GetState()
    if self.PreventDropping or state == STATE.INSERTING or state == STATE.EJECTING then return false end
    return true
end

function SWEP:Equip()
    self:CallBaseClass("Equip")
    if CLIENT then self:UpdateDescription() end
end
...
function SWEP:UseOmnitool(ent, data, status, msg, shared)
    ...
    if IsValid(owner) then
        if SERVER and not shared then net.Ping("SLCOmnitoolUsed", status, owner) end
        --if !status then
        --self:PrintMessage( "" )
        --return
        --end
        if self:GetState() == STATE.IDLE then
            local seq, dur = -1, 1
            local vm = owner:GetViewModel()
            if IsValid(vm) then seq, dur = vm:LookupSequence("use") end
            dur = dur * 0.5
            ...
            self:QueueAction(STATE.USING, dur, function(time, duration)
                if status then
                    ...
                    if SERVER then
                        if not isfunction(data.input_override) or data.input_override(owner) ~= true then
                    ...
                    --if IsValid( owner ) then
                    if SERVER then owner:EmitSound("Omnitool.denied") end
...
function SWEP:InstallChip(chip, ent)
    if chip then
        ...
        local vm = owner:GetViewModel()
        if IsValid(vm) then seq, dur = vm:LookupSequence("install") end
        local new_override = 0
        if IsValid(ent) then new_override = ent:GetAccessOverride() end
        ...
        self:QueueAction(STATE.INSTALLING, 2, function(time, duration)
            ...
            if IsValid(ent) then
                ...
                if CLIENT then if replace then ent:UpdateDescription(replace.id, override) end end
            end
        end)
    end
end
...
function SWEP:EjectChip(swap)
    ...
    if id ~= -1 then
        if self:GetState() == STATE.IDLE then
            ...
            local vm = owner:GetViewModel()
            if IsValid(vm) then seq, dur = vm:LookupSequence("extract") end
            ...
            self:QueueAction(STATE.IDLE, 0, function(time, duration)
                if not swap then
                    ...
                    if SERVER then
                        if chip then
                            if owner:GetFreeInventory() > 0 and not owner:HasWeapon("item_slc_access_chip") then
                                ...
                                if IsValid(phys) then phys:SetVelocity(ea:Forward() * 300) end
                            end
                        end
                    end
...
function SWEP:HasAccess(acc)
    ...
    if id > -1 then
        local chip = GetChipByID(id)
        if chip then return chip.obj:HasAccess(acc, self:GetAccessOverride()) end
    end
    return false
end

function SWEP:CanAccess(acc)
    if self:GetState() == STATE.IDLE then return self:HasAccess(acc) end
    return false
end
...
if CLIENT then
        ...
        elseif state == STATE.EJECTING then
            ...
            if show_text > CurTime() then
                ...
                if text and text ~= "" then
                    if LANG.NRegistry[text] then text = LANG.NRegistry[text] end
            ...
            if id > -1 then
                local chip = GetChipByID(id)
                if chip then name = self.CHIP_LANG.NAMES[chip.name] or id end
            end
    ...
    function SWEP:UpdateDescription(id, override)
        ...
        if chip then
            ...
            if cd then desc:print(string.format("%s\n\t%s", self.CHIP_LANG.hasaccess, cd)) end
        else
    ...
    net.ReceivePing("SLCOmnitool", function(data)
        ...
        if IsValid(omnitool) then omnitool:UseOmnitool(nil, nil, data == "true") end
    end)
end
