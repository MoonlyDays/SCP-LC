...
function SWEP:Disease()
	...
	if not self.PlayerCache[owner] or self.PlayerCache[owner][1] ~= sig then
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		if self.Markup then self.Markup:Draw(x + dw * 0.5, y + h * 0.06 + th * 0.5, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 255, true) end
	end
...
AddSCP1025Disease("arrest", 17, function(ply)
	local t = ply:AddTimer("1025disease", SLCRandom(30, 90), 1, function(self)
		if IsValid(ply) then
			ply:SetProperty("death_info_override", {
				type = "dead",
				args = {"table;WEAPONS;SCP1025;death_info_arrest"}
			})
	...
	ply:SetProperty("slc_1025_disease", {
		id = "arrest",
		remove = function() if IsValid(t) then t:Destroy() end end
	})
end)
...
local function gen_effect_cb(name)
	return function(ply)
		...
		ply:SetProperty("slc_1025_disease", {
			id = name,
			remove = function() if IsValid(ply) then ply:RemoveEffect(name) end end
		})
	end
end
...
local can_target = function(ply)
	...
	if team == TEAM_SPEC or team == TEAM_SCP then return false end
...
EFFECTS.RegisterEffect("light_bleeding", {
	...
	begin = function(self, ply, tier, args, refresh)
		if refresh then
			...
			if self.args[2] > 3 then self.args[2] = 3 end
...
hook.Add("SLCFallDamage", "SLCSCP1025Fall", function(ply, water, floater, speed) if ply:HasEffect("oste") then return speed, 110 end end)
hook.Add("SLCPlayerSanityChange", "SLCSCP1025Sanity", function(ply, sanitytype, num, data) if ply:HasEffect("urbach") then return true end end)
hook.Add("SLCSCP500Used", "SLCSCP1025SCP500", function(ply)
	local prop = ply:GetProperty("slc_1025_disease")
	if prop and prop.remove then prop.remove() end
	ply:SetProperty("slc_1025_disease", nil)
end)

hook.Add("SLCHealed", "SLCSCP1025Healed", function(target, healer) target:RemoveEffect("light_bleeding", true) end)
hook.Add("SLCNeedHeal", "SLCSCP1025NeedHeal", function(target, healer, heal_type) if target:HasEffect("light_bleeding") then return true end end)