...
SWEP.SpawnableItems = {"item_slc_medkit", "item_slc_battery", "item_slc_flashlight", "item_slc_radio", "item_slc_gasmask", "item_slc_nvg", "item_slc_adrenaline", "item_slc_morphine",}
...
function SWEP:Think()
    ...
    if SERVER then
        -- Check for mode swap
        if self:GetNextModeSwap() > 0 and ct >= self:GetNextModeSwap() then self:SwapMode() end
        ...
        if IsValid(self:GetCanceledTarget()) and ct >= self:GetCanceledEndTime() then
            ...
            if IsValid(target) then target:SetProperty("scp808j_canceled", nil) end
            self:SetCanceledTarget(NULL)
            self:SetCanceledEndTime(0)
        end
...
function SWEP:DeafenAttack()
    ...
    if affected > 0 then owner:ChatPrint("[SCP-808-J] Deafened " .. affected .. " player(s)!") end
end
...
function SWEP:StaminaDrainAttack()
    ...
    for _, ply in ipairs(player.GetAll()) do
        ...
        if ply:GetPos():Distance(pos) > radius then continue end
        -- Drain all stamina
        if ply.SetStamina then ply:SetStamina(0) end
        affected = affected + 1
    end

    if affected > 0 then owner:ChatPrint("[SCP-808-J] Drained stamina from " .. affected .. " player(s)!") end
end
...
function SWEP:CancelCultureAttack()
    ...
    if IsValid(oldTarget) then oldTarget:SetProperty("scp808j_canceled", nil) end
...
if CLIENT then
    ...
    hud:AddSkill("secondary"):SetButton("attack2"):SetMaterial("slc/hud/scp/049/choke.png", "smooth"):SetCooldownFunction("GetNextSecondaryAttack")
    hud:AddSkill("special"):SetButton("scp_special"):SetMaterial("slc/hud/scp/049/choke.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack")
    hud:AddBar("mode"):SetMaterial("slc/hud/scp/049/choke.png", "smooth"):SetColor(Color(255, 200, 50)):SetTextFunction(function(swep) return swep:GetEvilMode() and "EVIL KANYE" or "GOOD KANYE" end):SetProgressFunction(function(swep)
    ...
    end):SetVisibleFunction(function(swep) return true end)
    ...
    hook.Add("PreDrawHalos", "SCP808J_Paparazzi", function()
        ...
        if IsValid(target) then halo.Add({target}, Color(255, 0, 0), 2, 2, 2, true, true) end
    end)
end
