...
SWEP.TextLines = {"client_init();", "CCTV Client Initialized!", "connect();", "CCTV Connected on IP: 192.168.0.100", "ping_cctv();", "Pinging all CCTV Cameras..."}
...
function SWEP:Think()
    ...
    if #self.CCTVCache ~= #CCTV then
        for i, v in ipairs(ents.FindByClass("slc_cctv")) do
            if v.GetCameraID then self.CCTVCache[v:GetCameraID()] = v end
        end
    end
    ...
    if state ~= STATE_ACTIVE and owner:IsHolding(self, "cctv_scan") then owner:InterruptHold(self, "cctv_scan") end
    ...
    if state == STATE_LOADING and self:GetNextPrimaryFire() <= ct then self:SetState(STATE_ACTIVE) end
    ...
    if self.ReloadDown and (not owner:KeyDown(IN_RELOAD) or state ~= STATE_ACTIVE) then
        self.ReloadDown = false
        gui.EnableScreenClicker(false)
    end
end

hook.Add("ShouldDrawLocalPlayer", "SLCCCTVRender", function(ply) if ply.InCCTVRender then return true end end)
function SWEP:PrimaryAttack()
    if self:GetBattery() <= 0 or not self:CanEnable() or self:GetState() == STATE_LOADING then return end
    local owner = self:GetOwner()
...
function SWEP:Scan()
    ...
    if not IsValid(self.CCTVCache[cctv]) then return end
    local detected = {}
    for i, v in ipairs(SCPTeams.GetPlayersByTeam(TEAM_SCP)) do
        ...
        if not tr.Hit then table.insert(detected, v) end
    end
...
if SERVER then
    hook.Add("SetupPlayerVisibility", "CCTVPVS", function(ply, viewentity)
        ...
        if not IsValid(wep) or wep:GetClass() ~= "item_slc_cctv" or not wep:GetEnabled() or wep:GetState() ~= STATE_ACTIVE then return end
        local tab = CCTV[wep:GetCamera()]
        if not tab or not IsValid(tab.ent) then return end
        AddOriginToPVS(tab.pos)
    end)

    net.ReceivePing("SLCCCTVChange", function(data, ply)
        local cctv = tonumber(data)
        if not cctv or cctv < 1 or cctv > #CCTV then return end
        local wep = ply:GetActiveWeapon()
        if not IsValid(wep) or wep:GetClass() ~= "item_slc_cctv" or not wep:GetEnabled() or wep:GetState() ~= STATE_ACTIVE then return end
...
function SWEP:DrawScreen(x, y, w, h)
    ...
    if self.CCTVAlpha > 0 then
        ...
        local tab = CCTV[cctv]
        if tab then draw.OutlinedText(tab.name, "SCPHUDMedium", x + w * 0.5, y + 8 + th, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, color_black, 2, 3) end
        ...
        if LANG.MISC.buttons[map_btn] then map_btn = LANG.MISC.buttons[map_btn] end
        local _, mth = draw.OutlinedText(string.format(self.Lang.map_info, map_btn), "SCPHUDMedium", x + w * 0.5, y + h * 0.75, scan_text_color, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, color_black, 2, 3)
        if not valid then
            surface.SetAlphaMultiplier(old_alpha)
            return
        end
        ...
        if cd > 0 then
            ...
            local _, cd_th = draw.OutlinedText(self.Lang.scan_cd, "SCPHUDMedium", x + w * 0.5, y + h * 0.75 + mth, scan_text_color, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, color_black, 2, 3)
            local cd_w = w * 0.2
        ...
        elseif owner:IsHolding(self, "cctv_scan") then
            ...
            if LANG.MISC.buttons[btn] then btn = LANG.MISC.buttons[btn] end
            draw.OutlinedText(string.format(self.Lang.scan_info, btn), "SCPHUDMedium", x + w * 0.5, y + h * 0.75 + mth, scan_text_color, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, color_black, 2, 3)
        end
    ...
    if self.NextTextLine <= ct then
        ...
        if self.NextTextLine < ct then self.NextTextLine = ct + time_per_line end
    ...
    for i = 1, self.CurrentTextLine do
        ...
        ty = ty + th
        if ty > y + h then self.TextLineScroll = self.TextLineScroll + th end
    end
...
function SWEP:WorldToMap(pos)
    if not map_origin then setup_map() end
    local data = CCTV_CONFIG[self.MapID]
    if not data then return end
    local mat_size = data.u_size
    if data.v_size > mat_size then mat_size = data.v_size end
...
function SWEP:HandleMouse()
    ...
    if mouse_down and not was_mouse_down then
        ...
        return x, y
    elseif not mouse_down and was_mouse_down then
        was_mouse_down = false
    end
end

function SWEP:DrawMap(x, y, w, h)
    if not self.ReloadDown then return false end
    local data = CCTV_CONFIG[self.MapID]
    ...
    for i, v in ipairs(data.cams) do
        ...
        if mx and point_within_circle(mx, my, dx, dy, dr * 1.5) then net.Ping("SLCCCTVChange", v) end
    end
    ...
    draw.OutlinedText(string.format(data.name, btn), "SCPHUDMedium", x + w * 0.5, arrow_y, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, color_black, 2, 3)
    ...
    if mx and point_within_circle(mx, my, arrow_x_l, arrow_y, arrow_r * 1.5) then
        ...
        if self.MapID < 1 then self.MapID = #CCTV_CONFIG end
    elseif mx and point_within_circle(mx, my, arrow_x_r, arrow_y, arrow_r * 1.5) then
        self.MapID = self.MapID + 1
        if self.MapID > #CCTV_CONFIG then self.MapID = 1 end
    end
    return true
end
