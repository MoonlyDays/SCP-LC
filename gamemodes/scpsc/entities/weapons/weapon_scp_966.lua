...
function SWEP:Think()
    ...
    if IsValid(ent) and ent:IsPlayer() and self:CanTargetPlayer(ent) and not ent:GetNoDraw() and hook.Run("CanPlayerSeePlayer", owner, ent) ~= false then
    ...
    if next_passive <= ct and channeling_time == 0 then
        ...
        if next_passive <= ct then next_passive = ct + passive_cd end
        ...
        for i, v in ipairs(player.GetAll()) do
            if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(owner_pos) > passive_radius then continue end
            local is_safe, safe_pos = v:IsInSafeSpot()
            if is_safe and not owner_pos:WithinAABox(safe_pos.mins, safe_pos.maxs) then return end
            if v:ApplyEffect("scp966_effect", owner) then stacks = stacks + 1 end
        end
    ...
    if channeling_time ~= 0 then
        ...
        if channeling_time < ct or channeling_a and (not IsValid(target) or not target:Alive() or not target:CheckSignature(self.ChannelingSignature)) then
        ...
        elseif self.NextChannelingTick <= ct then
            ...
            if self.NextChannelingTick <= ct then self.NextChannelingTick = ct + self:GetUpgradeMod("channeling_rate") end
            local owner_pos = owner:GetPos()
            local range = self:GetUpgradeMod("channeling_range") * self:GetUpgradeMod("channeling_range_mul", 1)
            range = range * range
            if channeling_a then
                if owner_pos:DistToSqr(target:GetPos()) > range then self:SetChanneling(1) end
                ...
                local stacks = 0
                for i, v in ipairs(player.GetAll()) do
                    if not v:Alive() or not v:HasEffect("scp966_effect") or v:GetPos():DistToSqr(owner_pos) > range then continue end
    ...
    if mark_time ~= 0 then
        if mark_time < ct or not IsValid(self.MarkedPlayer) or not self.MarkedPlayer:Alive() or not self.MarkedPlayer:CheckSignature(self.MarkedSignature) then
            self:SetMarkTime(0)
        ...
        elseif self.NextMarkTick <= ct then
            ...
            if self.NextMarkTick <= ct then self.NextMarkTick = ct + mark_tick end
            local target_pos = self.MarkedPlayer:GetPos()
            local radius = self.MarkRadius * self:GetUpgradeMod("mark_range", 1)
            local stacks = 0
            radius = radius * radius
            for i, v in ipairs(player.GetAll()) do
                if v == self.MarkedPlayer or not v:Alive() or not v:HasEffect("scp966_effect") or v:GetPos():DistToSqr(target_pos) > radius then continue end
                v:DecreaseEffect("scp966_effect")
                stacks = stacks + 1
            end

            if stacks > 0 then self.MarkedPlayer:ApplyEffect("scp966_effect", stacks, owner) end
        end
    end
end
...
function SWEP:PrimaryAttack()
    ...
    elseif upg2 and stacks >= self:GetUpgradeMod("bleed2_thr") or upg1 and stacks >= self:GetUpgradeMod("bleed1_thr") and not ent:HasEffect("bleeding") then
    ...
    if upg1 and stacks >= self:GetUpgradeMod("slow_thr") then
        ...
        ent:AddTimer("SCP966Slow", self:GetUpgradeMod("slow_dur"), 1, function() ent:PopSpeed("SLC_SCP966Slow", true) end)
    end

    local heal = self:GetUpgradeMod("heal_rate", 0) * stacks * ply_stacks
    if heal > 0 then owner:AddHealth(math.ceil(heal)) end
end
...
function SWEP:SecondaryAttack()
    ...
    elseif not self:HasUpgrade("channeling_b") then
        return
    end
...
if CLIENT then
    ...
    SCPHook("SCP966", "PreDrawEffects", function()
        ...
        for i = 1, steps do
            ...
            render.DrawQuad(Vector(pos.x + prev_s * radius, pos.y + prev_c * radius, pos.z + 10), Vector(pos.x + s * radius, pos.y + c * radius, pos.z + 10), Vector(pos.x + s * radius, pos.y + c * radius, pos.z), Vector(pos.x + prev_s * radius, pos.y + prev_c * radius, pos.z), channeling and channeling_color or passive_color)
            prev_s = s
            prev_c = c
        end
    end)
    ...
    SCPHook("SCP966", "CanPlayerSeePlayer", function(ply, target)
        ...
        if IsValid(wep) and wep:GetClass() == "item_slc_nvg" and wep:GetBattery() > 0 then return end
        local nvgp = ply:GetWeapon("item_slc_nvgplus")
        if IsValid(nvgp) and nvgp:GetEnabled() then return end
        local thermal = ply:GetWeapon("item_slc_thermal")
        if IsValid(thermal) and thermal:GetEnabled() then return end
        return false
    end)
end
...
EFFECTS.RegisterEffect("scp966_effect", {
    ...
    begin = function(self, ply, tier, args, refresh, decrease)
        ...
        if IsValid(args[1]) then dmg:SetAttacker(args[1]) end
...
DefineUpgradeSystem("scp966", {
    ...
    upgrades = {
        ...
        },
        {
            ...
            pos = {3, 1},
            mod = {
                channeling_range = 140,
                channeling_time = 15,
                channeling_rate = 0.75,
                channeling_cd = 30
            },
            icon = icons.channeling_a,
            active = {icons.channeling_a}
        },
        {
            name = "channeling_b",
            cost = 3,
            ...
            pos = {4, 1},
            mod = {
                channeling_range = 500,
                channeling_time = 10,
                channeling_rate = 1,
                channeling_cd = 90
            },
            icon = icons.channeling_b,
            active = {icons.channeling_b}
        },
        {
            name = "channeling",
            cost = 2,
            req = {"channeling_a", "channeling_b"},
            reqany = true,
            pos = {3.5, 2},
            mod = {
                channeling_range_mul = 1.25,
                channeling_time_mul = 1.2
            },
            icon = icons.channeling,
            active = false
        },
        {
            name = "mark1",
    ...
    rewards = {
        --19/20 + 1 points -> ~60% = 14 (-1 base) = 13 points
        {25, 1},
        {50, 1},
        {100, 1},
        {150, 1},
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("attack"):SetButton("attack"):SetMaterial("slc/hud/scp/966/attack.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire")
    hud:AddSkill("channeling"):SetButton("attack2"):SetMaterial("slc/hud/scp/966/channeling.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetActiveFunction(function(swep) return (swep:HasUpgrade("channeling_a") or swep:HasUpgrade("channeling_b")) and swep:GetChanneling() == 0 end)
    hud:AddSkill("mark"):SetButton("scp_special"):SetMaterial("slc/hud/scp/966/mark.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack")
    hud:AddSkill("passive"):SetOffset(0.5):SetMaterial("slc/hud/scp/966/passive.png", "smooth"):SetCooldownFunction("GetNextPassive"):SetActiveFunction(function(swep) return swep:GetChanneling() == 0 end):SetTextFunction("GetStacks")
    hud:AddBar("channeling_bar"):SetMaterial("slc/hud/scp/966/channeling.png", "smooth"):SetColor(Color(60, 85, 95)):SetTextFunction(function(swep)
        local time = swep:GetChanneling() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time, 1) .. "s"
    end):SetProgressFunction(function(swep) return (swep:GetChanneling() - CurTime()) / swep:GetUpgradeMod("channeling_time", 1) * swep:GetUpgradeMod("channeling_time_mul", 1) end):SetVisibleFunction(function(swep) return swep:GetChanneling() > 0 end)

    hud:AddBar("mark_bar"):SetMaterial("slc/hud/scp/966/mark.png", "smooth"):SetColor(Color(175, 60, 25)):SetTextFunction(function(swep)
        local time = swep:GetMarkTime() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time, 1) .. "s"
    end):SetProgressFunction(function(swep) return (swep:GetMarkTime() - CurTime()) / swep.MarkDuration end):SetVisibleFunction(function(swep) return swep:GetMarkTime() > 0 end)
end
