...
function SWEP:Think()
	...
	if SERVER then
		...
		if not IsValid(target) or not self:IsValidTarget(target) then
			self:AssignNewTarget()
		end

		-- Clear highlight when expired
		if self:GetHighlightEndTime() > 0 and ct >= self:GetHighlightEndTime() then
			self:SetHighlightEndTime(0)
		end
	end
end
	...
	if not IsValid(target) then
		if SERVER then
			...
			if IsValid(owner) then
				owner:ChatPrint("[SCP-1983-J] No target assigned!")
			end
		end
		return
	end
...
function SWEP:SpecialAttack()
	...
	if affected > 0 then
		owner:ChatPrint("[SCP-1983-J] Affected " .. affected .. " player(s) with insanity!")
	end
end
...
EFFECTS.RegisterEffect("scp1983j_insanity", {
	...
	begin = function(self, ply, tier, args, refresh)
		...
		if CLIENT then
			ply:EmitSound("SCP1983J.InsanityEffect")
		end
	end,
	finish = function(self, ply, tier, args, interrupt)
		if CLIENT then
			ply:StopSound("SCP1983J.InsanityEffect")
		end
	end,
})
...
if CLIENT then
	...
	hook.Add("RenderScreenspaceEffects", "SCP1983J_Insanity", function()
		...
		if insanity_blur > 0.01 then
			DrawMotionBlur(0.1, insanity_blur * 0.8, 0.01)
		end
	end)
...
if CLIENT then
	...
	hud:AddSkill("highlight")
		:SetButton("attack2")
		:SetMaterial("slc/hud/scp/049/choke.png", "smooth")
		:SetCooldownFunction("GetNextSecondaryAttack")

	hud:AddSkill("insanity")
	...
	hud:AddBar("damage")
		...
		:SetProgressFunction(function(swep)
			return math.Clamp(swep:GetBonusDamage() / 50, 0, 1)
		end)
	...
	net.Receive("SLC1983JTarget", function()
		...
		local ply = LocalPlayer()
		if IsValid(ply) then
			ply:ChatPrint("[SCP-1983-J] New target: " .. (IsValid(target) and target:Nick() or "None"))
		end
	end)
	...
	hook.Add("PreDrawHalos", "SCP1983J_Highlight", function()
		...
		if IsValid(highlightTarget) and highlightEndTime > CurTime() then
			halo.Add({highlightTarget}, Color(255, 0, 0), 3, 3, 2, true, true)
		end

		-- Always show current target with subtle halo
		local target = wep:GetCurrentTarget()
		if IsValid(target) and (not IsValid(highlightTarget) or highlightEndTime <= CurTime()) then
			halo.Add({target}, Color(139, 0, 0, 255), 3, 3, 2, true, false)
		end
	end)
end
