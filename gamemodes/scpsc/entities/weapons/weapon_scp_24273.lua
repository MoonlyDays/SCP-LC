...
local hud_materials = {
    primary = {Material("slc/hud/scp/24273/dash.png", "smooth"), Material("slc/hud/scp/24273/camo.png", "smooth"),},
    secondary = {Material("slc/hud/scp/24273/drain.png", "smooth"), Material("slc/hud/scp/24273/spect.png", "smooth"),},
    special = {Material("slc/hud/scp/24273/special.png", "smooth"), Material("slc/hud/scp/24273/ghost.png", "smooth"),},
}
...
function SWEP:Think()
    ...
    if camo > 0 and (camo < ct or self.CamouflageRestrict and owner:GetPos():DistToSqr(self.CamouflageRestrict) > self:GetUpgradeMod("camo_limit", 25) ^ 2) then self:StopCamouflage() end
    ...
    elseif ghost >= ct and self.NextGhostTick < ct and owner:Health() < owner:GetMaxHealth() then
        ...
        if self.NextGhostTick < ct then self.NextGhostTick = ct + self.GhostDelay end
        local pos = owner:GetPos()
        local radius = self.GhostRadius ^ 2
        local heal = 0
        for i, v in ipairs(player.GetAll()) do
            local tab = self.Evidence[v]
            if not tab or tab[1] <= 0 or not v:CheckSignature(tab[2]) or not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(pos) > radius then continue end
            heal = heal + 1
            tab[1] = tab[1] - 0.01
            if tab[1] < 0 then tab[1] = 0 end
        end

        if heal > 0 then owner:AddHealth(math.ceil(heal * self:GetUpgradeMod("ghost_heal", self.GhostHeal))) end
    end
    ...
    if ghost == 0 then self:EvidenceThink() end
...
function SWEP:EvidenceThink()
    ...
    local best_ply = nil
    for i, v in ipairs(player.GetAll()) do
        if not self:CanTargetPlayer(v) or v:GetNoDraw() or hook.Run("CanPlayerSeePlayer", owner, v) == false then continue end
    ...
    if best_ply then
        ...
        if not tab or not best_ply:CheckSignature(tab[2]) then
...
function SWEP:DashThink()
    local ct = CurTime()
    local owner = self:GetOwner()
    if not controller.IsEnabled(owner, "scp24273_dash") then return end
    ...
    if not dash_trace.Hit then return end
...
function SWEP:DrainThink()
    ...
    elseif drain > 0 and drain < ct then
        ...
        target:AddTimer("SCP24273Drain", slow_time, 1, function() target:PopSpeed("SLC_SCP24273Drain") end)
...
function SWEP:SpecialThink()
    ...
    for i, v in ipairs(player.GetAll()) do
        if not self:CanTargetPlayer(v) or self.SpecialFilter[v] then continue end
        local pos = v:GetPos()
        local should_affect = not v:GetSCP714() and owner:TestVisibility(v, nil, nil, false)
        local target = v:GetProperty("scp24273_target")
        if should_affect and not target then
            target = target_pos
        ...
        elseif not should_affect and target then
            target = nil
            controller.Stop(v)
...
function SWEP:PrimaryAttack()
    if CLIENT or ROUND.preparing or ROUND.post or not self:CanAttack() then return end
    self:StopCamouflage()
    local ct = CurTime()
    local owner = self:GetOwner()
    if self:GetMode() then
        ...
        owner:AddTimer("SCP24273Camouflage", 0.5, 1, function() self.CamouflageRestrict = owner:GetPos() end)
    else
...
function SWEP:SecondaryAttack()
    ...
    local ct = CurTime()
    local spectate = self:GetSpectateTime()
    if (spectate > 0 and spectate < ct or self.SpectateFly >= ct) and not self:CanAttack() then return end
    self:StopCamouflage()
    local owner = self:GetOwner()
    if self:GetMode() then
        if spectate >= ct then self:SetSpectateTime(1) end
        ...
        local len = 0
        for i, v in ipairs(player.GetAll()) do
            if not self:CanTargetPlayer(v) or v:GetPos():DistToSqr(owner_pos) > radius then continue end
            len = len + 1
            targets[len] = v
        end
        ...
        util.TraceLine(drain_trace)
        owner:LagCompensation(false)
        if not drain_trace.Hit then return end
...
function SWEP:Reload()
    local ct = CurTime()
    if self:GetNextMode() > ct or not self:CanAttack() then return end
    if not self:HasUpgrade("change2") then self:StopCamouflage() end
    self:SetNextMode(ct + self.ChangeCooldown * self:GetUpgradeMod("change_cd", 1))
    local new = not self:GetMode()
    self:SetMode(new)
    if CLIENT then
...
function SWEP:CanAttack()
    return self:GetDrain() == 0 and self:GetGhost() == 0 and self:GetSpectateTime() == 0 and not controller.IsEnabled(self:GetOwner())
end
...
function SWEP:CalcView(ply, cur_pos, cur_ang, fov)
    ...
    if self.CameraPosition:Distance(target) < 30 then self.CameraSegment = self.CameraSegment + 1 end
...
Net
---------------------------------------------------------------------------]]
if SERVER then net.AddTableChannel("SCP24273Path") end
if CLIENT then
...
SCPHook("SCP24273", "EntityTakeDamage", function(target, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not IsValid(target) or not target:IsPlayer() or target:SCPClass() ~= CLASSES.SCP24273 then return end
    local wep = target:GetSCPWeapon()
    if not IsValid(wep) then return end
    if wep:GetGhost() ~= 0 and not dmg:IsDamageType(DMG_BLAST) and not dmg:IsDamageType(DMG_FALL) then return true end
    if not dmg:IsDamageType(DMG_BULLET) then return end
    wep:StopCamouflage()
    if wep:GetMode() then
        local def = 1 - wep:GetUpgradeMod("p_prot", wep.PPassiveDefense)
        if wep:GetSpectateTime() > CurTime() then def = def - wep:GetUpgradeMod("spect_prot", wep.SpectateDefense) end
        if def < 0.1 then def = 0.1 end
        dmg:ScaleDamage(def)
...
SCPHook("SCP24273", "CalcMainActivity", function(ply, speed)
    ...
    if spectate < 0 then spectate = -spectate end
...
SCPHook("SCP24273", "SLCPlayerFootstep", function(ply, foot, snd)
    if ply:SCPClass() ~= CLASSES.SCP24273 then return end
    local wep = ply:GetSCPWeapon()
    if not IsValid(wep) or wep:GetCamouflage() == 0 then return end
    return true
end)
...
SCPHook("SCP24273", "CanPlayerSeePlayer", function(lp, other)
    ...
    if not IsValid(wep) or wep:GetSpectateTime() == 0 then return end
...
SCPHook("SCP24273", "SetupPlayerVisibility", function(ply, ve)
    ...
    local ct = CurTime()
    local wep = ply:GetSCPWeapon()
    if not IsValid(wep) or wep:GetSpectateTime() < ct then return end
    if wep.SpectateFly < ct then
        local target = wep:GetSpectateEntity()
        if IsValid(target) then AddOriginToPVS(target:EyePos()) end
        return
    end
    ...
    if not time[act] then return end
...
if CLIENT then
    ...
    SCPHook("SCP24273", "PreDrawEffects", function()
        ...
        if ply:SCPClass() ~= CLASSES.SCP24273 then return end
        local wep = ply:GetSCPWeapon()
        if not IsValid(wep) or wep:GetDrain() <= 0 then return end
...
DefineUpgradeSystem("scp24273", {
    ...
    rewards = {
        --15 + 1 points -> 60% = 10 (-1 base) = 9 points
        {50, 1},
        {100, 1},
        {175, 1},
        {275, 1},
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("primary"):SetButton("attack"):SetMaterial(hud_materials.primary[1]):SetCooldownFunction("GetNextPrimaryFire")
    hud:AddSkill("secondary"):SetButton("attack2"):SetMaterial(hud_materials.secondary[1]):SetCooldownFunction("GetNextSecondaryFire")
    hud:AddSkill("special"):SetButton("scp_special"):SetMaterial(hud_materials.special[1]):SetCooldownFunction("GetNextSpecialAttack")
    hud:AddSkill("change"):SetOffset(0.5):SetButton("reload"):SetMaterial("slc/hud/scp/24273/passive.png", "smooth"):SetCooldownFunction("GetNextMode")
    hud:AddBar("camo_bar"):SetMaterial("slc/hud/scp/24273/camo.png", "smooth"):SetColor(Color(90, 225, 80)):SetTextFunction(function(swep, hud_obj, this) return math.Round(this.last_time or 0) .. "s" end):SetProgressFunction(function(swep, hud_obj, this)
        local time = swep:GetCamouflage() - CurTime()
        if time > 0 then this.last_time = time end
        return (this.last_time or 0) / swep.CamouflageDuration
    end):SetVisibleFunction(function(swep) return swep:GetCamouflage() ~= 0 end)

    hud:AddBar("spectate_bar"):SetMaterial("slc/hud/scp/24273/spect.png", "smooth"):SetColor(Color(33, 215, 160)):SetTextFunction(function(swep, hud_obj, this) return math.Round(this.last_time or 0) .. "s" end):SetProgressFunction(function(swep, hud_obj, this)
    ...
    end):SetVisibleFunction(function(swep) return swep:GetSpectateTime() >= CurTime() end)

    hud:AddBar("drain_bar"):SetMaterial("slc/hud/scp/24273/drain.png", "smooth"):SetColor(Color(190, 50, 145)):SetTextFunction(function(swep, hud_obj, this) return math.Round(this.last_time or 0) .. "s" end):SetProgressFunction(function(swep, hud_obj, this)
        local f = swep:GetDrain() - CurTime()
        if f > 0 then this.last_time = f end
        return (this.last_time or 0) / swep.DrainTime
    end):SetVisibleFunction(function(swep) return swep:GetDrain() >= CurTime() end)

    hud:AddBar("ghost_bar"):SetMaterial("slc/hud/scp/24273/ghost.png", "smooth"):SetColor(Color(30, 140, 15)):SetTextFunction(function(swep, hud_obj, this)
        local time = swep:GetGhost() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep, hud_obj, this) return (swep:GetGhost() - CurTime()) / swep.GhostDuration end):SetVisibleFunction(function(swep) return swep:GetGhost() ~= 0 end)

    hud:AddBar("special_bar"):SetMaterial("slc/hud/scp/24273/special.png", "smooth"):SetColor(Color(120, 40, 210)):SetTextFunction(function(swep, hud_obj, this)
        local time = swep:GetSpecial() - CurTime()
        if time < 0 then time = 0 end
        return math.Round(time) .. "s"
    end):SetProgressFunction(function(swep, hud_obj, this) return (swep:GetSpecial() - CurTime()) / swep.SpecialDuration end):SetVisibleFunction(function(swep) return swep:GetSpecial() ~= 0 end)
end
...
controller.Register("scp24273_dash", {
    ...
    Move = function(self, ply, mv)
        ...
        if ply:IsOnGround() then vel.z = 251 end
        mv:SetVelocity(vel)
    end,
})
