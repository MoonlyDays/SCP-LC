...
function SWEP:Think()
    ...
    if state == STATE.COOLDOWN then
        if time >= self.Cooldown then self:SetWeaponState(0) end
        return
    end

    owner:UpdateHold(self, "particle_cannon")
    if not owner:IsHolding(self, "particle_cannon") then
    ...
    while self.NextFire <= ct do
        ...
        if not attack_trace.Hit or not IsValid(target) or target:IsPlayer() and target:SCPTeam() == TEAM_SPEC then util.TraceLine(attack_trace) end
        if attack_trace.Hit and IsValid(target) and (not target:IsPlayer() or target:SCPTeam() ~= TEAM_SPEC) then
...
function SWEP:WeaponState()
    ...
    if state == 0 then return STATE.IDLE, state end
    local ct = CurTime()
    if state < 0 then
        state = -state - ct
    ...
    if state < ct then return STATE.SHOOTING, state end
    state = state - ct
    if state > self.WindUpPhase2Duration then return STATE.WINDUP_1, state - self.WindUpPhase2Duration end
...
function SWEP:StateChanged(new, old)
    ...
    elseif new == STATE.WINDDOWN then
        ...
        if old == STATE.SHOOTING then self:EmitSound("SLC.ParticleCannon.WindDown") end
...
function SWEP:DrawBeams(ent)
    ...
    render.AddBeam(beam_start, 15, tx, color_white255)
    for i = 1, count - 2 do
        ...
        render.AddBeam(beam_start + pos, 15, tx - f * i, laser_color)
    end

    render.AddBeam(beam_end, 15, tx - 1, color_white0)
    render.EndBeam()
end
...
controller.Register("particle_cannon", {
    StartCommand = function(self, ply, cmd)
        ...
        if not IsValid(wep) or wep:GetClass() ~= "weapon_slc_pc" then
            self:Stop()
            return
        end
        ...
        if abs_diff > max_diff then diff:Div(abs_diff / max_diff) end
        self.Angle:Add(diff)
