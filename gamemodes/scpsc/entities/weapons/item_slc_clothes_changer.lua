...
function SWEP:PrimaryAttack()
    ...
    if owner:GetVest() > 0 then
        if CLIENT then chat.AddText(self.Lang.vest) end
        return
    end
    ...
    local ent = tr.Entity
    if not IsValid(ent) or ent:GetClass() ~= "prop_ragdoll" or not ent.Data or not ent.Data.team or ent.Data.team == TEAM_SCP then return end
    owner:StartSLCTask("clothes_changer", 10, function(ply) return IsValid(self) and owner:KeyDown(IN_ATTACK) end, {
        ...
        if not ent.Data or not ent.Data.team or not ent.Data.class or ent.Data.id_stolen then
...
if CLIENT then
    ...
    function SWEP:Think()
        ...
        if self.LastCD > 0 and time <= 0 or self.LastCD <= 0 and time > 0 then self:PrintCooldown() end
    end
