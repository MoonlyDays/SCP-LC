...
function SWEP:OnSelect()
    ...
    if owner:GetProperty("inventory_transition", 0) > CurTime() then
        if CLIENT and IsFirstTimePredicted() then surface.PlaySound("common/wpn_denyselect.wav") end
        return
    end

    local new = not self:GetEnabled()
    self:SetEnabled(new)
...
function SWEP:Equip()
    ...
    if not self.StoredItems then return end
    local owner = self:GetOwner()
...
function SWEP:StoreBackpack()
    ...
    self.StoredItems = {}
    for i = 7, owner:GetInventorySize() do
        if not IsValid(inventory[i]) or inventory[i].Droppable == false then continue end
