...
function SWEP:Think()
    ...
    elseif status == false then
        ...
        if owner:IsHolding(self, "turret_place") then
            if not valid then owner:InterruptHold(self, "turret_place") end
        else
            self.LastPos = pos
            self.LastAng = ang
...
function SWEP:Deploy()
    ...
    if CLIENT then
        ...
        if not IsValid(self.Indicator) then self.Indicator = CreateIndicator("models/alski/scp/turret/turret1.mdl", false, nil, nil, function() return not LocalPlayer():HasWeapon("item_slc_turret") end) end
...
function SWEP:Holster()
    if CLIENT and IsValid(self.Indicator) then self.Indicator:SetNoDraw(true) end
    return true
end

function SWEP:OnRemove()
    if CLIENT and IsValid(self.Indicator) then self.Indicator:Remove() end
end
...
function SWEP:TraceIndicator(pos, ep, ang)
    ...
    if not result then result = ep end
