...
function SWEP:Think()
    ...
    if owner:WaterLevel() > 0 then
        if self.NextDamage < ct and owner:Health() > 1 then
            ...
            if self.NextDamage < ct then self.NextDamage = ct + 0.1 end
            owner:TakeDamage(10, owner, owner)
        end
    elseif not owner:IsBurning() then
    ...
    if self.IgniteActive and self.NextIgniteTick < ct then
        ...
        if self.NextIgniteTick < ct then self.NextIgniteTick = ct + cd end
        self:IgniteTick()
    end
end
...
function SWEP:SecondaryAttack()
    ...
    if not tr.Hit or tr.HitNormal.z < 0.35 then return end
...
function SWEP:IgniteTick()
    ...
    self:SetFuel(fuel)
    if not any or fuel == 0 then self.IgniteActive = false end
end
...
SCPHook("SCP457", "EntityTakeDamage", function(target, dmg)
    if dmg:IsDamageType(DMG_DIRECT) or not IsValid(target) or not target:IsPlayer() or target:SCPClass() ~= CLASSES.SCP457 then return end
...
SCPHook("SCP457", "SLCFireOnBurnPlayer", function(fire, target, dmg)
    ...
    if not owner:IsPlayer() or owner:SCPClass() ~= CLASSES.SCP457 then return end
    ...
    if heal then owner:AddHealth(math.ceil(dmg_num * heal)) end
end)

SCPHook("SCP457", "SLCPlayerFootstep", function(ply, foot, snd) if ply:SCPClass() == CLASSES.SCP457 then return true end end)
if CLIENT then
    ...
    --Remove blood
    SCPHook("SCP457", "ScalePlayerDamage", function(ply, group, dmg) if IsValid(ply) and ply:SCPClass() == CLASSES.SCP457 then return true end end)
end
...
DefineUpgradeSystem("scp457", {
    ...
    rewards = {
        --23 + 1 points -> 60% = 14 (-1 base) = 13 points
        _auto = {13, 100}
    }
}, SWEP)
...
if CLIENT then
    ...
    hud:AddCommonSkills()
    hud:AddSkill("fireball"):SetButton("attack"):SetMaterial("slc/hud/scp/457/fireball.png", "smooth"):SetCooldownFunction("GetNextPrimaryFire"):SetActiveFunction(function(swep) return swep:GetFuel() >= math.ceil(swep.FireballCost * swep:GetUpgradeMod("fireball_cost", 1)) end):SetParser(function(swep, lang)
        return {
            cost = MarkupBuilder.StaticPrint(math.ceil(swep.FireballCost * swep:GetUpgradeMod("fireball_cost", 1)), color_green)
        }
    end)

    hud:AddSkill("trap"):SetButton("attack2"):SetMaterial("slc/hud/scp/457/trap.png", "smooth"):SetCooldownFunction("GetNextSecondaryFire"):SetTextFunction("GetTraps"):SetActiveFunction(function(swep) return swep:GetFuel() >= math.ceil(swep.TrapCost * swep:GetUpgradeMod("trap_cost", 1)) end):SetParser(function(swep, lang)
        return {
            cost = MarkupBuilder.StaticPrint(math.ceil(swep.TrapCost * swep:GetUpgradeMod("trap_cost", 1)), color_green)
        }
    end)

    hud:AddSkill("ignite"):SetButton("scp_special"):SetMaterial("slc/hud/scp/457/ignite.png", "smooth"):SetCooldownFunction("GetNextSpecialAttack"):SetActiveFunction(function(swep) return swep:GetFuel() >= swep.IgniteCost * swep:GetUpgradeMod("ignite_cost", 1) end):SetParser(function(swep, lang)
        return {
            cost = MarkupBuilder.StaticPrint(math.ceil(swep.IgniteCost * swep:GetUpgradeMod("ignite_cost", 1)), color_green)
        }
    end)

    hud:AddSkill("passive"):SetOffset(0.5):SetMaterial("slc/hud/scp/457/passive.png", "smooth"):SetTextFunction(function(swep) return math.floor(swep:GetFuel()) end)
end
