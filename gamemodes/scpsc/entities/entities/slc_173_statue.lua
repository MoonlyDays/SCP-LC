...
function ENT:Think()
    if not self:GetOnGround() then self:MakeFall() end
    if self:GetMove() then self:Move() end
...
function ENT:MakeFall()
    ...
    if trace_tab.Hit then self:SetOnGround(true) end
end
...
function ENT:Move()
    ...
    if self.MovePct > 1 then self.MovePct = 1 end
    self:SetPos(LerpVector(self.MovePct, self.MovePos, target))
    if self.MoveThink then self.MoveThink() end
