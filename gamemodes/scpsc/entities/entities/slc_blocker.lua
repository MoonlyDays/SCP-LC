...
function ENT:Initialize()
    ...
    if filter.teams then
        for i = 1, #filter.teams do
            ...
            if team then self:AddTeamToFilter(team) end
        end
    end
    ...
    if filter.classes then
        for i = 1, #filter.classes do
            ...
            if class then self:AddClassToFilter(class) end
        end
    end

    --group
    if filter.group then self:LoadFilterGroup(filter.group) end
    --custom check
    if filter.custom_check then self:AddCustomCheckToFilter(filter.custom_check) end
    if filter.round then
        self.RoundFilter = {
            preparing = not not filter.round.preparing,
            post = not not filter.round.post,
        }
    end
end
...
function ENT:LoadFilterGroup(group)
    ...
    if data then
        ...
        if data.base then self:LoadFilterGroup(data.base) end
    end
end

function ENT:EnableBlacklistMode(enable)
    self.Blacklist = not not enable
end

function ENT:AllowUse(allow)
    self.UseAllowed = not not allow
end
...
hook.Add("ShouldCollide", "SLCBlocker", function(ent1, ent2)
    ...
    if not blocker or not blocker.Initialized or not other:IsPlayer() then return end
    if blocker.UseCustomChecks then
        local all_nil = true
        for i, v in ipairs(blocker.CustomChecks) do
            local result = v(other)
            if result == true and not blocker.Blacklist then return false end
            if result == false and blocker.Blacklist then return true end
            if result ~= nil then all_nil = false end
        end

        if not all_nil then return not blocker.Blacklist end
    end
