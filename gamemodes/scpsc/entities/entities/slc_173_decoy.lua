...
function ENT:Think()
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        if not v:Alive() or team == TEAM_SPEC or team == TEAM_SCP or v:GetBlink() or not self:TestVisibility(v) then continue end
        freeze = true
        if not self.TargetsLookup[v] then
    ...
    if not freeze then
        if #self.Targets > 0 and not self.Attacked then
            self:Attack()
...
function ENT:Attack()
    ...
    if not IsValid(victim) or not victim:CheckSignature(self.TargetsLookup[victim]) or not victim:GetBlink() then return end
    local pos = victim:GetPos() + victim:OBBCenter()
    local angles = victim:GetAngles()
    angles.p = 0
    angles.r = 0
    local point = pos + angles:Forward() * 100
    if not util.IsInWorld(point) then return end
