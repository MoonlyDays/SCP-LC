...
function ENT:Think()
    ...
    if self.DieTime ~= -1 and self.DieTime <= CurTime() then self:TriggerIgnite() end
end

function ENT:StartTouch(ent)
    if self.ArmTime >= CurTime() or not ent:IsPlayer() then return end
    local t = ent:SCPTeam()
...
function ENT:OnTakeDamage(dmg)
    ...
    if not IsValid(att) or not att:IsPlayer() or SCPTeams.IsAlly(att:SCPTeam(), TEAM_SCP) then return end
...
function ENT:TriggerIgnite()
    ...
    if IsValid(owner) and owner:IsPlayer() and owner:SCPClass() == CLASSES.SCP457 then fire:SetOwner(owner) end
