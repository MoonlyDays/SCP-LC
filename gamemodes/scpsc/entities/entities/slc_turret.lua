...
ENT.Modes = {
    {
        "off", --off
        "slc/misc/turret/off.png"
    },
    {
        "filter", --filter staff
        "slc/misc/turret/not_staff.png"
    },
    {
        "target", --only staff
        "slc/misc/turret/staff.png"
    },
    {
        "scp", --scp only
        "slc/misc/turret/scp.png"
    },
    {
        "all", --all
        "slc/misc/turret/all.png"
    },
    {
        "supp", --suppressive
        "slc/misc/turret/supp.png"
    },
}
...
function ENT:Think()
    ...
    if SERVER then
                ...
                if self.NFireBullets <= ct then
                    if 2 * math.pi * line:Length2D() * math.abs(ang.y - self.PredictedAngle.y) / 360 < 5 then
                        ...
                        if not IsValid(attacker) then
                            attacker = self
                        elseif not attacker:CheckSignature(self:GetOwnerSignature()) then
                        ...
                        if attacker:IsPlayer() then attacker:SetProperty("kill_team_override", nil) end
        ...
        if speed > 0 then self.PredictedAngle = self.PredictedAngle:Approach(Angle(pitch, yaw, 0), FrameTime() * speed) end
    end
...
function ENT:CSUse(ply)
    ...
    ply:StartHold(self, "slc_turret_mode", IN_USE, 0, function() CloseWheelMenu() end, true)
    local mode = self:GetMode()
    local options = {
        {
    ...
    end, function() return not IsValid(self) or ply:UpdateHold(self, "slc_turret_mode") ~= nil end, "slc/misc/turret/exit.png")
end
...
function ENT:RequestMode(ply, mode)
    ...
    if mode == 0 then
        ...
        if SERVER then ply:EnableProgressBar(true, ct + 5, "lang:WEAPONS.TURRET.pickup_turret", Color(200, 200, 200, 255), Color(50, 225, 25, 255)) end
...
if SERVER then
    ...
    function AddTurretFilterModels(mdl)
        if not istable(mdl) then mdl = {mdl} end
    ...
    local function turret_test_range_vis(self, target, shoot_pos, shoot_ang, vertical, horizontal)
        ...
        if math.abs(ang.p) <= vertical + 3 and math.abs(ang.y) <= horizontal + 3 then
            ...
            if tr.Entity == target or tr.Entity == NULL then return true end
        end
        return false
    end

    local function test_target(model, mode, ply)
        if mode == 2 then return not SLC_TURRET_FILTER_MODELS[model] end
    ...
    function ENT:FindTarget(ply)
        ...
        if mode > 1 then
            ...
            if IsValid(ply) and ply:Alive() and test_target(ply:GetModel(), mode, ply) then if turret_test_range_vis(self, ply, shoot_pos, shoot_ang, self.VerticalRange, self.HorizontalRange) then return ply end end
            for i, v in ipairs(SCPTeams.GetPlayersByInfo(SCPTeams.INFO_ALIVE, true)) do
                if test_target(v:GetModel(), mode, v) then if turret_test_range_vis(self, v, shoot_pos, shoot_ang, self.VerticalRange, self.HorizontalRange) then return v end end
            end
        end
    end
    ...
    AddTurretFilterModels({"models/scp/guard_med.mdl", "models/alski/mtfsupport/mtf1.mdl", "models/alski/mtfsupport/mtf_medyk.mdl", "models/scp/captain.mdl", "models/alski/mtfsupport/saper_mtf.mdl"})
...
hook.Add("StartCommand", "SLCTurretENTCMD", function(ply, cmd)
    if not ply.PickingTurret then return end
