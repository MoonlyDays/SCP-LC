...
function ENT:Initialize()
    ...
    for i = 1, self.ParticleNum do
        self.Particles[i] = {
            pos = pos + ang:Forward() * 64 * size,
            roll = SLCRandom() * math.pi * 2 - math.pi,
            delta = (SLCRandom(0, 1) * 2 - 1) * 0.08
        }
...
function ENT:Think()
    ...
    if self.NextDamageTick < ct then self.NextDamageTick = ct + 0.5 end
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        if wep_valid and not wep:CanTargetPlayer(v) then continue end
        if not wep_valid then
            local t = v:SCPTeam()
        ...
        if wep_valid then dmg:SetAttacker(owner) end
...
function ENT:OnRemove()
    ...
    for i, v in ipairs(self.Particles) do
        if v.particle then v.particle:SetDieTime(0) end
    end
end
