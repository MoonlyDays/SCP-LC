...
local CARD_POS = {
    {
        pos = Vector(-1, 3.3, 18.8),
        ang = Angle(0, 90, 0)
    },
    {
        pos = Vector(-1, 3.3, 8.9),
        ang = Angle(0, 90, 0)
    },
}
...
function ENT:Think()
    if SERVER then
        ...
        if state == STATE_NOREMOTE and self.NUse <= ct then self:SetState(STATE_READY) end
    end
end
...
function ENT:AccessOverride(ply, data)
    ...
    if not self.Usable then
        if not self.FirstCard and ply:HasWeapon("item_slc_alpha_card1") then
            self.FirstCard = true
        ...
        elseif not self.SecondCard and ply:HasWeapon("item_slc_alpha_card2") then
    ...
    if not self.Usable or self.NUse > CurTime() then return false, "" end
end

function ENT:OnUse(ply)
    if MAP_CHECKERS.ALPHA_REMOTE() and not GetRoundStat("goc_countdown") and not IsGOCDeviceValid() then
