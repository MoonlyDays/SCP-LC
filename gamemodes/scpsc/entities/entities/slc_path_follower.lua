...
function ENT:Initialize()
    if SERVER then self.BehaviourThread = coroutine.create(function() self:Behaviour() end) end
end
...
function ENT:Move()
    while true do
        ...
        if not self.Path then return "no path" end
        local len = #self.Path
        if len == 0 then return "no path" end
        local result = self:OnMove()
        if result then return result ~= true and result or "canceled" end
        ...
        new_dir:Normalize()
        if new_dir:Dot(dir) <= -0.9 then pos = goal end
        self:SetPos(pos)
        local ang = dir:Angle()
...
function ENT:CutCorner(cur_point, prev_point, next_point)
    ...
    if vec_dist < radius then radius = vec_dist end
    ...
    if DEVELOPER_MODE then debugoverlay.Line(center, center + center_angle:Forward() * radius, 30, Color(0, 255, 255), true) end
...
function ENT:Repath(max_steps)
    ...
    if DEVELOPER_MODE then SLCDrawPath(path, 30) end
    table.remove(path, 1)
    self.Path = path
end
...
function ENT:MoveTo(vec)
    ...
    if repath == nil then return self:Move() end
    return repath
end
