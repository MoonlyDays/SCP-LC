...
function ENT:Think()
	if SERVER then
		...
		if time >= ct then
			...
			if state == STATE_ACTIVE and (not IsValid(ply) or not ply:Alive() or ply:GetPos():DistToSqr(self:GetPos()) > 10000) then self:StopTransmit() end
			return
		end
...
function ENT:AccessOverride(ply, data)
	if not self.Usable or self.NUse > CurTime() or self:GetState() ~= STATE_IDLE or ply:GetPos():DistToSqr(self:GetPos()) > 10000 or IsPAActive() then return false, "" end
end
...
hook.Add("PlayerSpeakOverride", "SLCIntercom", function(talker)
	if not IsValid(INTERCOM_SCREEN) or not IsValid(INTERCOM_SCREEN.ActivePlayer) then return end
	if talker == INTERCOM_SCREEN.ActivePlayer then return true end
end)
...
if CLIENT then
	...
	function ENT:RenderScreen(width, height)
		...
		if text then draw.MultilineText(width / 2, height * 0.09, text, "SCPAlphaDisplay", tc, width, 10, 5, TEXT_ALIGN_CENTER) end
	end
end