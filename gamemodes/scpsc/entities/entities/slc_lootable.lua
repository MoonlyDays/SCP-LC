...
function ENT:Initialize()
    ...
    if SERVER then self:InstallTable("Lootable") end
end
...
function ENT:Think()
    ...
    if self.NCheck <= rt then
        self.NCheck = rt + 1
        self:CheckListeners()
        if self.RemoveOnEmpty and not next(self.LootItems) then
...
if CLIENT then
    ...
    function ENT:Draw()
        if self:GetShouldRender() then self:DrawModel() end
        ...
        if len <= 160000 then
            if len >= 8100 and not GetSettingsValue("search_indicator") then return end
    ...
    hook.Add("SLCRegisterSettings", "SLCSearchSettings", function() RegisterSettingsEntry("search_indicator", "switch", true, nil, "hud_config") end)
end
