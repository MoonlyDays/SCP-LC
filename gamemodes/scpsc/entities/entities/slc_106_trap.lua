...
function ENT:Think()
    ...
    if self:GetDieTime() <= CurTime() or not IsValid(owner) or owner:SCPClass() ~= CLASSES.SCP106 then
        self.Destroyed = true
        self:Remove()
        return
    end
    ...
    if not trace_tab.Hit then return end
    local ent = trace_tab.Entity
    if not IsValid(ent) or not ent:IsPlayer() or SCPTeams.IsAlly(ent:SCPTeam(), TEAM_SCP) then return end
    self.Triggered = true
