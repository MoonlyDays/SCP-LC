...
function ENT:Initialize()
    ...
    if SERVER then
        ...
        if self:Get_DieTime() == 0 then self:Set_DieTime(CurTime() + self:Get_BurnTime()) end
    end
...
function ENT:OnRemove()
    if SERVER then self:StopSound("General.BurningObject") end
    if CLIENT and IsValid(self.Particles) then self.Particles:StopEmission() end
end
...
function ENT:Think()
    ...
    if CLIENT then
        if not IsValid(self.Particles) then
            if not self:GetDontCreateParticles() and not self.DontCreateParticlesOverride then self.Particles = CreateParticleSystem(self, "scp_457_fire", PATTACH_ABSORIGIN, 0, Vector(0, 0, 10)) end
    ...
    if self:Get_DieTime() ~= -1 and self:Get_DieTime() < CurTime() or water_level == 4 or IsValid(parent) and parent:IsPlayer() and not parent:Alive() then
        if CLIENT and IsValid(self.Particles) then self.Particles:StopEmission() end
        if SERVER then
            self:Remove()
            return
    ...
    if self.Signature and IsValid(owner) then if not owner:CheckSignature(self.Signature) then self:Remove() end end
    ...
    if IsValid(parent) and self.HurtParent and (self.HurtOwner or parent ~= owner) then
        ...
        if hook.Run("SLCFireOnBurnPlayer", self, parent, dmg) ~= true then parent:TakeDamageInfo(dmg) end
    end
    ...
    for k, v in pairs(FindInCylinder(self:GetPos() + self.Offset, self.Radius, -48, 48, nil, MASK_SOLID_BRUSHONLY, player.GetAll())) do
        if not v:IsPlayer() or v == parent or v:SCPTeam() == TEAM_SPEC or not self.HurtOwner and v == owner then continue end
        ...
        if final_dmg > 0 then
            ...
            if hook.Run("SLCFireOnBurnPlayer", self, v, dmg) ~= true then v:TakeDamageInfo(dmg) end
        end

        if self.ShouldSpread then if SLCRandom(100) <= self.Power then v:Burn(math.max(self:Get_BurnTime() * 0.75, 1), self.Power * 0.75, owner, self.Damage, self.Signature ~= nil, self.DontOverride) end end
    end
end
