...
function ENT:Initialize()
    ...
    if self:GetFuse() == 0 then self:SetBodygroup(1, 1) end
end
...
function ENT:Use(activator, caller, use_type, value)
    ...
    if not self.Open then
        ...
        local isscp = activator:SCPTeam() == TEAM_SCP and not activator:GetSCPHuman()
...
function ENT:InstallFuse(ent)
    ...
    ent:Remove()
    if self.Time > 0 then self.BreakTime = CurTime() + self.Time end
    self:EmitSound("SLC.FuseBox.Disable")
...
function ENT:RemoveFuse(ply, drop)
    if drop then
        ...
        if IsValid(ent) then
            ...
            if IsValid(phys) then phys:SetVelocity(forward * 150) end
        end
    else
        local ent = ply:Give("item_slc_fuse")
        if IsValid(ent) then ent:SetRating(self:GetFuse()) end
    end
...
sound.Add({
    ...
    volume = 1,
    level = 80,
    sound = {"ambient/energy/zap1.wav", "ambient/energy/zap2.wav", "ambient/energy/zap3.wav",},
    channel = CHAN_STATIC,
})
