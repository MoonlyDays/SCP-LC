...
function ENT:Think()
    ...
    if not self:GetDestroyed() and self:Finished() and not self.Detonated and self:GetStartTime() > 0 then
        self.Detonated = true
        ...
        for k, v in pairs(SCPTeams.GetPlayersByTeam(TEAM_GOC)) do
            if not v:IsInZone(ZONE_WARHEAD) then continue end
            v:AddXP(xp)
            PlayerMessage("xp_goc_device$" .. xp, v)
            local mask = v:GetWeaponByGroup("gasmask")
            if IsValid(mask) then mask:Refill() end
        end
    end
end
...
function ENT:OnTakeDamage(dmginfo)
    ...
    if not IsValid(att) or not att:IsPlayer() or SCPTeams.IsAlly(att:SCPTeam(), TEAM_GOC) then return end
