...
function ENT:Think()
    if SERVER then
        ...
        if state == STATE_WAITING and self.Waiting <= ct then self.Listener:Broadcast(self, true, 0, self.NUse) end
        if (state == STATE_FAILED or state == STATE_NOREMOTE) and self.NUse <= ct then self:SetState(STATE_IDLE) end
    end
end
...
if CLIENT then
    ...
    function ENT:RenderScreen(width, height)
        ...
        if text then draw.MultilineText(width / 2, height * 0.09, text, "SCPOmegaDisplay", tc, width, 10, 5, TEXT_ALIGN_CENTER) end
    end
