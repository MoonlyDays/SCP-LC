...
function ENT:Think()
    ...
    if self.NextAttack >= 0 then
        ...
        for i, v in ipairs(SCPTeams.GetPlayersByInfo(SCPTeams.INFO_HUMAN)) do
            if not self:CanTarget(v) or not self:TestVisibility(v, nil, false, 44, 58, 0) and v:GetPos():Distance2DSqr(pos) > 900 then continue end
            if not self.Targets[v] and v:GetSCP714() then continue end
            if self.NextAttack < ct + 3 then self.NextAttack = ct + 3 end
    ...
    if self.NextAttack > 0 and self.NextAttack <= ct then
        ...
        AddTimer("SCP689Attack" .. self:EntIndex(), SLCRandom(15, 90), 1, function() self:DoAttack() end)
    end
...
function ENT:CanTarget(ply)
    local t = ply:SCPTeam()
    return t ~= TEAM_SPEC and not SCPTeams.IsAlly(TEAM_SCP, t)
end

function ENT:CheckTargets()
    local num = 0
    for k, v in pairs(self.Targets) do
        if not IsValid(k) or not k:CheckSignature(v) then
            self.Targets[k] = nil
        else
            num = num + 1
        end
...
function ENT:GetTargets()
    ...
    local num = 0
    for k, v in pairs(self.Targets) do
        if not k:IsInZone(ZONE_FLAG_ANOMALY) then
            num = num + 1
            plys[num] = k
        end
    end
