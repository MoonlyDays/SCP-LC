...
function ENT:Initialize()
    ...
    if SERVER then self:SetMaxHealth(self.HP) end
...
function ENT:Think()
    ...
    if not IsValid(owner) or not owner:CheckSignature(self.OwnerSignature) then
        self:Remove()
        return
    end
    ...
    if self.NextPassive <= ct and wep:GetPassiveCooldown() < ct then
        ...
        for i, v in ipairs(player.GetAll()) do
            if not wep:CanTargetPlayer(v) or v:GetPos():DistToSqr(pos) > wep.PassiveRadius or v:IsBurning() then continue end
        ...
        for i, v in ipairs(player.GetAll()) do
            ...
            if invis_radius > 0 and dist > invis_radius or not wep:CanTargetPlayer(v) or not self:TestVisibility(v) then continue end
            if dist < closest_dist then
                closest_dist = dist
                closest_prey = v
            end

            if not wep.Preys[v] then wep:AddPrey(v) end
        end

        if closest_prey then self.ClosestPrey = closest_prey end
        wep:CheckPreys()
    end
    return true
end
...
function ENT:OnTakeDamage(dmg)
    ...
    if not IsValid(attacker) or not attacker:IsPlayer() or SCPTeams.IsAlly(attacker:SCPTeam(), TEAM_SCP) then return end
...
function ENT:Behaviour()
    ...
    while true do
        ...
        if IsValid(self.ClosestPrey) and self.ClosestPrey:GetPos():DistToSqr(self:GetPos()) <= 9000000 then
            ...
            if pos then self:MoveTo(pos) end
        end

        self:ResetSequence(self:SelectWeightedSequence(ACT_HL2MP_IDLE_FIST))
        if not IsValid(self.ClosestPrey) then
            local wait_time = CurTime() + 2
            while wait_time > CurTime() and not IsValid(self.ClosestPrey) do
                coroutine.yield()
            end
        end
    end
