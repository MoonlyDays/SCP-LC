...
local function snake_load()
	...
	if snake_dir_x ~= 0 or snake_dir_y ~= 0 then pause = RealTime() + 3 end
	snake_score = snake_segments:Size()
end

local function snake_save()
	if game_over then snake_new_game() end
	local segments = {}
...
local function snake_update()
	...
	elseif tile == 2 then
		...
		if snake_score > snake_high_score then snake_high_score = snake_score end
		for i, v in ipairs(snake_apples) do
	...
	if #snake_apples < 3 then snake_next_apple = snake_next_apple - 1 end
	--Render changes
	render.PushRenderTarget(snake_texture)
	cam.Start2D()
	surface.SetDrawColor(200, 48, 48, 255)
...
local function snake_open()
	if IsValid(SNAKE_FRAME) or not CanOpenMinigame(LocalPlayer()) then return end
	...
	board.Paint = function(this, pw, ph)
		...
		if this.NextUpdate < rt then
			...
			if this.NextUpdate < rt then this.NextUpdate = rt + 0.125 end
			snake_update()
		end
...
hook.Add("SLCCloseMinigames", "SLCSnake", function()
	if not IsValid(SNAKE_FRAME) then return end
	snake_save()
	SNAKE_FRAME:Remove()
end)