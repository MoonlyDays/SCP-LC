...
if CLIENT then hook.Add("OnEntityCreated", "SCP106Collision", function(ent) timer.Simple(0, function() setup_collision_106(ent) end) end) end
local scp106_bounds = Vector(32, 15, 55)
local scp106_mid = Vector(2304, 4612.5, 568) --Vector( 2304, 4975, 568 )
local scp106_num = 8
local scp106_rad = 350
local scp106_off = {-0.5, -0.8, -0.65, 0, 0.6, 0.8, 0.6, 0}
local pos_106tp_pd = {Vector(2297.00, 4588.00, 523.00), Vector(2369.00, 6085.00, 347.00), Vector(1718.00, 6072.00, 139.00), Vector(2698.00, 6059.00, 139.00), Vector(3711.00, 4865.00, 651.00), Vector(3717.00, 3877.00, 907.00),}
local pos_106tp_hcz = {Vector(2410.00, 2889.00, -373.00), Vector(4693.00, 3020.00, -117.00), Vector(4159.00, 3026.00, 11.00), Vector(4156.00, 1077.00, 12.00), Vector(3470.00, -192.00, 11.00), Vector(5447.00, -194.00, 12.00), Vector(2863.00, 1741.00, 11.00), Vector(2113.00, 4060.00, 12.00),}
local pos_106tp_ez = {Vector(836.00, 2976.00, 11.00), Vector(-780.00, 2821.00, 11.00), Vector(-2361.00, 2394.00, 11.00), Vector(-1661.00, 3655.00, 11.00), Vector(-1054.00, 3839.00, -53.00), Vector(-647.00, 4321.00, -53.00), Vector(136.00, 3426.00, -117.00), Vector(-1532.00, 2526.00, -117.00), Vector(-1930.00, 2543.00, -117.00), Vector(-3378.00, 2261.00, 139.00), Vector(-1087.00, 2224.00, 11.00),}
local pos_106tp_surf = {Vector(-638.00, 5023.00, 2571.00), Vector(-992.00, 5500.00, 2315.00), Vector(-482.00, 5503.00, 2315.00), Vector(-77.00, 7294.00, 2571.00), Vector(-2491.00, 6592.00, 3115.00), Vector(426.00, 6724.00, 2187.00), Vector(-1983.00, 6631.00, 2187.00), Vector(-3128.00, 5329.00, 2187.00), Vector(-690.00, 6887.00, 2571.00), Vector(-2741.00, 7373.00, 3115.00), Vector(-77.00, 6533.00, 3115.00), Vector(-2422.00, 6744.00, 2571.00),}
local function scp106_tp_func(self, ent)
    ...
    if status then
        local tab = not GetRoundStat("omega_warhead") and (GetGasPower(ZONE_HCZ) == 0 and pos_106tp_hcz or (GetRoundStat("alpha_warhead") or GetGasPower(ZONE_EZ) == 0) and pos_106tp_ez) or pos_106tp_surf
...
function CanDestroyDoor(ent, check_velocity)
    return IsValid(MAP_DOOR_PAIRS[ent]) and (not check_velocity or ent:GetVelocity():LengthSqr() < 1)
end

function GetDoorEnts(ent)
    local other = MAP_DOOR_PAIRS[ent]
    if not other then return {ent, MAP_DOOR_PROPS[ent]} end
    return {ent, other, MAP_DOOR_PROPS[ent], MAP_DOOR_PROPS[other],}
end
...
function TranslateDoorModel(ent, breach_ang)
    ...
    if math.abs(math.AngleDifference(breach_ang.y, ang.y)) > 90 then ang.y = ang.y + 180 end
    ...
    if zone == "hcz" and math.abs(math.abs(prop:GetAngles().y) - 90) < 1 then pos = pos + hcz_fix[prop:GetModel()] end
...
local function fix_french_map()
    ...
    for k, v in pairs(MAP_DOOR_PAIRS) do
        if k:GetPos():DistToSqr(v:GetPos()) > 2500 then mismatch[k] = true end
    end
    ...
    for door, _ in pairs(mismatch) do
        ...
        if not fix then FRENCH_DOOR_SWAP[door:GetName()] = false end
    end
end
...
local function new_914_logic(ent, input, activator, caller, data)
    ...
    AddTimer("Site19SCP914Logic_LockButton", 0.5, 1, function() use:Fire("Lock") end)
...
hook.Add("SLCPreround", "Site19Preround", function()
    ...
    for i, v in ipairs(ents.GetAll()) do
        ...
        elseif name == "049_door" then
            v:SetKeyValue("forceclosed", "1")
        elseif mapid == 3240 and CVAR.slc_disable_fuseboxes:GetInt() == 0 then
            --Unlock Gate B
            v:Fire("Unlock")
        elseif name == "bt_914_tirette" then
            v:ClearAllOutputs("OnPressed")
            CatchInput("bt_914_tirette", new_914_logic)
        elseif mapid == 3034 or mapid == 3551 then
            --Make forest teleports bigger
            v:UseTriggerBounds(true, 32)
        end

        if remove_name[name] or remove_mapid[mapid] then v:Remove() end
        if class == "func_door" and string.match(name, "^elev_") then v:SetKeyValue("dmg", 0) end
        if class == "func_door" and string.match(name, "_containment_door_") then v:SetKeyValue("dmg", 0.2) end
        if class == "func_areaportal" then MAP_DOOR_AREAPORTALS[v:GetKeyValues().target] = v end
        local door_zone, door_id = string.match(name, "^(%w-)_door_[12]_(%d+)$")
        if door_zone and door_id and door_allowed_zones[door_zone] then
            door_lookup[name] = v
            if not MAP_DOOR_PAIRS_TMP[door_zone] then MAP_DOOR_PAIRS_TMP[door_zone] = {} end
            if not MAP_DOOR_PAIRS_TMP[door_zone][door_id] then MAP_DOOR_PAIRS_TMP[door_zone][door_id] = {} end
            table.insert(MAP_DOOR_PAIRS_TMP[door_zone][door_id], v)
            local prop = v:GetChildren()[1]
            MAP_DOOR_PROPS[v] = prop
            if IsValid(prop) then MAP_DOOR_POS[v] = prop:GetPos() end
        end
    ...
    CatchInput("gate_b_enter", function(ent, input, activator, caller, data) if input == "Lock" then return true end end)
