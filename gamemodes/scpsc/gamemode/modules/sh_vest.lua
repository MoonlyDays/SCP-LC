...
function VEST.Register(name, data, randomvest, callback)
    assert(not VEST.vests[name], "Vest '" .. name .. "' is already registered!")
    assert(data.damage and data.model, "Invalid data for vest '" .. name .. "'!")
    if not data.weight then data.weight = 2 end
    if not data.mobility then data.mobility = 0.9 end
    if not data.durability then data.durability = -1 end
    ...
    if randomvest then table.insert(VEST.rand, id) end
    if callback then VEST.callbacks[id] = callback end
    data.name = name
    data.id = id
...
function VEST.GetData(id)
    ...
    if name then return VEST.vests[name] end
end
...
timer.Simple(0, function() GUARD_MODELS_LOOKUP = CreateLookupTable(GUARD_MODELS) end)
local function guard_callback(ply, vest, data)
    local mdl = ply:GetModel()
    if GUARD_MODELS_LOOKUP[mdl] then return mdl end
end
...
VEST.Register("ntf", {
    model = "models/alski/mtfsupport/mtf1.mdl",
    damage = {
        [DMG_BULLET] = 0.55,
        [DMG_FALL] = 1.45
    },
...
VEST.Register("mtf_medic", {
    model = "models/alski/mtfsupport/mtf_medyk.mdl",
    damage = {
        [DMG_BULLET] = 0.55,
        [DMG_FALL] = 1.45
    },
...
VEST.Register("alpha1", {
    model = "models/alski/mtfsupport/saper_mtf.mdl",
    damage = {
        [DMG_BULLET] = 0.35,
        [DMG_FALL] = 1.65
    },
...
VEST.Register("ci", {
    model = "models/player/alski/ci/ci_soldier.mdl",
    damage = {
        [DMG_BULLET] = 0.5,
        [DMG_FALL] = 1.5
    },
...
VEST.Register("cimedic", {
    model = "models/player/alski/ci/ci_medyk.mdl",
    damage = {
        [DMG_BULLET] = 0.5,
        [DMG_FALL] = 1.5
    },
...
VEST.Register("goc", {
    model = "models/player/cheddar/goc/goc_soldier2.mdl",
    damage = {
        [DMG_BULLET] = 0.4,
        [DMG_FALL] = 1.45
    },
...
VEST.Register("gocmedic", {
    ...
    mobility = 0.9,
    weight = 3,
    bodygroups = goc_medic
})

VEST.Register("goccom", {
    model = "models/player/cheddar/goc/goc_soldier2.mdl",
    damage = {
        [DMG_BULLET] = 0.38,
        [DMG_FALL] = 1.45
...
VEST.Register("hazmat", {
    ...
    mobility = 0.9,
    weight = 2.5,
    prop_model = "models/mishka/models/hazmat.mdl"
}, true)
