...
local showdetails = {"name", "team", "tier", "health", "sanity", "walk_speed", "run_speed", "chip", "slots", "persona", "loadout", "weapons", "hp", "speed", "no_select"}
local headers = {"classes", "support", "scp"}
...
local function addClass(tab, p, h)
    ...
    if override ~= true and not ply:IsClassUnlocked(tab.name) and tab.hide then return end
    local button = vgui.Create("DButton", p)
    button:Dock(TOP)
    button:DockMargin(5, 5, 5, 0)
    button:SetTall(h)
    button:SetText("")
    if not ply:CanUnlockClassTier(tab.tier) then
    ...
    local use_roman = not GetSettingsValue("hud_avoid_roman")
    button.Paint = function(self, pw, ph)
        surface.SetDrawColor(COLOR.outline)
        surface.DrawOutlinedRect(pw * 0.1, 0, pw * 0.8, ph)
        local tier_lock = not ply:CanUnlockClassTier(tab.tier)
        ...
        if not unlocked and tier_lock then
    ...
    button.DoClick = function(self)
        ...
        if mdl_override then
            if mdl_override.model then model[1] = mdl_override.model end
            if mdl_override.skin then model[2] = mdl_override.skin end
            if mdl_override.bodygroups then model[3] = mdl_override.bodygroups end
        end

        showinfo.mvp:SetModel(model[1])
        local ent = showinfo.mvp:GetEntity()
        if model[2] then ent:SetSkin(model[2]) end
        if model[3] then
            for k, v in pairs(model[3]) do
                if isstring(k) then k = ent:FindBodygroupByName(k) end
        ...
        if seq > -1 then ent:SetSequence(seq) end
        showinfo.mvp:Update(tab.name, tab.tier == -1)
    end

    button.DoDoubleClick = function(self)
        if not ply:IsClassUnlocked(tab.name) and ply:UnlockClass(tab.name) then
...
function rebuildView()
    if not IsValid(showinfo.panel) then return end
    if not LANG.headers then return end
    local h = ScrH()
...
local function openViewer()
    local ply = LocalPlayer()
    if not ply.playermeta then return end
    local w, h = ScrW(), ScrH()
    ...
    exit_b.DoClick = function(self) window:Close() end
    ...
    mv.Update = function(self, name, prestige)
        ...
        self.buy:SetVisible(not ply:IsClassUnlocked(name))
    end
    ...
    details.Paint = function(self, pw, ph)
        ...
        if tab then
            for i, v in ipairs(showdetails) do
                local val = tab[v]
                if v == "loadout" then
                    if not GetLoadout(val) then continue end
                    val = {val}
                end
                    ...
                    for key, data in pairs(val) do
                        ...
                        if v == "weapons" then
                            ...
                            if istable(data) then
                                txt = txt .. LANG.random
                                if data.amount and data.amount > 1 then txt = txt .. " (" .. data.amount .. ")" end
                                txt = txt .. ":"
    ...
    details.OnMouseWheeled = function(self, delta) self.scroll = math.Clamp(self.scroll - delta * 8, 0, self.maxScroll) end
    details.scroll = 0
    details.maxScroll = 0
    ...
    info.OnMouseWheeled = function(self, delta) self.scroll = math.Clamp(self.scroll - delta * 8, 0, self.maxScroll) end
    info.scroll = 0
    info.maxScroll = 0
end
...
net.ReceivePing("SLCrefundClasses", function(data)
    ...
    if sp > 0 or pp > 0 then SLCLegacyPopup(LANG.classviewer, string.format(LANG.refunded, sp, pp)) end
end)
