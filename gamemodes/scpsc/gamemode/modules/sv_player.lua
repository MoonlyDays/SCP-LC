...
function PLAYER:SetSCPClass(class)
    if not self.Set_SCPClass then self:DataTables() end
...
function PLAYER:Cleanup(norem)
    ...
    if IsValid(holster) then self:SetActiveWeapon(holster) end
    if not norem then
        self:SetVest(0)
        self:RemoveAllAmmo()
        self:StripWeapons()
...
function PLAYER:Despawn()
    for k, v in pairs(self:GetWeapons()) do
        if v.DespawnDrop then self:PlayerDropWeapon(v:GetClass(), true, true) end
    end
...
function PLAYER:DropEQ()
    ...
    local holster = self:GetWeaponByBase("item_slc_holster")
    if IsValid(holster) and holster ~= wep then self:SetActiveWeapon(holster) end
    if IsValid(wep) then self:PlayerDropWeapon(wep:GetClass(), true, true) end
end
...
local function give_player_weapon(ply, class, ammo)
    ...
    if IsValid(wep) then
        if not ammo then return wep end
        local clip_size = wep.Primary.ClipSize
        if clip_size == -1 then clip_size = 1 end
        ammo = clip_size * ammo
        local clip = math.min(ammo, wep:GetMaxClip1())
        if clip == -1 then clip = 1 end
        local reserve = ammo - clip
        wep:SetClip1(clip)
        if reserve > 0 then ply:GiveAmmo(reserve, wep:GetPrimaryAmmoType()) end
    else
...
local function setup_player_internal(self, class, spawn)
    ...
    if class.persona and class.persona.class then self:Set_SCPPersonaC(class.persona.class) end
    if class.persona and class.persona.team then self:Set_SCPPersonaT(class.persona.team) end
    ...
    if class.backpack then
        ...
        if bp_id then
            ...
            if IsValid(bp) then bp:SetBackpack(bp_id) end
        end
    end
    ...
    if class.omnitool then
        ...
        if IsValid(omnitool) and chip then omnitool:SetChipData(chip, GenerateAccessOverride(chip)) end
    elseif chip then
        local wep = self:Give("item_slc_access_chip")
        if IsValid(wep) then wep:SetChipData(chip) end
    end

    if class.loadout then give_player_weapon(self, "loadout:" .. class.loadout) end
    ...
    self:SetupHands()
    if class.spawn_protection then self:ApplyEffect("spawn_protection") end
    self.ClassData = table.Copy(class)
    if class.vest then self:EquipVest(class.vest, true) end
    if class.callback then class.callback(self, class) end
...
function PLAYER:SetupPlayer(class, spawn, instant)
    if instant then
        ...
        self:AddTimer("Spawn", INFO_SCREEN_DURATION, 1, function() setup_player_internal(self, class, spawn) end)
    end
end
...
function PLAYER:EquipVest(vest, silent, dur)
    ...
    if data then
        if not hook.Run("SLCArmorPickedUp", self) then
            self:SetVest(vest)
            self:SetVestDurability(dur or data.durability or -1)
            local use_model
            local callback = VEST.GetCallback(vest)
            if callback then use_model = callback(self, vest, data) end
            local bodygroups = {}
...
function PLAYER:DropVest(silent)
    ...
    if info[2] then self:SetSkin(info[2]) end
    if info[3] then
        for k, v in pairs(info[3]) do
            if isstring(k) then k = self:FindBodygroupByName(k) end
            if k > -1 then self:SetBodygroup(k, v) end
        end
    end
...
function PLAYER:CreatePlayerRagdoll(disable_loot)
    ...
    if disable_loot ~= true and self:IsHuman() then
        ...
        for i, v in pairs(self:GetWeapons()) do
            if not IsValid(v) or v.Droppable == false or v.DespawnDrop or v.SCP then continue end
            ...
            table.insert(loot, {
                ...
                func = function(p, w_class, data) return nil, nil, p:RestoreWeapon(data) end
            })
        end
        ...
        if disable_loot == "force_backpack" then
            ...
            if backpack then backpack:SetLootData(w, h, loot) end
        else
            ...
            rag:CallOnRemove("looting", function(ent)
                ...
                if backpack then backpack:CopyLootData(ent) end
            end)
    ...
    if t ~= TEAM_SCP then self._RagEntity = rag end
    return rag
end
...
function PLAYER:Blink(delay, duration)
    ...
    if not duration then duration = 0.25 end
    if delay < duration then delay = duration end
    self:SetBlink(true)
    ...
    net.Send(self)
    self:AddTimer("PlayerUnBlink", duration + 0.05, 1, function() if IsValid(self) then self:SetBlink(false) end end)
...
function PLAYER:PushSpeed(walk, run, crouch, id, maxstack, force_mult)
    ...
    if self.SpeedStack then
        if type(id) == "string" then
            for i, v in ipairs(self.SpeedStack) do
                if v.id == id then
                    if not maxstack or maxstack == -1 or not isnumber(maxstack) or v.stacks < maxstack then v.stacks = v.stacks + 1 end
                    return true
                end
            end
        else
    ...
    if crouch ~= -1 then self:SetCrouchedWalkSpeed(crouch) end
    local spid = self.SpeedStack[#self.SpeedStack].spid + 1
    if not id then id = spid end
    table.insert(self.SpeedStack, {
        id = id,
        spid = spid,
        walk = walk,
        run = run,
...
function PLAYER:PopSpeed(id, all)
    if not self.SpeedStack then return end
...
function PLAYER:SetBaseSpeed(walk, run, crouch)
    if not self.SpeedStack then self.SpeedStack = {} end
    self.SpeedStack[1] = {
        id = 0,
        spid = 0,
        walk = walk,
...
function PLAYER:PlayerDropWeapon(class, all, force)
    ...
    if not force and (wep.PreventDropping == true or wep.CanDrop and wep:CanDrop() == false or hook.Run("SLCPlayerCanDropWeapon", self, class, all) == false or wep.eq_slot and wep.eq_slot > 6) then return end
        ...
        if count > 1 then
            ...
            for i = 1, all and count or 1 do
                ...
                if IsValid(new) then
                    ...
                    if IsValid(phys) then phys:SetVelocity(forward * 300 + VectorRand() * 10) end
                end
    ...
    if wep.SLCPreDrop then wep:SLCPreDrop(force, all) end
    self:DropWeapon(wep)
...
function PLAYER:StoreWeapon(class, norem)
    ...
    if IsValid(wep) then
        ...
        if wep.GetCustomClip then data.clip_custom = wep:GetCustomClip() end
        if wep.StoreWeapon then wep:StoreWeapon(data, self) end
        if not norem then wep:Remove() end
        return data
    end
end
...
function PLAYER:RestoreWeapon(data, wep)
    if not data._stored then return end
    local class = data.class
    if not IsValid(wep) and self:GetFreeInventory() > 0 and hook.Run("SLCCanPickupWeaponClass", self, class) ~= false then wep = self:Give(class) end
    if IsValid(wep) then
        wep:SetClip1(data.clip1)
        wep:SetClip2(data.clip2)
        if data.clip_custom and wep.SetCustomClip then wep:SetCustomClip(data.clip_custom) end
        if wep.RestoreWeapon then wep:RestoreWeapon(data, self) end
        return wep
    end
end
...
function PLAYER:UpdateEQ()
    ...
    for k, v in pairs(inventory) do
        if not IsValid(v) or not lookup_weps[v] then
            --print( "REM WEP", k, v )
            inventory[k] = nil
        end
    end
...
function PLAYER:MoveItem(from, to, from_class, to_class)
    if not self:Alive() or self:SCPTeam() == TEAM_SPEC then return end
    local ct = CurTime()
    local inventory = self:GetProperty("inventory", {})
    ...
    if backpack then
        if transition < ct then transition = ct end
    ...
    if time and time > 0 then
        AddTimer("Stasis_" .. self:SteamID64(), time, 1, function(this, n)
            if IsValid(self) then
                ...
                if s_data and self:CheckSignature(s_data.signature) then self:DisableStasis() end
            end
        end)
    end
end
...
function PLAYER:MakeAFK(timeout)
    ...
    if timeout then self.SoftAFK = rt + timeout end
end
...
Timer("SLCAFKCheck", 10, 0, function(self, n)
    ...
    for k, v in pairs(players) do
        ...
        if not v:IsAFK() then
            ...
            if v:Alive() and afk_autoslay > 0 and v.SLCAFKTimer and v.SLCAFKTimer + afk_autoslay < rt then
                ...
                v:AddTimer("SLCAFKSlay", 15, 1, function()
                    ...
                    if not v:Alive() or afk_autoslay <= 0 or not v.SLCAFKTimer or v.SLCAFKTimer + afk_autoslay >= rt then return end
        ...
        elseif not SLCAuth.HasAccess(v, "slc afkdontkick") then
...
function PLAYER:IsDoingSLCTask(name)
    ...
    return task and (not name or task.name == name) or false
end
...
function PLAYER:StartSLCTask(name, time, check, show_bar, block_movement, args)
    local promise = SLCPromise(function(resolve, reject)
        if self:IsDoingSLCTask() then self:StopSLCTask(2) end
        ...
        if block_movement then self:DisableControls("slc_task_" .. name, isnumber(block_movement) and block_movement or 0) end
    end)
    return promise
end
...
function PLAYER:StopSLCTask(data)
    local task = self:GetProperty("slc_task")
    if task.show_bar then self:EnableProgressBar(false) end
    if task.block_movement then self:StopDisableControls("slc_task_" .. task.name) end
    if data then task.reject(data) end
...
hook.Add("Tick", "SLCTaskTick", function()
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        elseif task_data and task_data.check then
            if not task_data.check() then v:StopSLCTask(1) end
        end
    end
end)
...
function PLAYER:PlayStepSound(no_update)
    ...
    if mv == MOVETYPE_LADDER then
        ...
        if not step_trace.Hit then util.TraceHull(step_trace) end
        if not step_trace.Hit then return end
        local surf = util.GetSurfaceData(step_trace.SurfaceProps)
        if surf then snd = self.slc_foot == 0 and surf.stepLeftSound or surf.stepRightSound end
    end

    if self:Crouching() then
        snd = snd .. "Crouch"
    elseif self:GetRunSpeed() == self:GetWalkSpeed() or not self:KeyDown(IN_SPEED) then
        snd = snd .. "Walk"
    end

    if hook.Run("SLCPlayerFootstep", self, self.slc_foot, snd) == true then return end
    self:EmitSound(snd)
    if not no_update then self:UpdateStepTime() end
end
...
function PLAYER:AddFrags(f)
    ...
    if f > 0 then
        ...
        if team ~= TEAM_SPEC then SCPTeams.AddScore(team, f) end
    end
...
local xp_scale_mfn = function(tab, mul, cat)
    ...
    if not tab.categories[cat] then tab.categories[cat] = 0 end
...
function PLAYER:AddXP(xp, category)
    ...
    if not self._SLCXPCategories then self._SLCXPCategories = {} end
    ...
    if self:IsPremium() then ref(CVAR.slc_premium_xp:GetFloat() - 1, "vip") end
    ...
    if daily_bonus > 0 then
        ...
        if bonus > daily_bonus then bonus = daily_bonus end
        plyxp = plyxp + bonus
    ...
    if sp_xp > 0 then self:SetSpectatorPoints(self:GetSpectatorPoints() + math.floor((plyxp - orig_xp) / sp_xp)) end
...
function PLAYER:IsAboutToSpawn()
    return not not self:GetProperty("spawning") or not not self:GetProperty("spawning_scp")
end

function PLAYER:IsValidSpectator(ignore_death_screen, softafk)
    return not self:Alive() and self:IsActive() and self:SCPTeam() == TEAM_SPEC and not self:IsAboutToSpawn() and (not self:IsAFK() or softafk and self.SoftAFK and self.SoftAFK >= RealTime()) and (ignore_death_screen or not self.SetupAsSpectator and not self.DeathScreen)
end
