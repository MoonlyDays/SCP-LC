hook.Add("PlayerPostThink", "SLCSanity", function(ply)
	if not ply.NSanity then ply.NSanity = 0 end
	if not ply.NSanityRegen then ply.NSanityRegen = 0 end
	if not SCPTeams.HasInfo(ply:SCPTeam(), SCPTeams.INFO_HUMAN) then return end
	if ply.NSanity < CurTime() then
		...
		if hook.Run("SLCCalcSanity", ply) ~= true then
			...
			for i, v in ipairs(FindInCylinder(ply:GetPos(), 500, 0, 128, nil, nil, player.GetAll())) do
				if v == ply or not SCPTeams.HasInfo(v:SCPTeam(), SCPTeams.INFO_HUMAN) then continue end
			...
			if sanity == 0 and not ply:HasEffect("insane") then ply:ApplyEffect("insane") end
		end
...
function PLAYER:TakeSanity(num, sanitytype, data)
	if not SCPTeams.HasInfo(self:SCPTeam(), SCPTeams.INFO_HUMAN) then return end
	local new = hook.Run("SLCPlayerSanityChange", self, sanitytype, num, data)
	if new == true then return end
	if isnumber(new) then num = new end
	if sanitytype then if SCPTeams.HasInfo(self:SCPTeam(), sanitytype) then num = num * 2 end end
...
function SanityEvent(amount, sanitytype, origin, distance, inflictor, victim)
	...
	if sanitytype then
		...
		if cb then cb(tab, inflictor, victim) end
	end

	for k, v in pairs(tab) do
		if v.amount ~= 0 then v.ply:TakeSanity(v.amount, v.sanitytype) end
	end
end
...
AddSanityType("DEATH", function(data, inflictor, victim)
	for k, v in pairs(data) do
		...
		elseif v.ply == inflictor and v.sanitytype == SANITY_TYPE.DEATH then
			...
			if not IsValid(wep) or not SLC_WEAPONS_REG[wep:GetClass()] then
				table.insert(data, {
					ply = v.ply,
