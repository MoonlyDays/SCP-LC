...
setmetatable(SCPSkillObject, {
    __call = SCPSkillObject.New
})

hook.Add("PlayerButtonDown", "SLCSCPHUDButtons", function(ply, code)
    if not IsFirstTimePredicted() or vgui.CursorVisible() or ply:SCPTeam() ~= TEAM_SCP then return end
    local wep = ply:GetSCPWeapon()
    if not IsValid(wep) or not wep.HUDObject then return end
...
function SCPHUDObjectInstance:ButtonPressed(code)
    for i, v in ipairs(self.Skills) do
        if v.Button == code then v:ButtonPressed() end
    end
end
...
function SCPHUDObjectInstance:Draw()
    ...
    if self.DamageModSkill and not lp:GetSCPHuman() then
        self.DamageModSkill:Draw(dx, y, self.Size)
        dx = dx - self.Size - self.Margin
    end

    if self.OverloadSkill and not lp:GetSCPHuman() and not lp:GetSCPDisableOverload() then
    ...
    if self.NotificationTime > ct then
        ...
        if diff < 0.5 then surface.SetAlphaMultiplier(diff / 0.5) end
...
function SCPHUDObject:New(name, swep)
    ...
    if swep then swep.HUDName = name end
    return tab
end
...
function SCPHUDObject:AddCommonSkills()
    ...
    overload:SetMaterial("slc/hud/scp/door_overload.png", "smooth"):SetCooldownFunction(function(swep, hud) return _SCPOverloadCooldown end)
    dmg_mod:SetMaterial("slc/hud/scp/dmg_mod.png", "smooth"):SetTextFunction(function(swep, hud) return string.format("%.1f%%", GetSCPModifiers(LocalPlayer()).def * 100) end):SetParser(function(swep, lang)
        local ply = LocalPlayer()
        local mods = GetSCPModifiers(ply)
        local def = mods.def * 100
        local buff
        if not ply:GetProperty("scp_buff") then
            buff = MarkupBuilder.StaticPrint(lang.not_bought, color_red)
        elseif not ply:IsInZone(ZONE_SURFACE) then
            buff = MarkupBuilder.StaticPrint(lang.not_surface, color_red)
        else
            buff = string.format(lang.buff, MarkupBuilder.StaticPrint(math.Round(mods.regen_scale * 100, 1) .. "%", color_green), MarkupBuilder.StaticPrint(math.Round(mods.heal_scale * 100, 1) .. "%", color_green))
        end
        return {
...
setmetatable(SCPHUDObject, {
    __call = SCPHUDObject.New
})
...
hook.Add("SLCScreenMod", "SCPVisionMod", function(clr)
    if not GetSettingsValue("scp_nvmod") then return end
    local ply = LocalPlayer()
