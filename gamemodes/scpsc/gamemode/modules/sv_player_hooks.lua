...
function GM:PlayerInitialSpawn(ply)
    ...
    if ply:IsBot() then hook.Run("PlayerReady", ply) end
...
function GM:PlayerSpawn(ply)
    ...
    if IsValid(ply._RagEntity) then ply._RagEntity:Remove() end
...
function GM:PlayerReady(ply)
    ...
    ply:SetActive(true)
    if not ROUND.preparing or ply:GetInfoNum("cvar_slc_preparing_classd", 0) == 0 then
        QueueInsert(ply)
    else
...
function GM:PlayerShouldTakeDamage(ply, attacker)
    ...
    if not attacker:IsPlayer() or ply == attacker then return true end
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
    ...
    if IsValid(attacker) and attacker:IsPlayer() then attacker.Logger:AddKill(ply) end
    if SCPTeams.HasInfo(ply:SCPTeam(), SCPTeams.INFO_HUMAN) then SanityEvent(10, SANITY_TYPE.DEATH, ply:GetPos() + Vector(0, 0, 20), 1000, attacker, ply) end
    ply:DropEQ()
    ply.StoredProperties = ply.SLCProperties
    if not ply._SkipNextRagdoll then ply:CreatePlayerRagdoll() end
...
function GM:PlayerDeath(victim, inflictor, attacker)
    ...
    if killinfo then
        ...
        if not IsValid(attacker) or not attacker:IsPlayer() or victim == attacker then
            if len > 0 then
        ...
        if not victim._SkipNextKillRewards then
            if IsValid(attacker) and attacker:IsPlayer() and victim ~= attacker then
                local t_vic = victim:SCPTeam()
                local t_att = attacker:GetProperty("kill_team_override") or attacker:SCPTeam()
                local rdm = not preventrdm and not attacker:GetProperty("prevent_rdm") and SCPTeams.IsAlly(t_att, t_vic)
                ...
                if len == 0 then
                    ...
                    if pc > reward then reward = pc end
                end
                ...
                if len > 0 then
                    while pool > 0 do
                        ...
                        for i = 1, len do
                            ...
                            if points > pool then points = pool end
                ...
                --phase 2: giving
                if rdm and not preventrdm then
                    AddRoundStat("rdm")
    ...
    if pprop.death_info_override then victim:SetProperty("death_info_override", pprop.death_info_override) end
    if pprop.death_cause_override then victim:SetProperty("death_cause_override", pprop.death_cause_override) end
    ...
    timer.Simple(3, function()
        if IsValid(victim) and victim.SetupAsSpectator then
            ...
            if victim:SCPTeam() == TEAM_SPEC then victim.DeathScreen = true end
        end
    end)
...
function GM:PlayerDeathThink(ply)
        ...
        if override then
            ...
            if IsValid(killer) then
                ...
                elseif killer:IsPlayer() then
                    ...
                    if kteam == TEAM_SPEC then kteam = killer:GetProperty("last_team") end
                    msg = {{"killed_by", "text;" .. EscapeMessage(killer:Nick())},}
                    if kteam and kteam ~= TEAM_SPEC then table.insert(msg, {"killer_t", "team;" .. kteam}) end
                else
                    msg = {"hazard"}
                end
            end
    ...
    elseif ply.DeathScreen and ply.DeathScreen < CurTime() then
        ply.DeathScreen = nil
        local killer = ply:GetProperty("last_killer")
        if ply._ForceSuicideQueue or not ply._SkipNextSuicide and (killer == ply or not IsValid(killer) or not killer:IsPlayer()) then
        ...
        ply:SetupSpectator()
    elseif not ply.DeathScreen then
        local obs = ply:GetObserverTarget()
        if IsValid(obs) then ply:SetPos(obs:GetPos()) end
    end
end

hook.Add("StartCommand", "SLCDeadFreeze", function(ply, cmd) if ply:SCPTeam() == TEAM_SPEC and (ply.SetupAsSpectator or ply.DeathScreen) then cmd:ClearMovement() end end)
...
function GM:PlayerUse(ply, ent)
    ...
    if not ply.NextSLCUse then ply.NextSLCUse = 0 end
    if ply.NextSLCUse > ct then return false end
    --Lootables
    if not ent.LootingDisabled and ply:IsHuman() and ent:InstanceOf("Lootable") then
    ...
    if result then
        if not data then BUTTONS_CACHE[ent] = result end
        data = result
    end

    --Cache door
    if not data and not ent.ButtonCacheChecked or DEVELOPER_MODE then
    ...
    --Door cooldown
    if ent.NextSLCUse and ent.NextSLCUse >= ct then return false end
    --Handle usage
    if data then
        --ply.NextSLCUse = ct + 1
        if data.disabled then return false end
        if data.scp_disallow and ply:SCPTeam() == TEAM_SCP and not ply:SCPCanInteract() then return false end
        if data.warhead_lockdown and GetRoundProperty("warhead_lockdown") then return false end
        local omega = GetRoundStat("omega_warhead")
        if omega and data.omega_disable then return false end
        local alpha = GetRoundStat("alpha_warhead")
        if alpha and data.alpha_disable then return false end
        if omega and data.omega_override then return true end
        if alpha and data.alpha_override then return true end
        if istable(data.decon_override) then
            for i, v in ipairs(data.decon_override) do
                if GetGasPower(v) > 0 then return true end
            end
        elseif data.decon_override and GetGasPower(data.decon_override) > 0 then
            return true
        end

        if not data.suppress_check then
            local handle_result = HandleButtonUse(ply, ent, data)
            if not handle_result then ply.NextSLCUse = ct + 0.5 end
...
function GM:AcceptInput(ent, input, activator, caller, data)
    ...
    if catch and catch(ent, input, activator, caller, data) == true then return true end
    local btn_data = BUTTONS_CACHE[ent]
    if btn_data and btn_data.accept_input and btn_data.accept_input(ent, input, activator, caller, data) == true then return true end
    if string.lower(input) == "use" and (ent:GetClass() == "func_button" or btn_data and btn_data.cooldown or ent.UseCooldown) then
        ...
        if not ent.NextSLCUse or new > ent.NextSLCUse then ent.NextSLCUse = new end
    end
end

hook.Add("EntityKeyValue", "SLCButtons", function(ent, key, value) if ent:GetClass() == "func_button" and key == "wait" then ent.UseCooldown = tonumber(value) + 0.5 end end)
...
function GM:PlayerCanHearPlayersVoice(listener, talker)
    ...
    if not listener:IsActive() then return false end
    local override, ret2 = hook.Run("PlayerSpeakOverride", talker, listener)
    if override ~= nil then return override, ret2 end
    ...
    if IsValid(radio_t) and IsValid(radio_l) then if radio_t:GetEnabled() and radio_l:GetEnabled() and radio_t:GetChannel() == radio_l:GetChannel() then return true end end
...
function GM:WeaponEquip(wep, ply)
    timer.Simple(0, function() ply:UpdateEQ() end)
end
