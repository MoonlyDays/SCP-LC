...
function GetRoundMVP()
    ...
    if not plys then return false end
    local len = #plys
    if len == 1 then return plys[1], max end
    return false
end
...
function GetRoundSummary(stats)
    ...
    for k, v in pairs(ROUND.stats) do
        ...
        if v.player_entry then
            if not PlayerStats._STATS[v.player_entry] then
            ...
            for _, ply in ipairs(player.GetAll()) do
                if ply.PlayerData then
                    ...
                    if not best or lower and stat < best or not lower and stat > best then best = stat end
                end
            end
...
Base round stats
---------------------------------------------------------------------------]]
RegisterRoundStat("kill", {
    init = 0,
    ref = 30,
    importance = 80
})

RegisterRoundStat("rdm", {
    init = 0,
    ref = 5,
    importance = 60
})

RegisterRoundStat("rdmdmg", {
    init = 0,
    ref = 1250,
    importance = 50
})

RegisterRoundStat("dmg", {
    init = 0,
    ref = 7500,
    importance = 30
})

RegisterRoundStat("escapes", {
    init = 0,
    ref = 12,
    importance = 60
})

RegisterRoundStat("escorts", {
    init = 0,
    ref = 9,
    importance = 60
})

RegisterRoundStat("bleed", {
    init = 0,
    ref = 300,
    importance = 80
})

RegisterRoundStat("023", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("049", {
    init = 0,
    ref = 16,
    importance = 30
})

RegisterRoundStat("0492", {
    init = 0,
    ref = 12,
    importance = 30
})

RegisterRoundStat("058", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("066", {
    init = 0,
    ref = 20,
    importance = 30
})

RegisterRoundStat("096", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("106", {
    init = 0,
    ref = 20,
    importance = 30
})

RegisterRoundStat("173", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("457", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("682", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("8602", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("939", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("966", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("3199", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("24273", {
    init = 0,
    ref = 10,
    importance = 30
})

RegisterRoundStat("106recontain", {
    init = false,
    ref = true,
    importance = 25
})

RegisterRoundStat("omega_warhead", {
    init = false,
    ref = true,
    importance = 10
})

RegisterRoundStat("alpha_warhead", {
    init = false,
    ref = true,
    importance = 10
})

RegisterRoundStat("goc_warhead", {
    init = false,
    ref = true,
    importance = 15
})
