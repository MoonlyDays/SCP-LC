...
EQ_ACTIONS = {
	{
		...
		can_see = function() return SCPTeams.CanEscort(LocalPlayer():SCPTeam(), true) end,
		callback = cmd_func,
		args = "slc_escort"
	},
	{
...
function UpdatePlayerEQ()
	...
	for k, v in pairs(INVENTORY) do
		if not IsValid(v) or not lookup_weps[v] then --[[or v:GetClass() == "item_slc_id" or v:IsDerived( "item_slc_holster" )]]
			--print( "REM WEP", k, v )
			INVENTORY[k] = nil
		end
	end
	...
	for i, v in ipairs(weps) do
		...
		if lookup_tab[v] or v:IsScripted() and (not v.ClassName or class == "item_slc_id" or v:IsDerived("item_slc_holster")) then continue end
		local target = 0
...
function OpenBackpack()
	...
	if not RunGUISkinFunction("eq", "is_visible") then return end
...
hook.Add("SLCRegisterSettings", "SLCEQ", function() RegisterSettingsEntry("eq_instant_desc", "switch", false, nil, "hud_config") end)