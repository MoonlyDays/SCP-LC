...
local CHIP = {
	...
	AddAccess = function(self, access)
		if istable(access) then access = bit.bor(unpack(access)) end
	...
	HasAccess = function(self, access, override) return bit.band(self.Access, access) == access or bit.band(override, access) == access end,
	...
	AddUpgrade = function(self, mode, weight, result)
		if istable(mode) then mode = bit.bor(unpack(mode)) end
		if not self.Upgrades[mode] then self.Upgrades[mode] = {} end
...
function RegisterChip(name, level, access, skin, model)
	...
	if not ACC_REGISTRY.LEVELS[level] then ACC_REGISTRY.LEVELS[level] = {} end
...
function SelectUpgrade(name, mode)
	...
	if not weightlist then return false end
	local len = #weightlist
	local total = weightlist[len][1]
	if total == 0 then
		local result = weightlist[len][2]
		if not result or result == true then return result end
...
if CLIENT then
	...
	function GetChipDescription(name, override, feed, refresh)
		...
		if override == 0 and desc_cache[name] and not refresh then return desc_cache[name] end
		local chip = ACC_REGISTRY.CHIPS[name]
		if chip then
			local obj = chip.obj
			local desc
...
if SERVER then
	...
	function HandleButtonUse(ply, ent, data)
		...
		if IsValid(wep) and wep:GetClass() == "item_slc_omnitool" then omnitool = wep end
		local access, msg, target = hook.Run("SLCButtonUse", ply, ent, data, omnitool)
		if access == nil and isfunction(data.override) then access, msg, target = data.override(ply, ent, data) end
		if access == nil and ent.AccessOverride then access, msg, target = ent:AccessOverride(ply, data) end
		if not target then target = data.msg_target end
		if access == nil and data.access then
			access = false
			--SCP Overload
			if not data.disable_overload and SCPButtonOverload(ply, ent, data) then return false end
			--if access == nil then
			if omnitool then
				omnitoolused = true
				...
				if not data.suppress_texts and msg ~= true then PlayerMessage(msg or data.msg_omnitool or "acc_omnitool", ply, true) end
				--end
				return false
			end
			--end
		...
		if access ~= false then
			--access granted
			if isfunction(data.access_granted) and data.access_granted(ply, ent, data) == true then return false end
			if omnitoolused then
				--has omnitool
				omnitool:UseOmnitool(ent, data, true, msg, false)
				if data.msg_access == "" or target ~= 2 then return false end
			end

			if (not data.suppress_texts or target and target >= 1) and msg ~= true then PlayerMessage(msg or data.msg_access or "acc_granted", ply, true) end
			if not omnitoolused then return true end
		else
			if omnitoolused then
			...
			if (not data.suppress_texts or target and target >= 1) and msg ~= true then
				if omnitoolused then
	...
	local function overload_effect(ply)
		...
		timer.Simple(0.2, function() util.Effect("ElectricSpark", effect, false, true) end)
	end
	...
	function SCPButtonOverload(ply, ent, data)
		...
		if ply:IsDoingSLCTask() then return ply:IsDoingSLCTask("button_overload") end
		local ct = CurTime()
		...
		ply:StartSLCTask("button_overload", CVAR.slc_overload_time:GetFloat() * (adv and 2 or 1), function() return ply:KeyDown(IN_USE) end, true, IN_USE):Then(function()
			local in_ct = CurTime()
			if not IsValid(ent) or ent.OverloadCooldown and ent.OverloadCooldown >= in_ct then return true end
			...
			if adv and not ent.AdvancedOverload then
	...
	hook.Add("EntityTakeDamage", "SLCButtonOverload", function(ply, info)
		if IsValid(ply) and ply:IsPlayer() and ply:IsDoingSLCTask("button_overload") and not info:IsDamageType(DMG_POISON) then
			ply:StopSLCTask(3)
	...
	function RegisterButton(data, ent, override)
		if IsValid(ent) then
			if override or not BUTTONS_CACHE[ent] then
...
hook.Add("SLCRegisterChips", "SLCBaseChips", function()
	RegisterChip("jan", 1, {ACCESS_SAFE, ACCESS_GENERAL}, 3):AddUpgrade(UPGRADE_MODE.FINE, 12, "sci2"):AddUpgrade(UPGRADE_MODE.FINE, 8, "spec"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 8, "sci2"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 12, "spec")
	RegisterChip("sci1", 1, {ACCESS_SAFE, ACCESS_GENERAL, ACCESS_OFFICE}, 5):AddUpgrade(UPGRADE_MODE.FINE, 12, "sci2"):AddUpgrade(UPGRADE_MODE.FINE, 8, "spec"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 8, "sci2"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 12, "spec")
	RegisterChip("sci2", 2, {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_OFFICE, ACCESS_GENERAL}, 6):AddUpgrade(UPGRADE_MODE.ONE_ONE, 10, "spec"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 10, "guard"):AddUpgrade(UPGRADE_MODE.FINE, 15, "sci3"):AddUpgrade(UPGRADE_MODE.FINE, 5, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 10, "sci3"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 4, "hacked3"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 4, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, true)
	RegisterChip("sci3", 3, {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_KETER, ACCESS_OFFICE, ACCESS_GENERAL, ACCESS_MEDBAY}, 7):AddUpgrade(UPGRADE_MODE.ONE_ONE, 0, "hacked3"):AddUpgrade(UPGRADE_MODE.FINE, 10, "hacked4"):AddUpgrade(UPGRADE_MODE.FINE, 10, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 10, "hacked4"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 4, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 4, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, true)
	RegisterChip("spec", 2, {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_EC, ACCESS_CHECKPOINT_LCZ, ACCESS_WARHEAD_ELEVATOR}, 8):AddUpgrade(UPGRADE_MODE.ONE_ONE, 5, "sci2"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 10, "guard"):AddUpgrade(UPGRADE_MODE.FINE, 0, "hacked3"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 8, "hacked3"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, true)
	RegisterChip("guard", 2, {ACCESS_SAFE, ACCESS_OFFICE, ACCESS_EC, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ}, 9):AddUpgrade(UPGRADE_MODE.ONE_ONE, 10, "sci2"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 10, "spec"):AddUpgrade(UPGRADE_MODE.FINE, 0, "chief"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 10, "chief"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, false)
	RegisterChip("chief", 3, {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_OFFICE, ACCESS_GENERAL, ACCESS_EC, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ}, 10):AddUpgrade(UPGRADE_MODE.ONE_ONE, 0, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, "com"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, true)
	RegisterChip("mtf", 3, {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_KETER, ACCESS_EC, ACCESS_MEDBAY, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ, ACCESS_GATE_A, ACCESS_GATE_B}, 11):AddUpgrade(UPGRADE_MODE.ONE_ONE, 0, "chief"):AddUpgrade(UPGRADE_MODE.FINE, 15, "com"):AddUpgrade(UPGRADE_MODE.FINE, 5, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 18, "com"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, true)
	RegisterChip("com", 4, {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_KETER, ACCESS_EC, ACCESS_ARMORY, ACCESS_MEDBAY, ACCESS_WARHEAD_ELEVATOR, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ, ACCESS_GATE_A, ACCESS_GATE_B}, 12):AddUpgrade(UPGRADE_MODE.ONE_ONE, 10, "director"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 10, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "hacked3"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "hacked4"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "hacked5"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "chief"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "director"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "o5"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, true)
	RegisterChip("hacked3", 3, {
		access = {ACCESS_SAFE, ACCESS_EUCLID},
		randomtable = {ACCESS_OFFICE, ACCESS_GENERAL, ACCESS_KETER, ACCESS_MEDBAY, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ, ACCESS_WARHEAD_ELEVATOR, ACCESS_EC, ACCESS_GATE_A},
		randomnum = 5
	}, 15):AddUpgrade(UPGRADE_MODE.ONE_ONE, 5, "sci3"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 15, "hacked3"):AddUpgrade(
		--generate new access
		UPGRADE_MODE.FINE, 10, "hacked4"):AddUpgrade(UPGRADE_MODE.FINE, 10, "chief"):AddUpgrade(UPGRADE_MODE.FINE, 5, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 8, "chief"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 8, "hacked4"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 2, true)

	RegisterChip("hacked4", 4, {
		access = {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_KETER, ACCESS_CHECKPOINT_EZ},
		randomtable = {ACCESS_OFFICE, ACCESS_GENERAL, ACCESS_MEDBAY, ACCESS_CHECKPOINT_LCZ, ACCESS_WARHEAD_ELEVATOR, ACCESS_EC, ACCESS_GATE_A, ACCESS_GATE_B, ACCESS_ARMORY, ACCESS_FEMUR, ACCESS_PARTICLE},
		randomnum = 6
	}, 16):AddUpgrade(UPGRADE_MODE.ONE_ONE, 15, "hacked4"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 5, false):AddUpgrade(UPGRADE_MODE.FINE, 10, "hacked5"):AddUpgrade(UPGRADE_MODE.FINE, 5, "mtf"):AddUpgrade(UPGRADE_MODE.FINE, 10, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 10, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, "hacked5"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, true)

	RegisterChip("hacked5", 5, {
		access = {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_KETER, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ},
		randomtable = {ACCESS_OFFICE, ACCESS_GENERAL, ACCESS_MEDBAY, ACCESS_WARHEAD_ELEVATOR, ACCESS_EC, ACCESS_GATE_A, ACCESS_GATE_B, ACCESS_ARMORY, ACCESS_FEMUR, ACCESS_ALPHA, ACCESS_OMEGA, ACCESS_PARTICLE},
		randomnum = 7
	}, 17):AddUpgrade(UPGRADE_MODE.ONE_ONE, 15, "hacked5"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 5, "o5"):AddUpgrade(UPGRADE_MODE.ONE_ONE, 5, false):AddUpgrade(UPGRADE_MODE.FINE, 10, "com"):AddUpgrade(UPGRADE_MODE.FINE, 10, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 15, "com"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, true)

	RegisterChip("director", 4, {ACCESS_SAFE, ACCESS_GENERAL, ACCESS_OFFICE, ACCESS_MEDBAY, ACCESS_EC, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ, ACCESS_WARHEAD_ELEVATOR, ACCESS_FEMUR, ACCESS_ARMORY, ACCESS_OMEGA, ACCESS_GATE_A, ACCESS_GATE_B, ACCESS_PARTICLE}, 13):AddUpgrade(UPGRADE_MODE.ONE_ONE, 0, "com"):AddUpgrade(UPGRADE_MODE.FINE, 10, "o5"):AddUpgrade(UPGRADE_MODE.FINE, 10, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 15, "o5"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, true)
	RegisterChip("o5", 5, {ACCESS_SAFE, ACCESS_EUCLID, ACCESS_KETER, ACCESS_OFFICE, ACCESS_MEDBAY, ACCESS_GENERAL, ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ, ACCESS_WARHEAD_ELEVATOR, ACCESS_EC, ACCESS_ARMORY, ACCESS_GATE_A, ACCESS_GATE_B, ACCESS_FEMUR, ACCESS_ALPHA, ACCESS_OMEGA, ACCESS_PARTICLE}, 14):AddUpgrade(UPGRADE_MODE.ONE_ONE, 0, "hacked5"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, "jan"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 20, false)
	RegisterChip("goc", 3, {ACCESS_CHECKPOINT_LCZ, ACCESS_CHECKPOINT_EZ, ACCESS_WARHEAD_ELEVATOR, ACCESS_GATE_A, ACCESS_GATE_B}, 16):AddUpgrade(UPGRADE_MODE.FINE, 10, "hacked3"):AddUpgrade(UPGRADE_MODE.FINE, 5, "hacked4"):AddUpgrade(UPGRADE_MODE.FINE, 10, false):AddUpgrade(UPGRADE_MODE.VERY_FINE, 10, "hacked4"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, "mtf"):AddUpgrade(UPGRADE_MODE.VERY_FINE, 5, true)
end)