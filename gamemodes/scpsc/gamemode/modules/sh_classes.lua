...
if slc_classes_fully_registered and slc_classes_fully_registered > CurTime() then --DEBUG function
    return
end
...
local function assert_warn(b, s)
    if not b then MsgC(Color(255, 50, 50), "WARNING! " .. s .. "\n") end
end

function AddClassGroup(name, weight, spawn)
    if not name or not weight then return end
    assert_warn(not gwarn, "Using 'AddClassGroup' function outside 'RegisterClassGroups' hook can cause errors!")
    assert(name ~= "SUPPORT", "Forbidden group name: 'SUPPORT'! To add support group use 'AddSupportGroup' function instead")
    assert(not string.match(name, "%s"), "Group name can not contain any whitespace characters!")
    ...
    if registered then table.insert(AllGroups, name) end
end

function AddSupportGroup(name, weight, spawn, max, callback, spawnrule)
    if not name or not weight then return end
    assert_warn(not gwarn, "Using 'AddSupportGroup' function outside 'RegisterClassGroups' hook can cause errors!")
    assert(not string.match(name, "%s"), "Group name can not contain any whitespace characters!")
    ...
    if SERVER and not cvar then cvar = CreateConVar(cvar_name, weight, {FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE}) end
...
function RegisterClass(name, group, model, data, support)
    ...
    assert_warn(not cwarn, "Using 'RegisterClass' function outside 'RegisterPlayerClasses' hook can cause errors!")
    ...
    assert(not string.match(name, "%s"), "Class name can not contain any whitespace characters!")
    assert(group == true or usetab[group] ~= nil, "Invalid group: " .. group)
    --assert( usetab[group][name] == nil, "Class '"..name.."' is already registered!" )
    if AllClasses[name] and not usetab[group][name] then
    ...
    if not ClassTiers[data.tier] then ClassTiers[data.tier] = {} end
    if data.tier > highest_tier then highest_tier = data.tier end
...
function SelectSupportGroup()
    ...
    for i, v in ipairs(newlist) do
        ...
        if total >= dice then return v.name, SupportData[v.name] end
    end
end
...
hook.Add("SLCVersionChanged", "ClassesOverride", function(new, old) file.Rename("slc/classes_data.txt", "slc/classes_data_" .. old.signature .. ".txt") end)
--[[-------------------------------------------------------------------------
Player functions
---------------------------------------------------------------------------]]
...
function PLAYER:IsClassUnlocked(name)
    ...
    if class then
        if class.tier and class.tier == -1 then return self.playermeta.prestige[name] end
        return not class.tier or class.tier == 0 or self.playermeta.classes[name]
    end
    return true
end
...
function PLAYER:CanUnlockClass(name)
    ...
    if class.tier and not self:CanUnlockClassTier(class.tier) then return false end
    return true
end
...
function PLAYER:CanUnlockClassTier(tier)
    ...
    if tier and tier > 1 then
        for k, v in pairs(ClassTiers[tier - 1]) do
            if not self.playermeta.classes[k] then return false end
        end
    end
    return true
end
...
function PLAYER:UnlockClass(name)
    if not self:CanUnlockClass(name) then return false end
    local class = AllClasses[name]
    if not class then return false end
...
function PLAYER:PerformPrestige()
    ...
    if CLIENT then net.Ping("SLCPrestige") end
end
...
if SERVER then
    ...
    net.ReceivePing("SLCPrestige", function(data, p) p:PerformPrestige() end)
    hook.Add("SLCRegisterGamerules", "SLCLANRule", function()
        gamerule.Register("lan", {
            value = false
        })
    end)
