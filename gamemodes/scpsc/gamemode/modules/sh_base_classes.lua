...
hook.Add("SLCRegisterClassGroups", "BaseGroups", function()
    ...
    AddSupportGroup("mtf_alpha", 20, SPAWN_SUPPORT_MTF, function(max) return CVAR.slc_alpha1_amount:GetInt() end, function()
        SetRoundProperty("mtfs_spawned", true)
        SetRoundProperty("mtf_alpha_spawned", true)
        if GetRoundProperty("goc_spawned") then SetRoundProperty("mtf_alpha_goc", true) end
        PlayPA("scp_lc/announcements/alpha1_entered.ogg", 24)
    end, function()
        if GetRoundProperty("goc_spawned") and not GetRoundProperty("mtf_alpha_goc") then return true, true end
        if GetRoundProperty("mtf_alpha_spawned") then return false end
        local round = GetTimer("SLCRound")
        return not IsValid(round) or round:GetRemainingTime() <= round:GetTime() / 2
    end)
    ...
    AddSupportGroup("goc", 15, SPAWN_SUPPORT_GOC, -1, function()
        ...
        if not GetRoundProperty("goc_device") then
    ...
    -- SCP-001-S - The Judge
    -- Spawns mid-round to punish players who have dealt excessive damage
    AddSupportGroup("scp_001s", 5, SPAWN_SUPPORT_001S, 1, function()
        ...
        local round = GetTimer("SLCRound")
        if not IsValid(round) then return false end
        -- Spawn after 180 seconds (3 minutes) into the round
        return round:GetTime() - round:GetRemainingTime() >= 180
    end)
end)
...
hook.Add("SLCRegisterPlayerClasses", "BaseClasses", function()
    ...
    RegisterClass("classd", "classd", CLASSD_MODELS, {
        ...
        max = 0,
        tier = 0,
        override = function(ply) if ply:IsClassUnlocked("veterand") then return false end end,
    })
    ...
    RegisterClass("expd", "classd", VETERAND_MODELS, {
        ...
        max = 3,
        tier = 3,
        rng_effects = {"expd_rubber_bones", "expd_stamina_tweaks", "expd_revive", "expd_lifesteal", "expd_glass_cannon", "expd_speedy_gonzales", "expd_reflect", "expd_invis", "expd_enderman"},
        callback = function(ply, this) ply:ApplyEffect(this.rng_effects[SLCRandom(#this.rng_effects)]) end
    })
    ...
    RegisterClass("sciassistant", "sci", SCI_MODELS, {
        ...
        max = 0,
        tier = 0,
        override = function(ply) if ply:IsClassUnlocked("sci") then return false end end,
    })
    ...
    RegisterClass("cispy", "guard", GUARD_MODELS, {
        ...
        skin = 4,
        select_override = function(name, cur_class, cur_group, total) return cur_group < math.floor(total / 2) end,
    })
    ...
    RegisterSupportClass("ntfmedic", "mtf_ntf", MTF_MODELS, {
        team = TEAM_MTF,
        weapons = {"item_slc_radio", "item_slc_gasmask", "item_slc_thermal", "item_slc_medkitplus", {"item_slc_morphine", "item_slc_adrenaline", "item_slc_ephedrine", "item_slc_hemostatic"}},
        ammo = {},
        chip = "mtf",
        omnitool = true,
        health = 100,
    ...
    RegisterSupportClass("alpha1", "mtf_alpha", MTF_MODELS, {
        ...
        backpack = "medium",
        spawn_protection = true,
        callback = function(ply, this) ply:SetProperty("escape_override", false) end,
    })
    ...
    RegisterSupportClass("alpha1medic", "mtf_alpha", MTF_MODELS, {
        team = TEAM_MTF,
        weapons = {"item_slc_radio", "item_slc_cctv", "item_slc_heavymask", "item_slc_thermal", {"item_slc_morphine", "item_slc_adrenaline", "item_slc_ephedrine", "item_slc_hemostatic"}, "item_slc_medkitplus"},
        ammo = {},
        chip = "o5",
        omnitool = true,
        health = 150,
    ...
    local function goc_rdm(att, vic)
        ...
        if not alpha[vic:SCPClass()] then return end
        return true, false
    end
    ...
    RegisterSupportClass("gocmedic", "goc", "models/player/cheddar/goc/goc_soldier2.mdl", {
        team = TEAM_GOC,
        weapons = {"item_slc_goc_tablet", "item_slc_radio", "item_slc_heavymask", "item_slc_thermal", "item_slc_medkitplus", {"item_slc_morphine", "item_slc_adrenaline", "item_slc_ephedrine", "item_slc_hemostatic"}},
        ammo = {},
        chip = "goc",
        omnitool = true,
        health = 105,
	...
	---------------------------------------------------------------------------]]
    RegisterSupportClass("scp_001s_judge", "scp_001s", "", {
        team = TEAM_SCP,
        weapons = {},
        ammo = {},
        chip = "",
...
if CLIENT then
    timer.Simple(0, function()
        ClassViewerOverride("guard", {
            skin = 4
        })

        ClassViewerOverride("lightguard", {
            skin = 4
        })

        ClassViewerOverride("heavyguard", {
            skin = 4
        })

        ClassViewerOverride("specguard", {
            skin = 3,
            nvg = 1
        })

        ClassViewerOverride("chief", {
            skin = 2
        })

        ClassViewerOverride("guardmedic", {
            skin = 1
        })

        ClassViewerOverride("tech", {
            skin = 3
        })

        ClassViewerOverride("cispy", {
            skin = 4
        })
    end)
end
