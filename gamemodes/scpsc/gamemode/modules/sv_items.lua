...
local function calc_weight(tab)
    ...
    for i, v in ipairs(tab) do
        if not v.weight then return end
...
function ItemSpawnRule(name, data, auto)
    if data.item then
        ...
        if tab and tab.spawns then
            ...
            if auto then SLC_ITEMS_AUTOSPAWN[name] = true end
        end
    ...
    elseif data.item_set then
        ...
        for i, v in pairs(data.item_set) do
            ...
            if r then
                ...
                if data.spawns then table.insert(tmp, r) end
            end
        end
...
local function select_item(tab)
    ...
    if not tab.weight_data then
        ...
        for i = 1, len do
            local v = tab.items[i]
            total = total + v.weight
            if total >= dice then return v, i end
        end
...
function SpawnUsingRule(name)
    ...
    for i = 1, amount do
        ...
        if key == "loadout" and fn_name then ent_class = GetLoadoutWeapon(fn_name, true) end
        if ent_class == "_none" then
            if post_func then post_func(nil, i) end
        elseif key == "func" and fn_name then
            if not SLC_ITEMS_FUNCTION[fn_name] then
            ...
            if IsValid(ent) then
                ...
                if data.offset then pos = pos + data.offset end
                ent:SetPos(pos)
                if ang then ent:SetAngles(ang) end
                if post_tab then apply_table(ent, post_tab) end
                ent:Spawn()
                if post_func then post_func(ent, i) end
            end
...
function SpawnItemGeneric(class, pos, num, post_tab, post_func)
    ...
    for i = 1, num do
        ...
        if IsValid(item) then
            ...
            if post_tab then apply_table(item, post_tab) end
            item:Spawn()
...
local GAS_TABLE = {
    Touch = function(self, ent)
        if ent:IsPlayer() then
            if not ent.NextGasChoke or ent.NextGasChoke < CurTime() then
...
function GM:SpawnItems()
    ...
    if USE_LEGACY_ITEMS_SPAWN then
        ...
        SpawnItemGeneric("cw_ammo_kit_regular", SPAWN_AMMO_CW, -1, {
            AmmoCapacity = 20
        })
        ...
        for k, v in pairs(CHIPS) do
            ...
            if istable(amount) then amount = SLCRandom(amount[1], amount[2]) end
            for i = 1, amount do
                local len = #spawns
                if len == 0 then break end
    ...
    for k, v in pairs(LOOTABLES) do
        ...
        if IsValid(ent) then
            ...
            if v.bounds then ent:SetCollisionBounds(v.bounds.mins, v.bounds.maxs) end
        end
    end
    ...
    for i, v in ipairs(CCTV) do
        ...
        if IsValid(cctv) then
            ...
            cctv:SetCameraID(i)
            cctv:Spawn()
            if v.destroy_alpha or v.destroy_omega then DestroyOnWarhead(cctv, v.destroy_alpha, v.destroy_omega) end
            if v.destroy_gatea then DestroyOnGateA(cctv) end
            v.ent = cctv
        end
    end
...
ItemSpawnFunction("chip", function(args) return CreateChip(SelectChip(tonumber(args[1]) or 1)) end)
...
ItemSpawnFunction("backpack", function(args, data) return BACKPACK.Create(args[1]) end)
