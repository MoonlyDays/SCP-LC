...
function PLAYER:SetProperty(key, value, sync)
    if not self.SLCProperties then self.SLCProperties = {} end
...
function PLAYER:GetProperty(key, def)
    if not self.SLCProperties then self.SLCProperties = {} end
    if not self.SLCProperties[key] and def then self.SLCProperties[key] = def end
...
function PLAYER:StartHold(tab, id, key, time, cb, never)
    if not self:KeyDown(key) then return end
    if not tab._ButtonHold then tab._ButtonHold = {} end
    id = id .. self:SteamID()
    local data = {
        key = key,
        duration = time,
...
function PLAYER:UpdateHold(tab, id)
    if not tab._ButtonHold then return end
    id = id .. self:SteamID()
    local data = tab._ButtonHold[id]
    if not data then return end
    if data.interrupted or not self:KeyDown(data.key) or not self:CheckSignature(data.sig) then
        if data.cb then data.cb(self, tab, false) end
        tab._ButtonHold[id] = nil
        return false, data
    end

    if not data.never and data.time <= CurTime() then
...
function PLAYER:IsHolding(tab, id)
    if not tab._ButtonHold then return false end
    id = id .. self:SteamID()
    local data = tab._ButtonHold[id]
    if not data or not self:CheckSignature(data.sig) then return false end
...
function PLAYER:DisableControls(id, mask)
    if not self.DisableControlsRegistry then self.DisableControlsRegistry = {} end
    mask = mask or 0
...
function PLAYER:StopDisableControls(id)
    if not self.DisableControlsRegistry or not self.DisableControlsRegistry[id] then return end
    self.DisableControlsRegistry[id] = nil
    if not next(self.DisableControlsRegistry) then
...
local function accessor(func, var)
    ...
    PLAYER[func] = function(self)
        if not self[var] then self:DataTables() end
...
local function db_functions(func, db)
    ...
    PLAYER["Get" .. func] = function(self) return self[getter](self) end
...
function PLAYER:RequiredXP(lvl)
    if not lvl then lvl = self:SCPLevel() end
...
function PLAYER:SCPClass()
    if not self.Get_SCPClass then self:DataTables() end
    return self:Get_SCPClass()
end

function PLAYER:SCPPersona()
    if not self.Get_SCPPersonaC or not self.Get_SCPPersonaT then self:DataTables() end
...
function PLAYER:TimeSignature()
    if not self.GetTimeSignature then self:DataTables() end
...
function PLAYER:IsSpectator()
    return not self:Alive() and self:SCPTeam() == TEAM_SPEC
end

function PLAYER:GetInventorySize()
    local backpack = self:GetWeapon("item_slc_backpack")
    if IsValid(backpack) then return 6 + backpack:GetSize() end
    return 6
end
...
function PLAYER:CheckGasmask(dmg)
    ...
    if IsValid(mask) and mask:GetEnabled() then
        if dmg then mask:Damage(dmg) end
        return true
    end
end
...
function PLAYER:GetWeaponByGroup(group)
    for i, v in ipairs(self:GetWeapons()) do
        if v:IsGroup(group) then return v end
    end
end
...
function PLAYER:GetMainWeapon()
    for i, v in ipairs(self:GetWeapons()) do
        if SLC_WEAPONS_REG[v:GetClass()] then return v end
    end
end
