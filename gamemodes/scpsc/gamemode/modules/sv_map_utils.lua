...
function CacheEntity(arg)
    ...
    if SLC_ENTITY_CACHE[arg] then return SLC_ENTITY_CACHE[arg] end
...
function GM:SLCOnDoorClosed(ply, retry, act)
    if not CVAR.slc_door_unblocker:GetBool() then return end
    local activator = act or ACTIVATOR
    if retry and retry > 1 then timer.Simple(0.25, function() hook.Run("SLCOnDoorClosed", ply, retry - 1, activator) end) end
    ...
    for _, item in ipairs(found) do
        ...
        if not is_blocker then continue end
...
function DestroyDoor(ent, breach_ang, ignore_moving)
    ...
    if not CanDestroyDoor(ent, ignore_moving) then return false end
    ...
    if IsValid(areaportal) then areaportal:Fire("open") end
    ...
    if not prop_model then return true end
...
sound.Add({
    ...
    sound = {"ambient/materials/metal_stress5.wav", "ambient/materials/metal_stress4.wav"},
    channel = CHAN_STATIC,
    volume = 1,
    level = 80,
    pitch = {95, 105}
...
function IsDoorDestroyed(ent)
    return not not SLC_DESTROYED_DOORS_LOOKUP[ent:EntIndex()]
end
...
function RestoreDoor(id)
    ...
    if IsValid(tab.areaportal) then tab.areaportal:Fire("close") end
    if IsValid(tab.prop) then tab.prop:Remove() end
end
...
hook.Add("Think", "SLCRepairDoors", function()
    ...
    for id, data in pairs(SLC_DESTROYED_DOORS) do
        if not data.time or data.time >= ct then continue end
...
local function internal_use(ply, ent, data)
    ...
    if SLC_CONTROLLERS_LAST_SETID[data.name] == cached[2] then return true end
    if data.cooldown then
        if not SLC_CONTROLLERS_USE_TIME[data.name] then SLC_CONTROLLERS_USE_TIME[data.name] = 0 end
        if SLC_CONTROLLERS_USE_TIME[data.name] > CurTime() then return true end
    end

    if isfunction(data.on_use) and data.on_use(ply, ent, data, cached[2]) then return true end
    if data.cooldown then SLC_CONTROLLERS_USE_TIME[data.name] = CurTime() + data.cooldown end
    if data.debug_use and not data.access then ent:Fire("Use") end
    SLC_CONTROLLERS_LAST_SETID[data.name] = cached[2]
end

local function handle_button_use(ply, ent, data)
    if not data.access then return internal_use(ply, ent, data) end
end
...
function ButtonController(name, data)
    ...
    if not istable(data.buttons_set) then return end
    local btn_set = {}
    for id, set in pairs(data.buttons_set) do
        if not istable(set) then continue end
...
hook.Add("SLCUseOverride", "SLCButtonController", function(ply, ent, data)
    if not ent.ControllersSearched then
        ent.ControllersSearched = true
        for name, cdata in pairs(SLC_BUTTON_CONTROLLERS) do
            if not istable(cdata.buttons_set) then continue end
            for id, set in pairs(cdata.buttons_set) do
                if not istable(set) then continue end
                for _, pos in pairs(set) do
                    if ent:GetPos() == pos then SLC_CONTROLLERS_CACHE[ent] = {name, id} end
                end
            end
        end
    end
...
local selobj = {
    ...
    AddEntity = function(self, ent) table.insert(self.entities, ent) end,
    Broadcast = function(self, ent, selfnotify, ...) self:BroadcastInternal(ent, selfnotify, SEL_USER, ...) end,
    BroadcastInternal = function(self, ent, selfnotify, t, ...)
        for k, v in pairs(self.entities) do
            if IsValid(v) then
                if selfnotify or v ~= ent then if v.DataBroadcast then v:DataBroadcast(ent, t, ...) end end
            else
                self.entities[k] = nil
            end
        end
...
hook.Add("SLCPreround", "SLCFuseBox", function()
    ...
    for i, v in ipairs(FUSE_BOXES) do
        ...
        if IsValid(box) then
            box:SetPos(v.pos)
            box:SetAngles(v.ang)
            if v.rating then box:SetRating(v.rating) end
            box.BoxName = v.name
            box.Time = v.time or 0
            if isnumber(v.fuse) and v.fuse > 0 then
                if v.fuse < 1 then
                    if SLCRandom() < v.fuse then box:SetFuse(box:GetRating()) end
                else
                    box:SetFuse(v.fuse)
                end
...
hook.Add("SLCButtonUse", "SLCFuseBox", function(ply, ent, data, omni)
    if not data.fuse_box or CVAR.slc_disable_fuseboxes:GetInt() ~= 0 then return end
    ...
    for _, v in ipairs(tab) do
        ...
        if not rok and tab.all then
            break
        elseif rok and not tab.all then
            return
        end
    end
...
hook.Add("SLCRoundCleanup", "SLCMapUtilsCleanup", function()
    ...
    for k, v in pairs(SLC_BUTTON_CONTROLLERS) do
        if v.initial_state then SLC_CONTROLLERS_LAST_SETID[v.name] = v.initial_state end
    end
end)
