...
function InsaneTick(ply)
    if not ply.NextSanityEffect then ply.NextSanityEffect = 0 end
...
function InterruptInsane(ply)
    ...
    local eff = insane_effects[id]
    if eff and eff[3] then eff[3](ply) end
end
...
AddInsaneEffect(12, function(ply) ply:EmitSound("SLCEffects.Whispers") end, function(ply) ply:StopSound("SLCEffects.Whispers") end)
--[[-------------------------------------------------------------------------
...
end, function(ply) ply.InsaneScreenEffect = 0 end)
...
AddInsaneEffect(11, function(ply) ply.InsaneBlinkChange = 1 end, function(ply) ply.InsaneBlinkChangeOn = false end)
