...
function SCP914Use(ply)
    ...
    AddTimer("SCP914Upgrade", 10, 1, function() SCP914Upgrade(mode, ply) end)
    AddTimer("SCP914UpgradeEnd", 16, 1, function() SetRoundProperty("scp914_in_use", false) end)
    return true
end
...
function SCP914Upgrade(mode, ply)
    ...
    for i, v in ipairs(ents.FindInBox(SCP_914_INTAKE_MINS, SCP_914_INTAKE_MAXS)) do
        ...
        elseif v.SCP914Upgrade then
            ...
            if istable(item_class) then item_class = item_class[SLCRandom(#item_class)] end
...
local function IsGateAOpen()
    for i, v in ipairs(ents.FindInSphere(POS_MIDDLE_GATE_A, 125)) do
        if v:GetClass() == "prop_dynamic" then if table.HasValue(POS_GATE_A_DOORS, v:GetPos()) then return false end end
    end
    return true
end
...
local function DestroyGateA()
    for i, v in ipairs(ents.FindInSphere(POS_MIDDLE_GATE_A, 125)) do
        ...
        if class == "prop_dynamic" or class == "func_door" then v:Remove() end
    end
...
local function TakeDamageA(ent, ply)
    for i, v in ipairs(player.GetAll()) do
        if not v:Alive() or v:SCPTeam() == TEAM_SPEC then continue end
        local pos = v:GetPos()
        for _, tab in pairs(EXPLOSION_AREAS_A) do
            if pos:WithinAABox(tab[1], tab[2]) then
                ...
                if dmg > 0 then
                    if dmg >= v:Health() then v:SkipNextSuicide() end
...
local function spawn_fire_ents(pos, time)
    ...
    for x = pos1.x, pos2.x, step do
        for y = pos1.y, pos2.y, step do
            fire_trace.start = Vector(x + (SLCRandom() * 2 - 1) * noise, y + (SLCRandom() * 2 - 1) * noise, pos2.z)
            if not util.IsInWorld(fire_trace.start) then continue end
            fire_trace.endpos = fire_trace.start * 1
            fire_trace.endpos.z = pos1.z
            _DrawNextTrace = true
            util.TraceLine(fire_trace)
            if not fire_trace.Hit or fire_trace.StartSolid or fire_trace.AllSolid then continue end
    ...
    AddTimer("GateFireTimer", tick, time / tick, function()
        for i, v in ipairs(pos) do
            for _, ply in ipairs(player.GetAll()) do
                if not ply:GetPos():WithinAABox(v[1], v[2]) then continue end
                local t = ply:SCPTeam()
                if not ply:Alive() or t == TEAM_SPEC then continue end
                if t == TEAM_SCP then
...
function Recontain106(ply)
    ...
    if not MAP_CHECKERS.SCP106_CAGE() then
    ...
    if not MAP_CHECKERS.SCP106_SOUND() then
    ...
    local plys = {}
    for i, v in ipairs(player.GetAll()) do
        if IsValid(v) and v:IsPlayer() and SCPTeams.HasInfo(v:SCPTeam(), SCPTeams.INFO_HUMAN) and v:GetPos():WithinAABox(CAGE_BOUNDS.MINS, CAGE_BOUNDS.MAXS) then table.insert(plys, v) end
    end
...
local function omega_shelter()
    ...
    for i, v in ipairs(SCPTeams.GetPlayersByInfo(SCPTeams.INFO_HUMAN)) do
        if v:GetPos():WithinAABox(OMEGA_SHELTER[1], OMEGA_SHELTER[2]) then
            ...
            InfoScreen(v, "escaped", INFO_SCREEN_DURATION, {"escape3", {"escape_xp", "text;" .. xp}})
            v:AddXP(xp, "escape")
...
hook.Add("SLCPreround", "SLCOmegaWarhead", function()
    local listener = SynchronousEventListener("OmegaWarhead", 2, 2, function(lis, info)
        if MAP_CHECKERS.OMEGA_REMOTE() and not GetRoundStat("goc_countdown") and not IsGOCDeviceValid() then
...
function OMEGAWarhead(ply1, ply2)
    ...
    AddTimer("OmegaSiren", 3, 1, function()
        ...
        local a_doors = {
            [OMEGA_GATE_A_DOOR_L] = true,
            [OMEGA_GATE_A_DOOR_R] = true
        }

        local s_doors = {
            [OMEGA_SHELTER_DOOR_L] = true,
            [OMEGA_SHELTER_DOOR_R] = true
        }
        ...
        AddTimer("OmegaCountdown", time - 10, 1, function()
            ...
            for k, v in ipairs(ents.GetAll()) do
                if v:GetClass() == "func_door" and s_doors[v:GetName()] then v:Fire("Close") end
            end
            ...
            AddTimer("OmegaExplosion", 10, 1, function()
                ...
                if sel_omega then sel_omega:Broadcast(NULL, false, 2) end
...
function ALPHAWarhead(ply)
    ...
    AddTimer("AlphaSiren", 3, 1, function()
        ...
        AddTimer("AlphaCountdown", time - 10, 1, function()
            ...
            AddTimer("AlphaSiren", 5, 1, function() TransmitSound("scp_lc/warhead/siren.wav", false, 1) end)
...
function ALLWarheads(time)
    ...
    AddTimer("GOCSiren", 3, 1, function()
        ...
        AddTimer("GOCCountdown", time - 10, 1, function()
            ...
            local escape_timer = GetTimer("EscapeTimer")
            if IsValid(escape_timer) then escape_timer:Destroy() end
            ...
            AddTimer("GOCExplosion", 10, 1, function()
                ...
                for i, v in ipairs(player.GetAll()) do
                    ...
                    if v:Alive() then
                        v:SetProperty("death_info_override", {
                            type = "mia",
                            args = {v:IsInZone(ZONE_SURFACE) and "alpha_mia" or "omega_mia"}
                        })
...
function IsGOCDeviceValid()
    ...
    if IsValid(device_placed) then return not device_placed:GetDestroyed() end
...
function DestroyOnWarhead(ent, alpha, omega)
    if alpha then ALPHA_DESTROY[ent] = true end
    if omega then OMEGA_DESTROY[ent] = true end
end
...
function InitiateLockdown(ply, ent, cb)
    ...
    if status > 0 then
        if status == 2 then PlayerMessage("lockdown_once", ply, true) end
        return false
    end
    ...
    AddTimer("DisableLockdown", dur, 1, function()
        ...
        if IsValid(ent) then ent:Fire("Use") end
    end)
...
hook.Add("SLCRound", "SLCAutoDestroyGateA", function(time)
    ...
    if auto > 0 then
        AddTimer("auto_destroy_gatea", time - auto - 5, 1, function()
            ...
            if remote then remote:Fire("lock") end
            AddTimer("auto_destroy_gatea2", 5, 1, function() ExplodeGateA() end)
        end)
    end
end)
