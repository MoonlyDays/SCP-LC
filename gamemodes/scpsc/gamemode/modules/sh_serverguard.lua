...
SLCAuth.AddLibrary("serverguard", "Serverguard", {
    CheckAccess = function(ply, access) return serverguard.player:HasPermission(ply, access) end,
...
function command:Execute(player, silent, arguments)
    ...
    if not IsValid(target) then
        serverguard.Notify(player, SERVERGUARD.NOTIFY.RED, "Player invalid!")
        return
    end

    if not target:GetActive() then
    ...
    if class and spawn then
        ...
        if not silent then serverguard.Notify(player, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(player), SERVERGUARD.NOTIFY.WHITE, " spawned as ", SERVERGUARD.NOTIFY.GREEN, classs, SERVERGUARD.NOTIFY.WHITE, " player ", SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(target)) end
    else
...
function command:ContextMenu(player, menu, rankData)
    ...
    for k, v in pairs(class_names) do
        local option = kickMenu:AddOption(v, function() serverguard.command.Run("forcespawn_hum", false, player:Name(), v) end)
...
function command:ContextMenu(player, menu, rankData)
    ...
    for k, v in pairs(SCPS) do
        local option = kickMenu:AddOption(v, function() serverguard.command.Run("forcespawn_scp", false, player:Name(), v) end)
        option:SetImage("icon16/user.png")
    end
end
...
function command:Execute(player, silent, arguments)
    ...
    if isnumber(count) then
        ...
        if not silent then serverguard.Notify(player, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(player), SERVERGUARD.NOTIFY.WHITE, " gived ", SERVERGUARD.NOTIFY.GREEN, tostring(arguments[2]), SERVERGUARD.NOTIFY.WHITE, " to player ", SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(target)) end
    else
...
function command:ContextMenu(player, menu, rankData)
    local kickMenu = menu:AddOption("Give level", function() Derma_StringRequest("Give level", "Give level", "", function(text) serverguard.command.Run("givelevel_scp", false, player:Name(), text) end, function(text) end, "Give", "Cancel") end)
    kickMenu:SetSkin("serverguard")
...
function command:Execute(player, silent, arguments)
    RestartRound()
    if not silent then serverguard.Notify(player, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(player), SERVERGUARD.NOTIFY.WHITE, " restarted the round") end
end
...
function command:Execute(player, silent, arguments)
    ExplodeGateA()
    if not silent then serverguard.Notify(player, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(player), SERVERGUARD.NOTIFY.WHITE, " triggered Gate A destroy") end
end
...
function command:ContextMenu(player, menu, rankData)
    ...
    for k, v in pairs(dataType) do
        local option = kickMenu:AddOption(v, function() serverguard.command.Run("rdata", false, player:Name(), k) end)
        option:SetImage("icon16/user.png")
    end
end
