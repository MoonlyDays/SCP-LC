...
local function DrawEffectsHUD()
    ...
    if not ply.EFFECTS then return end
    local effects = {}
    for i, v in ipairs(ply.EFFECTS) do
        local eff = EFFECTS.effects[v.name]
        if eff and not eff.hide then table.insert(effects, v) end
    end
    ...
    if showtext then
        ...
        if not istable(showtext) then showtext = {showtext} end
        local maxw = 0
        local fh = 0
        for i = 1, #showtext do
        ...
        if my + fh + 10 > h then my = h - fh - 10 end
...
hook.Add("SLCRegisterSettings", "SLCEffectsHUD", function() RegisterSettingsEntry("hud_avoid_roman", "switch", false, nil, "hud_config") end)
