...
function SLCAddDocument(name, mat_path, reward, merge)
	...
	local tbl = {
		...
		Selectable = not not mat_path,
		Name = name,
		Reward = reward,
		Page = mat,
	}

	if merge then table.Merge(tbl, merge) end
...
local function document_escape(ply)
	...
	for k, v in pairs(ply:GetWeapons()) do
		if not v.IS_DOCUMENT then continue end
		num = num + 1
		if v.Reward and v.Reward > 0 then total = total + v.Reward end
	end
...
if SERVER then
	SLC_ROUND_DOCUMENTS = {}
	hook.Add("SLCRoundCleanup", "SLCDocuments", function() SLC_ROUND_DOCUMENTS = table.Copy(SLC_DOCUMENTS) end)
	hook.Add("SLCFullyLoaded", "SLCDocuments", function()
		ItemSpawnFunction("document", function()
			local class
			if not SLC_ROUND_DOCUMENTS or #SLC_ROUND_DOCUMENTS == 0 then
				class = SLC_DOCUMENTS[SLCRandom(#SLC_DOCUMENTS)]
			else
				class = table.remove(SLC_ROUND_DOCUMENTS, SLCRandom(#SLC_ROUND_DOCUMENTS))
			end

			if class then return ents.Create(class) end
		end)
	end)
end
...
local function handle_upgrade(self, mode, exit, ply)
	...
	elseif mode == 3 then
		local s = syringes[class]
		if s and s.better_class then new_ent = s.better_class end
	elseif mode == 4 then
		local s = syringes[class]
...
function AddSyringe(class, data)
	...
	if data.can_use or (data.id and not data.base) then
		local id = data.id
		swep.CanUseOn = data.can_use or function(self, target, owner)
			if not target:Alive() or not target:IsHuman() then return end
	...
	if not data.no_reg then table.insert(data.is_better and list_big or list_small, class) end
...
AddSyringe("item_slc_ephedrine", {
	...
	can_use = function(self, target, owner)
		if not target:Alive() or not target:IsHuman() or target:HasEffect("ephedrine") then return end
...
AddSyringe("item_slc_hemostatic", {
	...
	can_use = function(self, target, owner)
		if not target:Alive() or not target:IsHuman() or target:HasEffect("hemostatic") then return end
		if hook.Run("SLCCanHeal", target, owner, "ephedrine") == false then return end
		return true
	end,
	callback = function(self, target, owner)
...
AddSyringe("item_slc_antidote", {
	...
	can_use = function(self, target, owner)
		if not target:Alive() or not target:IsHuman() or target:HasEffect("antidote") then return end
		if hook.Run("SLCCanHeal", target, owner, "ephedrine") == false then return end
		return true
	end,
	callback = function(self, target, owner)
...
AddSyringe("item_slc_poison", {
	...
	can_use = function(self, target, owner) return target:Alive() and target:IsHuman() end,
	callback = function(self, target, owner)
		if CLIENT then return end
