...
function GM:PlayerButtonDown(ply, button)
    if SERVER then
        ...
        local rt = RealTime()
        if not ply.SLCAFKTimer then ply.SLCAFKTimer = 0 end
    ...
    if CLIENT and IsFirstTimePredicted() then
        ...
        if button == GetBindButton("eq_button") and hook.Run("SLCCanUseBind", "eq") ~= false then
            ...
            HUDDrawInfo = true
            if ply:SCPTeam() == TEAM_SPEC then HUDSpectatorInfo = true end
...
function GM:PlayerButtonUp(ply, button)
    ...
    if CLIENT and IsFirstTimePredicted() then
        if button == GetBindButton("eq_button") then
            HideGUIElement("eq")
            HUDDrawInfo = false
            if ply:SCPTeam() == TEAM_SPEC then HUDSpectatorInfo = false end
...
if CLIENT then
    hook.Add("Tick", "SLCClientSideUse", function()
        ...
        if not IsValid(ply) or not ply:KeyDown(IN_USE) then return end
        ...
        if not tr.Entity.CSUse then return end
        tr.Entity:CSUse(ply)
    end)
end
...
function GM:PlayerCanPickupWeapon(ply, wep)
    if ply:GetAdminMode() then
        ...
        return not not admin_weapons[class]
    end

    local t = ply:SCPTeam()
    if t == TEAM_SPEC then return false end
    if ply:GetFreeInventory() <= 0 then
        if not wep.Stacks or wep.Stacks <= 1 then return false, "max_eq" end
        local pwep = ply:GetWeapon(wep:GetClass())
        if not IsValid(pwep) then return false, "max_eq" end
        if pwep.CanStack and not pwep:CanStack() then return false, "cant_stack" end
    end

    if t == TEAM_SCP then
        if wep.SCP then return true end
        if not ply:GetSCPHuman() then return false end
    end

    if wep.SCP then return false end
    local status, msg = hook.Run("SLCCanPickupWeaponClass", ply, wep:GetClass())
    if status == false then return false, msg end
    local class = wep:GetClass()
    local has = false
    ...
    if has then
        if not wep.Stacks or wep.Stacks <= 1 then return false, "has_already" end
        local pwep = ply:GetWeapon(wep:GetClass())
        if IsValid(pwep) then if pwep.CanStack and not pwep:CanStack() then return false, "cant_stack" end end
    end

    if wep.CanPickUp then
        local s, m = wep:CanPickUp(ply)
        if s ~= nil then return s, m end
    end
...
function GM:SLCCanPickupWeaponClass(ply, class)
    if not WeaponInAnyGroup(class) then return end
    local lowest = math.huge
    local count = 0
    for i, v in ipairs(ply:GetWeapons()) do
        local result, limit = v:TestGroup(class)
        if not result then continue end
        if limit <= 1 then return false, "same_type" end
        if limit < lowest then lowest = limit end
        count = count + 1
    end

    if count >= lowest then return false, "max_type" end
end
...
function GM:UpdateAnimation(ply, velocity, maxseqgroundspeed)
    ...
    local rate = 1
    if len > 0.2 then rate = len / maxseqgroundspeed end
    local n_rate, noclamp = hook.Run("SLCMovementAnimSpeed", ply, velocity, maxseqgroundspeed, len, rate)
    if isnumber(n_rate) then rate = n_rate end
    if not noclamp and rate > 2 then rate = 2 end
...
function GM:StartCommand(ply, cmd)
    ...
    if ply:Alive() and not IsValid(ply:GetActiveWeapon()) then
        ...
        if IsValid(holster) then cmd:SelectWeapon(holster) end
    end
    ...
    if ply.GetDisableControls and ply:GetDisableControls() then
        ...
        if bit.band(mask, CAMERA_MASK) == 0 then
            if not ply.DisableControlsAngle then ply.DisableControlsAngle = cmd:GetViewAngles() end
            cmd:SetViewAngles(ply.DisableControlsAngle)
        end

        if bit.band(mask, MOVEMENT_MASK) == 0 then cmd:ClearMovement() end
...
function GM:Move(ply, mv)
    ...
    if ply:SCPTeam() == TEAM_SCP and ply:GetProperty("scp_buff") and ply:IsInZone(ZONE_SURFACE) then mod = mod * 1.1 end
    if mod ~= 1 then
        local speed = (mv:KeyDown(IN_SPEED) and ply:GetRunSpeed() or ply:GetWalkSpeed()) * mod
        if ply:Crouching() then speed = speed * ply:GetCrouchedWalkSpeed() end
...
function GM:FinishMove(ply, mv)
    ...
    if SERVER and (ply:OnGround() and (mvtype == MOVETYPE_WALK or mvtype == MOVETYPE_STEP) or mvtype == MOVETYPE_LADDER) then
        ...
        if ply.slc_next_footstep then
            ...
            if ply.slc_next_footstep <= 0 then ply:PlayStepSound() end
        end

        if not ply.slc_next_footstep or ply.slc_next_footstep <= 0 then ply:UpdateStepTime() end
    end
...
function GM:SLCFootstepParams(ply, st, vel, crouch)
    if st == STEPTYPE_LADDER then return 100 end
    local units = 50
    local len = vel:Length()
    if len > 125 then units = units + len / 10 end
    if crouch then units = units - 15 end
    if units < 25 then units = 25 end
    return units
end

if not SLCStepSoundsAdded then
    SLCStepSoundsAdded = true
    for i, v in ipairs(sound.GetTable()) do
        if not string.find(v, "StepLeft") and not string.find(v, "StepRight") then continue end
...
local function calc_fall_threshold(ply)
    ...
    if vest > 0 then
        ...
        if data then return math.max(300 / (1 + data.weight / 10), 75) end
    end
    return 300
end
...
function GM:OnPlayerHitGround(ply, water, floater, speed)
    ...
    if override then speed = override end
    ...
    if not water then
        ...
        if damage or speed > dmg_t then
            ...
            if dmg > 0 then
                ...
                if dmg > 25 then ply:ApplyEffect("fracture") end
