...
local function copy_spawns(spawns, prev)
    if not istable(spawns) then return spawns end
    if not istable(spawns[1]) then return table.Copy(spawns) end
...
local function select_weighted(tab, total_weight, rem)
    ...
    for i, v in ipairs(tab) do
        ...
        if rem then
            table.remove(tab, i)
            if not total_weight then tab.total_weight = tab.total_weight - v.weight end
        end
        return v.value
    end
end
...
local function assign_scps(plys, num)
    ...
    for i, v in ipairs(SCPS) do
        if skip_filter or not ROUND.LastRoundSCPs or not ROUND.LastRoundSCPs[v] then
            --print( "Add", v )
    ...
    local priority_players, other_players = {
        total_weight = 0
    }, {}
    ...
    for i = 1, num do
        ...
        if priority_players.total_weight > 0 then
            ...
            if other_len > 0 then ply = table.remove(other_players, SLCRandom(other_len)) end
        end
...
function get_player_classes(plys, groups)
    ...
    for _, ply in ipairs(plys) do
        ...
        for group, classes in pairs(groups) do
            ...
            if isnumber(override) then ply_weight = math.ceil(override) end
            if ply_weight <= 0 then --[[print( "    > weight <= 0", ply_weight )]]
                continue
            end

            local group_tab = {
                total_weight = 0,
                ply_weight = ply_weight
            }
            ...
            for class, data in pairs(classes) do
                ...
                if weight ~= false and not isnumber(weight) then
...
function assign_groups(plys, all_groups, player_classes)
    ...
    for idx, group in ipairs(groups_order) do
        ...
        --PrintTable( plys )
        local available_plys = {
            total_weight = 0
        }

        for _, ply in ipairs(plys) do
            local group_data = player_classes[ply][group]
            if not group_data or group_data.total_weight < 1 then --[[print( "Player has no class in group", ply, group )]]
                continue
            end

            available_plys.total_weight = available_plys.total_weight + group_data.ply_weight
            table.insert(available_plys, {
                value = ply,
                weight = group_data.ply_weight
            })
        end
        ...
        for i = 1, to_select do
            ...
            local karma_add = 0
            if karma_enabled then karma_add = math.floor(math.ClampMap(ply:GetSCPKarma(), 0, 1000, 0, 2.5)) end
            tmp_players[i] = {
                ply = ply,
                index = i,
                order = ply:GetPrestigeLevel() + karma_add
            }
    ...
    for group, plys in pairs(assigned_groups) do
        ...
        for i, ply in ipairs(plys) do
            ...
            while ply_classes.total_weight > 0 do
                ...
                --Msg( "    > "..tmp_class )
                if not classes_in_use[tmp_class] then classes_in_use[tmp_class] = 0 end
                if not select_groups[select_group] then select_groups[select_group] = 0 end
                ...
                if override == true or override == nil and (not tmp_data.max or tmp_data.max == 0 or classes_in_use[tmp_class] < tmp_data.max and select_groups[select_group] < tmp_data.max) then
            ...
            if not assigned_classes[ply] then
...
function SetupPlayers()
    ...
    for i, v in ipairs(active_plys) do
        local penalty = v:GetSCPPenalty()
        if penalty > 0 or scp_num > 0 then v:SetSCPPenalty(penalty - 1) end
    end
    ...
    for group, plys in pairs(assigned_groups) do
        ...
        for _, ply in ipairs(plys) do
            ...
            elseif stype == "table" then
                ...
                if not spawn_table or #spawn_table == 0 then
            ...
            elseif stype == "string" then
                ...
                if not group_spawns or #group_spawns == 0 then
            ...
            if not initial_teams[group] then
                initial_teams[group] = {}
            ...
            if class_data.id_group then
                if not initial_teams[class_data.id_group] then
...
hook.Add("SLCRound", "SLCSCPLastRound", function()
    ...
    for i, v in ipairs(player.GetAll()) do
        if v:SCPTeam() == TEAM_SCP then tab[v:SCPClass()] = true end
    end
end)
...
function MoveSuicideQueue()
    for i, v in ipairs(suicide_queue) do
        if not v:IsValid() or v:IsAFK() or v:SCPTeam() ~= TEAM_SPEC then continue end
...
function QueueCheck()
    ...
    for i, v in ipairs(suicide_queue) do
        ...
        if not v:IsValidSpectator() then
            v:SetQueuePosition(0)
            if not v:IsAFK() then suicide_queue_sid[v:SteamID64()] = nil end
        elseif not queue_lookup[v] then
    ...
    for i, v in ipairs(player.GetAll()) do
        if queue_lookup[v] or not v:IsValidSpectator() then continue end
...
function QueueUpdate()
    ...
    for i, v in ipairs(player.GetAll()) do
        if not done[v] and IsValid(v) then v:SetQueuePosition(0) end
    end
end

function QueueInsert(ply, front)
    if not ply:IsValidSpectator() or queue_lookup[ply] then return end
...
function QueueRemove(low)
    ...
    repeat
        if #queue == 0 then
            if not low then return NULL, false end
            ply = nil
            break
        end
    ...
    repeat
        ...
        queue_lookup[ply] = nil
        if IsValid(ply) then suicide_queue_sid[ply:SteamID64()] = nil end
    until IsValid(ply) and ply:IsValidSpectator()
    return ply, true
end
...
function SpawnSupport(group_override)
    ...
    if not group_name or not group_data then group_name, group_data = SelectSupportGroup() end
    ...
    while to_spawn > 0 do
        ...
        if not IsValid(ply) then --No more valid players in queue!
        ...
        for class_name, class in pairs(classes) do
            ...
            if weight ~= false and not isnumber(weight) then weight = (weight or ply:IsClassUnlocked(class.name)) and (class.weight or class.tier and (class.tier + 1) or 1) end
            if weight and weight > 0 then
                available_classes.total_weight = available_classes.total_weight + weight
                table.insert(available_classes, {
                    value = class_name,
                    weight = weight
                })
            end
    ...
    for ply, class_name in pairs(assigned_classes) do
        ...
        if class_data.spawn then
            if istable(class_data.spawn) then
                if not class_spawns[class_name] or #class_spawns[class_name] == 0 then class_spawns[class_name] = copy_spawns(class_data.spawn, class_spawns[class_name]) end
    ...
    if group_override then SetupSupportTimer() end
    MoveSuicideQueue()
    return true
end
...
function ForceSpawnSupport(group_name, amount)
    ...
    for _, ply in ipairs(player.GetAll()) do
        if ply:IsValidSpectator(true, true) then table.insert(available_players, ply) end
    end

    if #available_players == 0 then return false, "No valid spectators available" end
    -- Determine spawn amount
    local to_spawn = amount or #available_players
    to_spawn = math.min(to_spawn, #available_players)
    if to_spawn <= 0 then return false, "Invalid spawn amount" end
    ...
    local count = table.Count(assigned_classes)
    if count == 0 then return false, "Failed to assign any players" end
    -- Spawn all players
    local spawned = {}
    local spawned_plys = {}
    local class_spawns = {}
...
local function GetEscapeData()
    ...
    for i, v in ipairs(player.GetAll()) do
        ...
        if team == TEAM_SPEC or not v:IsInEscape() then continue end
        teams[team] = true
        table.insert(all, v)
        local override = v:GetProperty("escape_override")
        if override == false then continue end
        if override or SCPTeams.CanEscape(team) or GetRoundStat("alpha_warhead") then table.insert(players, v) end
    end

    for t1, v1 in pairs(teams) do
        for t2, v2 in pairs(teams) do
            if SCPTeams.IsEnemy(t1, t2) then return players, true, all end
        end
    end
...
hook.Add("Tick", "SLCEscapeCheck", function()
    ...
    if #tab == 0 then
        ...
        for k, v in ipairs(blocked and all or tab) do
            if not LAST_ESCAPE_LOOKUP[v] then
        ...
        if #transmit > 0 then TransmitEscapeInfo(transmit) end
        if #ntransmit > 0 then TransmitEscapeInfo(ntransmit, 0) end
        --end
        LAST_ESCAPE = blocked and all or tab
        LAST_ESCAPE_LOOKUP = CreateLookupTable(LAST_ESCAPE)
    end
    ...
    if ESCAPE_STATUS == 1 and ESCAPE_TIMER > 0 and ESCAPE_TIMER <= CurTime() then
        ...
        if GetRoundStat("alpha_warhead") then
            ...
            for k, v in ipairs(tab) do
                ...
                InfoScreen(v, "escaped", INFO_SCREEN_DURATION, {"escape2", {"escape_xp", "text;" .. give_xp}})
            ...
            if IsValid(t) or ROUND.aftermatch then
                ...
                if ROUND.aftermatch then
                    ...
                    xp = math.floor(xp)
                    msg = {"escape1", {"escape_time", "time;" .. diff}, xp_info}
                end
...
function GetEscapeBlockingPlayers(ply)
    ...
    for i, v in ipairs(LAST_ESCAPE) do
        ...
        local t = v:SCPTeam()
        if not SCPTeams.IsEnemy(our_team, t) and not SCPTeams.IsEnemy(t, our_team) then continue end
...
function IsBlockingEscape(ply1, ply2)
    if not LAST_ESCAPE_LOOKUP[ply1] or not LAST_ESCAPE_LOOKUP[ply2] then return false end
...
function GM:SLCEscapeMultiplier(plys, mult)
    local num_ply = -1
    for i, v in ipairs(plys) do
        if not mult.skip_count[v] then num_ply = num_ply + 1 end
    end

    local xp_mult = math.Clamp(num_ply / 10, 0, 1)
    for i, v in ipairs(plys) do
        if not mult.skip_mult[v] then mult[v] = mult[v] + xp_mult * (mult.bonus_mult[v] or 1) end
    end
end
...
function StartAftermatch(endcheck)
    ...
    if IsValid(support_timer) then support_timer:Destroy() end
...
function PlayerEscort(ply)
    ...
    if not ply:GetPos():WithinAABox(pos[1], pos[2]) then return end
    local t = GetTimer("SLCRound")
    if IsValid(t) then
        ...
        local plys = {}
        for i, v in ipairs(player.GetAll()) do
            if v:GetPos():WithinAABox(pos[1], pos[2]) and SCPTeams.CanEscort(team, v:SCPTeam()) then table.insert(plys, v) end
        end

        local num = #plys
        if num == 0 then return end
        local msg = {"escorted", {"escape_time", "time;" .. diff}, {"escape_xp", "text;" .. xp}}
