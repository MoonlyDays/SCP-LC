...
function AddRoundType(name, tab, base, chance)
	...
	if isnumber(chance) and number > 0 then select_info[name] = chance end
	ROUNDS[name] = tab
end
...
AddRoundType("normal", {
	...
	postround = function(self, winner) PrintSCPNotice() end,
		...
		if num == 0 then
			...
			for t1, _ in pairs(teams) do
				for t2, _ in pairs(teams) do
					if SCPTeams.IsEnemy(t1, t2) then return false end
				end
			end
			return true
		end
	...
	getwinner = function(self)
		...
		if hs then
			if istable(hs) then
				...
				if #winner == 1 then winner = winner[1] end
				return winner
			end
		else
			return false
...
AddRoundType("multi", {
	name = "multi",
	init = function(self) self.BaseClass.init(self, true) end,
}, "normal")