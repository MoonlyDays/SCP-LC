...
util.AddNetworkString("SLCDamageNumbers")
...
net.Receive("PlayerReady", function(len, ply)
    if ply.FullyLoaded and not DEVELOPER_MODE then return end
    ...
    if not ply.playermeta then ErrorNoHalt("Error! Player.playermeta is nil!\n", ply) end
...
net.Receive("DropWeapon", function(len, ply)
    ...
    if ctrl and ctrl[1] < CurTime() then return end
...
net.Receive("WeaponDnD", function(len, ply)
    ...
    if IsValid(wep1) and IsValid(wep2) then if wep1:GetOwner() == ply and wep2:GetOwner() == ply then if wep2.DragAndDrop then wep2:DragAndDrop(wep1, false) end end end
end)
...
net.Receive("DropVest", function(len, ply) if ply:GetVest() > 0 then ply:DropVest() end end)
...
net.Receive("ClassUnlock", function(len, ply)
    local class = net.ReadString()
    if class then ply:UnlockClass(class) end
end)
...
function PLAYER:SLCVarUpdated(id, var_type, newVal)
    if newVal ~= nil then
        ...
        if cb then cb(self, newVal) end
    end

    for i, v in ipairs(self.scp_update_var) do
        if v[1] == id and v[2] == var_type then return end
    end
...
function PLAYER:AddSLCVar(name, id, var_type)
    if not name or not id or not var_type then return end
...
net.Receive("UpdateSLCVars", function(len, ply)
    if not ply.scp_var_table then return end
