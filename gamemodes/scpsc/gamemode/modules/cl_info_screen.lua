    ...
    for i, v in ipairs(text_data) do
        for line in string.gmatch(v.text, "[^\n]+") do
            ...
            table.insert(lines, {
                text = line,
                length = len,
                font = v.font or "SCPInfoScreenBig",
                x_offset = v.x_offset or 0,
                y_offset = v.y_offset or 0
            })
...
local function parse_key(data)
    ...
    if key then
        if #args > 0 then
            ...
            if macro then return macro(args) end
        else
...
function InfoScreen(ply, type, duration, data)
    ...
    if ply and ply ~= LocalPlayer() and not istab then return end
    ...
    text[1] = {
        text = string.format("%s: %s\n%s: %s\n%s: %s\n%s: %s", LANG.info_screen.subject, LocalPlayer():Nick(), LANG.info_screen.class, LANG.CLASSES[class] or class, LANG.info_screen.team, LANG.TEAMS[team] or team, LANG.info_screen.status, status or LANG.info_screen_type.unknown),
    }
...
hook.Add("DrawOverlay", "SLCInfoScreen", function()
    if not INFO_SCREEN.show_screen then return end
    local ct = CurTime()
    if INFO_SCREEN.end_time < ct then INFO_SCREEN.done = true end
    if not INFO_SCREEN.done and input.IsMouseDown(MOUSE_LEFT) and SLCAuth.HasAccess(LocalPlayer(), "slc skipintro") then INFO_SCREEN.done = true end
    if not INFO_SCREEN.done then
        if INFO_SCREEN.blackout < 1 then
            if INFO_SCREEN.time == 0 then INFO_SCREEN.time = ct + 0.3 end
        ...
        elseif INFO_SCREEN.alpha < 1 then
            if INFO_SCREEN.time == 0 then INFO_SCREEN.time = ct + 0.2 end
            INFO_SCREEN.alpha = 1 - (INFO_SCREEN.time - ct) / 0.2
            if INFO_SCREEN.alpha >= 1 then
                INFO_SCREEN.alpha = 1
                INFO_SCREEN.time = 0
            end
        elseif not INFO_SCREEN.calc_text then
...
InfoScreenMacro("team", function(args)
    local t = tonumber(args[1])
    if not t then return LANG.TEAMS.unknown end
