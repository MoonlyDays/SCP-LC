...
local function add_button(text, func, pnl, dock)
    ...
    btn.DoClick = function(this) if func and this:IsEnabled() and not IsValid(pnl.LockIfValid) then func(this) end end
    btn.pct = 0
    btn.Paint = function(this, pw, ph)
        local delta = -1
        local enabled = this:IsEnabled() and not IsValid(pnl.LockIfValid)
        if enabled and this:IsHovered() then delta = 1 end
...
function OpenMenuScreen()
    ...
    if system.IsWindows() then SLCMenuScreen:SetHTML([[<!DOCTYPE html>
<style type="text/css">
html, body {
	width: 100%;
	height:100%;
 ...
 <body>
  <video id="vid" video autobuffer autoplay loop>
    ...
    SLCMenuScreen.PaintOver = function(this, pw, ph)
        ...
        if this.DebugNotif < RealTime() then draw.SimpleText("Stuck on this screen? Write slc_menu_debug in console", "SCPHUDSmall", 16, h - offset_y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM) end
    end
    ...
    sbt.Think = function(this) this:SetEnabled(_SLCPlayerReady and (SLC_CONTENT.status == SCS_DONE or SLC_CONTENT.status == SCS_WAITING)) end
    --Settings button
    add_button("settings", function()
        OpenSettingsWindow()
        pnl.LockIfValid = SLC_SETTINGS_WINDOW
        SLC_SETTINGS_WINDOW.Think = function(this) if not this:HasFocus() and not IsValid(SLC_POPUP) and not IsValid(SLC_SETTINGS_WINDOW) then this:MakePopup() end end
    end, pnl)
    ...
    add_button("credits", function()
        ...
        vbar.Paint = function(this, pw, ph) draw.RoundedBox(15, 0, 0, pw, ph, color_light) end
        grip.Paint = function(this, pw, ph) draw.RoundedBox(15, 2, 2, pw - 4, ph - 4, color_dark) end
    end, pnl)

    --Quit button
    add_button("quit", function()
        SLCLegacyPopup(lang.quit_server, lang.quit_server_confirmation, false, function(i) if i == 1 then RunConsoleCommand("disconnect") end end, lang.yes, lang.no)
        pnl.LockIfValid = SLC_POPUP
    end, pnl, BOTTOM)
    ...
    btn_ws.DoClick = function(this) gui.OpenURL("https://steamcommunity.com/sharedfiles/filedetails/?id=" .. SLC_CONTENT_WORKSHOP_ID) end
    ...
    cb:SetText("Auto")
    cb:SetState(auto)
    cb.Think = function(this) this:SetText(LANG.MISC.content_checker.auto_check) end
    cb.OnUpdate = function(this, new) cookie.Set("slc_auto_content_check", new and 1 or 0) end
    ...
    status_pnl.Paint = function(this, pw, ph)
        ...
        elseif status == SCS_DONE then
            if SLC_CONTENT.info.nok > 0 then
                local txt = ""
                if SLC_CONTENT.info.was_nsub > 0 then txt = txt .. clang.nsub_warn end
                if SLC_CONTENT.info.was_disabled > 0 then txt = txt .. clang.disabled_warn end
        ...
        if p_show then
            ...
            if p_txt then draw.SimpleText(p_txt, "SCPHUDSmall", pw * 0.5, dy, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM) end
        end
    end

    if auto then CheckContent() end
end

concommand.Add("slc_menu_debug", function()
    if not IsValid(SLCMenuScreen) then return end
