...
function ChaseArea:Think()
    ...
    for i, v in ipairs(player.FindInSphere(self.Position, self.Radius)) do
        if v:SCPTeam() ~= TEAM_SCP or not v:GetSCPChase() or hook.Run("SLCChase", ply, v) then continue end
        if not tab[v] then
            tab[v] = {
                start_time = ct,
        ...
        if not last_id or self.UID > last_id then
...
setmetatable(ChaseArea, {
    __call = ChaseArea.New
})
...
function ChaseController:Think()
    ...
    if not self.LastArea or self.LastArea:DistToSqr(pos) > self.LastAreaRadiusSqr and not ply:KeyDown(IN_DUCK) then
        local area = ChaseArea(self)
        table.insert(self.Areas, 1, area)
        if #self.Areas == 9 then self.Areas[9] = nil end
    ...
    for i = #self.Areas, 1, -1 do
        if self.Areas[i]:Think() then table.remove(self.Areas, i) end
    end

    for chase_ply, chase_data in pairs(self.ChasedBy) do
        if not IsValid(chase_ply) or chase_data.last_area_dietime <= ct then
            self.ChasedBy[chase_ply] = nil
            continue
        end

        if not chase_data.is_active then chase_data.is_active = chase_data.last_area_id > chase_data.start_area_id end
    end
end

setmetatable(ChaseController, {
    __call = ChaseController.New
})
...
hook.Add("Think", "SLCChaseThink", function()
    ...
    for _, ply in ipairs(player.GetAll()) do
        ...
        for chase_ply, chase_data in pairs(cont.ChasedBy) do
            if not chase_data.is_active then continue end
            ply_tab[ply] = true
    ...
    for _, ply in ipairs(player.GetAll()) do
        ...
        if not data and not ply_tab[ply] then continue end
        ...
        if ply_tab[ply] then
            if not data then
                ...
                ply:PlayAmbient("scp_chase", true, true, function(pl) return not pl:GetProperty("slc_chase_data") end)
            end
