...
function AddChatCommand(cmd, callback, realm, hide)
    ...
    if not string.StartsWith(cmd, "!") then cmd = "!" .. cmd end
...
if SERVER then
    hook.Add("PlayerSay", "SLCChatCommands", function(ply, msg, teamonly)
        ...
        if isstring(cmd) then cmd = string.lower(cmd) end
        local tab = commands_registry[cmd]
        if not tab then
            local alias = commands_aliases[cmd]
            if alias then tab = commands_registry[alias] end
        end

        if not tab or not tab.realm then
            net.Ping("SLCChatCommand", msg, ply)
            if tab and tab.hide then return "" end
            return
        end

        if tab.callback then tab.callback(ply, args) end
        if tab.hide then return "" end
    end)
else
    net.ReceivePing("SLCChatCommand", function(data)
        ...
        if tab.callback then tab.callback(LocalPlayer(), args) end
    end)
end
...
hook.Add("SLCFullyLoaded", "SLCChatCommandsAliases", function()
    for _, lang in pairs(_LANG) do
        if not lang.MISC.commands_aliases then continue end
        for alias, cmd in pairs(lang.MISC.commands_aliases) do
            if not string.StartsWith(alias, "!") then alias = "!" .. alias end
            if not string.StartsWith(cmd, "!") then cmd = "!" .. cmd end
...
AddChatCommand("afk", function(ply)
    if ply.ncmdcheck and ply.ncmdcheck > CurTime() or not ply:IsValidSpectator() then return end
    ply.ncmdcheck = CurTime() + 2
    ply:MakeAFK(30)
end, SERVER, false)
...
AddChatCommand("adminmode", function(ply)
    if ply.ncmdcheck and ply.ncmdcheck > CurTime() then return end
    ply.ncmdcheck = CurTime() + 2
    if SLCAuth.HasAccess(ply, "slc adminmode") then ply:ToggleAdminMode() end
end, SERVER, true)
...
AddChatCommand("settings", function(ply) OpenSettingsWindow() end, CLIENT, true)
