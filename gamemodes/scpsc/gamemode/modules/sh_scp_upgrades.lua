...
local custom_skill_parsers = {
    outside_buff = function(wep, lang, info)
        ...
        if mods.regen_max > SCP_BUFF_REGEN_CAP then mods.regen_max = SCP_BUFF_REGEN_CAP end
        return mods
    end,
}
...
function DefineUpgradeSystem(name, data, swep)
    ...
    for i, v in ipairs(data.upgrades) do
        ids[v.name] = i
        if not v.icon and generic_skill_icons[v.name] then v.icon = GetMaterial(generic_skill_icons[v.name]) end
    end
...
function InstallUpgradeSystem(name, swep)
    ...
    swep.GetUpgradePoints = function(self) return self.UpgradeSystemRegistry.points, self.UpgradeSystemRegistry.total end
    swep.SetUpgradePoints = function(self, num, total)
        self.UpgradeSystemRegistry.points = num
        if total then self.UpgradeSystemRegistry.total = total end
        if SERVER then
    ...
    swep.HasUpgrade = function(self, u_name) return self.UpgradeSystemRegistry.upgrades[u_name] == true end
    swep.AddScore = function(self, score)
        ...
        if self.UpgradeSystemRegistry.lastreward < count then
            ...
            if changed then hook.Run("SCPUpgradePoint", self, self.UpgradeSystemRegistry.points, self.UpgradeSystemRegistry.lastreward, count) end
        ...
        if id then
            ...
            if block and #block > 0 then
                for k, v in pairs(block) do
                    if self.UpgradeSystemRegistry.upgrades[v] then return true end
                end
            end
        end
    end
        ...
        if id then
            ...
            for k, v in pairs(req) do
                if self.UpgradeSystemRegistry.upgrades[v] then
                    if reqany then return true end
                else
    ...
    swep.CanBuyUpgrade = function(self, u_name)
        ...
        if not self:UpgradeUnlocked(u_name) then return false end
        --if self:UpgradeBlocked( u_name ) then return false end
        local id = upg.upgradeid[u_name]
        if id then return self.UpgradeSystemRegistry.points >= upg.upgrades[id].cost end
    end
    ...
    swep.BuyUpgrade = function(self, id)
        ...
        if upgrade then
            if self:CanBuyUpgrade(upgrade.name) then
                ...
                if upgrade.active and self.OnUpgradeBought then self:OnUpgradeBought(upgrade.name, upgrade.active, upgrade.group) end
                hook.Run("SCPUpgradeBought", self, upgrade)
            end
        end
    end

    swep.GetUpgradeMod = function(self, m_name, def) return self.UpgradeSystemRegistry.mod[m_name] or def end
...
function RestoreUpgradeSystem(swep, data)
    if data.UpgradeSystemMounted then
        ...
        if IsValid(owner) then net.SendTable("SCPUpgradeSync", {swep:GetClass(), swep.UpgradeSystemRegistry}, owner) end
        if swep.UpgradeSystemRestored then swep:UpgradeSystemRestored(swep.UpgradeSystemRegistry.upgrades) end
        local upg = SLC_SCP_UPGRADES[swep.UpgradeSystemName]
        if upg then
            for k, v in pairs(upg.upgrades) do
                if v.active and v.rowr and swep.OnUpgradeBought and swep.UpgradeSystemRegistry.upgrades[v.name] then swep:OnUpgradeBought(v.name, v.active, v.group) end
            end
        end
    end
end
...
if SERVER then
    ...
    net.Receive("SCPUpgrade", function(len, ply)
        ...
        if not IsValid(wep) or not wep.UpgradeSystemMounted then return end
        wep:BuyUpgrade(id)
    end)
end
...
if CLIENT then
        ...
        WaitForSync(function()
            ...
            if IsValid(wep) then
                if wep.UpgradeSystemMounted then
                    ...
                    if upg then
                        for k, v in pairs(upg.upgrades) do
                            if v.active and v.rowr and wep.OnUpgradeBought and wep.UpgradeSystemRegistry.upgrades[v.name] then wep:OnUpgradeBought(v.name, v.active, v.group) end
                        end
                    end
                end
            end
    ...
    local function drawUpgrades()
        if not HUDSCPUpgradesOpen then return end
        next_frame = true
        ...
        for i, v in ipairs(upg.upgrades) do
            ...
            if not wep:HasUpgrade(v.name) then
            ...
            if not wep:UpgradeUnlocked(v.name) then
            ...
            if rnum > 1 and not v.reqany then
                surface.SetDrawColor(150, 150, 150, 255)
                draw.NoTexture()
        ...
        if info then
            ...
            local width = w * 0.3
            cy = cy + nf_yoffset
            if cx + width > w then cx = w - width end
            ...
            if dsc then
                if lang.parse_description == true or clang.parse_description == true then
                    if not wep.cached_description then wep.cached_description = {} end
                    if not wep.cached_description[info.name] then
                        ...
                        wep.cached_description[info.name] = string.gsub(dsc, "%[([%/%+%-%%]?)(%+?)([%w_]+)%]", function(prefix, plus, name)
                            ...
                            if not value then return "?" .. name end
                            if prefix == "-" then
                                local num = 1 - value
            ...
            if info.req and #info.req > 0 and not wep:UpgradeUnlocked(info.name) then
            ...
            if info.block and #info.block > 0 and not wep:HasUpgrade(info.name) then
