...
function PLAYER:SpectatePlayerNext()
    ...
    local index
    if not IsValid(cur_target) then
        index = 1
    else
    ...
    if index > #plys then index = 1 end
...
function PLAYER:SpectatePlayerPrev()
    ...
    if not index then index = 1 end
    if index < 1 then index = #plys end
    local target = plys[index]
    if target ~= cur_target then
...
function PLAYER:GetValidSpectateTargets(all)
    ...
    if CVAR.slc_allow_scp_spectate:GetBool() == true or hook.Run("SLCCanSpectateSCP", self) == true or SLCAuth.HasAccess(self, "slc spectatescp") then info = SCPTeams.INFO_ALIVE end
    local plys = {}
    local tab = SCPTeams.GetPlayersByInfo(info)
    for i, v in ipairs(tab) do
        if all or not v:IsAboutToSpawn() then table.insert(plys, v) end
    end
    return plys
end
...
function CheckSpectatorMode(all)
    for i, v in ipairs(player.GetAll()) do
        if v:SCPTeam() == TEAM_SPEC and not v:GetAdminMode() then v:CheckSpectatorMode(all) end
    end
end
