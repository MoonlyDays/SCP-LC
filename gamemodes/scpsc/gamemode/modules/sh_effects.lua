...
hook.Add("PlayerPostThink", "SLCEffectsThink", function(ply)
    if not ply.EFFECTS then ply.EFFECTS = {} end
    local ct = CurTime()
    for i, v in rpairs(ply.EFFECTS) do
        ...
        if v.endtime ~= -1 and v.endtime <= ct then
            ...
            if keep then
                ...
                if eff.begin then eff.begin(v, ply, v.tier, v.args, true, true) end
                if v.duration then v.endtime = ct + v.duration end
            else
                if eff.finish then if eff.finish(v, ply, v.tier, v.args, false, false) then continue end end
                ...
                if not found then ply.EFFECTS_REG[v.name] = nil end
            end
        elseif eff.think then
            if eff.wait then
                if v.nextthink and v.nextthink > ct then return end
                v.nextthink = ct + eff.wait
            end

            local override = eff.think(v, ply, v.tier, v.args)
            if isnumber(override) then v.nextthink = ct + override end
        end
    end
end)
...
function PLAYER:ApplyEffect(name, ...)
    ...
    if not self.EFFECTS_REG then self.EFFECTS_REG = {} end
    if hook.Run("SLCApplyEffect", self, name, args) == true then return false end
    if not effect.stacks or effect.stacks == 0 or effect.stacks == 2 then
        if self.EFFECTS_REG[name] then
            for k, v in ipairs(self.EFFECTS) do
                if v.name == name then
                    if effect.duration >= 0 then v.endtime = CurTime() + effect.duration end
                    if effect.stacks == 2 then
                        local ntier = v.tier + tier
                        if ntier > effect.tiers then ntier = effect.tiers end
                        v.tier = ntier
                        --v.icon = EFFECTS.registry[name.."_"..ntier]
                    end

                    if effect.begin then effect.begin(v, self, v.tier, args, true, false) end
                    if v.duration then v.endtime = CurTime() + v.duration end
                    if SERVER then
    ...
    local endtime = -1
    if effect.duration >= 0 then endtime = CurTime() + effect.duration end
    ...
    if effect.begin then effect.begin(tab, self, tier, args, false, false) end
    if tab.duration then tab.endtime = CurTime() + tab.duration end
    if SERVER then
        net.Start("PlayerEffect")
        net.WriteBool(false)
        net.WriteString(name)
...
function PLAYER:RemoveEffect(name, all)
    ...
    if not name or name == "" then
        for i, v in ipairs(self.EFFECTS) do
            ...
            if effect and effect.finish then effect.finish(v, self, v.tier, v.args, true, true) end
        end
        ...
        for i, v in rpairs(self.EFFECTS) do
            if v.name == name then
                if effect.stacks ~= 2 or all or v.tier == 1 then
                    ...
                    if effect.finish then effect.finish(v, self, v.tier, v.args, true, false) end
                    if effect.stacks ~= 1 or all then
                        ...
                        if not found then self.EFFECTS_REG[name] = nil end
                    end
                else
                    v.endtime = CurTime() + effect.duration
                    v.tier = v.tier - 1
                    if effect.begin then effect.begin(v, self, v.tier, v.args, true, true) end
                    if v.duration then v.endtime = CurTime() + v.duration end
                end

                if effect.stacks == 0 or effect.stacks == 2 or not all then break end
            end
        end
...
function PLAYER:RefreshEffect(name, all)
    if not self.EFFECTS_REG[name] then return end
    local effect = EFFECTS.effects[name]
    if not effect then return end
    if SERVER then
    ...
    for i, v in rpairs(self.EFFECTS) do
        ...
        v.endtime = CurTime() + effect.duration
        if effect.begin then effect.begin(v, self, v.tier, v.args, true, false) end
        if v.duration then v.endtime = CurTime() + v.duration end
...
function PLAYER:GetEffect(name)
    if not self.EFFECTS_REG or not self.EFFECTS_REG[name] then return end
    for i, v in rpairs(self.EFFECTS) do
...
function scp_spec_hazard_filter(dmg)
    return function(ply)
        if not scp_spec_filter(ply) then return false end
        if ply:GetSCP714() then return false end
        return not ply:CheckHazardProtection(dmg)
    end
end
...
EFFECTS.RegisterEffect("heavy_bleeding", {
    ...
    begin = function(self, ply, tier, args, refresh)
        ...
        if refresh then self.args[2] = 1 end
    ...
    finish = function(self, ply, tier, args, interrupt, all)
        if CLIENT or not interrupt or all then return end
        ply:AddTimer("reopen_wound", SLCRandom(30, 120), 1, function() if not ply:HasEffect("heavy_bleeding") and SLCRandom(1, 100) <= 50 / args[2] then ply:ApplyEffect("heavy_bleeding", args[1], args[2] + 1) end end)
    end,
    think = function(self, ply, tier, args)
        if CLIENT then return end
        local att = args[1]
        ...
        dmg:SetDamage(2)
        dmg:SetDamageType(DMG_DIRECT)
        if IsValid(att) and att:CheckSignature(args[3]) then dmg:SetAttacker(att) end
        ply:TakeDamageInfo(dmg)
...
EFFECTS.RegisterEffect("insane", {
    ...
    think = function(self, ply, tier, args)
        if SERVER then
            if ply:GetSanity() >= 20 or not SCPTeams.HasInfo(ply:SCPTeam(), SCPTeams.INFO_HUMAN) then ply:RemoveEffect("insane") end
        else
            InsaneTick(ply)
        end
    end,
    finish = function(self, ply, tier, args, interrupt) if CLIENT then InterruptInsane(ply) end end,
    wait = 3,
})
...
EFFECTS.RegisterEffect("gas_choke", {
    ...
    begin = function(self, ply, tier, args, refresh)
        if SERVER and not refresh then
            ply:PushSpeed(0.65, 0.65, -1, "SLC_Choke")
            ply:SetNextBlink(0)
        end
    end,
    finish = function(self, ply, tier, args, interrupt) if SERVER then ply:PopSpeed("SLC_Choke") end end,
    think = function(self, ply, tier, args) if CLIENT then ply:EmitSound("SLCEffects.Choke") end end,
    wait = 2.75
})

hook.Add("SLCBlinkParams", "ChokeEffect", function(ply) if ply:HasEffect("gas_choke") then return CVAR.slc_blink_delay:GetFloat() * 0.25 end end)
if CLIENT then
...
EFFECTS.RegisterEffect("radiation", {
    ...
    think = function(self, ply, tier, args)
        if CLIENT then ply:EmitSound("SLCEffects.Radiation") end
...
EFFECTS.RegisterEffect("deep_wounds", {
    ...
    cantarget = scp_spec_filter,
    begin = function(self, ply, tier, args, refresh) if SERVER then ply:PushSpeed(0.9, 0.9, -1, "SLC_DeepWounds", 1) end end,
    finish = function(self, ply, tier, args, interrupt) if SERVER then ply:PopSpeed("SLC_DeepWounds") end end,
})
...
hook.Add("StartCommand", "SLCSpawnProtection", function(ply, cmd)
    if not ply:HasEffect("spawn_protection") then return end
    if bit.band(cmd:GetButtons(), action_mask) ~= 0 then ply:RemoveEffect("spawn_protection") end
end)
...
EFFECTS.RegisterEffect("poison", {
    ...
    finish = function(self, ply, tier, args, interrupt)
        if SERVER then ply:PopSpeed("SLC_Poison") end
        ply:SetDSP(0)
    end,
    think = function(self, ply, tier, args)
        if SERVER then
...
EFFECTS.RegisterEffect("fracture", {
    ...
    cantarget = scp_spec_filter,
    begin = function(self, ply, tier, args, refresh) if SERVER then ply:PushSpeed(0.4, 0.4, -1, "SLC_Fracture") end end,
    finish = function(self, ply, tier, args, interrupt) if SERVER then ply:PopSpeed("SLC_Fracture") end end,
})

local fcv = 0
hook.Add("SLCCalcView", "SLCFracture", function(ply, view)
    if not ply:OnGround() or not ply.HasEffect or not ply:HasEffect("fracture") then return end
    if not view.vm and ply:GetVelocity():LengthSqr() > 900 then fcv = fcv + FrameTime() * 3 end
...
EFFECTS.RegisterEffect("decay", {
    ...
    finish = function(self, ply, tier, args, interrupt)
        if SERVER then ply:PopSpeed("SLC_Decay") end
        ply:SetDSP(0)
    end,
    think = function(self, ply, tier, args)
        if not SERVER then return end
        local calc = math.ceil((CurTime() - self.start) / 30 * self.damage)
        if not ply:CheckHazmat(calc * 2.5) then
            local dmg = DamageInfo()
            dmg:SetDamage(calc)
...
hook.Add("SLCFootstepParams", "SLCDecaySteps", function(ply, st, vel, crouch) if SERVER and ply:Alive() and ply:HasEffect("decay") then return vel:LengthSqr() > 10000 and 75 or 50 end end)
local dcv = 0
hook.Add("SLCCalcView", "SLCDecay", function(ply, view)
    if not ply:OnGround() or not ply.HasEffect or not ply:HasEffect("decay") then return end
    if not view.vm then
        local len = ply:GetVelocity():Length()
        if len > 30 then dcv = dcv + FrameTime() * 0.06 * len end
    end
...
hook.Add("PlayerUse", "SLCDoorLock", function(ply, ent) if ply:HasEffect("doorlock") then return false end end)
--[[-------------------------------------------------------------------------
Chase effects
---------------------------------------------------------------------------]]
EFFECTS.RegisterEffect("scp_chase", {
    ...
    cantarget = function(ply) return ply:SCPTeam() == TEAM_SCP end,
    begin = function(self, ply, tier, args, refresh, decrease)
        if CLIENT then return end
...
EFFECTS.RegisterEffect("scp009", {
    ...
    cantarget = function(ply)
        if not scp_spec_filter(ply) then return false end
        if ply:GetSCP714() then return false end
        return not ply:CheckHazmat(175, true)
    end,
    begin = function(self, ply, tier, args, refresh, decrease)
        if IsValid(args[1]) then
            self.attacker = args[1]
        ...
        if SERVER then ParticleEffectAttach("SLC_SCP009_Smoke", PATTACH_ABSORIGIN_FOLLOW, ply, 0) end
    end,
    finish = function(self, ply, tier, args, interrupt)
        ply:StopParticles()
        ...
        ply:TakeDamageInfo(dmg)
        ply._RagEntity = nil
        if IsValid(self.attacker) then self.attacker:SetProperty("kill_team_override", nil) end
    end,
    think = function(self, ply, tier, args)
        if CLIENT then return end
        local pos = ply:GetPos()
        ...
        for i, v in ipairs(player.GetAll()) do
            if SLCRandom(4) < 4 and not v:HasEffect("scp009") and v:GetPos():DistToSqr(pos) <= radius then v:ApplyEffect("scp009", self.attacker, self.team) end
        end
    end,
    wait = 0.5
})
...
EFFECTS.RegisterEffect("electrical_shock", {
    ...
    finish = function(self, ply, tier, args, interrupt) if SERVER then ply:PopSpeed("SLC_ElectricalShock") end end,
})
...
EFFECTS.RegisterEffect("ephedrine", {
    ...
    cantarget = scp_spec_filter,
    begin = function(self, ply, tier, args, refresh) if SERVER then ply:PushSpeed(args[1], args[1], -1, "SLC_EphedrineBoost", 1) end end,
    finish = function(self, ply, tier, args, interrupt) if SERVER then ply:PopSpeed("SLC_EphedrineBoost") end end,
    ignore500 = true,
})
...
EFFECTS.RegisterEffect("hemostatic", {
    ...
    cantarget = scp_spec_filter,
    begin = function(self, ply, tier, args, refresh) self.duration = args[1] end,
    think = function(self, ply, tier, args)
        if CLIENT then return end
        if ply:HasEffect("bleeding") then ply:RemoveEffect("bleeding") end
        if ply:HasEffect("heavy_bleeding") then ply:RemoveEffect("heavy_bleeding") end
    end,
    wait = 1,
    ignore500 = true,
})
...
EFFECTS.RegisterEffect("antidote", {
    ...
    think = function(self, ply, tier, args)
        if CLIENT then return end
        if ply:HasEffect("poison") then ply:RemoveEffect("poison") end
        if ply:HasEffect("poison_syringe") then ply:RemoveEffect("poison_syringe") end
    end,
    wait = 1,
    ignore500 = true,
})
...
EFFECTS.RegisterEffect("poison_syringe", {
    ...
    cantarget = scp_spec_filter,
    begin = function(self, ply, tier, args, refresh)
        if not IsValid(args[1]) or not args[1]:IsPlayer() then return end
        if args[2] > self.args[2] then self.args[2] = args[2] end
        if args[3] < self.args[3] then self.args[3] = args[3] end
        self.args[1] = args[1]
    ...
    think = function(self, ply, tier, args)
        ...
        dmg:SetDamage(args[2])
        dmg:SetDamageType(DMG_DIRECT)
        if IsValid(att) and att:IsPlayer() and att:CheckSignature(args[4]) then dmg:SetAttacker(att) end
        ply:TakeDamageInfo(dmg)
        return args[3]
    end,
    wait = 1,
...
EFFECTS.RegisterEffect("expd_rubber_bones", {
    ...
    cantarget = function(ply) return ply:SCPClass() == CLASSES.EXPD end,
...
EFFECTS.RegisterEffect("expd_stamina_tweaks", {
    ...
    begin = function(self, pl)
        ...
        if SERVER then pl.ClassData.stamina = 200 end
        local id = pl:SteamID64()
    ...
    begin = function(self, pl)
        if SERVER then
            ...
            for k, v in pairs(pl.EFFECTS_REG) do
                ...
                if not obj or obj.ignore500 then continue end
        ...
        local id = pl:SteamID64()
        AddRoundHook("EntityTakeDamage", "expd_effect" .. id, function(ply, dmg) if ply == pl then return true end end)
        AddRoundHook("PlayerSwitchWeapon", "expd_effect" .. id, function(ply) if ply == pl then return true end end)
        AddRoundHook("SLCPlayerFootstep", "expd_effect" .. id, function(ply) if ply == pl then return true end end)
    end,
    finish = function(self, pl)
        if SERVER then
            pl:GodDisable()
...
EFFECTS.RegisterEffect("expd_lifesteal", {
    ...
    begin = function(self, pl)
        local id = pl:SteamID64()
        AddRoundHook("SLCPostScaleDamage", "expd_effect" .. id, function(ply, dmg)
            if not IsValid(ply) or not ply:IsPlayer() or dmg:IsDamageType(DMG_DIRECT) then return end
        ...
        AddRoundHook("SLCCanHeal", "expd_effect" .. id, function(target, healer, heal_type) if target == pl then return false end end)
    end,
    finish = function(self, pl)
        local id = pl:SteamID64()
        RemoveRoundHook("SLCPostScaleDamage", "expd_effect" .. id)
...
EFFECTS.RegisterEffect("expd_glass_cannon", {
    ...
    begin = function(self, pl)
        ...
        AddRoundHook("SLCPlayerDeath", "expd_effect" .. id, function(victim, attacker)
            if attacker ~= pl or not IsValid(victim) or not victim:IsPlayer() or not IsValid(attacker) or not attacker:IsPlayer() then return end
...
EFFECTS.RegisterEffect("expd_reflect", {
    ...
    begin = function(self, pl)
        local id = pl:SteamID64()
        AddRoundHook("SLCPostScaleDamage", "expd_effect" .. id, function(ply, dmg)
            if not IsValid(ply) or not ply:IsPlayer() or dmg:IsDamageType(DMG_DIRECT) or ply:GetProperty("expd_reflected") then return end
            if ply == pl then
                local attacker = dmg:GetAttacker()
...
EFFECTS.RegisterEffect("expd_invis", {
    ...
    begin = function(self, pl)
        ...
        AddRoundHook("PlayerPostThink", "expd_effect" .. id, function(ply)
            ...
            if not is_crouching then return end
...
hook.Add("SLCScaleHealing", "SLCEffectsScaleHeal", function(ply, source, heal)
    if ply:HasEffect("fracture") then return 0 end
    if ply:HasEffect("deep_wounds") then return heal * 0.25 end
end)
...
EFFECTS.RegisterEffect("frightened", {
    ...
    cantarget = scp_spec_filter,
    begin = function(self, ply, tier, args, refresh)
        if SERVER and not refresh then ply:PushSpeed(0.7, 0.7, -1, "SLC_Frightened") end
        if CLIENT then ply:EmitSound("SLCEffects.Frightened") end
    end,
    finish = function(self, ply, tier, args, interrupt) if SERVER then ply:PopSpeed("SLC_Frightened") end end,
})
...
EFFECTS.RegisterEffect("scp1987j_blackout", {
    ...
    cantarget = scp_spec_filter,
    begin = function(self, ply, tier, args, refresh) if CLIENT then ply:SetDSP(14) end end,
    finish = function(self, ply, tier, args, interrupt) if CLIENT then ply:SetDSP(0) end end,
})
