hook.Add("StartCommand", "SLCSprint", function(ply, cmd)
    ...
    elseif cmd:KeyDown(IN_JUMP) and IsFirstTimePredicted() then
        if not ply.WasJumpDown and ply:OnGround() and not ply:InVehicle() then
            local stamina = ply:GetStamina()
            local mask = ply:GetWeaponByGroup("gasmask")
            if not IsValid(mask) or not mask:GetEnabled() or not mask:GetUpgraded() or stamina - 10 > 30 then
    ...
    if (ply:OnGround() or ply:WaterLevel() ~= 0) and not ply:InVehicle() then
    ...
    if exhausted then
        ...
        if not ply.ExhaustCam then ply.ExhaustCam = ang.p end
...
hook.Add("OnPlayerHitGround", "SLCBHop", function(ply, water, floater, speed) ply.JumpPenalty = CurTime() + 0.3 end)
hook.Add("Move", "SLCBHop", function(ply, mv)
    if ply.JumpPenalty and ply.JumpPenalty >= CurTime() and not ply:GetProperty("allow_bhop") then
...
local function CalcStamina(ply)
    ...
    if stamina_limit > max_stamina then stamina_limit = max_stamina end
    if ply.StaminaRegen <= ct then
        ply.StaminaRegen = ply.StaminaRegen + data.regen_delay
        if ply.StaminaRegen < ct then ply.StaminaRegen = ct + data.regen_delay end
        if exhausted then
    ...
    if stamina > stamina_limit then stamina = stamina_limit end
    if ply.RunCheck < ct then
        ply.RunCheck = ply.RunCheck + data.use_delay
        if ply.RunCheck < ct then ply.RunCheck = CurTime() + data.use_delay end
        if ply.Running then
            ply.Running = false
            ply.StaminaRegen = CurTime() + data.regen_delay_running
            local mask = ply:GetWeapon("item_slc_gasmask")
            if not IsValid(mask) or not mask:GetEnabled() or not mask:GetUpgraded() or stamina > 30 then
    ...
    if stamina <= 0 and not exhausted then
        ...
        if SERVER then ply:PushSpeed(0.2, -1, -1, "SLC_Exhaust") end
    ...
    if SERVER then
        if stamina < 50 and not ply.Breathing then
            ply.Breathing = true
