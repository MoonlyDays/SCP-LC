...
local function translateButton(button)
	...
	if t and key then
		...
		elseif t == "code" then
			...
			if num_key then return num_key, input.GetKeyName(num_key), nil end
		elseif t == "bind" then
			local bind_key = input.LookupBinding(key)
			if bind_key then return input.GetKeyCode(bind_key), bind_key, key end
			return nil, nil, key
		end
	end
end
...
local function processSettingsEntry(k)
	...
	if v then
		if v.cvar then v.value = v.cvar:GetString() end
...
function GetBindButton(name)
	...
	if v and v.stype == "bind" then return v.key_code, v.key end
end
...
function OpenSettingsWindow()
	...
	close_btn.DoClick = function(self) window:Close() end
	...
	side_menu.VBar.Paint = function(this, pw, ph) draw.RoundedBox(15, 0, 0, pw, ph, COLOR.vbar) end
	side_menu.VBar.btnGrip.Paint = function(this, pw, ph) draw.RoundedBox(15, 1, 1, pw - 2, ph - 2, COLOR.vbar_grip) end
	...
	main_panel.VBar.Paint = function(this, pw, ph) draw.RoundedBox(15, 0, 0, pw, ph, COLOR.vbar) end
	main_panel.VBar.btnGrip.Paint = function(this, pw, ph) draw.RoundedBox(15, 1, 1, pw - 2, ph - 2, COLOR.vbar_grip) end
	main_panel.Paint = function(self, pw, ph) end
	table.sort(WINDOW_PANELS, function(a, b) return (a.z or 0) > (b.z or 0) end)
...
function OpenSettingsPanel(num)
	if not IsValid(SLC_SETTINGS_WINDOW) then return end
...
hook.Add("SLCGamemodeLoaded", "SLCSettings", function()
	...
	local data = {}
	if not file.Exists("slc/client_settings.json", "data") then
...
AddSettingsPanel("binds", function(parent)
	local binds = {}
	for k, v in pairs(SLC_SETTINGS) do
		if v.stype == "bind" and (not v.panel or v.panel == "binds") then
			table.insert(binds, {
				name = k,
				data = v
			})
	...
	for i, v in ipairs(binds) do
		...
		reset_btn.Paint = function(self, pw, ph)
			...
			surface.SetDrawColor(COLOR.white)
			surface.DrawOutlinedRect(0, 0, pw, ph)
			if not self:IsEnabled() then surface.SetDrawColor(COLOR.inactive) end
		...
		binder.Paint = function(self, pw, ph)
			...
			if self.Trapping then
				...
				if v.data.bind then b_text = b_text .. "  /  " .. v.data.bind end
				if v.data.key_conflict then b_color = COLOR.conflict end
			end
...
function AddConfigControl(parent, data, marg)
	if data.stype == "switch" then
		...
		switch.DoClick = function(this) inner:DoClick() end
		...
		inner.DoClick = function(this)
			...
			if data.data and data.data.callback then if data.data.callback(data.value, data.name) == true then return end end
			this.anim = true
			local anim = this:NewAnimation(0.25, 0, -1, function(tab, pnl) this.anim = false end)
			anim.Think = function(tab, pnl, f)
				if not switch.state then f = 1 - f end
				switch.color = Color(175 * (1 - f), 175 * f, 0)
				pnl:SetPos(switch:GetWide() * 0.5 * f)
			end

			if data.cvar then data.cvar:SetInt(data.value and 1 or 0) end
			processSettingsEntry(data.name)
			SaveSettings()
		end

		inner.Paint = function(this, pw, ph) draw.RoundedBox(8, 1, 1, pw - 2, ph - 2, color_gray) end
		switch.PerformLayout = function(this, pw, ph)
			this:SetWide(ph * 2)
			inner:SetSize(ph, ph)
			if this.state then inner:SetPos(ph, 0) end
		end
	elseif data.stype == "dropbox" then
		...
		dropbox.OnSelect = function(this, id, value)
			...
			data.value = value
			if data.data.callback then if data.data.callback(value, old, data.name) == true then return end end
			if data.cvar then data.cvar:SetString(value) end
			processSettingsEntry(data.name)
			SaveSettings()
		end
	end
...
function AddConfigPanel(panel_name, sort, def)
	AddSettingsPanel(panel_name, function(parent)
		local tab = {}
		for k, v in pairs(SLC_SETTINGS) do
			if order[v.stype] and (def and not v.panel or v.panel == panel_name) then
				table.insert(tab, {
					name = k,
					data = v,
					sorting = order[v.stype]
				})
			end
		end

		table.sort(tab, function(a, b)
			if a.sorting ~= b.sorting then return a.sorting < b.sorting end
...
AddSettingsPanel("reset", function(parent)
	...
	if LocalPlayer():IsSuperAdmin() then
		...
		server_btn.DoClick = function(self) SLCLegacyPopupIfEmpty(LANG.settings.server_reset, LANG.settings.server_reset_desc, true, nil, LANG.settings.popup_ok) end
	end
end, nil, 100)
