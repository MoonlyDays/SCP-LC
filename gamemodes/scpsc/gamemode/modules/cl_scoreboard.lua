...
local custom_badges = {
	vip = {
		...
		func = function(ply) return ply:IsPremium() end,
	},
...
if not DEVELOPER_MODE then timer.Simple(10, function() fetch_web_badges() end) end
local player_ranks = {
...
function ScoreboardPlayerData(ply)
	if not DEVELOPER_MODE and web_badges_status == 0 then fetch_web_badges() end
	local lp = LocalPlayer()
	local sorting = ply:IsActive() and SORT.unknown or SORT.inactive
	local color = not ply:IsActive() and Color(100, 100, 100, 255)
	...
	if ply == lp then sorting = SORT.localplayer end
	return sorting, color
end

function ScoreboardPlayerBadges(ply, tab)
	local badges = {}
	if tab then tab.badges = badges end
	...
	for k, v in pairs(custom_badges) do
		...
		if isstring(v.func) then
			if ply:SteamID64() == v.func then shouldhave = true end
		...
		if shouldhave then table.insert(badges, {k, v.color, v.sorting}) end
	end
