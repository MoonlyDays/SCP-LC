...
function GetSCPModifiers(ply)
    ...
    if not IsRoundLive() then return mods end
    ...
    if mods.def > 0.25 then mods.def = 0.25 end
    if mods.flat < -2 then mods.flat = -2 end
    ...
    if scp and scp.prot_scale then
        if mods.def > 0 then mods.def = mods.def * scp.prot_scale end
        if mods.flat < 0 then mods.flat = mods.flat * scp.prot_scale end
    end
    ...
    if scp and scp.buff_scale then
        ...
        if mods.regen_scale > SCP_BUFF_REGEN_CAP then mods.regen_scale = SCP_BUFF_REGEN_CAP end
    end
    return mods
end
...
hook.Add("SLCRound", "SCPSurfaceCheck", function(time)
    AddTimer("SLCSurfaceCheck", 3, 0, function()
        for i, v in ipairs(player.GetAll()) do
            ...
            if v:GetProperty("scp_buff_applied") or not v:GetProperty("scp_buff") or not v:IsInZone(ZONE_SURFACE) then continue end
            ...
            v:AddTimer("SCPOutsideBuff", SCP_BUFF_TICK, 0, function(this)
                ...
                if not v:IsInZone(ZONE_SURFACE) then
                ...
                if buff_heal > 0 then
                    ...
                    if to_heal > buff_heal then to_heal = buff_heal end
                    v:AddHealth(to_heal)
                ...
                if buff_regen > 0 and v:GetProperty("scp_buff_regen_time", 0) <= CurTime() then
                    ...
                    if to_regen > buff_regen then to_regen = buff_regen end
...
hook.Add("SCPUpgradeBought", "SCPOutsideBuff", function(wep, upgrade) if upgrade.name == "outside_buff" then wep:GetOwner():SetProperty("scp_buff", true, true) end end)
--[[-------------------------------------------------------------------------
Damage handling
---------------------------------------------------------------------------]]
SLC_DAMAGE_EFFECTS = {}
function AddDamageEffect(cb, ...)
    for k, v in pairs({...}) do
        if not SLC_DAMAGE_EFFECTS[v] then SLC_DAMAGE_EFFECTS[v] = {} end
        table.insert(SLC_DAMAGE_EFFECTS[v], cb)
    end
end

AddDamageEffect(function(target, info, prot) if SLCRandom(1, 100) <= (prot and 4 or 20) then target:ApplyEffect("bleeding", info:GetAttacker()) end end, DMG_BULLET, DMG_SLASH)
...
local function handle_prevent_break(target, info)
    if not PREVENT_BREAK then return end
    local cache = GetRoundProperty("prevent_break_cache")
    if not cache then cache = SetRoundProperty("prevent_break_cache", {}) end
...
local function handle_bullets(target, weapon, mult)
    ...
    --print( "[VEST]", vest )
    if not pen then return mult, vest end
    local new_mult
    if pen <= 100 then
...
local function handle_vest(target, info)
    ...
    for k, v in pairs(data.damage) do
        if not info:IsDamageType(k) then continue end
        local vest_mult = 1
...
function handle_scps(target, info, attacker)
    if target:SCPTeam() ~= TEAM_SCP or not info:IsDamageType(DMG_BULLET) then return end
    ...
    if scale >= 0 then info:ScaleDamage(scale) end
end
...
function GM:PlayerTakeDamage(target, info)
    if not target:IsPlayer() or info:IsDamageType(DMG_DIRECT) then return end
    local attacker = info:GetAttacker()
    if hook.Run("PlayerShouldTakeDamage", target, attacker) == false then --TODO test if called by default?
        return true
    end
    ...
    if IsValid(attacker) then
        ...
        if attacker:IsPlayer() then
            ...
            if target ~= attacker then
                ...
                if SCPTeams.IsAlly(attacker:SCPTeam(), target:SCPTeam()) then AddRoundStat("rdmdmg", dmg_orig) end
            end
        end
    end
    ...
    while dmg_flag ~= 0 do
        ...
        if SLC_DAMAGE_EFFECTS[b] then
            local p = not not (prot and prot[b])
...
function GM:EntityTakeDamage(target, info)
    ...
    local dmg_orig = info:GetDamage()
    if hook.Run("PlayerTakeDamage", target, info) == true then return result end
    if target:IsPlayer() then
        local dmg_type = info:GetDamageType()
        local dmg = info:GetDamage()
        local hp = target:Health()
        local attacker = info:GetAttacker()
        target.Logger:DamageTaken(dmg, attacker, {
            dmg_type = dmg_type,
            dmg_orig = dmg_orig,
        ...
        if IsValid(attacker) and attacker:IsPlayer() then
            attacker.Logger:DamageDealt(dmg_orig, target, {
                dmg_type = dmg_type,
                dmg_final = dmg,
                hp = hp
            })
    ...
    if prevent == true and (direct == true or not info:IsDamageType(DMG_DIRECT)) then return true end
    local dmgtype = info:GetDamageType()
    if bit.band(dmgtype, DMG_POISON) == DMG_POISON then info:SetDamageType(bit.bor(bit.band(dmgtype, INVERSE_POISON), DMG_NERVEGAS)) end
end

function GM:SLCPostScaleDamage(target, info)
    if not IsValid(target) or not target:IsPlayer() then return end
    local att = info:GetAttacker()
    if target == att or not IsValid(att) or not att:IsPlayer() or SCPTeams.IsAlly(att:SCPTeam(), target:SCPTeam()) then return end
    if target:SCPTeam() == TEAM_SCP and target:GetProperty("scp_buff") and target:IsInZone(ZONE_SURFACE) then
        if info:IsDamageType(DMG_DIRECT) or not info:IsDamageType(DMG_BULLET) then return end
...
function ApplyDamageHUDEvents(target, dmg)
    ...
    if IsValid(att) then
        ...
        if IsValid(owner) then att = owner end
    else
        att = dmg:GetAttacker()
    end

    if IsValid(att) and att ~= target and dmg:IsBulletDamage() then
        if (not target_valid or not target.slc_dmg_ind[att]) and att:IsPlayer() then
            net.Start("SLCHitMarker")
            net.Send(att)
        end

        if target_valid then target.slc_dmg_ind[att] = (target.slc_dmg_ind[att] or 0) + dmg:GetDamage() end
