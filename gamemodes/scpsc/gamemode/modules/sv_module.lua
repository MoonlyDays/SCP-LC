...
function GM:SCPDamage(ply, ent, dmg)
    ...
    if not ent.SCPBreakable and ent:GetClass() ~= "func_breakable" then return end
...
function GM:PlayerPostThink(ply)
    ...
    for k, v in pairs(ply:GetWeapons()) do
        if IsValid(v) and v ~= active then if v.EnableHolsterThink and v.HolsterThink then v:HolsterThink() end end
    end
...
function GetActivePlayers()
    ...
    for i, v in ipairs(player.GetAll()) do
        if v:IsActive() and not v:IsAFK() then table.insert(tab, v) end
    end
    return tab
end
...
hook.Add("PlayerPostThink", "SLCBlink", function(ply)
    ...
    if ply:GetBlink() or nb == -1 or nb > ct or not SCPTeams.HasInfo(ply:SCPTeam(), SCPTeams.INFO_HUMAN) then return end
...
Timer("PlayerBlink", blinkdelay, 0, function(self, n)
    ...
    timer.Create("PlayerUnBlink", 0.3, 1, function()
        for k, v in pairs(plys) do
            if IsValid(v) then v:SetBlink(false) end
        end
    end)
end)
    ...
    if IsValid(radio) and radio:GetEnabled() then
        ...
        for i, v in ipairs(player.GetAll()) do
            if v:SCPTeam() ~= TEAM_SCP and v:SCPTeam() ~= TEAM_SPEC and v ~= ply then
                ...
                if IsValid(r) and r:GetEnabled() and r:GetChannel() == ch then table.insert(transmit, v) end
            end
        end
    end
...
function LocateEntity(class)
    for i, v in ipairs(player.GetAll()) do
        ...
        if IsValid(wep) then return wep, v end
    end
    ...
    for i, v in ipairs(ents.GetAll()) do
        ...
        elseif v:InstanceOf("Lootable") then
            for _, item in pairs(v.LootItems) do
                if item.info.class == class then return true, v end
            end
        end
    end
end

hook.Add("PostGamemodeLoaded", "SCPLCLightStyle", function() timer.Simple(0, function() engine.LightStyle(0, "k") end) end)
