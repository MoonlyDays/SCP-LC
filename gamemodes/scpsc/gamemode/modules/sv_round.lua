...
function SetupSupportTimer()
	...
	if override then
		...
		time = tonumber(time)
		max = tonumber(max)
		if max then time = SLCRandom(time, max) end
	end

	AddTimer("SupportTimer", time, 1, function(self, n)
		if not SpawnSupport() then
			self:Change(10, 0)
			self:Start()
		else
			self:Destroy()
...
function FinishRoundInternal(winner, endcheck)
	...
	if winner == nil then
		...
		if winner == nil then winner = false end
	end
	...
	DestroyTimers()
	if IsValid(endckeck) then endcheck:Destroy() end
...
function CheckRoundEnd()
	if not ROUND.active or ROUND.post or ROUND.freeze then return end
	if ROUND.roundtype:endcheck() then InterruptRound() end
end

local abouttostart = false
function CheckRoundStart()
	if not ROUND.active and #GetActivePlayers() >= CVAR.slc_min_players:GetInt() then
		if not abouttostart then
			abouttostart = true
...
cvars.AddChangeCallback(CVAR.slc_min_players:GetName(), function() CheckRoundStart() end, "CheckRoundStart")
--[[-------------------------------------------------------------------------
GM hooks
---------------------------------------------------------------------------]]
...
function GM:SLCPostround(winner)
	...
	if winner ~= true then
		...
		if mvp then sb:append(";mvp$", EscapeMessage(mvp:Nick()), ",", points) end
	...
	elseif istable(winner) then
		...
		if specialinfo and specialinfo ~= "" then msg = msg .. specialinfo end
		CenterMessage(msg)
	else
