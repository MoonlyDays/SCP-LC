...
concommand.Add("slc_hud_scale", function(ply, cmd, args)
    local size = args[1]
    if not size or size == "" or not HUD_SCALES[size] then
    ...
    for k, v in pairs(HUD_SCALES) do
        if string.find(k, args) then table.insert(tbl, cmd .. " " .. k) end
    end
    return tbl
end)
...
hook.Add("SLCRegisterSettings", "SLCHUDSettings", function()
    ...
    table.sort(tab, function(a, b) return HUD_SCALES[a] < HUD_SCALES[b] end)
...
function GM:HUDShouldDraw(name)
    ...
    if name == "CHudCrosshair" and (not GetSettingsValue("hud_draw_crosshair") or not GetSettingsValue("hud_hl2_crosshair")) then return false end
    return true
end
...
function GM:HUDPaint()
    ...
    if not scaled_fonts_built then
    ...
    for i, v in rpairs(SCPMarkers) do
        ...
        if v.time < CurTime() then table.remove(SCPMarkers, i) end
    end

    --[[-------------------------------------------------------------------------
	ESCAPE_TIMER
	---------------------------------------------------------------------------]]
    if not ROUND.post then
    ...
    if ply:SCPTeam() == TEAM_SPEC and not admin_mode then
	...
	Pickup hint
	---------------------------------------------------------------------------]]
    if HUDPickupHint and not IsValid(HUDPickupHint) then HUDPickupHint = nil end
    if HUDPickupHint then
        ...
        if tw < th then tw = th end
        if not HUDPickupHintMsg then
	...
	Crosshair
	---------------------------------------------------------------------------]]
    if not GetSettingsValue("hud_draw_crosshair") then return true end
    if GetSettingsValue("hud_hl2_crosshair") then return end
    local wep = ply:GetActiveWeapon()
    if admin_mode or IsValid(wep) and (not wep:IsScripted() or wep.DrawCrosshair == true and (not wep.DoDrawCrosshair or wep:DoDrawCrosshair() ~= true)) then DrawSLCCrossHair(w / 2, h / 2) end
end
...
function GM:DrawOverlay()
    local w, h = ScrW(), ScrH()
    local ply = LocalPlayer()
    if not ply.FullyLoaded then return end
    --[[-------------------------------------------------------------------------
	XP Bar
	---------------------------------------------------------------------------]]
    if ROUND.post then
        if HUDXPSummary then
            ...
            elseif not show_text and next_anim - rt <= fade_dur then
                local tab = HUDXPSummary[1]
                if tab then
                    show_text = (LANG.XP_BAR[tab[2]] or tab[2]) .. ": " .. tab[1] .. " XP"
            ...
            if show_text and fade_time then text_alpha = 1 - (fade_time - rt) / fade_dur end
            local bar_w = w * 0.325
            local bar_h = h * 0.03
            ...
            local bar_out = {
                ...
                {
                    x = bar_x + bar_w + xoffset + 2 + r2,
                    y = bar_y + bar_h + 2
                },
                {
                    x = bar_x + xoffset - 2 + r2,
                    y = bar_y + bar_h + 2
                },
            }
...
function GM:SLCShouldDrawSCPHUD()
    return not hud_disabled and not HUDDrawingInfo and not ROUND.post --[[and !ROUND.preparing]]
end
...
function draw_escort_zones()
    ...
    local t = ply:SCPTeam()
    local teams = {}
    if SCPTeams.CanEscort(t, true) then teams[t] = true end
    ...
    for k, v in pairs(teams) do
        ...
        if alpha2 < alpha then surface.SetAlphaMultiplier(alpha2) end
...
hook.Add("SLCSettingsLoaded", "SLCEscortZones", function()
    if not GetSettingsValue("hud_escort") then return end
    hook.Add("PostDrawOpaqueRenderables", "SLCEscortZones", draw_escort_zones)
end)
