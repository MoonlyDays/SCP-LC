...
function RegisterPlayerStatus(name, default, set, reset, transmit)
    ...
    assert(not transmit or t == "number" or t == "boolean", "Failed to register PlayerStatus! Transmited PlayerStatus only supports numbers and booleans!")
    PlayerStatus[name] = {
        def = default,
        set = set,
        reset = reset,
        transmit = transmit and t,
        db = db_key
    }
end
...
function PlayerData:Create(ply)
    ...
    for k, v in pairs(PlayerStatus) do
        data.Status[k] = v.def
        ply["Set" .. k] = function(p, value) data:SetStatus(k, value, v.db) end
        ply["Get" .. k] = function(p) return data:GetStatus(k) end
        if SERVER and v.db then ply:GetSCPData(v.db, v.def):Then(function(db_val) data:SetStatus(k, tonumber(db_val) or db_val) end) end
    end
...
function PlayerData:AddStat(name, value, replace)
    ...
    if stat then
        ...
        if has_flag(stat, PlayerStats.STAT_ARCHIVE) then self.Player:SetSCPData(name, globalstat) end
    end
end
...
function PlayerData:Reset(roundend)
    ...
    for k, v in pairs(PlayerStatus) do
        ...
        if v.reset == false then
            reset = false
        elseif v.reset == true and not roundend then
            reset = false
        elseif isfunction(v.reset) then
            reset = not v.reset(self.Player, self.Status[k], v.def)
        end
        ...
        if reset then
            ...
            if v.transmit then table.insert(to_transmit, k) end
        end
    end
...
function PlayerData:SetStatus(name, value, db)
    ...
    if self.Status[name] ~= value then
        if not obj.set or obj.set(self.Player, self.Status[name], value) ~= true then
            self.Status[name] = value
            if db then self.Player:SetSCPData(db, value) end
