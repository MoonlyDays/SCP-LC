...
function AddItemToLootPool(name, item_tab)
    ...
    if not pool or not pool.items then return end
    table.insert(pool.items, item_tab)
end

function GenerateLootTable(name, num)
    local result = {}
    if not SLCLootPools[name] then return result end
    local tab
    ...
    for i = 1, num do
        ...
        if tab.max and tab.max > 0 and total >= tab.max then break end
        if tab.value_limit and tab.value_limit > 0 and value >= tab.value_limit then break end
    end
    return result
end
        ...
        if status then
            if ent:CanLoot(ply) then
                ...
                if obj and cache[item] then
                    ...
                    if obj.func then result, upd, wep = obj.func(ply, obj.info.class, obj.info.data, obj.ntd) end
                    if result == nil and not upd then
                        if not IsValid(wep) and ply:GetFreeInventory() > 0 and hook.Run("SLCCanPickupWeaponClass", ply, obj.info.class) ~= false then wep = ply:Give(obj.info.class) end
                        if IsValid(wep) then
                            local data = obj.info.data
                            if data and data.amount then data.amount = data.amount - 1 end
                    ...
                    elseif upd then
                        local send = {}
                        for k, v in pairs(ent.Listeners) do
                            if ent.PlayersCache[k] and ent.PlayersCache[k][item] then table.insert(send, k) end
                        end
                ...
                elseif not ply.SearchingLoot or ply.SearchingLoot < CurTime() then
            ...
            if IsValid(t) then t:Destroy() end
...
local function chip_roll(data)
    if not data.ntd then return end
    if data.ntd.chip_chance then if SLCRandom() >= data.ntd.chip_chance then return end end
...
local function chip_post(ply, item, data, ntd)
    if not ntd.chip then return end
...
local function ammo_func(ply, class, data, ntd)
    ...
    if num >= data.amount then return true end
...
local function fuse_post(ply, item, data, ntd)
    if not ntd.rating then return end
...
AddLootPool("lcz_loose_loot", {
    ...
    items = {
        ...
        {
            class = "item_slc_flashlight",
            weight = 10,
            value = 30,
            max = 1
        },
        {
            class = "item_slc_radio",
            weight = 5,
            value = 75,
            max = 1
        },
...
AddLootPool("lcz_rare_loot", {
    ...
    items = {
        ...
        {
            class = "item_slc_nvgplus",
            weight = 2,
            max = 1
        },
        {
            class = "item_slc_thermal",
            weight = 1,
            max = 1
        },
        {
            class = "item_slc_medkit",
            weight = 5,
            max = 1
        },
        {
            class = "item_slc_morphine",
            weight = 1,
            max = 1
        },
        {
            class = "item_slc_adrenaline",
            weight = 1,
            max = 1
        },
        {
            class = "item_slc_ephedrine",
            weight = 1,
            max = 1
        },
        {
            class = "item_slc_hemostatic",
            weight = 1,
            max = 1
        },
        {
        ...
        {
            class = "loadout:pistol_low",
            weight = 2,
            max = 1
        },
        {
            class = "loadout:melee_low",
            weight = 3,
            max = 1
        },
        {
...
AddLootPool("lcz_valuable_loot", {
    ...
    items = {
        ...
        {
            class = "item_slc_antidote",
            weight = 2,
            value = 165
        },
        {
            class = "item_slc_poison",
            weight = 1,
            value = 225
        },
        {
...
AddLootPool("lcz_military_crate", {
    ...
    items = {
        ...
        {
            class = "cw_kk_ins2_nade_m84",
            weight = 3,
            max = 1
        },
        {
            class = "cw_kk_ins2_nade_m18",
            weight = 3,
            max = 1
        },
        {
            class = "cw_kk_ins2_nade_anm14",
            weight = 2,
            max = 1
        },
        {
...
AddLootPool("hcz_035", {
    ...
    items = {
        ...
        {
            class = "loadout:shotgun_mid",
            weight = 6,
            value = 450,
            max = 1
        },
        {
            class = "loadout:pistol_high",
            weight = 8,
            value = 250
        },
        {
        ...
        {
            class = "cw_kk_ins2_nade_m67",
            weight = 4,
            value = 200,
            max = 1
        },
        {
            class = "cw_kk_ins2_nade_molotov",
            weight = 3,
            value = 200,
            max = 1
        },
...
AddLootPool("toilet", {
    ...
    items = {
        {
            class = "item_slc_alpha_card1",
            weight = 1
        },
        {
            class = "item_slc_alpha_card2",
            weight = 1
        },
        {
            class = "item_slc_commander_tablet",
            weight = 1
        },
        {
            class = "weapon_slc_pc",
        ...
        },
        {
            class = "item_slc_morphine",
            weight = 1,
            post = function(ply, item, data, ntd)
                item.ExtraHealth = 999
                item.SelfInjectSpeed = 0.1
                item.InjectSpeed = 0.1
            end
        },
        {
            class = "item_slc_adrenaline",
        ...
        },
        {
            class = "item_slc_gasmask",
            weight = 3,
            post = function(ply, item, data, ntd) item:SetUpgraded(true) end
        },
        {
            class = "item_slc_snav_ultimate",
            weight = 4
        ...
        {
            class = "weapon_slc_glass_knife",
            weight = 5,
            post = function(ply, item, data, ntd)
                item.AttackDamage = 1
                item.StrongAttackDamage = 99
            end
        },
        {
            class = "item_slc_turret",
...
AddLootPool("medbay", {
    ...
    items = {
        ...
        {
            class = "item_slc_morphine_big",
            weight = 2
        },
        {
            class = "item_slc_adrenaline_big",
            weight = 2
        },
        {
            class = "item_slc_ephedrine_big",
            weight = 2
        },
        {
            class = "item_slc_hemostatic_big",
            weight = 2
        },
        {
            class = "item_slc_antidote_big",
            weight = 2
        },
    }
})
