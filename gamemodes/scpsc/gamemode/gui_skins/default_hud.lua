...
local function draw_admin_hud()
	...
	_, th = draw_admin_text("Nick: " .. target:Nick(), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("HP: " .. target:Health() .. " / " .. target:GetMaxHealth(), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("Score: " .. target:Frags(), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("Player ID: " .. target:UserID(), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("SteamID: " .. target:SteamID(), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("SteamID64: " .. target:SteamID64(), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("Level: " .. target:SCPLevel() .. "  ( " .. target:SCPExp() .. " XP )", dx, dy)
	dy = dy + th
	_, th = draw_admin_text("Active: " .. tostring(target:IsActive()), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("Premium: " .. tostring(target:IsPremium()), dx, dy)
	dy = dy + th
	_, th = draw_admin_text("Class: " .. target:SCPClass() .. "  ( " .. SCPTeams.GetName(target:SCPTeam()) .. " )", dx, dy)
	dy = dy + th
	local pc, pt = target:SCPPersona()
	_, th = draw_admin_text("Fake ID: " .. pc .. "  ( " .. SCPTeams.GetName(pt) .. " )", dx, dy)
	dy = dy + th
end
...
local function draw_spectator_hud()
	...
	if not IsValid(spectarget) then return end
...
local function draw_hud()
	...
	if has_battery then
		...
		if not dont_draw then surface.DrawTexturedRect(bat_x, bat_y, bat_w, bat_h) end
	...
	local show = boost or stamina < max_stamina or stamina_pct > 0 and not stamina_visible
	if not show and GetSettingsValue("hud_stamina_always") and ply:GetWalkSpeed() ~= ply:GetRunSpeed() then show = true end
	...
	elseif not boost and stamina >= max_stamina and stamina_pct > 0 then
	...
	if (ROUND.preparing or HUDDrawInfo or time_pct > 0 and not time_visible or GetSettingsValue("hud_timer_always")) and time_pct < 1 then
	...
	elseif not HUDDrawInfo and time_pct > 0 then
...
local function draw_all()
	...
	pre_draw_hud()
	local ply = LocalPlayer()
	if HUDSpectatorInfo and SLCAuth.HasAccess(ply, "slc spectateinfo") then draw_admin_hud() end
...
AddGUISkin("hud", "default", {
	...
	remove = function() hook.Remove("SLCDrawHUD", "HUD.Default.Paint") end,
	show = nil,
	hide = nil,
})