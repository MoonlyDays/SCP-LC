...
local function createPlayerTab(ply, pnl)
    ...
    avatar_btn.DoClick = function(self) ply:ShowProfile() end
    avatar_btn.PerformLayout = function(self, pw, ph) self:SetWide(ph) end
    ...
    name.PerformLayout = function(self, pw, ph) self:SetWide(pnl:GetWide() * 0.25) end
    ...
    mute.PerformLayout = function(self, pw, ph) self:SetWide(ph) end
    tab.Mute = mute
    ...
    ping.PerformLayout = function(self, pw, ph) self:SetWide(pnl:GetWide() * 0.075) end
    ...
    level.PerformLayout = function(self, pw, ph) self:SetWide(pnl:GetWide() * 0.1) end
    ...
    score.Paint = function(self, pw, ph)
        ...
        if IsValid(ply) and (ply == lp or not lp:Alive()) then text = math.Clamp(ply:Frags(), 0, 9999) end
        draw.Text{
            text = text,
            pos = {pw * 0.5, ph * 0.5},
            color = COLOR.white, --tab.TextColor,
    ...
    score.PerformLayout = function(self, pw, ph) self:SetWide(pnl:GetWide() * 0.1) end
    ...
    ranks.Paint = function(self, pw, ph)
        ...
        if totalw <= pw then
            ...
            for i, v in ipairs(tab.badges) do
                ...
                if not cw then cw = 8 end
                local btn = Button(px + totalw, py + 4, cw + 8, ph - 8)
                if btn > 0 then tab.crank = i end
                if tab.crank == i then
                    if btn == 0 then
                        tab.crank = 0
                    else
...
local function create_scoreboard()
    ...
    container.Paint = function(self, pw, ph)
        ...
        if diff_h > 0 then
            ...
            if scroll > 0 then self.DrawArrowUp = true end
            if scroll < diff_h then self.DrawArrowDown = true end
        end
    end

    container.Think = function(self)
        for i, v in ipairs(player.GetAll()) do
            if not IsValid(PLAYERS[v]) then
                --print( "Creating", v )
    ...
    container.Players.OnMouseWheeled = function(self, delta)
        if container.Players:GetVBar():IsVisible() then SCOREBOARD.VolumeSlider = nil end
...
function overlay_scoreboard()
    next_frame = true
    if holdingVC and not input.IsButtonDown(MOUSE_LEFT) then holdingVC = false end
    ...
    if tab and IsValid(tab.Player) then
        ...
        if holdingVC then
            cpx = mx
            if cpx < nx then cpx = nx end
            if cpx > nx + nw then cpx = nx + nw end
            vol = (cpx - nx) / nw
            tab.Player:SetVoiceVolumeScale(vol)
        end
        ...
        if holdingVC or hover then surface.SetDrawColor(225, 225, 225, 255) end
...
AddGUISkin("scoreboard", "legacy", {
    ...
    show = function()
        if not IsValid(SCOREBOARD) then create_scoreboard() end
        SCOREBOARD:Show()
    end,
    hide = function()
        if not IsValid(SCOREBOARD) then return end
