...
local function create_eq()
    if IsValid(EQ) then EQ:Remove() end
    local h = ScrH()
    ...
    panel.SizeToContents = function(this) this:SetWide(this.BasicEQ:GetWide() + this.EQ:GetWide() + eq_pad) end
    ...
    actions:Hide()
    actions.Linked = true
    actions.Think = function(this) if not IsValid(panel) then this:Remove() end end
    ...
    a_move.OnMove = function(this, nx, ny) actions.Linked = false end
    ...
    main_move.OnMove = function(this, nx, ny)
        if not actions.Linked then return end
    ...
    vest.Think = function(this)
        ...
        if not this.Tooltip and has_vest then this:RebuildDescription() end
    end
    ...
    vest.RebuildDescription = function(this)
        ...
        for k, v in pairs(data.damage) do
            ...
            vest_text:Print("    • " .. (LANG.DMG[dmg_name] or dmg_name) .. ": "):Print(math.Round(v * 100, 1) .. "%", v < 1 and Color(30, 200, 30) or Color(200, 30, 30)):Print("\n")
        end
    ...
    eq.SizeToContents = function(this) this:SetWide((this:GetTall() - eq_pad) * 0.5 * 3 + eq_pad * 4) end
    ...
    panel.UpdateActions = function(this)
        ...
        for i, v in ipairs(EQ_ACTIONS) do
            if v.can_see ~= true and (not isfunction(v.can_see) or not v.can_see()) then continue end
...
AddGUISkin("eq", "default", {
    create = function()
        create_eq()
        hook.Add("SLCLanguageChanged", "EQ.Default.Lang", function() create_eq() end)
        hook.Add("PlayerButtonDown", "DefaultEQ.ButtonDown", function(ply, btn)
            if not IsFirstTimePredicted() or EQ:IsVisible() or btn < KEY_0 or btn > KEY_9 then return end
    ...
    show = function()
        if not EQ:IsVisible() and EQ.SavedMouseX and EQ.SavedMouseY then input.SetCursorPos(EQ.SavedMouseX, EQ.SavedMouseY) end
        EQ:UpdateItems()
        EQ:UpdateActions()
        EQ:Show()
        EQ.Actions:Show()
    ...
    hide = function()
        if not EQ:IsVisible() or IsValid(EQ.BACKPACK) and not EQ.BACKPACK.Removed then return end
    ...
    close_backpack = function()
        ...
        if not input.IsButtonDown(GetBindButton("eq_button")) then HideGUIElement("eq") end
    end,
    is_visible = function() return EQ:IsVisible() end,
})
...
function PANEL:Init()
    ...
    image.OnMousePressed = function(this, code) self:OnMousePressed(code, true) end
end
...
function PANEL:Think()
    ...
    if not input.IsMouseDown(MOUSE_LEFT) then
        self.Moving = false
        if self.FinishMove then self:FinishMove() end
        return
    end
    ...
    if self.OnMove then self:OnMove(new_x, new_y) end
end
...
function PANEL:OnMousePressed(code, override)
    if not self.MovePanel or code ~= MOUSE_LEFT then return end
    local _, my = self:ScreenToLocal(0, gui.MouseY())
    if not override and self.DraggableHeight > 0 and my > self.DraggableHeight then return end
    self.Moving = true
...
function PANEL:HoveredButtonDown(code)
    if not IsFirstTimePredicted() or hook.Run("SLCCanUseBind", "eq_loadout") == false then return end
    if code < KEY_0 or code > KEY_9 or not IsValid(self.Item) then return end
    if self.Item.Selectable == false then --[[and self.Item.Toggleable == false]]
        return
    end
...
function PANEL:RebuildDescription()
    ...
    if not wep_name then wep_name = self.Item.PrintName or self.Item.GetPrintName and self.Item:GetPrintName() or LANG.eq_unknown end
    desc:Print(wep_name):Print("\n")
    if self.Item.Stacks and self.Item.Stacks > 1 then desc:Print(LANG.EQ.count .. ": "):Print(self.Item:GetCount(), Color(200, 200, 30)):Print("\n") end
...
function PANEL:Paint(w, h)
    ...
    if not is_active and self:IsHovered() and self:IsEnabled() then
...
function PANEL:Think()
    if self:IsHovered() and not dragndrop.IsDragging() and IsValid(self.Item) then
        local instant = GetSettingsValue("eq_instant_desc")
        if self.HoverTime == 0 then
            self.HoverTime = RealTime()
            self:RebuildDescription()
    ...
    UpdatePlayerEQ()
    if self.Item and not IsValid(self.Item) then
...
function PANEL:Paint(w, h)
    if not self.PaintingDragging then
        surface.SetDrawColor(0, 0, 0, 100)
        surface.DrawRect(0, 0, w, h)
    end
    ...
    if not self.PaintingDragging then surface.DrawOutlinedRect(0, 0, w, h) end
    if not self.ItemClass then
        ...
        if self.DropHovered and (not self.Backpack or self.DropHovered.ItemClass ~= "item_slc_backpack" and not is_dragged_active) then
    ...
    if (not is_active or dragndrop.IsDragging()) and not self.PaintingDragging and self:IsHovered() then
        surface.SetDrawColor(100, 100, 100, 75)
        surface.DrawRect(0, 0, w, h)
    end
    ...
    if self.Transition > ct then surface.SetAlphaMultiplier(math.TimedSinWave(1, 0, 1)) end
    ...
    elseif wep_tab.SelectFont then
        ...
        local dy = h * 0.5
        if font == "SCPCSSIcons" then dy = dy + h * 0.25 end
        draw.Text{
    ...
    if self.DropHovered and self.DropHovered ~= self and IsValid(self.DropHovered.Item) and self.Transition < ct then
        ...
        if not backpack or (self.ItemClass ~= "item_slc_backpack" and self.DropHovered.ItemClass ~= "item_slc_backpack" and not is_dragged_active and not is_active) then
            if not backpack and IsValid(self.Item) and self.Item.DragAndDrop and self.Item:DragAndDrop(self.DropHovered.Item, true) then
    ...
    if self:IsDragging() and not self.PaintingDragging then
        ...
        if hovered and hovered ~= self and hovered.ItemButton and hovered.Transition < ct then
            ...
            if not backpack or (self.ItemClass ~= "item_slc_backpack" and hovered.ItemClass ~= "item_slc_backpack" and not is_active and not is_other_active) then
                if not backpack and IsValid(hovered) and IsValid(hovered.Item) and hovered.Item.DragAndDrop and hovered.Item:DragAndDrop(self.Item, true) then
                    surface.SetMaterial(mat_combine)
                else
                    surface.SetMaterial(mat_swap)
                end
...
function PANEL:DoClick()
    if self.Backpack or not IsValid(self.Item) or self.Transition > CurTime() then return end
    input.SelectWeapon(self.Item)
end

function PANEL:DoRightClick()
    if self.Backpack or not IsValid(self.Item) or self.Item.Droppable == false or self.Transition > CurTime() then return end
    net.Start("DropWeapon")
    net.WriteString(self.ItemClass)
    net.SendToServer()
    if not self.Item.Stacks or self.Item.Stacks <= 1 or self.Item:GetCount() <= 1 then
...
function PANEL:OnDrop(tab, dropped)
    ...
    elseif self ~= dragged then
        if not backpack and self.Item.DragAndDrop and self.Item:DragAndDrop(dragged.Item) ~= false then
            net.Start("WeaponDnD")
            ...
            if backpack and (IsValid(self.Item) and self.Item.Toggleable and self.Item:GetEnabled() or dragged.Item == active_wep or IsValid(dragged.Item) and dragged.Item.Toggleable and dragged.Item:GetEnabled() or dragged.Item == active_wep) then return end
            local from = dragged.ID
            local to = self.ID
            local from_item = INVENTORY[from]
            local to_item = INVENTORY[to]
...
function PANEL:BindItem(item)
    if not IsValid(item) then
        self:SetCursor("arrow")
        if self.m_DragSlot then self.m_DragSlot = nil end
        self.Item = nil
        self.ItemClass = nil
        self.Tooltip = nil
        return
...
function PANEL:Paint(w, h)
    ...
    surface.DrawRect(0, 0, w, h)
    if self:IsHovered() and not input.IsMouseDown(MOUSE_LEFT) then
        surface.SetDrawColor(100, 100, 100, 75)
        surface.DrawRect(0, 0, w, h)
    end
...
function PANEL:DoClick()
    ...
    if isfunction(cb) then cb(self.Action, self.Action.args) end
end
...
function PANEL:Init()
    ...
    self:SizeToContents()
    if not EQ.PBX or not EQ.BPY then
...
function PANEL:Think()
    if not EQ:IsVisible() or not IsValid(LocalPlayer():GetWeapon("item_slc_backpack")) then self:Remove() end
end
