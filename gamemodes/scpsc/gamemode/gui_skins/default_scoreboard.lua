...
local header_items = {
    ...
    {
        text = "badges", --fill, docking order is reversed after this!
        size = -1,
        font = "SCPHUDMedium"
    },
    {
    ...
    },
    {
        ...
        size = 0.09,
        font = "SCPNumbersMedium",
        func = function(ply) return ply:SCPLevel() end
    },
    {
        text = "score",
        size = 0.09,
        ...
        func = function(ply)
            ...
            if IsValid(ply) and (ply == lp or not lp:Alive()) then return math.Clamp(ply:Frags(), 0, 9999) end
            return "--"
        end
    },
}
...
local function create_player(ply, parent, marg)
    ...
    avatar.OnMousePressed = function(this, code)
        ...
        elseif code == MOUSE_RIGHT then
            ...
            m:AddOption(LANG.scoreboard_actions.open_profile, function() ply:ShowProfile() end)
            m:Open()
        end
    end
    ...
    for i, v in ipairs(header_items) do
        ...
        elem.Think = function(this)
            if not v.func or not IsValid(ply) then return end
...
local function create_scoreboard()
    ...
    players.OnMouseWheeled = function(this, delta)
        if IsValid(panel.VolumeSlider) then panel.VolumeSlider:Remove() end
...
AddGUISkin("scoreboard", "default", {
    create = function()
        create_scoreboard()
        hook.Add("SLCLanguageChanged", "Scoreboard.Default.Lang", function() create_scoreboard() end)
    end,
    remove = function()
        SCOREBOARD:Remove()
    ...
    hide = function() SCOREBOARD:Hide() end,
})
...
function PANEL:OnMousePressed(code)
    if not self.Collapsed or code ~= MOUSE_LEFT then return end
...
function PANEL:Paint(w, h)
    ...
    if not self.Collapsed or self.Expand == 1 then draw.SimpleText(self.Text, "SCPHUDSmall", w * 0.5, h * 0.5 - 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER) end
end
...
function PANEL:Init()
    ...
    self.Slider.Paint = function(this, pw, ph) draw.RoundedBox(ph * 0.5, 0, ph * 0.45, pw, ph * 0.1, color_white) end
...
function PANEL:Think()
    if not IsValid(SCOREBOARD) or not IsValid(self.Panel) or not SCOREBOARD:IsVisible() then self:Remove() end
end
