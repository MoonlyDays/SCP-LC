...
local function spec_hud()
	...
	local sh = h
	local sw = w
	if drawtarget then surface.DrawRect(w * 0.4 - 2, h * 0.06 + 8, w * 0.2 + 4, h * 0.05 + 4) end
	if showtime then surface.DrawRect(w * 0.45 - 2, 8, w * 0.1 + 4, h * 0.05 + 4) end
	...
	if drawtarget then surface.DrawRect(w * 0.4, h * 0.06 + 10, w * 0.2, h * 0.05) end
	if showtime then surface.DrawRect(w * 0.45, 10, w * 0.1, h * 0.05) end
...
local function hud()
	...
	local bar = SimpleMatrix(2, 4, {{start + w * 0.01, h * 0.81 + addy}, {start + w * 0.275, h * 0.81 + addy}, {start + w * 0.275 + xoffset, h * 0.845 + addy}, {start + w * 0.01 + xoffset, h * 0.845 + addy},})
	local bar_out = SimpleMatrix(2, 4, {{start + w * 0.01 - 4, h * 0.81 - 2 + addy}, {start + w * 0.275 + 2, h * 0.81 - 2 + addy}, {start + w * 0.275 + xoffset + 4, h * 0.845 + 2 + addy}, {start + w * 0.01 + xoffset - 2, h * 0.845 + 2 + addy},})
	local bar_offset = SimpleMatrix(2, 4, {{xoffset + cxo, h * 0.05}, {xoffset + cxo, h * 0.05}, {xoffset + cxo, h * 0.05}, {xoffset + cxo, h * 0.05},})
	local xico = ratio * (h * 0.025)
	local ico = SimpleMatrix(2, 4, {{start + w * 0.015, h * 0.815 + addy}, {start + w * 0.025, h * 0.815 + addy}, {start + w * 0.025 + xico, h * 0.84 + addy}, {start + w * 0.015 + xico, h * 0.84 + addy},})
	local ico_offset = SimpleMatrix(2, 4, {{w * 0.013, 0}, {w * 0.013, 0}, {w * 0.013, 0}, {w * 0.013, 0},})
	draw.NoTexture()
	--BLINK
	surface.SetDrawColor(COLOR.gray_bg)
	...
	if HUDBlink > 0 and cur > 0 then
		...
		surface.DrawPoly{
			...
			{
				x = start + w * 0.015 + width,
				y = h * 0.815 + addy
			},
			{
				x = start + w * 0.015 + width + xico,
				y = h * 0.84 + addy
			},
			{
				x = start + w * 0.015 + xico,
				y = h * 0.84 + addy
			},
		}
	end
	...
	if intense > 1 then intense = 1 end
	if extra then
...
local function a_hud()
	...
	surface.DrawPoly{
		...
		{
			x = start + w * 0.55 + 2 + h * 0.03,
			y = h * 0.75 - 2 + addy
		},
		{
			x = start + w * 0.625 + 3 + h * 0.03,
			y = h * 0.99 + 2 + addy
		},
		{
			x = start + w * 0.36 - 2,
	...
	surface.DrawPoly{
		...
		{
			x = start + w * 0.55 + h * 0.03,
			y = h * 0.75 + addy
		},
		{
			x = start + w * 0.625 + h * 0.03,
			y = h * 0.99 + addy
		},
		{
			x = start + w * 0.36,
	...
	local bar = SimpleMatrix(2, 4, {{start, h * 0.81 + addy}, {start + w * 0.265, h * 0.81 + addy}, {start + w * 0.265 + xoffset, h * 0.845 + addy}, {start + xoffset, h * 0.845 + addy},})
	local bar_out = SimpleMatrix(2, 4, {{start - 4, h * 0.81 - 2 + addy}, {start + w * 0.265 + 2, h * 0.81 - 2 + addy}, {start + w * 0.265 + xoffset + 4, h * 0.845 + 2 + addy}, {start + xoffset - 2, h * 0.845 + 2 + addy},})
	local ico = SimpleMatrix(2, 4, {{start + ixo + w * 0.004, h * 0.815 + addy}, {start + ixo + w * 0.014, h * 0.815 + addy}, {start + ixo + w * 0.014 + xico, h * 0.84 + addy}, {start + ixo + w * 0.004 + xico, h * 0.84 + addy},})
	--SANITY
	draw.NoTexture()
	surface.SetDrawColor(COLOR.gray_bg)
	surface.DrawDifference(bar:ToPoly(), bar_out:ToPoly())
...
AddGUISkin("hud", "legacy", {
	create = function() hook.Add("SLCDrawHUD", "HUD.Legacy.Paint", draw_all) end,
	remove = function() hook.Remove("SLCDrawHUD", "HUD.Legacy.Paint") end,
	show = nil,
	hide = nil,
})