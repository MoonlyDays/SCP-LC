...
function gamerule.Register(name, data)
    if not name or not data or data.value == nil then return end
    local t = data.type
    if not t then t = type(data.value) end
...
function gamerule.Set(name, value, silent)
    ...
    if data then
        ...
        if data.value ~= value then
            data.value = value
            gamerule.Save()
            if not data.protected and not silent then gamerule.SendAll(nil, name) end
        end
    end
end
...
function gamerule.SendAll(ply, list)
    if not list then
        ...
        for k, v in pairs(gamerule.data) do
            if not v.protected then table.insert(list, k) end
        end
    ...
    for k, v in pairs(list) do
        if gamerule.data[v] and not gamerule.data[v].protected then
            tab[v] = gamerule.data[v].value
            any = true
        end
    end

    if any then net.SendTable("SLCGameruleData", tab, ply) end
end
...
if SERVER then
    hook.Add("SLCGamemodeLoaded", "SLCGamerule", function()
        ...
        if not file.Exists("slc/gamerule.dat", "DATA") then
            file.Write("slc/gamerule.dat", util.TableToJSON(gamerule.data))
        else
            ...
            if not data then
                ...
                if f then file.Write("slc/gamerule_corrupted.dat", f) end
                file.Write("slc/gamerule.dat", util.TableToJSON(gamerule.data))
                return
            end
            ...
            for k, v in pairs(data) do
                ...
                if gamerule.data[k] and gamerule.data[k].registered then wasRegistered = true end
                gamerule.data[k] = v
                if v.registered and not wasRegistered then gamerule.data[k].registered = false end
            end
...
concommand.Add("slc_gamerule", function(ply, cmd, args)
    if not args[1] or args[1] == "" or args[1] == "help" or args[1] == "?" then
        --print help
        return
    end

    if not actions[args[1]] then
    ...
    if SERVER and (not IsValid(ply) or ply:IsListenServerHost()) then
        if args[1] == "get" then
            printRules(args[2])
        ...
        elseif args[1] == "remove" then
            ...
            if args[2] == "*" then
                for k, v in pairs(gamerule.data) do
                    if not v.registered then table.insert(tab, k) end
                end
            else
                tab = {args[2]}
            end
    ...
    if space then
        ...
        for k, v in pairs(gamerule.data) do
            if args == "" or string.find(string.lower(k), arg, 1, true) then table.insert(tab, cmdstr .. k) end
        end
        return tab
    else
        local tab = {}
        for k, v in pairs(actions) do
            if args == "" or string.find(string.lower(k), args, 1, true) then table.insert(tab, "slc_gamerule " .. k) end
        end
        return tab
    end
end, "")
