local heuristic_functions = {
    euclidean = function(a, b) return a:Distance(b) end,
    euclidean2d = function(a, b) return a:Distance2D(b) end,
...
function SLCAStar(start, goal, h, test, max)
    if isvector(start) then start = navmesh.GetNearestNavArea(start) end
    if isvector(goal) then goal = navmesh.GetNearestNavArea(goal) end
    if not start or not goal then return end
    if isstring(h) then h = heuristic_functions[h] end
    if not h then h = heuristic_functions.euclidean end
    if not isfunction(h) then return end
    ...
    while not start:IsOpenListEmpty() do
        iters = iters + 1
        if max and iters > max then return false end
        local current = start:PopOpenList()
        if current == goal then return reconstruct_path(path, current) end
        current:AddToClosedList()
        for i, v in ipairs(current:GetAdjacentAreas()) do
            if test and not test(current, v) then continue end
            local current_cost = current:GetCostSoFar() + get_cost(current, v)
            if current_cost < v:GetCostSoFar() or not v:IsOpen() and not v:IsClosed() then
...
function SLCCalculatePath(start, goal, h, test, max, bounds, offset, mask)
    ...
    if not istable(navs) then return navs end
    local len = #navs
    local path = {}
    ...
    for i = 2, len + 1 do
        ...
        util.TraceHull(trace_tab)
        if not trace_tab.Hit then path[i] = lerp end
    end

    if DEVELOPER_MODE and not DEV_NO_PATH then SLCDrawPath(path) end
    DEV_NO_PATH = false
    return path
end
