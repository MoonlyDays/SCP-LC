...
function SLCPromise:New(func)
    ...
    if func then func(function(data) tab:Resolve(data) end, function(err) tab:Reject(err) end) end
    return tab
end
            ...
            if success then
                ...
                if pcall_status then
                    if type(then_return) == "table" and then_return._ISPROMISE then
                        then_return:Then(function(new_data) ret:Resolve(new_data) end, function(err) ret:Reject(err) end)
                    else
...
function SLCPromise:Finally(func)
    local function fn(val)
        local ret = func(val)
        if not istable(ret) or not ret._ISPROMISE then return val end
        return SLCPromise(function(resolve, reject) ret:Then(function() resolve(val) end, function() reject(val) end) end)
    end
...
setmetatable(SLCPromise, {
    __call = SLCPromise.New
})
...
function SLCPromiseJoin(...)
    local tab = {...}
    if #tab == 1 and istable(tab[1]) and not tab[1]._ISPROMISE then tab = tab[1] end
    ...
    for i, v in pairs(tab) do
        if ispromise(v) then
            v:Then(function(data)
                ret[i] = data
                num = num - 1
                if num <= 0 then p:Resolve(ret) end
            end, function(err)
                ret[i] = err
                num = num - 1
                if num <= 0 then p:Resolve(ret) end
...
function SLCPromiseAny(...)
    local p = SLCPromise()
    for i, v in pairs({...}) do
        v:Then(function(data) p:Resolve(data) end)
    end
    return p
end
...
function SLCToPromise(obj)
    if ispromise(obj) then
        ...
        return SLCPromise(function(resolve) resolve(obj) end)
    end
end
