...
function AddWeaponToGroup(class, ...)
    if not SLC_WEAPON_GROUP_OVERRIDE[class] then SLC_WEAPON_GROUP_OVERRIDE[class] = {} end
...
function AddGroupToWeapon(group, ...)
    for i, v in ipairs({...}) do
        if not SLC_WEAPON_GROUP_OVERRIDE[v] then SLC_WEAPON_GROUP_OVERRIDE[v] = {} end
...
local function weapon_value(class, key, wep)
    if wep then return wep[key] end
    local done = {}
    repeat
...
function IsWeaponInGroup(class, group, wep)
    local override = SLC_WEAPON_GROUP_OVERRIDE[class]
    if override then return not not override[group] end
    local wep_group = weapon_value(class, "Group", wep)
    if not wep_group then return false end
...
function TestWeaponGroup(class, other, wep)
    ...
    for group in pairs(wep_group) do
        if not other_group[group] then continue end
        local limit = SLC_WEAPON_GROUP_LIMITS[group] or 1
        if limit <= 1 then return true, 1 end
        match = true
        if limit < lowest then lowest = limit end
    end

    if match then return true, lowest end
    return false
end
...
function WEAPON:SwingThink()
    ...
    if id > data.last_id then
        ...
        if data.last_id == 1 and data.on_start then data.on_start() end
        local owner = self:GetOwner()
        ...
        if data.correct_mins or data.correct_maxs then OrderVectors(trace_tbl.mins, trace_tbl.maxs) end
        ...
        if trace_tbl.Hit then
            ...
            if stop == true then
                if istable(center) then
                    ...
                    util.TraceHull(trace_tbl)
                    owner:LagCompensation(false)
                    if trace_tbl.Hit then data.callback(trace_tbl, 0, true) end
                end

                if data.cb_end then data.cb_end(trace_tbl) end
    ...
    if id > data.last_id then self:SwingThink() end
end
...
function WEAPON:InstallCustomDataTable()
    ...
    self.Data = setmetatable({}, {
        __index = function(_, key) return data.data[key] end,
...
net.Receive("SLCDataTable", function()
    ...
    if not IsValid(ent) or not ent._CutomDataTable then return end
...
hook.Add("PreRegisterSWEP", "SLCCustomDataTable", function(swep, class)
    if not swep.UseCustomDT then return end
    ...
    swep.OwnerChanged = function(this)
        if owner_changed then owner_changed(this) end
        local owner = this:GetOwner()
        if not IsValid(owner) then return end
