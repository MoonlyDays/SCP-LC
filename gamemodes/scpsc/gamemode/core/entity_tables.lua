...
function InstallTable(name, target)
    ...
    if not isentity(target) and not istable(target) then
        print("Entity or table expected in InstallTable!")
        return
    end

    if tab then
        if tab.Setup then tab.Setup(target) end
        table.Merge(target, tab.Table)
        if tab.Finish then tab.Finish(target) end
...
function InstanceOf(name, tab)
    if tab.InstalledTables and tab.InstalledTables[name] then return true end
    return false
end
...
RegisterEntityTable("ActionQueue", {
    Table = {
        ...
        ActionQueueInit = function(self) self.ActionQueue = {} end,
        ActionQueueThink = function(self)
            ...
            if a_finish ~= 0 and a_finish < ct then
                ...
                if #self.ActionQueue > 0 then
                    ...
                    if new.cb then new.cb(a_finish, new.duration, prev, new.state) end
                else
                    self:SetState(0)
        ...
        QueueAction = function(self, state, duration, cb)
            ...
            if self:GetActionFinish() == 0 then self:SetActionFinish(1) end
        end,
...
RegisterEntityTable("Lootable", {
    ...
    Table = {
        OpenInventory = function(self, ply)
            ...
            if ply:IsPlayer() and not self.Listeners[ply] and self:CanLoot(ply) then
                if IsValid(ply.ActiveLootable) then ply.ActiveLootable.Listeners[ply] = nil end
        ...
        CheckListeners = function(self)
            ...
            for k, v in pairs(self.Listeners) do
                if not self:CanLoot(k) then
                    self.Listeners[k] = nil
                    if IsValid(k) then table.insert(to_update, k) end
                end
            end
        ...
        CanLoot = function(self, ply) return IsValid(ply) and ply:IsHuman() and self:GetPos():DistToSqr(ply:GetPos()) <= 6400 and hook.Run("SLCPlayerCanLoot", ply, self) ~= false end,
        DropAllListeners = function(self)
            local listeners = {}
            for k, v in pairs(self.Listeners) do
                self.Listeners[k] = nil
                if IsValid(k) then table.insert(listeners, k) end
            end
