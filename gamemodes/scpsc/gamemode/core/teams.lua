...
function SCPTeams.AddTeamInfo(name)
    ...
    if SCPTeams[key] then return SCPTeams[key] end
    if info_num > 31 then
...
function SCPTeams.AddInfo(team, info)
    ...
    if t then t.info = bit.bor(t.info, info) end
end

function SCPTeams.RemoveInfo(team, info)
    local t = SCPTeams.REG[team]
    if t then t.info = bit.band(t.info, bit.bnot(info)) end
end

function SCPTeams.SetupAllies(tab, ally)
    if not istable(tab) then tab = {tab} end
    if not ally then
        ally = tab
    elseif ally == true then
        ally = SCPTeams.ALL
    elseif not istable(ally) then
        ally = {ally}
    end
    ...
    for k, v in pairs(tab) do
        ...
        for _k, _v in pairs(ally) do
            if v ~= _v then t.relations[_v] = true end
        end
    end
end
...
function SCPTeams.SetupNeutral(tab, neutral)
    ...
    elseif not istable(neutral) then
        neutral = {neutral}
    end
    ...
    for k, v in pairs(tab) do
        ...
        for _k, _v in pairs(neutral) do
            if v ~= _v then t.relations[_v] = false end
        end
    end
end
...
function SCPTeams.SetupEnemy(tab, enemy)
    ...
    for k, v in pairs(tab) do
        ...
        for _k, _v in pairs(enemy) do
            if v ~= _v then t.relations[_v] = nil end
        end
    end
end
...
function SCPTeams.IsEnemy(team1, team2)
    ...
    if t then return t.relations[team2] == nil end
    return true
end
...
function SCPTeams.IsAlly(team1, team2)
    ...
    local t = SCPTeams.REG[team1]
    if t then return t.relations[team2] == true end
    return false
end
...
function SCPTeams.IsNeutral(team1, team2)
    if team1 == team2 then return true end
    local t = SCPTeams.REG[team1]
    if t then return t.relations[team2] == false end
    return false
end
...
function SCPTeams.GetAllies(team, include_self)
    ...
    if t then
        ...
        for k, v in pairs(t.relations) do
            if v == true then table.insert(allies, k) end
        end

        if include_self then table.insert(allies, team) end
        return allies
    end
end

function SCPTeams.CanEscort(team1, team2)
    if not SCPTeams.REG[team1] then return false end
    if not next(SCPTeams.REG[team1].escort) then return false end
    if team2 == true then return true end
    return SCPTeams.REG[team1].escort[team2]
end

function SCPTeams.CanBeEscorted(team1, team2)
    if not SCPTeams.REG[team1] then return false end
    if not next(SCPTeams.REG[team1].escorted_by) then return false end
    if team2 == true then return true end
    if not SCPTeams.REG[team2] then return false end
    return SCPTeams.REG[team2].escort[team1]
end

function SCPTeams.GetEscort(team)
    if not SCPTeams.REG[team] then return {} end
    if not next(SCPTeams.REG[team].escort) then return {} end
    local tab = {}
...
function SCPTeams.GetEscortedBy(team)
    if not SCPTeams.REG[team] then return {} end
    if not next(SCPTeams.REG[team].escorted_by) then return {} end
    local tab = {}
...
function SCPTeams.CanEscape(team)
    if not SCPTeams.REG[team] then return false end
...
function SCPTeams.GetScore(team, score)
    if not SCPTeams.SCORE[team] then SCPTeams.SCORE[team] = 0 end
...
function SCPTeams.GetPlayersByTeam(team)
    ...
    for i, v in ipairs(player.GetAll()) do
        if v:SCPTeam() == team then table.insert(plys, v) end
    end
    return plys
end
...
function SCPTeams.GetPlayersByInfo(info, alive)
    local plys = {}
    for i, v in ipairs(player.GetAll()) do
        if SCPTeams.HasInfo(v:SCPTeam(), info) and (not alive or v:Alive()) then table.insert(plys, v) end
    end
    return plys
end
...
function PLAYER:SCPTeam()
    if not self.Get_SCPTeam then self:DataTables() end
    return self:Get_SCPTeam()
end

function PLAYER:SetSCPTeam(team)
    if not self.Set_SCPTeam then self:DataTables() end
