...
function SLCInventory:New(name, size_x, size_y, ent)
    ...
    if slot < s * 0.055 then tab.FontOverride = "SCPHUDESmall" end
    tab.SlotDim = slot
    tab.Margin = margin
    ...
    tab.SizeY = size_y
    local total_w = cw
    if tab.IsRagdoll then total_w = total_w - margin - tab.InfoW end
    ...
    if old and IsValid(old) then old:Remove() end
...
function SLCInventory:Draw()
    ...
    if self.MouseDown and (mx < x or mx > x + w or my < y or my > y + h) then self:OnClick(-1, -1, -1) end
    if self.IsRagdoll then self:DrawInfo() end
    --[[
...
function SLCInventory:DrawBackground(s, pass)
    ...
    if pass == 1 then
        ...
        if isp then surface.DrawRect(x - info_w - marg - 1, y - 1, info_w + 2, h + 2) end
    else
        surface.SetDrawColor(0, 0, 0, 150)
        surface.DrawRect(x, y, w, h)
        if isp then surface.DrawRect(x - info_w - marg, y, info_w, h) end
    end
end
...
function SLCInventory:DrawSlot(i, j, s)
    ...
    if item then
        ...
        if (not input.IsMouseDown(MOUSE_LEFT) or item.disabled) and in_slot then
            surface.SetDrawColor(75, 75, 75, 150)
            surface.DrawRect(x, y, slot, slot)
        end

        local name_h = slot * 0.2
        if item.icon and item.icon ~= default_icon or not item.select_font then
        ...
        if not item.disabled and in_slot and self.MouseDown then self:OnClick(x, y, num) end
    end
...
function SLCInventory:Remove()
    ...
    if self.Visible then gui.EnableScreenClicker(false) end
end
...
setmetatable(SLCInventory, {
    __call = SLCInventory.New
})

function IsAnyInventoryVisible()
    for k, v in pairs(_SLCInventoryCache) do
        if IsValid(v) and v:IsVisible() then return true end
    end
    return false
end
...
hook.Add("DrawOverlay", "SLCInventory", function()
    ...
    for k, v in pairs(_SLCInventoryCache) do
        if IsValid(v) and v:IsVisible() then v:Draw() end
    end
end)
...
hook.Add("PlayerButtonDown", "SLCInventory", function(ply, btn)
    if btn >= KEY_FIRST and btn <= KEY_LAST and btn ~= GetBindButton("eq_button") and GetSettingsValue("any_button_close_search") then
        for k, v in pairs(_SLCInventoryCache) do
            if IsValid(v) and v:IsVisible() and v.Drawn then v:OnClick(-1, -1, -1) end
        end
    end
end)

hook.Add("SLCRegisterSettings", "SLCSearching", function() RegisterSettingsEntry("any_button_close_search", "switch", true) end)
