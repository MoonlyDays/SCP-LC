...
function ToRoman(num)
    if roman_cache[num] then return roman_cache[num] end
    local result = ""
    for i = 13, 1, -1 do
...
function List:PushFront(item)
    ...
    if _head then _head.prev = self.head end
    return self.size
end
...
function List:PushBack(item)
    ...
    if _tail then _tail.next = self.tail end
    return self.size
end
...
function FindInCylinder(orig, radius, zmin, zmax, filter, mask, feed)
    ...
    for i, v in ipairs(feed or ents.GetAll()) do
        if filter and not filter[v:GetClass()] then continue end
