...
function GetSCPData(steamid, name, def)
    return SLCDatabase:FormatQuery("SELECT value FROM scplc_playerdata_v2 WHERE sid = ? AND datakey = ?;", steamid, name):Then(function(data)
        if not data or not data[1] then return def end
...
function SCPDataNumericAdd(steamid, name, num)
    return GetSCPData(steamid, name):Then(function(res) return SetSCPData(steamid, name, (tonumber(res or 0) or 0) + num) end)
end
...
function SLCInitializeMySQL()
    ...
    print("Loading MySQL...")
    require("mysqloo")
    if not mysqloo then MsgC(Color(200, 0, 0), "Couldn't load mysqloo module!\n") end
end
...
function SQL:Connect(cfg)
    ...
    return SLCPromise(function(resolve, reject)
        ...
        function db.onConnected(this)
            ...
            hook.Add("ShutDown", "SLCMySQL.Shutdown." .. db:hostInfo(), function() this:disconnect(true) end)
            resolve(this)
        end
...
function SQL:Query(query)
    if self.MySQL then
        if not self.MySQLConnected then return SLCPromise(function(resolve, reject) reject("MySQL is not connected!") end) end
        return SLCPromise(function(resolve, reject)
            local q = self.MySQLDatabase:query(query)
            q.onSuccess = function(this, data) resolve(data) end
...
function SQL:SimpleQuery(query, cb, err, data, abort)
    if self.MySQL then
        if not self.MySQLConnected then return false end
        local q = self.MySQLDatabase:query(query)
        ...
        local q = sql.Query(query)
        if q == false then
            if err then err(nil, sql.LastError(), query) end
        else
            if cb then cb(nil, q) end
        end
    end
    return true
end
...
function SQL:FormatQueryString(query, ...)
    ...
    return string.gsub(query, "%?([%!%d]*)", function(num)
        ...
        local rep = tab[num]
        if not rep then error("Missing value #" .. num .. " for SQL query: " .. orig) end
...
function SQL:FormatTableString(query, tab)
    ...
    if not key_list or not value_list or not set then return end
...
hook.Add("SLCFullyLoaded", "SetupDatabase", function()
	...
	---------------------------------------------------------------------------]]
    SLCDatabase = SQL()
    if SERVER then include("core/_mysql.lua") end
    --[[-------------------------------------------------------------------------
	Create tables
	---------------------------------------------------------------------------]]
    SLCDatabase:Query([[
    ...
    function slc_convert_db_v1_to_v2()
        ...
        SLCDatabase:Query("SELECT * FROM scpplayerdata;"):Then(function(data)
            ...
            for i, v in ipairs(loaded) do
                if i % 100 == 0 then print(i .. " / " .. size .. " (" .. math.Round(i / size * 100, 1) .. "%)") end
        ...
        end):Then(function() print("Migration done!") end)
    end
end
