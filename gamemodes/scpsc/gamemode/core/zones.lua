...
function SLCZones.AddFlag(zone, flag)
    if not SLCZones.ZONES[zone] then return end
...
function SLCZones.GetPlayersInZone(zone)
    ...
    for i, v in ipairs(player.GetAll()) do
        if v:IsInZone(zone) then table.insert(tab, v) end
    end
    return tab
end

function SLCZones.GetPlayersByFlag(flag)
    if not isnumber(flag) then return end
    local zones = {}
    local any = false
...
function SLCZones.IsInZone(vec, zone)
    if isnumber(zone) then return SLCZones.IsInZoneByFlag(vec, zone) end
    ...
    elseif t_type == "table" then
        for i, v in ipairs(tab) do
            if vec:WithinAABox(v[1], v[2]) then return true end
        end
    end
    return false
end
...
function SLCZones.IsInZoneByFlag(vec, flag)
    for _, v in ipairs(SLCZones.ALL) do
        if not SLCZones.HasFlag(v, flag) then continue end
        if SLCZones.IsInZone(vec, v) then return true end
    end
    return false
end
