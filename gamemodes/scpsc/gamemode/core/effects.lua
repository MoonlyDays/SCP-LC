...
function GenericBeam(data)
    if not data.material then return end
    local ct = CurTime()
...
function UpdateBeam(beam, dt)
    if bit.band(beam.flags, BEAM_USE_ENTITY) ~= 0 then
        if IsValid(beam.ent_start) then beam.vec_start = beam.ent_start:GetAttachment(beam.attachment_start).Pos end
        if IsValid(beam.ent_end) then beam.vec_end = beam.ent_start:GetAttachment(beam.attachment_end).Pos end
        local delta = beam.vec_end - beam.vec_start
        beam.delta = delta
    ...
    if beam.fade == 0 then beam.fade = beam.delta:Length() end
end
...
function RenderBeam(beam)
    ...
    if beam.width_end > max_width then max_width = beam.width_end end
    ...
    if segments > noise_div then segments = noise_div end
    div = 1 / (segments - 1)
    length = length * 0.01
