...
function StringBuilder:New(init)
    local t = setmetatable({}, {
        ...
        __tostring = function(obj) return obj.str end
    })
...
function StringBuilder:print(str, ...)
    if str then self:append(str, ...) end
    self.str = self.str .. "\n"
end
    ...
    for y = 1, self.Dim[1] do
        for x = 1, self.Dim[2] do
            ...
            if self.Data[y][x] == 1 then
                ...
                if len and (not cols[x] or cols[x] < len) then cols[x] = len end
            end
        end
    end
    ...
    for y = 1, self.Dim[1] do
        for x = 1, self.Dim[2] do
            ...
            if size > 1 then
                ...
                if len and (not cols[x] or cl < len) then cols[x + size - 1] = cols[x + size - 1] + len - cl end
            end
        end
    end
...
function StringTable:GetRow(y)
    ...
    for x = 1, self.Dim[2] do
        ...
        if len > 0 then
            ...
            local align = false
            if cont_tab and cont_tab[2] then align = string.match(cont_tab[2], "[%<%>%&][%<%>%&]") end
            if align == "<>" then
...
function string.UUID()
    return string.gsub("xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx", "[xy]", function(c) return string.format("%x", c == "x" and SLCRandom(0x00, 0x0f) or SLCRandom(0x08, 0x0b)) end)
end
