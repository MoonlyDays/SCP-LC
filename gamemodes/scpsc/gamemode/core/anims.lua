if SERVER then util.AddNetworkString("SLCAnimController") end
...
hook.Add("CalcMainActivity", "SLCAnimController", function(ply)
    ...
    if cycle >= 1 and not anim.loop then
        cycle = 1
...
hook.Add("SLCPlayerCleanup", "SLCAnimController", function(ply) ply._slc_anim_controller = {} end)
if CLIENT then
    net.Receive("SLCAnimController", function(len)
        local action = net.ReadUInt(action_bytes)
        if action == ANIM_PLAY then LocalPlayer():PlaySequence(net.ReadInt(32), net.ReadBool(), net.ReadDouble()) end
    end)
end
