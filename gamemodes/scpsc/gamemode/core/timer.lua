...
function Timer:Create(name, time, repeats, callback, endcallback, noactivete, nocache)
    if not name or not time or not repeats then return end
    ...
    if not nocache then
        if IsValid(_TimersCache[name]) then _TimersCache[name]:Destroy() end
        _TimersCache[name] = t
    end

    if not noactivate then t:Start() end
    return t
end
...
function Timer:Destroy()
    ...
    self:Stop()
    self.destroyed = true
    if not self.nocache then _TimersCache[self.name] = nil end
end
...
function Timer:Call(...)
    ...
    if self.callback then
        ...
        if not suc then
            ...
            print(err)
            if self.dof then self:Destroy() end
        end
    end
end
...
hook.Add("Tick", "TimersTick", function()
    for k, v in pairs(_TimersCache) do
        if v.alive and v.ncall <= CurTime() then v:Tick() end
    end
        ...
        for i, v in ipairs(player.GetAll()) do
            ...
            if tab then
                for _, t in pairs(tab) do
                    if t.alive and t.ncall <= CurTime() then t:Tick() end
                end
            end
        end
    end
...
function ThenableTimer(name, time, repeats, callback, endcallback, noactivete, nocache)
    ...
    local t = Timer(name, time, repeats, callback, function()
        if endcallback then endcallback(name) end
        p:Resolve()
...
if SERVER then
    ...
    function PLAYER:AddTimer(name, time, repeats, callback, endcallback, noactivete, nooverride)
        ...
        local old = tab[name]
        if IsValid(old) then
            if nooverride then return false end
            old:Destroy()
        end
