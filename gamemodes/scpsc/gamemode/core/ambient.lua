...
function AddAmbient(name, path, check, group, volume, duration, fade)
    ...
    if group then
        if not SLC_AMBIENTS.Groups[group] then SLC_AMBIENTS.Groups[group] = {} end
...
function SelectAmbient(group)
    ...
    if tab then return tab[SLCRandom(#tab)] end
end
...
if SERVER then
    function PlayAmbient(name, loop, ply, force, co)
        ...
        if not ply.SLCAmbient then ply.SLCAmbient = {} end
        if ply.SLCAmbient[name] and not force then return end
        ...
        if not loop and obj.duration then data[0] = CurTime() + obj.duration end
        if not loop and not obj.duration then return end
    ...
    function StopAmbient(ply, name)
        if not IsValid(ply) then return end
        if not ply.SLCAmbient then ply.SLCAmbient = {} end
        if name and not ply.SLCAmbient[name] then return end
        local obj = name and SLC_AMBIENTS.Registry[name]
        if not obj and not name then return end
        ply.SLCAmbient[name] = nil
        net.Start("SLCAmbient")
        net.WriteBool(false)
        net.WriteBool(not name)
        if name then net.WriteString(obj.name) end
        net.Send(ply)
    end

    hook.Add("PlayerPostThink", "SLCAmbientThink", function(ply)
        if not ply.SLCAmbient then return end
        local ct = CurTime()
        for name, data in pairs(ply.SLCAmbient) do
            ...
            if finish and finish ~= 0 and finish <= ct then ply.SLCAmbient[name] = nil end
        end
    end)
end
...
if CLIENT then
    function GetIGModAudio(name, flags, dontcreate, cb)
        ...
        if IsValid(c_igac) then
            if cb then cb(c_igac, obj, false) end
            return c_igac
        end
        ...
        sound.PlayFile(obj.path, flags or "", function(igac, errID, err)
            ...
            if cb then cb(igac, obj, true) end
        end)
    end
    ...
    function PlayAmbient(name, loop)
        ...
        if GetIGModAudio(name, "noblock", false, function(igac, obj, c)
            if loop then igac:EnableLooping(true) end
            igac:SetTime(0)
            igac:Play()
        end) == false then --print( "IGAC OK" )
    ...
    function StopAmbient(name)
        ...
        elseif name then
            ...
            SLC_AMBIENTS.CACHE[name] = nil
            if igac == true or not IsValid(igac) then return end
            if obj.fade then
