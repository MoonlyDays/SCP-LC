...
function SCPHook(scp, name, func)
    if not SCPHooks[scp] then SCPHooks[scp] = {} end
    SCPHooks[scp][name] = func
    if not SCPHooks.__ActiveSCPs[scp] then return end
    hook.Add(name, scp, func)
    if not DEVELOPER_MODE then return end
...
function EnableSCPHook(scp)
    ...
    if SERVER and not SCPHooks.__PreventUpdate then
        net.Start("SCPHooks")
        net.WriteBool(false)
        net.WriteString(scp)
        net.Broadcast()
...
function ClearSCPHooks()
    for scp, _ in pairs(SCPHooks.__ActiveSCPs) do
        if not SCPHooks[scp] then continue end
...
function PLAYER:AddHook(name, identifier, func)
    if not self._PlayerHooks then self._PlayerHooks = {} end
    if not self._PlayerHooks[name] then self._PlayerHooks[name] = {} end
...
function PLAYER:RemoveHook(name, identifier)
    if not self._PlayerHooks or not self._PlayerHooks[name] then return end
    local hook_id = identifier .. self:SteamID64()
    if not self._PlayerHooks[name][hook_id] then return end
    hook.Remove(name, hook_id)
end

function PLAYER:ClearHooks()
    if not self._PlayerHooks then return end
...
hook.Add("SLCPlayerCleanup", "ClearPlayerHooks", function(ply) ply:ClearHooks() end)
