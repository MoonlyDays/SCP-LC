...
function ENTITY:CallBaseClass(func, bc, ...)
    ...
    if not bc then bc = self.BaseClass end
    if bc.BaseClass and bc.ClassName ~= bc.Base and (not self.DeepBase or bc.ClassName ~= self.DeepBase) then self:CallBaseClass(func, bc.BaseClass, ...) end
    local val = bc[func]
...
function ENTITY:ResetBones(num)
    ...
    if num and num > count then count = num end
    for i = 0, count do
...
function ENTITY:TestVisibility(ply, mask, headonly, vlimit, hlimit, z_offset)
    ...
    if SERVER and not ply:TestPVS(self) then return false end
    local dist = self:GetPos():DistToSqr(ply:GetPos())
    local sight = ply:GetSightLimit()
    if sight ~= -1 then if dist > sight * sight then return false end end
    ...
    if not visibility_trace.Hit then return true end
    ...
    util.TraceLine(visibility_trace)
    return not visibility_trace.Hit
end
...
function ENTITY:GetNameEx(pname_limit, blacklist, rep)
    ...
    if self:IsPlayer() then
        if isnumber(pname_limit) and string.len(name) > pname_limit then name = string.sub(name, 1, pname_limit) end
        if blacklist then name = string.gsub(name, blacklist, rep or "?") end
    end
    return name
end
