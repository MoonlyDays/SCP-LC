...
function vector:IsInTolerance(dpos, tolerance)
    ...
    for k, v in pairs({"x", "y", "z"}) do
        if self[v] ~= dpos[v] then if not tolerance[v] or math.abs(dpos[v] - self[v]) > tolerance[v] then return false end end
    end
    return true
end
...
function angle:Clamp(pmin, pmax, ymin, ymax, rmin, rmax)
    if pmin and self.p < pmin then self.p = pmin end
    if pmax and self.p > pmax then self.p = pmax end
    if ymin and self.p < ymin then self.y = ymin end
    if ymax and self.y > ymax then self.y = ymax end
    if rmin and self.r < rmin then self.r = rmin end
    if rmax and self.r > rmax then self.r = rmax end
end
...
function math.AngleTo360(ang)
    ...
    if a360.p < 0 then a360.p = 360 + a360.p end
    if a360.y < 0 then a360.y = 360 + a360.y end
    if a360.r < 0 then a360.r = 360 + a360.r end
    return a360
end

function math.Angle360ToAngle(ang)
    math.NormalizeAngle360(ang)
    if ang.p > 180 then ang.p = -360 + ang.p end
    if ang.y > 180 then ang.y = -360 + ang.y end
    if ang.r > 180 then ang.r = -360 + ang.r end
...
function math.ApproachAngle360(ang, target, inc)
    ...
    for k, v in pairs(ang) do
        ...
        if abs_diff > inc then abs_diff = inc end
...
function math.TrueFOV(fov, aspect)
    if not aspect and CLIENT then aspect = ScrW() / ScrH() end
...
function math.Bin2Dec(bin, unsigned) --this version is about 400% faster
    local num = bit.lshift(string.byte(bin, 1), 24) + bit.lshift(string.byte(bin, 2), 16) + bit.lshift(string.byte(bin, 3), 8) + string.byte(bin, 4)
    if num < 0 and unsigned then
        num = num + 4294967296 --2^32
    end
    return num
...
function math.Dec2Bin(dec) --this version is about 30% faster
    return string.char(bit.rshift(bit.band(dec, -16777216), 24)) .. string.char(bit.rshift(bit.band(dec, 16711680), 16)) .. string.char(bit.rshift(bit.band(dec, 65280), 8)) .. string.char(bit.band(dec, 255))
end
...
function SimpleMatrix:Add(mx)
    ...
    if #self ~= #mx or #self[1] ~= #mx[1] then error("Dimensions of matrixes are not equal") end
...
function SimpleMatrix:Mul(mx)
    ...
    if type(mx) == "table" then
        if #self[1] ~= #mx then error("First matrix width is not eaqual to second matrix height") end
...
setmetatable(SimpleMatrix, {
    __call = SimpleMatrix.New
})
