...
function AddWeaponLoadout(weapon_class, ammo, ...)
    ...
    if SLC_GLOBAL_LOADOUT then
        if not SLC_LOADOUTS[SLC_GLOBAL_LOADOUT] then
...
function RemoveLoadout(name, class)
    ...
    for i, v in rpairs(lo) do
        if v.class == class then
            table.remove(lo, i)
            if v.weight then lo._total = lo._total - v.weight end
        end
    end
end
...
function GetLoadoutWeapon(name, use_weights)
    local lo = SLC_LOADOUTS[name]
    if not lo then return end
    if lo._total == 0 then use_weights = false end
    local tab
    if use_weights then
        local dice = SLCRandom(lo._total)
        local total = 0
        for i, v in ipairs(lo) do
            if not v.weight then continue end
    ...
    if other then
        ...
        if class then return class, ammo end
    end
...
hook.Add("SLCCanPickupWeaponClass", "SLCCW20Pickup", function(ply, class)
    if not SLC_WEAPONS_REG[class] then return end
    for k, v in pairs(ply:GetWeapons()) do
        if SLC_WEAPONS_REG[v:GetClass()] then return false, "one_weapon" end
    end
end)

hook.Add("SLCCanUseBind", "SLCCW20Binds", function(bind)
    local wep = LocalPlayer():GetActiveWeapon()
    if IsValid(wep) and wep.CW20Weapon and wep.dt.State == CW_CUSTOMIZE then return false end
end)
...
local function add_cw_weapon(class, stats, ...)
    ...
    if not noreg then SLC_WEAPONS_REG[class] = true end
    if #args > 0 then AddWeaponLoadout(class, ammo, unpack(args)) end
    local wep_tab = weapons.GetStored(class)
    if not wep_tab then return end
    ...
    if isstring(wep_tab.WorldModel) then table.insert(precache_list_cw, wep_tab.WorldModel) end
    if isstring(wep_tab.ViewModel) then table.insert(precache_list_cw, wep_tab.ViewModel) end
    wep_tab.SelectFont = "SCPCSSIcons"
    wep_tab.SelectIcon = nil
    if wep_tab.Primary and wep_tab.Primary.ClipSize then wep_tab.Primary.DefaultClip = wep_tab.Primary.ClipSize end
    for k, v in pairs(stats) do
        wep_tab[k] = v
    end

    if func then func(wep_tab) end
    rem = rem or {}
    local rem_lookup = {}
    ...
    if not wep_tab.Attachments then return end
    for button, category in pairs(wep_tab.Attachments) do
        if not category.atts then continue end
        for i, elem in rpairs(category.atts) do
            if not rem[elem] and not rem_lookup[elem] and not global_remove[elem] then continue end
...
hook.Add("SLCGamemodeLoaded", "SLCCWStuff", function()
    ...
    function ins_base:Think()
		...
		---------------------------------------------------------------------------]]
        local ct = CurTime()
        if self.dt.State == CW_RUNNING and (self:GetNextPrimaryFire() > ct or self:GetNextSecondaryFire() > ct) then self.dt.State = CW_IDLE end
    end
    ...
    function cw_base:playAnim(anim, speed, cycle, ent)
        if not IsValid(ent or self.CW_VM) then return end
    ...
    function ammo_base:Use(activator, caller)
        ...
        if not IsValid(wep) or wep.Base == "cw_kk_ins2_base_nade" then return end
    ...
    CustomizableWeaponry:registerRecognizedStat("PenetrationPowerMult", "Decreases penetration power", "Increases penetration power", CustomizableWeaponry.textColors.NEGATIVE, CustomizableWeaponry.textColors.POSITIVE)
    CustomizableWeaponry:registerRecognizedStat("ImpactPowerMult", "Decreases impact power", "Increases impact power", CustomizableWeaponry.textColors.NEGATIVE, CustomizableWeaponry.textColors.POSITIVE)
    CustomizableWeaponry:registerRecognizedVariable("PenetrationPowerFlat", "Decreases penetration power", "Increases penetration power", CustomizableWeaponry.textColors.NEGATIVE, CustomizableWeaponry.textColors.POSITIVE, function(weapon, attachmentData) weapon.PenetrationPower_Flat = weapon.PenetrationPower_Flat + attachmentData.PenetrationPowerFlat end, function(weapon, attachmentData) weapon.PenetrationPower_Flat = weapon.PenetrationPower_Flat - attachmentData.PenetrationPowerFlat end)
    if CLIENT and not SLC_CW_STATS_DISPLAY then
    ...
    local function mod_att(name, tab, fn_tab)
        ...
        if CLIENT then CustomizableWeaponry:createStatText(tbl) end
    end
    ...
    if CLIENT then CustomizableWeaponry:createStatText(att) end
    att = {}
    att.name = "slc_fmj"
    ...
    if CLIENT then
        ...
        att.description = {
            {
                t = "Allows penetration of thin objects",
    ...
    add_cw_weapon("cw_kk_ins2_grach", {
        IconLetter = "u"
    }, {
        am_magnum = "slc_hp",
        am_matchgrade = "slc_fmj"
    }, 8, "pistol_low")

    add_cw_weapon("cw_kk_ins2_tokarev", {
        IconLetter = "a"
    }, {
        am_magnum = "slc_hp",
        am_matchgrade = "slc_fmj"
    ...
    add_cw_weapon("cw_kk_ins2_makarov", {
        IconLetter = "a"
    }, {
        am_magnum = "slc_hp",
        am_matchgrade = "slc_fmj"
    ...
    add_cw_weapon("cw_kk_ins2_deagle", {
        IconLetter = "f"
    }, {
        am_magnum = "slc_hp",
        am_matchgrade = "slc_fmj"
    ...
    add_cw_weapon("cw_kk_ins2_hkuspbestgun", {
        IconLetter = "a"
    }, {
        am_magnum = "slc_hp",
        am_matchgrade = "slc_fmj"
    ...
    add_cw_weapon("cw_kk_ins2_mp5a4", {
        IconLetter = "x",
        ImpactPower = 1.1
    }, {
        am_magnum = "slc_hp",
    ...
    add_cw_weapon("cw_kk_ins2_mac10", {
        IconLetter = "l",
        ImpactPower = 1.1
    }, {
        am_magnum = "slc_hp",
    ...
    add_cw_weapon("cw_kk_ins2_ump45", {
        IconLetter = "q",
        ImpactPower = 1.1
    }, {
        am_magnum = "slc_hp",
    ...
    add_cw_weapon("cw_kk_ins2_scorpion", {
        IconLetter = "q",
        ImpactPower = 1.1
    }, {
        am_magnum = "slc_hp",
    ...
    add_cw_weapon("cw_kk_ins2_veresk", {
        IconLetter = "d",
        ImpactPower = 1.1
    }, {
        am_magnum = "slc_hp",
    ...
    add_cw_weapon("cw_kk_ins2_vityaz", {
        IconLetter = "x",
        ImpactPower = 1.1
    }, {
        am_magnum = "slc_hp",
    ...
    add_cw_weapon("cw_kk_ins2_vector", {
        IconLetter = "q",
        ImpactPower = 1.1
    }, {
        am_magnum = "slc_hp",
    ...
    add_cw_weapon("cw_kk_ins2_ak74", {
        IconLetter = "b", --g
        PenetrationPower = 115
    }, 8, "rifle_low")

    add_cw_weapon("cw_kk_ins2_aks74u", {
        IconLetter = "b",
        PenetrationPower = 115
    }, 8, "rifle_low")

    add_cw_weapon("cw_kk_ins2_akm", {
        IconLetter = "b", --g
        PenetrationPower = 115
    }, 8, "rifle_low")

    add_cw_weapon("cw_kk_ins2_abakan", {
        IconLetter = "b", --g
        PenetrationPower = 115
    }, 8, "rifle_low")

    add_cw_weapon("cw_kk_ins2_aek971", {
        IconLetter = "v",
        PenetrationPower = 115
    }, 8, "rifle_low")

    add_cw_weapon("cw_kk_ins2_m16a4", {
        IconLetter = "w", --g
        PenetrationPower = 115
    }, 8, "rifle_low")

    add_cw_weapon("cw_kk_ins2_m4a1", {
        IconLetter = "w", --g
        PenetrationPower = 115
    }, 8, "rifle_mid")

    add_cw_weapon("cw_kk_ins2_galil", {
        IconLetter = "v",
        PenetrationPower = 115
    }, 8, "rifle_mid")

    add_cw_weapon("cw_kk_ins2_cz805", {
        IconLetter = "A",
        PenetrationPower = 115
    }, 8, "rifle_mid")

    add_cw_weapon("cw_kk_ins2_aug", {
        IconLetter = "e",
        PenetrationPower = 115
    }, 8, "rifle_mid")

    add_cw_weapon("cw_kk_ins2_famasf1", {
        IconLetter = "t",
        PenetrationPower = 115
    }, 8, "rifle_mid")

    add_cw_weapon("cw_kk_ins2_sg551", {
        IconLetter = "v",
        PenetrationPower = 115
    }, 8, "rifle_mid")

    add_cw_weapon("cw_kk_ins2_sa58", {
        IconLetter = "A",
        PenetrationPower = 115
    }, 8, "rifle_high")

    add_cw_weapon("cw_kk_ins2_mk18mod1", {
        IconLetter = "A",
        PenetrationPower = 115
    }, 8, "rifle_high")

    add_cw_weapon("cw_kk_ins2_fnf2000", {
        IconLetter = "m",
        PenetrationPower = 115
    }, 8, "rifle_high")

    add_cw_weapon("cw_kk_ins2_asval", {
        IconLetter = "v",
        PenetrationPower = 115
    }, 8, "rifle_high")

    --add_cw_weapon( "cw_kk_ins2_scarh", { PenetrationPower = 115 }, 8, "rifle_high" )
    add_cw_weapon("cw_kk_ins2_m14", {
        IconLetter = "v",
        PenetrationPower = 115
    }, 8, "rifle_high")
    ...
    add_cw_weapon("cw_kk_ins2_chaser13", {
        IconLetter = "k",
        PenetrationPower = 145
    }, 8, "shotgun_low")

    add_cw_weapon("cw_kk_ins2_ks23", {
        IconLetter = "k",
        PenetrationPower = 145
    }, 8, "shotgun_low")

    add_cw_weapon("cw_kk_ins2_br99", {
        IconLetter = "k",
        PenetrationPower = 145
    }, 8, "shotgun_low")

    add_cw_weapon("cw_kk_ins2_m1014", {
        IconLetter = "B",
        PenetrationPower = 145
    }, 8, "shotgun_mid")

    add_cw_weapon("cw_kk_ins2_m590", {
        IconLetter = "k",
        PenetrationPower = 145
    }, 8, "shotgun_mid")

    add_cw_weapon("cw_kk_ins2_saigaspike", {
        IconLetter = "B",
        PenetrationPower = 145
    }, {"kk_ins2_saigaspike_syndicate", "kk_ins2_saigaspike_joke"}, 8, "shotgun_high")

    add_cw_weapon("cw_kk_ins2_spas12", {
        IconLetter = "B",
        PenetrationPower = 145
    }, 8, "shotgun_high")
    ...
    add_cw_weapon("cw_kk_ins2_sv98b", {
        IconLetter = "r",
        Damage = 95
    }, {
        am_magnum = "slc_ap",
    ...
    add_cw_weapon("cw_kk_ins2_t5000", {
        IconLetter = "o",
        Damage = 160
    }, {
        "kk_ins2_t5000_rage",
    ...
    add_cw_weapon("cw_kk_ins2_ax308", {
        IconLetter = "o",
        Damage = 205
    }, {
        "kk_ins2_ax308_oc",
    ...
    add_cw_weapon("cw_kk_ins2_stonerlmg", {
        IconLetter = "z"
    }, {
        am_magnum = "slc_sc",
        "am_matchgrade"
    ...
    add_cw_weapon("cw_kk_ins2_m240b", {
        IconLetter = "z"
    }, {
        am_magnum = "slc_sc",
        "am_matchgrade"
    ...
    add_cw_weapon("cw_kk_ins2_nade_anm14", {
        IconLetter = "Q" --inc
    }, true, "grenade")

    add_cw_weapon("cw_kk_ins2_nade_molotov", {
        IconLetter = "Q" --molo
    }, true, "grenade")

    add_cw_weapon("cw_kk_ins2_nade_m67", {
        IconLetter = "O" --frag
    }, true, "grenade")

    add_cw_weapon("cw_kk_ins2_nade_f1", {
        IconLetter = "O" --frag v2
    }, true, "grenade")

    add_cw_weapon("cw_kk_ins2_nade_m18", {
        IconLetter = "P" --smoke
    }, true, "grenade")

    add_cw_weapon("cw_kk_ins2_nade_m84", {
        IconLetter = "P" --flash
    }, true, "grenade")
    ...
    AddGroupToWeapon("grenade", "cw_kk_ins2_nade_anm14", "cw_kk_ins2_nade_molotov", "cw_kk_ins2_nade_m67", "cw_kk_ins2_nade_f1", "cw_kk_ins2_nade_m18", "cw_kk_ins2_nade_m84")
