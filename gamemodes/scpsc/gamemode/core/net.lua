...
net.Receive("SLCPing", function(len, ply)
    ...
    if net.PingReceivers[name] then
        ...
        if isfunction(net.PingReceivers[name]) then net.PingReceivers[name](data, ply) end
        --end
    end
end)

function net.ReceivePing(name, func)
    if not net.PingReceivers[name] then net.PingReceivers[name] = {} end
...
function net.AddTableChannel(name)
    ...
    if #net.TableChannels >= 255 then return false end
    if net.TableChannelsID[name] then return net.TableChannelsID[name] end
...
function net.SendTable(name, tab, ply, disablecrc, dnc)
    ...
    if not dnc then tab = table.Copy(tab) end
...
net.Receive("SLCSendTable", function(len, ply)
    ...
    if not net_cache[message_id] then
    ...
    if tab.num == tab.max then
        ...
        if not msg_name then
            if CLIENT and not FULLY_LOADED then
...
hook.Add("Think", "SLCNetTables", function()
    ...
    if SERVER then
        local plys = {}
        for i, v in ipairs(player.GetAll()) do
            if not v.TableChannelsUpdated then
        ...
        if #plys > 0 then
            ...
            for k, v in pairs(net.TableChannels) do
                if v ~= "_TableChannel" then ch[k] = v end
            end
