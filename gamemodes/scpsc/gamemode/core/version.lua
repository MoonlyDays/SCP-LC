local op_mt = {
    ...
    __tostring = function(this) return this.name end
}
...
function SLCVersion(sig)
    ...
    if not realm or not major or not minor or not patch or not rev then
        return setmetatable({
            signature = sig,
    ...
    local name = ""
    if realms[realm] then name = realms[realm] .. " " end
    name = name .. major .. "." .. minor .. "." .. patch
    if rev > 0 then name = name .. " rev. " .. rev end
    return setmetatable({
        signature = sig,
        name = name,
        realm = realm,
