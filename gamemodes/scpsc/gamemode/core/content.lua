SLC_CONTENT_WORKSHOP_ID = "3644692201"
...
local function fetch_collection()
	return SLCPromise(function(resolve, reject)
		...
		steamworks.FileInfo(SLC_CONTENT_WORKSHOP_ID, function(data)
			...
			if SERVER then print(string.format("Added %i addons as gamemode content from collection: %s", #data.children, SLC_CONTENT_WORKSHOP_ID)) end
			resolve()
		end)
	end)
end
...
if CLIENT then
	...
	local function download_addons()
		...
		if SLC_CONTENT.status ~= SCS_DOWNLOADING then progress.total = #SLC_CONTENT.info.nsub end
	...
	local function verify_addons()
		...
		joined:Then(function(tab)
			...
			for i, v in ipairs(tab) do
				if not v.installed then print(string.format("\t> '%s' (%s)", v.name, v.id)) end
			end

			print("\n#####################################\n")
			print("Disabled addons:")
			for i, v in ipairs(tab) do
				if v.installed and not v.enabled then print(string.format("\t> '%s' (%s)", v.name, v.id)) end
			end
...
if SERVER then
	hook.Add("SLCFullyLoaded", "SLCGamemodeContent", function()
		...
		if game.IsDedicated() then
			timer.Simple(30, function() fetch_collection():Catch(function() print("Failed to fetch collection info!") end) end)
		else
			fetch_collection():Catch(function() print("Failed to fetch collection info!") end)
		end
	end)
end