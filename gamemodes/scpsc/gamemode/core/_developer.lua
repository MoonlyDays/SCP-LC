...
if SERVER then
    ...
    timer.Create("slcdrawvectors", 0.5, 0, function()
        if not draw_key then return end
        local tab = _G
        for i, v in ipairs(draw_key) do
            tab = tab[v]
...
if SERVER then
    ...
    concommand.Add("draw_item_spawns", function(ply, cmd, args) enabled = not enabled end)
...
if SERVER then
    ...
    timer.Create("slcdrawzones", 0.5, 0, function()
        if not draw_zones then return end
        local zone = MAP_ZONES[draw_zones]
        if not zone or not istable(zone) then return end
...
function util.TraceLine(tr)
    ...
    if tr.output then res = tr.output end
...
concommand.Add("draw_blockers", function() slc_dev_blockers = not slc_dev_blockers end)
hook.Add("Think", "blockersdev", function()
    if not slc_dev_blockers then return end
    for i = 1, #BLOCKERS do
...
hook.Add("HUDPaint", "SpeedHUD", function()
    if not show_speed then return end
...
if CLIENT then concommand.Add("show_speed", function() show_speed = not show_speed end) end
--[[-------------------------------------------------------------------------
Dump loadouts
---------------------------------------------------------------------------]]
...
concommand.Add("trace_contents", function(ply)
    ...
    local n = 1
    while i <= mask do
        if bit.band(mask, i) == i then print("\t" .. (CONTENTS[n] or "???")) end
        n = n + 1
        if n > 64 then
            print("OVERFLOW")
            break
...
if CLIENT then
    ...
    concommand.Add("slc_snav", function()
        if enabled then return SNAV_GEN:Remove() end
        enabled = true
        ...
        btn:Dock(TOP)
        btn:SetText("SAVE")
        btn.DoClick = function(this) update_rt(true) end
        panel:SetWide(800)
    ...
    concommand.Add("dmg_record", function(ply)
        if recording then
            ...
            hook.Add("SLCPostScaleDamage", "developer_recording", function(target, info)
                ...
                if not IsValid(target) or not target:IsPlayer() or att ~= ply or not info:IsDamageType(DMG_BULLET) and not info:IsDamageType(DMG_BUCKSHOT) then return end
