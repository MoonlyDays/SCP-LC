...
function GetMaterial(name, args)
    ...
    if not mat or mat:IsError() then mat = Material("null") end
    mat_cache[name] = mat
    return mat
end

concommand.Add("slc_mat_clear_cache", function() mat_cache = {} end)
--[[---------------------------------------------------------------------------
...
function surface.DrawRingDC(x, y, radius, thick, angle, segments, fill, rotation, dist, func)
    ...
    for i = 1, math.Round(segments * fill) do
        ...
        local sin2 = math.sin(ang2)
        local cos2 = -math.cos(ang2)
        if func and isfunction(func) then func(i) end
        surface.DrawPoly({
            {
                x = x + sin1 * radius,
                y = y + cos1 * radius
...
function surface.DrawLoading(x, y, radius, thick)
    ...
    if ang > 360 then ang = 720 - ang end
        ...
        for i, v in ipairs(string.Split(text, "\n")) do
            ...
            if w <= maxWidth then
                ...
                repeat
                    ...
                    if index == nil then index = string.len(cur_txt) + 1 end
                ...
                if stack > 250 then print("Overflow!!!") end
            end
        end
    end

    local n = #final
    if maxRows and maxRows < n then n = maxRows end
    if simulate and not calcW then return margin * 2 + height * n, maxWidth, final end
    local maxw = 0
    local justify = false
...
function util.GlitchData(seg, scale, color_r, color_g, color_b)
    ...
    for i = 1, seg do
        ...
        if color_r then data[i].color = Color(SLCRandom(color_r, 255), SLCRandom(color_g or color_r, 255), SLCRandom(color_b or color_r, 255)) end
    end
    return data
end
...
function draw.Glitch(x, y, w, h, mode, data)
    if mode == GLITCH_VERTICAL then
        ...
        for i = 1, xseg do
            local tab = data[i]
            if tab.color then surface.SetDrawColor(tab.color) end
...
function draw.GlitchToTexture(texture, target, mode, data, clear, r, g, b, a)
    ...
    if clear then render.Clear(r or 0, g or 0, b or 0, a or 255, true, true) end
    cam.Start2D()
...
function draw.OutlinedText(text, font, x, y, color, xalign, yalign, outline_color, fallback_width, strength)
    ...
    if not of then return draw.SimpleTextOutlined(text, font, x, y, color, xalign, yalign, fallback_width or 3, outline_color) end
