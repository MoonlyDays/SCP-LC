...
timer.Simple(0, function()
    ...
    if CLIENT then net.ReceiveTable("DeathInfo", function(data) DamageLogger.Print(data) end) end
end)

DamageLogger = {}
function DamageLogger:New(ply)
    local logger = setmetatable({}, {
        __index = DamageLogger
    })
...
function DamageLogger:DamageTaken(dmg, attacker, data)
    ...
    table.insert(self.TakenLog, {
        attacker = attacker,
        attackername = ivp and attacker:GetName() or "NULL",
        attackerid = ivp and attacker:SteamID64() or "NULL",
        damage = dmg,
        data = data,
        rdm = rdm
    })

    if not self.TakenSourceLog[attacker] then
        self.TakenSourceLog[attacker] = {
            dmg = 0,
            orig = 0,
            nrdm = 0
        }
    ...
    if enemy then self.TakenSourceLog[attacker].nrdm = self.TakenSourceLog[attacker].nrdm + dmg end
...
function DamageLogger:DamageDealt(dmg, target, data)
    ...
    table.insert(self.DealtLog, {
        target = target,
        targetname = ivp and target:GetName() or "NULL",
        targetid = ivp and target:SteamID64() or "NULL",
        damage = dmg,
        data = data,
        rdm = rdm
    })

    if not self.DealtTargetLog[target] then
        self.DealtTargetLog[target] = {
            dmg = 0,
            final = 0,
            nrdm = 0
        }
    ...
    if enemy then self.DealtTargetLog[target].nrdm = self.DealtTargetLog[target].nrdm + data.dmg_final end
...
function DamageLogger:Dump(killer, inflictor)
    if (not IsValid(inflictor) or inflictor == killer) and killer:IsPlayer() then inflictor = killer:GetActiveWeapon() end
    local data = {
        Killer = killer,
...
function DamageLogger:GetDeathDetails(deathid)
    local tab = {}
    if not deathid or deathid == -1 then deathid = #self.LastRound end
    ...
    for k, v in pairs(log.TakenSourceLog) do
        if IsValid(v[1]) and v[1]:IsPlayer() then
            if v[1] ~= log.Killer then
                if v[2].nrdm > 0 then table.insert(assists, {v[1], v[2].nrdm}) end
            else
                kdmg = v[2].nrdm
            end
...
function DamageLogger:Reset(round)
    if round then self.LastRound = {} end
    self.Dealt = 0
    self.Taken = 0
    self.DealtLog = {}
    self.TakenLog = {}
...
setmetatable(DamageLogger, {
    __call = DamageLogger.New
})
