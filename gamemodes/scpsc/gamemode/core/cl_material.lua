...
local function find_and_insert(tab, base_path, path)
    ...
    if last ~= "/" and last ~= "\\" then base_path = base_path .. "/" end
...
function RegisterResourceOverride(id, data)
    if not resource_manifest[id] then
    ...
    if data.auto then find_and_insert(overrides, isstring(data.auto) and data.auto or path) end
end
...
function RebuildResourceOverrides()
    ...
    for i = #order, 1, -1 do
        ...
        if not tab.enabled or not resource_overrides[id] then continue end
        tab.order = i
...
hook.Add("SLCRegisterSettings", "SLCResourcePacks", function()
    AddSettingsPanel("resource_packs", function(parent)
        ...
        container.PerformLayout = function(this, pw, ph) container:SizeToChildren(true, true) end
        ...
        for k, v in pairs(resource_manifest) do
            ...
            down:SetText("⯆", true)
            down.Think = function(this) this:SetEnabled(v.order < max) end
            ...
            up:SetText("⯅", true)
            up.Think = function(this) this:SetEnabled(v.order > 1) end
...
if manifest then resource_manifest = util.JSONToTable(manifest) or {} end
