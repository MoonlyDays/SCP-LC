...
function BuildPrecacheList()
    ...
    for _, v in ipairs(file.Find(BASE_LUA_PATH .. "/entities/weapons/*.lua", "LUA")) do
        ...
        if wep and wep.WorldModel and wep.WorldModel ~= "" then table.insert(precache_list, wep.WorldModel) end
    end
...
function PrecacheNext()
    ...
    if not mdl or current_index == precache_len then
        current_index = -1
        precache_list = {}
        return false
    end
...
if SERVER then
    function PrecacheAll()
        ...
        hook.Add("Tick", "SLCPrecacheModels", function()
            for i = 1, 10 do
                if not PrecacheNext() then break end
            end
