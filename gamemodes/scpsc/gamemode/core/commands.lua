...
local function exec_cmd(ply, name, args)
    ...
    if not cmd or not slc_cmd.ExecCallback(ply, name, args) then return end
...
function slc_cmd.AddCommand(name, callback, cooldown)
    ...
    if CLIENT then concommand.Add(name, exec_cmd) end
end
...
function slc_cmd.ExecCallback(ply, name, args)
    ...
    for k, v in pairs(cmd.cd) do
        if not IsValid(k) then cmd.cd[k] = nil end
    end

    if cmd.callback and cmd.callback(ply, name, args) == true then return false end
    if cmd.cooldown then cmd.cd[ply] = ct + cmd.cooldown end
    return true
end
