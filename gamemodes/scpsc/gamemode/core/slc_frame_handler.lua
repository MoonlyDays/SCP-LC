...
function DEF_PAGE:ReceiveData_Internal(name, data, ply)
    ...
    if SERVER then
        if self.Handler.IsBanned(ply:SteamID64(), self.ID) then
            ...
            return
        elseif self.PANIC or self.IsVisible and not self:IsVisible(ply) then
            if req == "load" then
                self:SendData(name, {
                    __restrict = true
                }, ply)
    ...
    if not rid or SERVER then
        ...
        if rid then
            if ispromise(res) then
                res:Then(function(d) self:SendData(name, d, ply) end)
            else
...
function DEF_PAGE:Error(msg, fatal, ply)
    if SERVER then
        ...
        if fatal and IsValid(self.Handler.Instance) then self.Handler.Instance:Close() end
...
function DEF_PAGE:Message(id, ply)
    if SERVER then
        ...
        local msg
        if self.Lang.Messages then msg = self.Lang.Messages[id] end
        if not msg and self.Handler.Lang.Messages then msg = self.Handler.Lang.Messages[id] end
...
if CLIENT then
    function DEF_PAGE:Load(root, data)
        ...
        return p:Then(function(rdata)
            if not IsValid(root) or rdata.RequestID ~= self.LastRequestID then return end
            ...
            elseif rdata.__restrict then
                ...
                rdata.Root = root
                self.PageData = rdata
                if self.Build then self:Build(root, rdata) end
            end
            return rdata
        end)
        --[[:Catch( function( err )
    ...
    function DEF_PAGE:Request(name, data, to, override)
        ...
        if self.ReqID > 999 then self.ReqID = 0 end
        return p, rid
    end
...
function FrameHandler:SubPage(id, data)
    ...
    if not sub then sub = self:RegisterSubPage(id, data or {}) end
    SUBPAGE = sub
end
...
function FrameHandler:GetPages()
    ...
    for k, v in pairs(self.PAGES) do
        if SERVER or v.ID ~= "index" and (not v.IsVisible or v:IsVisible(LocalPlayer())) then table.insert(pages, k) end
    end

    table.sort(pages, function(a, b) return (self.PAGES[a].Order or 999) < (self.PAGES[b].Order or 999) end)
    return pages
end
...
function FrameHandler:GetButtons()
    ...
    local buttons = {}
    for k, v in pairs(self.PAGES) do
        if CLIENT and (v.ID == "index" or v.IsVisible and not v:IsVisible(ply)) then continue end
    ...
    for k, v in pairs(self.ACTIONS) do
        if CLIENT and v.is_visible and not v.is_visible(ply) then continue end
        table.insert(buttons, {
            name = k,
            action = v.callback,
            color = v.color,
...
function FrameHandler:CloseFrame()
    if SERVER or not IsValid(self.Instance) then return end
...
local function new_handler(_, name, path, cfg)
    if not path then path = BASE_GAMEMODE_PATH .. "/" .. name end
    path = path .. "/"
    ...
    for _, folder in ipairs(folders) do
        ...
        if not file.Exists(manifest, "LUA") then return end
        ...
        for _, f in ipairs(file.Find(path .. folder .. "/*.lua", "LUA")) do
            ...
            if SERVER and pre ~= "sv_" then AddCSLuaFile(full) end
...
if SERVER then
    ...
    net.Receive("SLCPages", function(len, ply)
        ...
        if net.ReadBit() == 1 then data.__bin = net.ReadData(net.ReadUInt(32)) end
