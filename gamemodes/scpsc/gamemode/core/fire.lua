...
if SERVER then
    function ENTITY:Burn(time, radius, attacker, dmg, sc, dontoverride, fulloverride)
        ...
        if fulloverride and IsValid(fire) then fire:Remove() end
        if fulloverride or not IsValid(fire) then
            ...
            if sc and attacker:IsPlayer() then fire:SetSignatureCheck(attacker:TimeSignature()) end
            fire:Spawn()
            FIRE_REGISTRY[self] = fire
        elseif not fire.DontOverride then
            local changed = false
            ...
            if changed then if IsValid(attacker) and attacker:IsPlayer() then if attacker ~= fire:GetOwner() then fire:SetOwner(attacker) end end end
        end
        return fire
    end
