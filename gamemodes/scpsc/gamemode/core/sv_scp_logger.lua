...
function SCPLogger:Save()
    if not self.Finalized then self:Finish() end
    ...
    until not file.Exists(file_path, "DATA")
...
hook.Add("DoPlayerDeath", "SCPLogger", function(ply)
    if not ply.SCPLogger then return end
    print("Death finish")
...
hook.Add("SLCPreround", "SCPLogger", function()
    ...
    AddRoundHook("PostEntityTakeDamage", "SCPLogger", function(ply, dmg)
        ...
        if not IsValid(att) or att == ply then return end
        if ply.SCPLogger then
            local data = ply.SCPLogger.Data
            if data.first_received == -1 then data.first_received = os.time() end
        ...
        if att.SCPLogger then
            ...
            if data.first_dealt == -1 and att ~= ply then data.first_dealt = os.time() end
            if ply:IsInZone(ZONE_FLAG_SURFACE) then
    ...
    AddRoundHook("SLCPlayerDeath", "SCPLogger", function(victim, attacker)
        --Test escape/quit/suicide/kill
        ...
        if not IsValid(attacker) or victim == attacker or not attacker.SCPLogger then return end
        if attacker.SCPLogger.Data.first_kill == -1 then attacker.SCPLogger.Data.first_kill = os.time() end
    ...
    AddTimer("SCPLogger", 30, 0, function()
        for i, v in ipairs(player.GetAll()) do
            if not v.SCPLogger then continue end
            local pos = v:GetPos()
