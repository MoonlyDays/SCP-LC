...
function controller.Register(name, data)
    registry[name] = data
    data.Name = name
    data.Stop = function(self) self.ShouldStop = true end
end
...
function controller.GetData(ply)
    ...
    if name == "" or ply._ControllerData and ply._ControllerData.Name ~= name then
        if ply._ControllerData then
            if ply._ControllerData.OnFinish then ply._ControllerData:OnStop(ply) end
            ply._ControllerData = nil
        end
        return
    end

    if not ply._ControllerData then
        local func = registry[name]
        if not func then return end
        ply._ControllerData = setmetatable({}, {
            __index = func
        })

        if ply._ControllerData.OnStart then ply._ControllerData:OnStart(ply) end
    end
