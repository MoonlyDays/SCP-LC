...
if CLIENT then
    ...
    function AddGUISkin(target, name, functbl)
        if not SLCSkins[name] then SLCSkins[name] = {} end
    ...
    function RunGUISkinFunction(target, fn, name)
        ...
        if name and SLCSkins[name] and SLCSkins[name][target] and SLCSkins[name][target][fn] then return SLCSkins[name][target][fn]() end
...
if CLIENT then
    hook.Add("SLCRegisterSettings", "SLCGUISkins", function()
        ...
        for k, v in pairs(SLCSkins) do
            if k ~= "default" then table.insert(names, k) end
            for fname, _ in pairs(v) do
                if not funcs[fname] then funcs[fname] = {} end
                table.insert(funcs[fname], k)
            end
        end

        RegisterSettingsEntry("hud_skin_main", "dropbox", "default", {
            list = names,
            callback = main_cb
        }, "skins")

        for k, v in pairs(funcs) do
            RegisterSettingsEntry("hud_skin_" .. k, "dropbox", "default", {
                list = v,
                callback = change_cb,
                lang = "hud_skin_main"
            }, "skins")
