...
hook.Add("PlayerButtonDown", "SLCVGUIVButtonDown", function(ply, btn)
    ...
    if IsValid(pnl) and pnl.HoveredButtonDown then pnl:HoveredButtonDown(btn) end
end)

function vgui.QuickPanel(parent, dock, dbg)
    local pnl = vgui.Create("DPanel", parent)
    pnl:Dock(dock or TOP)
    pnl.Paint = dbg and function(_, pw, ph)
...
function vgui.QuickLabel(text, parent, dock, font, color, copy)
    ...
    if not copy then return label end
...
function vgui.NickLabel(sid, parent, dock, font, color, copy)
    if not steamworks_player_info then steamworks_player_info = SLCPromisify(steamworks.RequestPlayerInfo) end
    local name
    local ply = player.GetBySteamID64(sid)
    if IsValid(ply) then
        name = ply:Nick()
    else
        name = {sid, steamworks_player_info(sid)}
    end
...
function vgui.QuickMarkup(text, align, parent, dock, dbg)
    ...
    pnl.SizeToContents = function(this)
        if not this.Markup then return end
