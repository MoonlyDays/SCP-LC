...
function AddGamemodeConfig(name, category, value, help, min, max, check)
    ...
    if min then help_text = help_text .. "\n" .. "Minimum value: " .. min end
    if max then help_text = help_text .. "\n" .. "Maximum value: " .. max end
...
function SLCCVar(name, category, value, flags, help, min, max, valid_check)
    ...
    if CLIENT then AddGamemodeConfig(name, category, value, help, min, max, valid_check) end
    return cv
end
...
hook.Add("SLCFactoryReset", "SLCResetConvars", function()
    if SERVER then
        for k, v in pairs(CVAR) do
            ...
            if default then v:SetString(default) end
        end
    end
end)
...
if CLIENT then
    ...
    local function BuildSettingsTree()
        ...
        for k, v in pairs(SLCGamemodeConfig) do
            ...
            if not primary_categories[primary] then primary_categories[primary] = {} end
            ...
            if istab then
                ...
                if len > 1 then
                    for i = 2, len do
                        ...
                        if not secondary_categories[category] then secondary_categories[category] = {} end
    ...
    local function settings_window(parent)
        ...
        local function refresh_text_boxes()
            for _, child in pairs(parent:GetCanvas():GetChildren()) do
                if child.RefreshAllTextBoxes then child:RefreshAllTextBoxes() end
            end
        end
        ...
        for i, v in ipairs(settings_data) do
            ...
            pnl.PerformLayout = function(self) self:SizeToChildren(false, true) end
            ...
            for _, item in ipairs(v.items) do
                ...
                reset_btn:SetText("")
                reset_btn.PerformLayout = function(self) self:SetWide(self:GetTall()) end
                reset_btn.Paint = function(self, pw, ph)
                    ...
                    surface.SetDrawColor(COLOR.white)
                    surface.DrawOutlinedRect(0, 0, pw, ph)
                    if not enabled then surface.SetDrawColor(COLOR.inactive) end
                ...
                discard_btn:SetText("")
                discard_btn.PerformLayout = function(self) self:SetWide(self:GetTall()) end
                ...
                apply_btn:SetText("")
                apply_btn.PerformLayout = function(self) self:SetWide(self:GetTall()) end
                ...
                apply_btn.DoClick = function(self)
                    ...
                    if data.value_type == "number" then value = tonumber(value) end
                ...
                text_box.OnChange = function(self)
                    ...
                    if num then value = tonumber(value) end
                    apply_btn:SetEnabled(value ~= data.value)
                    discard_btn:SetEnabled(value ~= data.value)
                    reset_btn:SetEnabled(data.value ~= data.default)
                    if data.custom_check then
                        self.has_error = not data.custom_check(value) or num and (data.min and value < data.min or data.max and value > data.max)
                        if self.has_error then apply_btn:SetEnabled(false) end
                    end
                end
    ...
    timer.Simple(0, function()
            ...
            FetchGamemodeConfig():Then(function(data)
                if IsValid(parent) and parent.SelectedMenuName == "cvars" then
                    ...
                    for k, v in pairs(data) do
                        ...
                        if tab then
                            if tab.value_type == "number" then v = tonumber(v) end
                            tab.value = v
                        end
                    end
        ...
        end, function() return SLCAuth.HasAccess(LocalPlayer(), "slc gamemodeconfig") end, 1000)
        ...
        net.ReceiveTable("SLCGamemodeConfig", function(data) if fetch_promise and fetch_promise:GetStatus() == PROMISE_PENDING then fetch_promise:Resolve(data) end end)
    end)
end
