hook.Add("SLCGamemodeLoaded", "SLCPlayerInfo", function() if not file.Exists("slc/playerinfo", "DATA") then file.CreateDir("slc/playerinfo") end end)
...
function PlayerInfo:New(arg)
    ...
    if not file.Exists(tab.File, "DATA") then
        file.Write(tab.File, "{}")
        tab.Data = {}
    else
        ...
        if not data then
            ...
            if f then
                if not file.Exists("slc/playerinfo/corrupted", "DATA") then file.CreateDir("slc/playerinfo/corrupted") end
        ...
        if data.compressed then data = util.JSONToTable(util.Decompress(data.data)) end
        if not data then
            print("Failed to load Playerdata for player: " .. tostring(tab.Player) .. " - " .. tab.ID)
            if t == "Player" then tab.Player.PlayerInfo = nil end
            return
        end
...
setmetatable(PlayerInfo, {
    __call = PlayerInfo.New
})

hook.Add("SLCRegisterGamerules", "SLCPlayerInfoRule", function()
    gamerule.Register("compressPlayerdata", {
        value = false
    })
end)
