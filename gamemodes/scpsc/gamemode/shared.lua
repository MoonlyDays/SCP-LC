...
function RegisterLanguage(tab, name, ...)
	...
	if _LANG[name] then print("WARNING! Language '" .. name .. "' is already registered!") end
	_LANG[name] = tab
	_LANG_FLAGS[name] = 0
	for k, v in pairs({...}) do
		_LANG_ALIASES[v] = name
		if v == "default" then _LANG_DEFAULT = name end
	end
...
function SetLanguageFlag(name, flag)
	if _LANG_FLAGS[name] and flag then _LANG_FLAGS[name] = bit.bor(_LANG_FLAGS[name], flag) end
end
	...
	for k, f in pairs(file.Find(path .. "/*.lua", "LUA")) do
		if string.len(f) > 3 then
			...
			elseif realm == "SERVER" or ext == "sv_" then
				...
				if SERVER then AddCSLuaFile(path .. "/" .. f) end
				if t then print(t .. f) end
				include(path .. "/" .. f)
			end
		end
...
if DEVELOPER_MODE then
	...
	if SERVER then AddCSLuaFile(dev_file) end
...
for k, f in pairs(file.Find(LANGUAGES_PATH .. "/*.lua", "LUA")) do
	if SERVER then AddCSLuaFile(LANGUAGES_PATH .. "/" .. f) end
...
for k, f in pairs(modules) do
	...
	if string.len(f) > 3 then
		...
		elseif ext == "sh_" then
			if SERVER then AddCSLuaFile(MODULES_PATH .. "/" .. f) end
			print("# Loading module: " .. f)
			include(MODULES_PATH .. "/" .. f)
		end
	else
...
if hook.Run("SLCCustomMapConfig", map, MAP_LOADED) and not MAP_LOADED then
