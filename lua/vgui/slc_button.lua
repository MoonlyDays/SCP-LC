...
function PANEL:Paint(w, h)
    ...
    local clr = 0.75
    if not self:IsEnabled() or warn > 0 then
        clr = 0.5
    ...
    if warn > 0 then text = math.ceil(warn) end
...
function PANEL:PerformLayout(w, h)
    ...
    if not self.ForceText and self.Text ~= "" and surface.GetTextSize(self.Text) > max_w then
        self.DrawText = self.Text .. "..."
        while surface.GetTextSize(self.DrawText) > max_w do
            self.DrawText = string.sub(self.DrawText, 1, -5) .. "..."
            if self.DrawText == "..." then break end
        end
    else
        self.DrawText = self.Text
    end
