...
function PANEL:Init()
    ...
    self.Button.Paint = function(this, pw, ph)
        ...
        if self.Radio then
            ...
            if self.b_State then surface.DrawFilledCircle(s2, s2, s2 - 3, 24) end
        else
            surface.DrawOutlinedRect(0, 0, pw, ph)
            if self.b_State then surface.DrawRect(3, 3, pw - 6, ph - 6) end
        end
    end

    self.Button.DoClick = function(this)
        if self.Radio then
            if not self.b_State then
...
function PANEL:SetRadio(b, id, tab)
    ...
    self.RadioID = id
    self.RadioTab = tab
    if not tab.Buttons then tab.Buttons = {} end
    if not table.HasValue(tab.Buttons, self) then table.insert(tab.Buttons, self) end
end
