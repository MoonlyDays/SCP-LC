...
function PANEL:Init()
    ...
    mask.Think = function(this) if not IsValid(self) then this:Remove() end end
    mask:MakePopup()
...
function PANEL:Think()
    if self:HasFocus() then
        if not self.HadFocus then self.HadFocus = true end
...
function PANEL:Paint(w, h)
    ...
    if self.Markup then self.Markup:Draw(self.Marg, self.Name:GetTall() + self.Marg) end
end
...
function PANEL:SetOptions(options)
    ...
    for i, v in ipairs(options) do
        ...
        if istable(v) then
            ...
            if v.warn then btn:SetWarning(5) end
        else
            btn:SetText(v)
        ...
        btn.DoClick = function(this)
            if self.Callback and self.Callback(i, self, this) then return end
            self:Remove()
        end
    end
end
...
function PANEL:SetData(data)
    ...
    self:SetKeep(not not data.keep)
    self:SetOptions(data.options)
    self.Callback = data.callback
    if data.callback then data.callback(-1) end
end
