...
function PANEL:Init()
    ...
    exit.DoClick = function(this) self:Close() end
    ...
    nav.PerformLayout = function(this, pw, ph)
        for i, v in ipairs(this.Buttons) do
            if not v.BaseWide then v.BaseWide = v:GetWide() end
...
function PANEL:SetHandler(handler)
    ...
    for k, v in pairs(handler:GetButtons()) do
        ...
        btn.Paint = function(this, pw, ph)
            local speed = 6
            local delta = -1
            if this:IsEnabled() and this:IsHovered() and not this.Lock then delta = 1 end
            this.Fill = math.Clamp(this.Fill + delta * speed * RealFrameTime(), 0, 1)
            if this.Fill > 0 then
                if this.PageID and this.PageID == root.ActivePage then
                    if this.PageObject and not this.PageObject.DisableRefresh then
        ...
        btn.DoClick = function(this)
            ...
            elseif self.Failed or this.PageID ~= root.ActivePage then
                ...
                this.Fill = 0
            elseif this.PageObject and not this.PageObject.DisableRefresh then
                self:LoadPage(this.PageID)
            end
        ...
        btn.OnCursorExited = function(this) this.Lock = false end
        btn.PerformLayout = function(this) if mark.Button ~= this then this:UpdateMark() end end
        btn.UpdateMark = function(this)
            ...
            anim.Think = function(tab, pnl, f) pnl:SetColor(old_color.r + (new_color.r - old_color.r) * f, old_color.g + (new_color.g - old_color.g) * f, old_color.b + (new_color.b - old_color.b) * f, old_color.a + (new_color.a - old_color.a) * f) end
            anim.OnEnd = function(tab, pnl) pnl:SetColor(new_color:Unpack()) end
        end
    end
